...
void LegendreCache::update( const double polynomialParameter, const bool checkConsistency )
{
    ...
    if( !( polynomialParameter == currentPolynomialParameter_ ) )
    {
        ...
        // Compute first derivatives of Legendre polynomials if needed
        if( computeFirstDerivatives_ )
        {
            for( int i = 0; i <= maximumDegree_; i++ )
            {
                jMax = std::min( i, maximumOrder_ );
                for( int j = 0; j <= jMax; j++ )
                {
                    if( j != 0 )
                    {
                        ...
                        if( useGeodesyNormalization_ )
                        {
                            legendreDerivatives_[ i * ( maximumOrder_ + 1 ) + ( j - 1 ) ] = computeGeodesyLegendrePolynomialDerivative(
                                    j - 1,
                        ...
                        else
                        {
                            legendreDerivatives_[ i * ( maximumOrder_ + 1 ) + ( j - 1 ) ] =
                                    computeLegendrePolynomialDerivative( j - 1,
                                                                         currentPolynomialParameter_,
                                                                         legendreValues_[ i * ( maximumOrder_ + 1 ) + ( j - 1 ) ],
                                                                         legendreValues_[ i * ( maximumOrder_ + 1 ) + j ] );
                        }
                ...
                if( jMax == i )
                {
                    if( useGeodesyNormalization_ )
                    {
                        legendreDerivatives_[ i * ( maximumOrder_ + 1 ) + jMax ] =
                                computeGeodesyLegendrePolynomialDerivative( jMax,
                                                                            currentPolynomialParameter_,
                                                                            currentOneOverPolynomialParameterComplement_,
                    ...
                    {
                        legendreDerivatives_[ i * ( maximumOrder_ + 1 ) + jMax ] = computeLegendrePolynomialDerivative(
                                jMax, currentPolynomialParameter_, legendreValues_[ i * ( maximumOrder_ + 1 ) + jMax ], 0.0 );
                    }
                }
            }
        }
        ...
        if( computeSecondDerivatives_ )
        {
            for( int i = 0; i <= maximumDegree_; i++ )
            {
                ...
                if( jMax == i )
                {
                    if( useGeodesyNormalization_ )
                    {
                        legendreSecondDerivatives_[ i * ( maximumOrder_ + 1 ) + jMax ] = computeGeodesyLegendrePolynomialSecondDerivative(
                                jMax,
                                currentPolynomialParameter_,
                                currentOneOverPolynomialParameterComplement_,
                                legendreValues_[ i * ( maximumOrder_ + 1 ) + jMax ],
                    ...
                    else
                    {
                        legendreSecondDerivatives_[ i * ( maximumOrder_ + 1 ) + jMax ] =
                                computeGeodesyLegendrePolynomialSecondDerivative( jMax,
                                                                                  currentPolynomialParameter_,
                                                                                  currentOneOverPolynomialParameterComplement_,
                                                                                  legendreValues_[ i * ( maximumOrder_ + 1 ) + jMax ],
                                                                                  0.0,
...
void LegendreCache::resetMaximumDegreeAndOrder( const int maximumDegree, const int maximumOrder )
{
    ...
    for( int i = 0; i <= maximumDegree_; i++ )
    {
        for( int j = 0; ( ( j <= i ) && ( j <= maximumOrder_ ) ); j++ )
        {
            ...
            derivativeNormalizations_[ i * ( maximumOrder_ + 1 ) + j ] =
                    std::sqrt( ( static_cast< double >( i + j + 1 ) ) * ( static_cast< double >( i - j ) ) );
            ...
            verticalLegendreValuesComputationMultipliersOne_[ i * ( maximumOrder_ + 1 ) + j ] =
                    std::sqrt( ( 2.0 * static_cast< double >( i ) + 1.0 ) /
                               ( ( static_cast< double >( i + j ) ) * ( static_cast< double >( i - j ) ) ) );
            verticalLegendreValuesComputationMultipliersTwo_[ i * ( maximumOrder_ + 1 ) + j ] =
                    std::sqrt( ( static_cast< double >( i + j ) - 1.0 ) * ( static_cast< double >( i - j ) - 1.0 ) /
...
double LegendreCache::getLegendrePolynomial( const int degree, const int order ) const
{
    ...
    {
        return getLegendrePolynomialWithoutCheck( degree, order );
    }
}

double LegendreCache::getLegendrePolynomialWithoutCheck( const int degree, const int order ) const
{
    return legendreValues_.at( degree * ( maximumOrder_ + 1 ) + order );
}

//! Get first derivative of Legendre polynomial value from the cache.
double LegendreCache::getLegendrePolynomialDerivative( const int degree, const int order ) const
{
    if( degree > ( maximumDegree_ ) || order > maximumOrder_ )
    {
    ...
    {
        return getLegendrePolynomialDerivativeWithoutCheck( degree, order );
    }
}
...
double LegendreCache::getLegendrePolynomialSecondDerivative( const int degree, const int order ) const
{
    if( degree > ( maximumDegree_ ) || order > maximumOrder_ )
    {
    ...
    {
        return getLegendrePolynomialSecondDerivativeWithoutCheck( degree, order );
    }
}
...
double computeLegendrePolynomialFromCache( const int degree, const int order, LegendreCache& legendreCache )
{
    ...
    return computeLegendrePolynomialFromCacheWithoutCheck( degree, order, legendreCache );
}
...
double computeLegendrePolynomialFromCacheWithoutCheck( const int degree, const int order, LegendreCache& legendreCache )
{
    ...
    else if( degree == order )
    {
        ...
        const double degreeOneOrderOnePolynomial = legendreCache.getLegendrePolynomialWithoutCheck( 1, 1 );

        // Obtain prior sectoral polynomial.
        const double priorSectoralPolynomial = legendreCache.getLegendrePolynomialWithoutCheck( degree - 1, order - 1 );

        // Compute polynomial.
        return computeLegendrePolynomialDiagonal( degree, degreeOneOrderOnePolynomial, priorSectoralPolynomial );
    }
    ...
    else
    {
        ...
        const double oneDegreePriorPolynomial = legendreCache.getLegendrePolynomialWithoutCheck( degree - 1, order );

        // Obtain two degrees prior polynomial.
        const double twoDegreesPriorPolynomial = legendreCache.getLegendrePolynomialWithoutCheck( degree - 2, order );

        // Compute polynomial.
        return computeLegendrePolynomialVertical(
                degree, order, legendreCache.getCurrentPolynomialParameter( ), oneDegreePriorPolynomial, twoDegreesPriorPolynomial );
    }
}
...
double computeGeodesyLegendrePolynomialFromCache( const int degree, const int order, LegendreCache& geodesyLegendreCache )
{
    ...
    return computeGeodesyLegendrePolynomialFromCacheWithoutCheck( degree, order, geodesyLegendreCache );
}
...
double computeGeodesyLegendrePolynomialFromCacheWithoutCheck( const int degree, const int order, LegendreCache& geodesyLegendreCache )
{
    ...
    {
        // Obtain polynomial of degree one and order one.
        double degreeOneOrderOnePolynomial = geodesyLegendreCache.getLegendrePolynomialWithoutCheck( 1, 1 );

        // Obtain prior sectoral polynomial.
        double priorSectoralPolynomial = geodesyLegendreCache.getLegendrePolynomialWithoutCheck( degree - 1, order - 1 );

        // Compute polynomial.
        return computeGeodesyLegendrePolynomialDiagonal( degree, degreeOneOrderOnePolynomial, priorSectoralPolynomial );
    }
    ...
    {
        // Obtain prior degree polynomial.
        double oneDegreePriorPolynomial = geodesyLegendreCache.getLegendrePolynomialWithoutCheck( degree - 1, order );

        // Obtain two degrees prior polynomial.
        double twoDegreesPriorPolynomial = geodesyLegendreCache.getLegendrePolynomialWithoutCheck( degree - 2, order );
        ...
        return computeGeodesyLegendrePolynomialVertical(
                ...
                geodesyLegendreCache.getVerticalLegendreValuesComputationMultipliersOne( degree, order ),
                geodesyLegendreCache.getVerticalLegendreValuesComputationMultipliersTwo( degree, order ),
                                            ...
                                            const double incrementedLegendrePolynomial )
{
    ...
    return incrementedLegendrePolynomial / std::sqrt( 1.0 - polynomialParameter * polynomialParameter ) -
            static_cast< double >( order ) * polynomialParameter / ( 1.0 - polynomialParameter * polynomialParameter ) *
                                                   ...
                                                   const double normalizationCorrection )
{
    ...
    return normalizationCorrection * incrementedLegendrePolynomial * oneOverPolynomialParameterComplement -
            ...
            oneOverPolynomialParameterComplement * currentLegendrePolynomial;
}
                                                   ...
                                                   const double incrementedLegendrePolynomial )
{
    ...
    double normalizationCorrection =
            std::sqrt( ( static_cast< double >( degree + order + 1 ) ) * ( static_cast< double >( degree - order ) ) );
    double oneOverPolynomialParameterComplement = 1.0 / std::sqrt( 1.0 - polynomialParameter * polynomialParameter );
    ...
    // Return polynomial derivative.
    return computeGeodesyLegendrePolynomialDerivative( order,
                                                       polynomialParameter,
                                                         ...
                                                         const double normalizationCorrection )
{
    ...
    return normalizationCorrection *
            ( incrementedLegendrePolynomialDerivative * oneOverPolynomialParameterComplement +
              polynomialParameter * oneOverPolynomialParameterComplement * oneOverPolynomialParameterComplement *
                      oneOverPolynomialParameterComplement * incrementedLegendrePolynomial ) -
            static_cast< double >( order ) *
            ( polynomialParameter * oneOverPolynomialParameterComplement * oneOverPolynomialParameterComplement *
                      currentLegendrePolynomialDerivative +
              ( 1.0 + polynomialParameterSquare ) * oneOverPolynomialParameterComplement * oneOverPolynomialParameterComplement *
                      oneOverPolynomialParameterComplement * oneOverPolynomialParameterComplement * currentLegendrePolynomial );
}
    ...
    switch( degree )
    {
        ...
        case 2:
            switch( order )
            {
                ...
                case 1:
                    return 3.0 * polynomialParameter * std::sqrt( 1.0 - polynomialParameter * polynomialParameter );
                case 2:
        ...
        case 3:
            ...
            {
                case 0:
                    return 0.5 * polynomialParameter * ( 5.0 * polynomialParameter * polynomialParameter - 3.0 );
                case 1:
        ...
        case 4:
            ...
            {
                case 0:
                    return ( 35.0 * polynomialParameter * polynomialParameter * polynomialParameter * polynomialParameter -
...
double computeGeodesyLegendrePolynomialExplicit( const int degree, const int order, const double polynomialParameter )
{
    ...
    else
    {
        // Set error message.
        std::stringstream errorMessage;
        errorMessage << "Error: computation of Legendre polynomial of = " << degree << " and order = " << order << " is not supported."
                     << std::endl;
...
double computeLegendrePolynomialDiagonal( const int degree, const double degreeOneOrderOnePolynomial, const double priorSectoralPolynomial )
{
    // Return polynomial.
    return ( 2.0 * static_cast< double >( degree ) - 1.0 ) * degreeOneOrderOnePolynomial * priorSectoralPolynomial;
}
...
{
    // Return polynomial.
    return std::sqrt( ( 2.0 * static_cast< double >( degree ) + 1.0 ) / ( 6.0 * static_cast< double >( degree ) ) ) *
...
{
    // Return polynomial.
    return ( ( 2.0 * static_cast< double >( degree ) - 1.0 ) * polynomialParameter * oneDegreePriorPolynomial -
             ( static_cast< double >( degree + order ) - 1.0 ) * twoDegreesPriorPolynomial ) /
                                                 ...
                                                 const double twoDegreesPriorPolynomial )
{
    ...
    return firstMultiplier *
            ( std::sqrt( 2.0 * static_cast< double >( degree ) - 1.0 ) * polynomialParameter * oneDegreePriorPolynomial -
              secondMultiplier * twoDegreesPriorPolynomial );
}
                                                 ...
                                                 const double twoDegreesPriorPolynomial )
{
    ...
    double secondMultipler =
            std::sqrt( ( static_cast< double >( degree + order ) - 1.0 ) * ( static_cast< double >( degree - order ) - 1.0 ) /
                       ( 2.0 * static_cast< double >( degree ) - 3.0 ) );
    return computeGeodesyLegendrePolynomialVertical(
            degree, order, polynomialParameter, firstMultiplier, secondMultipler, oneDegreePriorPolynomial, twoDegreesPriorPolynomial );
}
                                                         ...
                                                         Eigen::MatrixXd& normalizedSineCoefficients )
{
    ...
    for( unsigned degree = 0; degree < unnormalizedCosineCoefficients.rows( ); degree++ )
    {
        for( unsigned order = 0; ( order < unnormalizedCosineCoefficients.cols( ) && order <= degree ); order++ )
        {
            ...
            normalizedCosineCoefficients( degree, order ) = unnormalizedCosineCoefficients( degree, order ) / normalizationFactor;
            normalizedSineCoefficients( degree, order ) = unnormalizedSineCoefficients( degree, order ) / normalizationFactor;
        }
    }
}
        ...
        const Eigen::MatrixXd& unnormalizedSineCoefficients )
{
    ...
    convertUnnormalizedToGeodesyNormalizedCoefficients(
            unnormalizedCosineCoefficients, unnormalizedSineCoefficients, normalizedCosineCoefficients, normalizedSineCoefficients );
                                                         ...
                                                         Eigen::MatrixXd& unnormalizedSineCoefficients )
{
    ...
    for( unsigned degree = 0; degree < unnormalizedCosineCoefficients.rows( ); degree++ )
    {
        ...
        {
            normalizationFactor = calculateLegendreGeodesyNormalizationFactor( degree, order );
            unnormalizedCosineCoefficients( degree, order ) = normalizedCosineCoefficients( degree, order ) * normalizationFactor;
            unnormalizedSineCoefficients( degree, order ) = normalizedSineCoefficients( degree, order ) * normalizationFactor;
        }
    }
}
        ...
        const Eigen::MatrixXd& normalizedSineCoefficients )
{
    ...
    convertGeodesyNormalizedToUnnormalizedCoefficients(
            normalizedCosineCoefficients, normalizedSineCoefficients, unnormalizedCosineCoefficients, unnormalizedSineCoefficients );
