...
#include "tudat/math/geometric/geometricShapesToFile.h"
                                             ...
                                             const bool isIndependentVariableInverted )
{
    ...
    if( isIndependentVariableInverted == false )
    {
        // Set grid size 1.
        gridSize1_ =
                ( singleSurfaceGeometry->getMaximumIndependentVariable( 1 ) - singleSurfaceGeometry->getMinimumIndependentVariable( 1 ) ) /
                ( numberOfLines - 1 );

        // Set grid size 2.
        gridSize2_ =
                ( singleSurfaceGeometry->getMaximumIndependentVariable( 2 ) - singleSurfaceGeometry->getMinimumIndependentVariable( 2 ) ) /
                ( numberOfPoints - 1 );
    }
        ...
        for( int j = 0; j < numberOfPoints; j++ )
        {
            ...
            if( isIndependentVariableInverted == false )
            {
                ...
                point = singleSurfaceGeometry->getSurfacePoint(
                        singleSurfaceGeometry->getMinimumIndependentVariable( 1 ) + i * gridSize1_,
                        singleSurfaceGeometry->getMinimumIndependentVariable( 2 ) + j * gridSize2_ );
            }
            ...
            else
            {
                // Set point.
                point = singleSurfaceGeometry->getSurfacePoint(
                        singleSurfaceGeometry->getMinimumIndependentVariable( 1 ) + j * gridSize1_,
                        singleSurfaceGeometry->getMinimumIndependentVariable( 2 ) + i * gridSize2_ );
            }
                                                ...
                                                const std::vector< bool >& isIndependentVariableInvertedArray )
{
    ...
    for( unsigned int i = 0; i < compositeSurfaceGeometry->getNumberOfSingleSurfaceGeometries( ); i++ )
    {
        // Append single geometry to file.
        writeSingleSurfaceGeometryPointsToFile( compositeSurfaceGeometry->getSingleSurfaceGeometry( i ),
                                                arrayOfNumberOfLines[ i ],
                                                arrayOfNumberOfPoints[ i ],
                                                filename,
                                                1,
                                                isIndependentVariableInvertedArray[ i ] );
    }
}
