...
void to_json( nlohmann::json& jsonObject, const std::shared_ptr< AccelerationSettings >& accelerationSettings )
{
    ...
    switch( accelerationType )
    {
        ...
        case relativistic_correction_acceleration: {
            ...
            jsonObject[ K::calculateDeSitterCorrection ] = relativisticAccelerationCorrectionSettings->calculateDeSitterCorrection_;
            jsonObject[ K::primaryBody ] = relativisticAccelerationCorrectionSettings->primaryBody_;
    ...
    switch( accelerationType )
    {
        ...
        case spherical_harmonic_gravity: {
            accelerationSettings = std::make_shared< SphericalHarmonicAccelerationSettings >(
                    getValue< int >( jsonObject, K::maximumDegree ), getValue< int >( jsonObject, K::maximumOrder ) );
            return;
        }
        case mutual_spherical_harmonic_gravity: {
        ...
        case relativistic_correction_acceleration: {
            ...
            accelerationSettings = std::make_shared< RelativisticAccelerationCorrectionSettings >(
                    getValue( jsonObject, K::calculateSchwarzschildCorrection, defaults.calculateSchwarzschildCorrection_ ),
                    getValue( jsonObject, K::calculateLenseThirringCorrection, defaults.calculateLenseThirringCorrection_ ),
