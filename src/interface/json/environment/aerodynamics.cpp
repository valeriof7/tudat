...
void to_json( nlohmann::json& jsonObject, const std::shared_ptr< AerodynamicCoefficientSettings >& aerodynamicSettings )
{
    ...
    const AerodynamicCoefficientTypes aerodynamicCoefficientType = aerodynamicSettings->getAerodynamicCoefficientType( );
    ...
    switch( aerodynamicCoefficientType )
    {
        case constant_aerodynamic_coefficients: {
            ...
            jsonObject[ K::areCoefficientsInAerodynamicFrame ] = constantAerodynamicSettings->getAreCoefficientsInAerodynamicFrame( );
        ...
        case tabulated_coefficients: {
            ...
            if( tabulated1AerodynamicSettings )  // uni-dimensional
            {
                ...
                jsonObject[ K::forceCoefficients ] = getMapValues< std::map >( tabulated1AerodynamicSettings->getForceCoefficients( ) );
            ...
            if( hasMoments )
            {
                ...
                jsonObject[ K::lateralReferenceLength ] = fromFileAerodynamicCoefficientSettings->getLateralReferenceLength( );
                jsonObject[ K::momentReferencePoint ] = fromFileAerodynamicCoefficientSettings->getMomentReferencePoint( );
            }

            jsonObject[ K::independentVariableNames ] = fromFileAerodynamicCoefficientSettings->getIndependentVariableNames( );
        ...
        default:
            handleUnimplementedEnumValue( aerodynamicCoefficientType, aerodynamicCoefficientTypes, unsupportedAerodynamicCoefficientTypes );
    }
}
...
void from_json( const nlohmann::json& jsonObject, std::shared_ptr< AerodynamicCoefficientSettings >& aerodynamicSettings )
{
    ...
    const AerodynamicCoefficientTypes coefficientsType = getValue( jsonObject, K::coefficientsType, constant_aerodynamic_coefficients );

    // Reference area (either from the current object or from the current object's parent, i.e. the body)
    const double referenceArea = getValue< double >( jsonObject, { K::referenceArea, SpecialKeys::up / Keys::Body::referenceArea } );
    ...
    switch( coefficientsType )
    {
        case constant_aerodynamic_coefficients: {
            ...
            {
                ConstantAerodynamicCoefficientSettings defaults( TUDAT_NAN, TUDAT_NAN, TUDAT_NAN, Eigen::Vector3d( ), Eigen::Vector3d( ) );
                aerodynamicSettings = std::make_shared< ConstantAerodynamicCoefficientSettings >(
                        ...
                        getValue( jsonObject, K::areCoefficientsInAerodynamicFrame, defaults.getAreCoefficientsInAerodynamicFrame( ) ),
                        getValue( jsonObject,
                                  K::areCoefficientsInNegativeAxisDirection,
        ...
        case tabulated_coefficients: {
            ...
            const std::vector< AerodynamicCoefficientsIndependentVariables > independentVariableNames =
                    getValue< std::vector< AerodynamicCoefficientsIndependentVariables > >( jsonObject, K::independentVariableNames );
            const bool areCoefficientsInAerodynamicFrame = getValue( jsonObject, K::areCoefficientsInAerodynamicFrame, true );
            const bool areCoefficientsInNegativeAxisDirection = getValue( jsonObject, K::areCoefficientsInNegativeAxisDirection, true );
            ...
            if( readFromFiles )
            {
                ...
                {
                    aerodynamicSettings = readTabulatedAerodynamicCoefficientsFromFiles( forceCoefficientsFiles,
                                                                                         referenceArea,
                                                                                         independentVariableNames,
                                                                                         areCoefficientsInAerodynamicFrame,
                                                                                         areCoefficientsInNegativeAxisDirection );
        ...
        default:
            handleUnimplementedEnumValue( coefficientsType, aerodynamicCoefficientTypes, unsupportedAerodynamicCoefficientTypes );
    }
}
