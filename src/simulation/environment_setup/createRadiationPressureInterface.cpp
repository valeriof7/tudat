...
void getOccultingBodiesInformation( const SystemOfBodies& bodies,
                                    ...
                                    std::vector< double >& occultingBodyRadii )
{
    ...
    for( unsigned int i = 0; i < occultingBodies.size( ); i++ )
    {
        ...
        {
            std::shared_ptr< basic_astrodynamics::BodyShapeModel > shapeModel = bodies.at( occultingBodies.at( i ) )->getShapeModel( );
...
void getCentralBodyInformation( const SystemOfBodies& bodies,
        ...
        const SystemOfBodies& bodies )
{
    ...
    switch( radiationPressureInterfaceSettings->getRadiationPressureType( ) )
    {
        case cannon_ball_radiation_pressure_interface: {
            ...
            {
                std::shared_ptr< basic_astrodynamics::BodyShapeModel > sourceShapeModel = sourceBody->getShapeModel( );

                if( sourceShapeModel == nullptr )
                {
                    throw std::runtime_error( "Error when making occulted body, source body " +
                                              radiationPressureInterfaceSettings->getSourceBody( ) + " does not have a shape" );
                }
                else
                {
            ...
            if( defaultRadiatedPowerValues.count( radiationPressureInterfaceSettings->getSourceBody( ) ) == 0 )
            {
                throw std::runtime_error( "Error, no radiated power found for " + radiationPressureInterfaceSettings->getSourceBody( ) );
            }
            else
            {
            ...
            if( cannonBallSettings->getRadiationPressureCoefficient( ) != cannonBallSettings->getRadiationPressureCoefficient( ) )
            {
            ...
            //                            "Error when making occulted body, source body " +
            //                            radiationPressureInterfaceSettings->getSourceBody( ) + " does not have a shape" );
            //            }
            //            else
            //            {
            ...
            //        panelledSettings->getSurfaceNormalsInBodyFixedFrameFunctions();
            //
            //
            //        // Create radiation pressure interface.
            ...
            //    }
        default:
            throw std::runtime_error( "Error, radiation pressure type" +
                                      std::to_string( radiationPressureInterfaceSettings->getRadiationPressureType( ) ) +
...
std::function< double( const double ) > getOccultationFunction( const SystemOfBodies& bodyMap,
