        ...
        const std::vector< std::function< double( ) > > guidanceInputFunctions )
{
    ...
    std::shared_ptr< aerodynamics::FlightConditions > vehicleFlightConditions = bodyWithGuidance->getFlightConditions( );

    std::shared_ptr< aerodynamics::AtmosphericFlightConditions > vehicleAtmosphericFlightConditions =
            std::dynamic_pointer_cast< aerodynamics::AtmosphericFlightConditions >( bodyWithGuidance->getFlightConditions( ) );
    ...
    for( unsigned int i = 0; i < independentVariables.size( ); i++ )
    {
        switch( independentVariables.at( i ) )
        {
            ...
            case propulsion::throttle_dependent_thrust:
                ...
                {
                    throw std::runtime_error(
                            "Error when creating propulsion indput dependent variables, insufficient user-defined inputs found" );
                }
                inputFunctions.push_back( guidanceInputFunctions.at( numberOfCustomInputs ) );
                numberOfCustomInputs++;
                break;
    ...
    {
        std::cerr << "Warning when creating propulsion indput dependent variables, not all user-defined inputs have been parsed"
                  << std::endl;
    }
        ...
        const std::shared_ptr< interpolators::Interpolator< double, double > > specificImpulseInterpolator )
{
    int numberOfUserSpecifiedThrustInputs = std::count(
            thrustIndependentVariables_.begin( ), thrustIndependentVariables_.end( ), propulsion::guidance_input_dependent_thrust );
    int numberOfMaximumThrustMultipliers =
            std::count( thrustIndependentVariables_.begin( ), thrustIndependentVariables_.end( ), propulsion::throttle_dependent_thrust );
    ...
    {
        throw std::runtime_error(
                "Error in parameterized thrust settings, inconsistent number of user-defined input variables for thrust" );
    }
    ...
    if( numberOfMaximumThrustMultipliers == 1 )
    {
        std::vector< propulsion::ThrustIndependentVariables >::iterator findIterator = std::find(
                thrustIndependentVariables_.begin( ), thrustIndependentVariables_.end( ), propulsion::throttle_dependent_thrust );
        int entryForMaximumThrustMultiplierInIndependentVariables = std::distance( thrustIndependentVariables_.begin( ), findIterator );
    ...
    if( specificImpulseInterpolator != nullptr )
    {
        ...
        int numberOfUserSpecifiedSpecificImpulseInputs = std::count( specificImpulseDependentVariables_.begin( ),
                                                                     specificImpulseDependentVariables_.end( ),
                                                                     propulsion::guidance_input_dependent_thrust );
        int numberOfMaximumThrustMultipliersForSpecificImpulse = std::count( specificImpulseDependentVariables_.begin( ),
                                                                             specificImpulseDependentVariables_.end( ),
                                                                             propulsion::throttle_dependent_thrust );

        if( numberOfUserSpecifiedSpecificImpulseInputs != static_cast< int >( specificImpulseGuidanceInputVariables_.size( ) ) )
        {
            throw std::runtime_error(
                    "Error in parameterized thrust settings, inconsistent number of user-defined input variables for specific impulse " );
        }
        if( specificImpulseInterpolator->getNumberOfDimensions( ) != static_cast< int >( specificImpulseDependentVariables_.size( ) ) )
        {
        ...
        {
            throw std::runtime_error(
                    "Error in parameterized thrust settings, no maximum thrust multiplier may be defined for specific impulse." );
        }
    }
    else
    {
        ...
        {
            throw std::runtime_error(
                    "Error in parameterized thrust settings, no specific impulse interpolator define, but inptu data has been provided." );
        }
    }
}
...
std::shared_ptr< interpolators::Interpolator< double, double > > readCoefficientInterpolatorFromFile( const std::string coefficientFile )
{
    ...
    int numberOfThrustIndependentVariables = input_output::getNumberOfIndependentVariablesInCoefficientFile( coefficientFile );
    if( numberOfThrustIndependentVariables == 1 )
    {
        std::pair< boost::multi_array< double, 1 >, std::vector< std::vector< double > > > coefficientData =
                input_output::MultiArrayFileReader< 1 >::readMultiArrayAndIndependentVariables( coefficientFile );
        coefficientInterpolator = std::make_shared< interpolators::MultiLinearInterpolator< double, double, 1 > >( coefficientData.second,
                                                                                                                   coefficientData.first );
    }
    else if( numberOfThrustIndependentVariables == 2 )
    {
        std::pair< boost::multi_array< double, 2 >, std::vector< std::vector< double > > > coefficientData =
                input_output::MultiArrayFileReader< 2 >::readMultiArrayAndIndependentVariables( coefficientFile );
        coefficientInterpolator = std::make_shared< interpolators::MultiLinearInterpolator< double, double, 2 > >( coefficientData.second,
                                                                                                                   coefficientData.first );
    }
    else if( numberOfThrustIndependentVariables == 3 )
    {
        std::pair< boost::multi_array< double, 3 >, std::vector< std::vector< double > > > coefficientData =
                input_output::MultiArrayFileReader< 3 >::readMultiArrayAndIndependentVariables( coefficientFile );
        coefficientInterpolator = std::make_shared< interpolators::MultiLinearInterpolator< double, double, 3 > >( coefficientData.second,
                                                                                                                   coefficientData.first );
    }
    else
    {
...
{
    // Create thrust magnitude settings object.
    return createParameterizedThrustMagnitudeSettings( thrustInputParameterGuidance,
        ...
        const std::string nameOfCentralBody )
{
    std::shared_ptr< ThrustInputParameterGuidance > thrustGuidance =
            std::make_shared< AccelerationLimitedThrottleGuidance >( bodies,
...
std::shared_ptr< ParameterizedThrustMagnitudeSettings > createAccelerationLimitedParameterizedThrustMagnitudeSettings(
        ...
        const std::string nameOfCentralBody )
{
    return createAccelerationLimitedParameterizedThrustMagnitudeSettings( bodies,
                                                                          nameOfBodyWithGuidance,
                                                                          maximumAcceleration,
                                                                          readCoefficientInterpolatorFromFile( thrustMagnitudeDataFile ),
                                                                          thrustIndependentVariables,
        ...
        const std::string nameOfCentralBody )
{
    std::shared_ptr< ThrustInputParameterGuidance > thrustGuidance =
            std::make_shared< AccelerationLimitedThrottleGuidance >( bodies,
                                                                     ...
                                                                     thrustMagnitudeInterpolator,
                                                                     maximumAcceleration );
    return createParameterizedThrustMagnitudeSettings( thrustGuidance,
                                                       thrustMagnitudeInterpolator,
                                                       thrustIndependentVariables,
                                                       specificImpulseInterpolator,
                                                       specificImpulseDependentVariables );
