...
{
    {
        ...
        std::shared_ptr< propulsion::ThrustMagnitudeWrapper > thrustMagnitudeWrapper =
                createThrustMagnitudeWrapper( thrustSettings, bodies, bodyName, magnitudeUpdateSettings );

        std::shared_ptr< system_models::EngineModel > vehicleEngineModel =
                std::make_shared< system_models::EngineModel >( thrustMagnitudeWrapper, engineName, bodyFixedThrustDirection );
        ...
        const simulation_setup::SystemOfBodies& bodies )
{
    ...
    std::shared_ptr< system_models::VehicleExteriorPanel > exteriorPanel = std::make_shared< system_models::VehicleExteriorPanel >(
            localFrameSurfaceNormal, localFramePositionVector, panelArea, panelTemperature, trackedBodyName );
    ...
    if( bodies.at( bodyName )->getRotationalEphemeris( ) == nullptr )
    {
        throw std::runtime_error( "Error when creating body exterior panel model for body " + bodyName +
                                  ", no body rotation model provided" );
    }

    if( ( panelSettings->panelGeometry_->frameOrientation_ != "" ) &&
        ( panelSettings->panelGeometry_->frameOrientation_ !=
                                   ...
                                   const simulation_setup::SystemOfBodies& bodies )
{
    ...
    if( panelSettings == nullptr )
    {
        throw std::runtime_error( "Error when creating body exterior paneled shape for body " + bodyName +
                                  ", provided settings are empty" );
    }
    ...
    for( auto it: panelSettings->partRotationModelSettings_ )
    {
        ...
        vehiclePartOrientation[ it.first ] = createRotationModel( it.second, bodyName + "_" + it.first, bodies );
    }
