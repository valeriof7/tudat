        ...
        const std::vector< std::shared_ptr< GravityFieldVariationSettings > >& gravityFieldVariationSettings )
{
    ...
    {
        throw std::runtime_error( "Error when making gravity field variations of body " + body + ", body not found" );
    }
    ...
    for( unsigned int i = 0; i < gravityFieldVariationSettings.size( ); i++ )
    {
        ...
        variationObjects.push_back( createGravityFieldVariationsModel( gravityFieldVariationSettings.at( i ), body, bodies ) );

        if( gravityFieldVariationSettings.at( i )->getBodyDeformationType( ) == basic_solid_body )
        {
            std::vector< std::string > deformingBodies =
                    std::dynamic_pointer_cast< BasicSolidBodyGravityFieldVariationSettings >( gravityFieldVariationSettings.at( i ) )
                            ->getDeformingBodies( );
        ...
        {
            createInterpolators[ i ] = gravityFieldVariationSettings.at( i )->getInterpolatorSettings( )->interpolatorSettings_;
            initialTimes[ i ] = gravityFieldVariationSettings.at( i )->getInterpolatorSettings( )->initialTime_;
            finalTimes[ i ] = gravityFieldVariationSettings.at( i )->getInterpolatorSettings( )->finalTime_;
            timeSteps[ i ] = gravityFieldVariationSettings.at( i )->getInterpolatorSettings( )->timeStep_;
        }
    }

    // Create object with settings for updating variations from new parameter values.
    std::shared_ptr< GravityFieldVariationsSet > fieldVariationsSet = std::make_shared< GravityFieldVariationsSet >(
            variationObjects, variationTypes, variationIdentifiers, createInterpolators, initialTimes, finalTimes, timeSteps );

    if( std::dynamic_pointer_cast< TimeDependentSphericalHarmonicsGravityField >( bodies.at( body )->getGravityFieldModel( ) ) == nullptr )
    {
        throw std::runtime_error( "Error when making gravity field variations of body " + body + ", base type is not time dependent" );
    }

    std::dynamic_pointer_cast< TimeDependentSphericalHarmonicsGravityField >( bodies.at( body )->getGravityFieldModel( ) )
            ->setFieldVariationSettings( fieldVariationsSet, false );
    ...
    switch( gravityFieldVariationSettings->getBodyDeformationType( ) )
    {
        ...
        case mode_coupled_solid_body: {
            ...
            if( gravityFieldVariationSettings->getBodyDeformationType( ) == basic_solid_body )
            {
                std::shared_ptr< BasicSolidBodyGravityFieldVariationSettings > basicSolidBodyGravityVariationSettings =
                        std::dynamic_pointer_cast< BasicSolidBodyGravityFieldVariationSettings >( gravityFieldVariationSettings );
            ...
            {
                std::shared_ptr< ModeCoupledSolidBodyGravityFieldVariationSettings > modeCoupledSolidBodyGravityVariationSettings =
            ...
            for( unsigned int i = 0; i < deformingBodies.size( ); i++ )
            {
                ...
                {
                    throw std::runtime_error( "Error when making basic solid body gravity field variation, " + deformingBodies[ i ] +
                                              " deforming body not found." );
                }
                ...
                {
                    throw std::runtime_error( "Error, could not find gravity field model in body " + deformingBodies[ i ] +
            ...
            if( gravityFieldVariationSettings->getBodyDeformationType( ) == basic_solid_body )
            {
                std::shared_ptr< BasicSolidBodyGravityFieldVariationSettings > basicSolidBodyGravityVariationSettings =
                        std::dynamic_pointer_cast< BasicSolidBodyGravityFieldVariationSettings >( gravityFieldVariationSettings );
                gravityFieldVariationModel = std::make_shared< BasicSolidBodyTideGravityFieldVariations >(
                        deformedBodyStateFunction,
            ...
            else if( gravityFieldVariationSettings->getBodyDeformationType( ) == mode_coupled_solid_body )
            {
                std::shared_ptr< ModeCoupledSolidBodyGravityFieldVariationSettings > modeCoupledSolidBodyGravityVariationSettings =
                        std::dynamic_pointer_cast< ModeCoupledSolidBodyGravityFieldVariationSettings >( gravityFieldVariationSettings );
                gravityFieldVariationModel = std::make_shared< ModeCoupledSolidBodyTideGravityFieldVariations >(
                        deformedBodyStateFunction,
                        deformedBodyOrientationFunction,
                        deformingBodyStateFunctions,
                        gravityField->getReferenceRadius( ),
        ...
        case tabulated_variation: {
            ...
            std::shared_ptr< TabulatedGravityFieldVariationSettings > tabulatedGravityFieldVariationSettings =
                    std::dynamic_pointer_cast< TabulatedGravityFieldVariationSettings >( gravityFieldVariationSettings );
        ...
        case periodic_variation: {
            ...
            std::shared_ptr< PeriodicGravityFieldVariationsSettings > periodicGravityFieldVariationSettings =
                    std::dynamic_pointer_cast< PeriodicGravityFieldVariationsSettings >( gravityFieldVariationSettings );
        ...
        case polynomial_variation: {
            ...
            std::shared_ptr< PolynomialGravityFieldVariationsSettings > polynomialGravityFieldVariationSettings =
                    std::dynamic_pointer_cast< PolynomialGravityFieldVariationsSettings >( gravityFieldVariationSettings );
        ...
        default: {
            throw std::runtime_error( "Error, case " + std::to_string( gravityFieldVariationSettings->getBodyDeformationType( ) ) +
...
std::map< int, std::vector< std::complex< double > > > getFullLoveNumbersVector( const std::complex< double > constantLoveNumber,
...
std::map< int, std::vector< std::complex< double > > > getFullLoveNumbersVector( const double constantLoveNumber,
                                                                                 const int maximumDegree,
                                                                                 const int maximumOrder )
{
...
std::vector< std::complex< double > > getLoveNumberPerDegree( const std::complex< double > loveNumber, const int degree )
{
...
std::vector< std::complex< double > > getLoveNumberPerDegree( const double loveNumber, const int degree )
{
