        ...
        const double lowThrustLegInitialTime )
{
    ...
    accelerationSettingsMap[ centralBody ].push_back(
            std::make_shared< simulation_setup::AccelerationSettings >( basic_astrodynamics::point_mass_gravity ) );
    accelerationSettingsMap[ bodyToPropagate ].push_back( getLowThrustLegAccelerationSettings(
        ...
        const std::shared_ptr< propagators::DependentVariableSaveSettings > dependentVariablesToSave )
{
    ...
    terminationConditions.second =
            std::make_shared< propagators::PropagationTimeTerminationSettings >( lowThrustLeg->getTimeOfFlight( ), true );

    // Compute state vector at half of the time of flight.
    double independentVariableAtHalfTimeOfFlight = lowThrustLeg->convertTimeToIndependentVariable( lowThrustLeg->getTimeOfFlight( ) / 2.0 );
    Eigen::Vector6d stateAtHalfOfTimeOfFlight = lowThrustLeg->computeCurrentStateVector( independentVariableAtHalfTimeOfFlight );
                                   ...
                                   const std::shared_ptr< propagators::DependentVariableSaveSettings >& dependentVariablesToSave )
{
    ...
    basic_astrodynamics::AccelerationMap lowThrustTrajectoryAccelerations =
            retrieveLowThrustAccelerationMap( lowThrustLeg, bodies, bodyToPropagate, centralBody, specificImpulseFunction, 0.0 );
    ...
    std::pair< std::shared_ptr< propagators::TranslationalStatePropagatorSettings< double > >,
               std::shared_ptr< propagators::TranslationalStatePropagatorSettings< double > > >
            translationalStatePropagatorSettings = createLowThrustTranslationalStatePropagatorSettings(
    ...
    std::pair< std::shared_ptr< propagators::MassPropagatorSettings< double > >,
               std::shared_ptr< propagators::MassPropagatorSettings< double > > >
    ...
    massPropagatorSettings.first = std::make_shared< propagators::MassPropagatorSettings< double > >(
            std::vector< std::string >{ bodyToPropagate },
            massRateModels,
            ( Eigen::Matrix< double, 1, 1 >( ) << massHalfOfTimeOfFlight ).finished( ),
            terminationConditions.first );
    ...
    std::pair< std::vector< std::shared_ptr< propagators::SingleArcPropagatorSettings< double > > >,
               std::vector< std::shared_ptr< propagators::SingleArcPropagatorSettings< double > > > >
