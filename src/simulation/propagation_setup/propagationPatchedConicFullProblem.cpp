        ...
        const std::string& frameOrientation )
{
    ...
    simulation_setup::BodyListSettings bodySettings = simulation_setup::getDefaultBodySettings( bodiesToCreate, frameOrigin );
    ...
    for( unsigned int i = 0; i < nameTransferBodies.size( ); i++ )
    {
        ...
        bodies.at( nameTransferBodies[ i ] )
                ->setGravityFieldModel( simulation_setup::createGravityFieldModel(
        ...
        const double arrivalEccentricity )
{
    ...
    Eigen::VectorXd semiMajorAxesVector = ( Eigen::VectorXd( 2 ) << departureSemiMajorAxis, arrivalSemiMajorAxis ).finished( );
    Eigen::VectorXd eccentricityVector = ( Eigen::VectorXd( 2 ) << departureEccentricity, arrivalEccentricity ).finished( );

    return transfer_trajectories::Trajectory( numberOfLegs,
                                              transferLegTypes,
                                              ephemerisVector,
                                              gravitationalParameterVector,
                                              utilities::convertStlVectorToEigenVector( trajectoryIndependentVariables ),
                                              centralBodyGravitationalParameter,
                                              utilities::convertStlVectorToEigenVector( minimumPericenterRadii ),
                                              semiMajorAxesVector,
                                              eccentricityVector,
                                              includeDepartureDeltaV,
                                              includeArrivalDeltaV );
}
        ...
        std::map< double, Eigen::VectorXd >& dependentVariableResultCurrentLeg )
{
    ...
    propagators::propagateLambertTargeterAndFullProblem( timeOfFlight,
                                                         initialTime,
                                                         bodies,
                                                         centralBody,
                                                         propagatorSettings,
        ...
        std::map< double, Eigen::VectorXd >& dependentVariablesFromDsmToArrival )
{
    ...
    propagateKeplerianOrbitLegAndFullProblem( timeDsm - initialTime,
                                              initialTime,
                                              bodies,
                                              centralBody,
                                              legDepartureAndArrival,
                                              velocityAfterDeparture,
                                              propagatorSettingsBeforeDsm,
                                              integratorSettings,
                                              patchedConicsResultFromDepartureToDsm,
    ...
    propagateLambertTargeterAndFullProblem( timeArrival - timeDsm,
                                            timeDsm,
                                            bodies,
                                            centralBody,
                                            propagatorSettingsAfterDsm,
                                            integratorSettings,
                                            patchedConicsResultFromDsmToArrival,
        ...
        std::map< double, Eigen::VectorXd >& dependentVariablesFromDsmToArrival )
{
    ...
    propagateLambertTargeterAndFullProblem( timeDsm - initialTime,
                                            initialTime,
                                            bodies,
                                            centralBody,
                                            propagatorSettingsBeforeDsm,
        ...
        const Eigen::Vector3d& cartesianPositionAtDeparture )
{
    ...
    Eigen::Vector6d keplerianStateAtHalvedTimeOfFlight = orbital_element_conversions::propagateKeplerOrbit(
            keplerianStateAtDeparture, halvedTimeOfFlight, gravitationalParameterCentralBody );
    ...
    for( std::map< double, Eigen::VectorXd >::iterator itr = stateHistoryFullProblemForwardPropagation.begin( );
         itr != stateHistoryFullProblemForwardPropagation.end( );
         itr++ )
    {
        cartesianStateKeplerianOrbit = orbital_element_conversions::convertKeplerianToCartesianElements(
                orbital_element_conversions::propagateKeplerOrbit( orbital_element_conversions::convertCartesianToKeplerianElements(
    ...
    for( std::map< double, Eigen::VectorXd >::iterator itr = stateHistoryFullProblemBackwardPropagation.begin( );
         itr != stateHistoryFullProblemBackwardPropagation.end( );
         itr++ )
    {
        cartesianStateKeplerianOrbit = orbital_element_conversions::convertKeplerianToCartesianElements(
                orbital_element_conversions::propagateKeplerOrbit( orbital_element_conversions::convertCartesianToKeplerianElements(
        ...
        const double terminationDistanceScaler )
{
    ...
    {
        Eigen::Vector6d cartesianStateArrivalBody = bodies.at( arrivalBody )->getEphemeris( )->getCartesianState( finalTimeCurrentLeg );
    ...
    double gravitationalParameterCentralBody = bodies.at( centralBody )->getGravityFieldModel( )->getGravitationalParameter( );
    double gravitationalParameterDepartureBody = bodies.at( departureBody )->getGravityFieldModel( )->getGravitationalParameter( );
    double gravitationalParameterArrivalBody = bodies.at( arrivalBody )->getGravityFieldModel( )->getGravitationalParameter( );

    double radiusSphereOfInfluenceDeparture;
    double radiusSphereOfInfluenceArrival;
    {
        double distanceDepartureToCentralBodies =
                ( bodies.at( centralBody )->getEphemeris( )->getCartesianState( initialTimeCurrentLeg ).segment( 0, 3 ) -
                  cartesianPositionAtDeparture.segment( 0, 3 ) )
                        .norm( );
        double distanceArrivalToCentralBodies =
                ( bodies.at( centralBody )->getEphemeris( )->getCartesianState( finalTimeCurrentLeg ).segment( 0, 3 ) -
                  cartesianPositionAtArrival.segment( 0, 3 ) )
                        .norm( );
    ...
    double synodicPeriod;
    {
        ...
        double orbitalPeriodDepartureBody = basic_astrodynamics::computeKeplerOrbitalPeriod(
                ...
                gravitationalParameterCentralBody,
                gravitationalParameterDepartureBody );

        double orbitalPeriodArrivalBody = basic_astrodynamics::computeKeplerOrbitalPeriod(
                orbital_element_conversions::convertCartesianToKeplerianElements(
                        bodies.at( arrivalBody )->getEphemeris( )->getCartesianState( initialTimeCurrentLeg ),
                        gravitationalParameterCentralBody )[ orbital_element_conversions::semiMajorAxisIndex ],
                gravitationalParameterCentralBody,
                gravitationalParameterArrivalBody );
    ...
    if( !useBackwardLegToDepartureBody )
    {
        ...
        forwardPropagationTerminationSettingsList.push_back( std::make_shared< PropagationDependentVariableTerminationSettings >(
                std::make_shared< SingleDependentVariableSaveSettings >(
                        relative_distance_dependent_variable, bodyToPropagate, arrivalBody ),
                terminationDistanceScaler * radiusSphereOfInfluenceArrival,
                true ) );
    ...
    else
    {
        ...
        backwardPropagationTerminationSettingsList.push_back( std::make_shared< PropagationDependentVariableTerminationSettings >(
                std::make_shared< SingleDependentVariableSaveSettings >(
                        relative_distance_dependent_variable, bodyToPropagate, departureBody ),
                terminationDistanceScaler * radiusSphereOfInfluenceDeparture,
                true ) );
...
getSingleLegSphereOfInfluenceTerminationSettings( simulation_setup::SystemOfBodies& bodies,
                                                  ...
                                                  const double terminationDistanceScaler )
{
    return std::make_pair( getSingleLegPartSphereOfInfluenceTerminationSettings( bodies,
                                                                                 bodyToPropagate,
                                                                                 centralBody,
                                                                                 departureBody,
                                                                                 arrivalBody,
                                                                                 initialTimeCurrentLeg,
                                                                                 finalTimeCurrentLeg,
                                                                                 true,
                                                                                 terminationDistanceScaler ),
                           getSingleLegPartSphereOfInfluenceTerminationSettings( bodies,
                                                                                 bodyToPropagate,
                                                                                 centralBody,
                                                                                 departureBody,
                                                                                 arrivalBody,
                                   ...
                                   const double terminationDistanceScaler )
{
    ...
    transfer_trajectories::Trajectory trajectory = propagators::createTransferTrajectoryObject( bodies,
                                                                                                transferBodyOrder,
                                                                                                centralBody,
                                                                                                legTypeVector,
                                                                                                trajectoryVariableVector,
                                                                                                minimumPericenterRadiiVector,
                                                                                                true,
                                                                                                semiMajorAxesVector[ 0 ],
                                                                                                eccentricitiesVector[ 0 ],
                                                                                                true,
                                                                                                semiMajorAxesVector[ 1 ],
                                                                                                eccentricitiesVector[ 1 ] );
    ...
    std::vector< std::pair< std::shared_ptr< propagators::PropagationTerminationSettings >,
                            std::shared_ptr< propagators::PropagationTerminationSettings > > >
            terminationSettings;
    ...
    for( int i = 0; i < numberOfLegs - 1; i++ )
    {
        if( ( ( legTypeVector[ i ] == transfer_trajectories::mga_Departure || legTypeVector[ i ] == transfer_trajectories::mga_Swingby ) ) )
        {
            ...
            {
                terminationSettings.push_back( getSingleLegSphereOfInfluenceTerminationSettings( bodies,
                                                                                                 bodyToPropagate,
                                                                                                 centralBody,
                                                                                                 transferBodyOrder.at( i ),
                                                                                                 transferBodyOrder.at( i + 1 ),
                                                                                                 initialTimeCurrentLeg,
                                                                                                 finalTimeCurrentLeg,
                                                                                                 terminationDistanceScaler ) );
            }
    ...
    for( int i = 0; i < numberOfLegs - 1; i++ )
    {
        // If the leg includes one DSM, add another element to the propagator settings vector to take the second part of the leg into
        // account.
        if( ( legTypeVector[ i ] == transfer_trajectories::mga_Departure || legTypeVector[ i ] == transfer_trajectories::mga_Swingby ) )
        {
            ...
            propagatorSettings.push_back( std::make_pair( std::make_shared< TranslationalStatePropagatorSettings< double > >(
                                                                  centralBodyPropagation,
                                                                  accelerationMap[ i ],
                                                                  bodyToPropagatePropagation,
                                                                  initialState,
                                                                  terminationSettings[ counterLegsIncludingDsm ].first,
                                                                  propagator,
                                                                  currentDependentVariablesToSave ),
                                                          std::make_shared< TranslationalStatePropagatorSettings< double > >(
                                                                  centralBodyPropagation,
                                                                  accelerationMap[ i ],
                                                                  bodyToPropagatePropagation,
                                                                  initialState,
                                                                  terminationSettings[ counterLegsIncludingDsm ].second,
                                                                  propagator,
                                                                  currentDependentVariablesToSave ) ) );
        ...
        std::map< int, std::map< double, Eigen::VectorXd > >& dependentVariableResultForEachLeg )
{
    ...
    // Include manoeuvres between transfer bodies when required (considering that a deep space manoeuvre divides a leg into two smaller
    // ones).
    ...
    for( int i = 0; i < numberOfLegs - 1; i++ )
    {
        ...
        if( legTypeVector[ i ] == transfer_trajectories::mga_Departure || legTypeVector[ i ] == transfer_trajectories::mga_Swingby )
        {
            ...
            propagators::propagateMgaWithoutDsmAndFullProblem( bodies,
                                                               departureAndArrivalBodies,
                                                               patchedConicCentralBody,
                                                               positionVector[ counterLegs ],
                                                               positionVector[ counterLegs + 1 ],
                                                               timeVector[ counterLegs ],
                                                               timeVector[ counterLegs + 1 ] - timeVector[ counterLegs ],
                                                               propagatorSettings[ counterLegs ],
                                                               integratorSettings,
                                                               patchedConicsResultCurrentLeg,
        ...
        if( legTypeVector[ i ] == transfer_trajectories::mga1DsmVelocity_Departure ||
            legTypeVector[ i ] == transfer_trajectories::mga1DsmVelocity_Swingby )
        {
            std::vector< std::string > departureAndArrivalBodies;
            departureAndArrivalBodies.push_back( bodiesAndManoeuvresOrder[ counterLegs ] );
            ...
            propagators::propagateMga1DsmVelocityAndFullProblem( bodies,
                                                                 departureAndArrivalBodies,
                                                                 bodiesAndManoeuvresOrder[ counterLegs + 1 ],
                                                                 patchedConicCentralBody,
                                                                 positionVector[ counterLegs ],
                                                                 positionVector[ counterLegs + 1 ],
                                                                 positionVector[ counterLegs + 2 ],
                                                                 timeVector[ counterLegs ],
                                                                 timeVector[ counterLegs + 1 ],
                                                                 timeVector[ counterLegs + 2 ],
                                                                 legTypeVector[ i ],
                                                                 trajectoryVariableVectorLeg,
                                                                 semiMajorAxesVector[ 0 ],
                                                                 eccentricitiesVector[ 0 ],
                                                                 velocityAfterDeparture,
                                                                 velocityBeforeArrival,
                                                                 propagatorSettings[ counterLegs ],
                                                                 propagatorSettings[ counterLegs + 1 ],
                                                                 integratorSettings,
                                                                 patchedConicsResultFromDepartureToDsm,
                                                                 fullProblemResultFromDepartureToDsm,
                                                                 dependentVariablesFromDepartureToDsm,
            ...
            legTypeVector[ i ] == transfer_trajectories::mga1DsmPosition_Swingby )
        {
            ...
            // Compute patched conics and full problem results along the leg.
            propagators::propagateMga1DsmPositionAndFullProblem( bodies,
                                                                 departureAndArrivalBodies,
                                                                 bodiesAndManoeuvresOrder[ counterLegs + 1 ],
                                                                 patchedConicCentralBody,
                                                                 positionVector[ counterLegs ],
                                                                 ...
                                                                 legTypeVector[ i ],
                                                                 trajectoryVariableVectorLeg,
                                                                 minimumPericenterRadiiVector[ i ],
                                                                 semiMajorAxesVector[ 0 ],
                                                                 eccentricitiesVector[ 0 ],
                                                                 velocityAfterDeparture,
                                                                 velocityBeforeArrival,
        ...
        const TranslationalPropagatorType propagator )
{
    std::vector< std::pair< std::shared_ptr< propagators::TranslationalStatePropagatorSettings< double > >,
                            std::shared_ptr< propagators::TranslationalStatePropagatorSettings< double > > > >
            propagatorSettings = getPatchedConicPropagatorSettings( bodies,
                                                                    accelerationMap,
                                                                    transferBodyOrder,
                                                                    centralBody,
                                                                    bodyToPropagate,
        ...
        const TranslationalPropagatorType propagator )
{
    ...
    fullPropagationPatchedConicsTrajectory( bodies,
                                            ...
                                            dependentVariableResultForEachLeg,
                                            terminationSphereOfInfluence,
                                            dependentVariablesToSaveForEachLeg,
                                            propagator );
}
        ...
        const TranslationalPropagatorType propagator )
{
    ...
    stateDifferenceAtArrivalAndDepartureForEachLeg = getDifferenceFullProblemWrtPatchedConicsTrajectory( bodies,
                                                                                                         accelerationMapForEachLeg,
                                                                                                         transferBodyOrder,
                                                                                                         centralBody,
                                                                                                         bodyToPropagate,
