        ...
        const std::string& nameOfBodyExertingTorque )
{
    ...
    {
        throw std::runtime_error( "Error when making aerodynamic torque, body " + nameOfBodyUndergoingTorque +
                                  "has no aerodynamic coefficients." );
    }

    if( bodyExertingTorque->getAtmosphereModel( ) == nullptr )
    {
        throw std::runtime_error( "Error when making aerodynamic torque, central body " + nameOfBodyExertingTorque +
    ...
    std::shared_ptr< aerodynamics::AtmosphericFlightConditions > bodyFlightConditions =
            std::dynamic_pointer_cast< aerodynamics::AtmosphericFlightConditions >( bodyUndergoingTorque->getFlightConditions( ) );
    ...
    if( bodyFlightConditions == nullptr && bodyUndergoingTorque->getFlightConditions( ) == nullptr )
    {
        ...
        bodyUndergoingTorque->setFlightConditions( bodyFlightConditions );
    }
    ...
    reference_frames::AerodynamicsReferenceFrames torqueFrame =
            aerodynamics::getCompleteFrameForCoefficients( aerodynamicCoefficients->getMomentCoefficientsFrame( ) );
        ...
        const std::string& nameOfBodyExertingTorque )
{
    ...
    return std::make_shared< gravitation::SecondDegreeGravitationalTorqueModel >( positionOfBodySubjectToTorqueFunction,
                                                                                  gravitationalParameterOfAttractingBodyFunction,
                                                                                  inertiaTensorOfRotatingBodyFunction,
                                                                                  positionOfBodyExertingTorqueFunction,
                                                                                  rotationToBodyFixedFrameFunction );
}
        ...
        const std::string& nameOfBodyExertingTorque )
{
    ...
    std::shared_ptr< AccelerationSettings > sphericalHarmonicAccelerationSettings =
            std::make_shared< SphericalHarmonicAccelerationSettings >( sphericalHarmonicTorqueSettings->maximumDegree_,
    ...
    std::shared_ptr< gravitation::SphericalHarmonicsGravitationalAccelerationModel > sphericalHarmonicAcceleration =
            std::dynamic_pointer_cast< gravitation::SphericalHarmonicsGravitationalAccelerationModel >(
                    createSphericalHarmonicsGravityAcceleration( bodyExertingTorque,
        ...
        const SystemOfBodies& bodies )
{
    ...
    std::shared_ptr< electromagnetism::IsotropicPointSourceRadiationPressureAcceleration > radiationPressureAcceleration =
            std::dynamic_pointer_cast< electromagnetism::IsotropicPointSourceRadiationPressureAcceleration >(
                    createRadiationPressureAccelerationModel(
                            bodyExertingTorque, bodyUndergoingTorque, nameOfBodyExertingTorque, nameOfBodyUndergoingTorque, bodies ) );
    ...
    if( massProperties == nullptr )
    {
        throw std::runtime_error(
                "Error when making radiation pressure torque model, no mass properties are defined, which are required for center of mass "
                "definition" );
    }
    else
    {
...
std::shared_ptr< basic_astrodynamics::CustomTorqueModel > createCustomTorqueModel( const std::shared_ptr< TorqueSettings > torqueSettings,
                                                                                   const std::string& nameOfBodyUndergoingTorque )
{
    std::shared_ptr< CustomTorqueSettings > customTorqueSettings = std::dynamic_pointer_cast< CustomTorqueSettings >( torqueSettings );
    if( customTorqueSettings == nullptr )
    {
        throw std::runtime_error( "Error, expected custom torque settings when making torque model on " + nameOfBodyUndergoingTorque );
    }
                                                                       ...
                                                                       const SystemOfBodies& bodies )
{
    ...
    switch( torqueSettings->torqueType_ )
    {
        case basic_astrodynamics::inertial_torque: {
            torqueModel = createInertialTorqueModel( bodyUndergoingTorque, nameOfBodyUndergoingTorque );
            break;
        }
        ...
        case basic_astrodynamics::radiation_pressure_torque: {
            torqueModel = createRadiationPressureTorqueModel( bodyUndergoingTorque,
                                                              bodyExertingTorque,
                                                              torqueSettings,
                                                              nameOfBodyUndergoingTorque,
                                                              nameOfBodyExertingTorque,
        ...
        case basic_astrodynamics::custom_torque: {
            torqueModel = createCustomTorqueModel( torqueSettings, nameOfBodyUndergoingTorque );
            break;
        }
        default:
            throw std::runtime_error( "Error, did not recognize type " + std::to_string( torqueSettings->torqueType_ ) +
                                                           ...
                                                           const std::vector< std::string >& propagatedBodies )
{
    ...
    for( SelectedTorqueMap::const_iterator acceleratedBodyIterator = selectedTorquePerBody.begin( );
         acceleratedBodyIterator != selectedTorquePerBody.end( );
         acceleratedBodyIterator++ )
    {
        if( bodies.count( acceleratedBodyIterator->first ) == 0 )
        {
            throw std::runtime_error( "Error, could not find body " + acceleratedBodyIterator->first + " when making torque model map." );
        }
        else
        {
            torqueModelMap[ acceleratedBodyIterator->first ][ acceleratedBodyIterator->first ].push_back(
                    createTorqueModel( bodies.at( acceleratedBodyIterator->first ),
                                       bodies.at( acceleratedBodyIterator->first ),
                                       std::make_shared< TorqueSettings >( basic_astrodynamics::inertial_torque ),
                                       acceleratedBodyIterator->first,
                                       acceleratedBodyIterator->first,
                                       bodies ) );

            for( std::map< std::string, std::vector< std::shared_ptr< TorqueSettings > > >::const_iterator acceleratingBodyIterator =
                         acceleratedBodyIterator->second.begin( );
                 acceleratingBodyIterator != acceleratedBodyIterator->second.end( );
                 acceleratingBodyIterator++ )
            {
                if( bodies.count( acceleratingBodyIterator->first ) == 0 )
                {
                    throw std::runtime_error( "Error, could not find body " + acceleratingBodyIterator->first +
                                              " when making torque model map." );
                }
                ...
                for( unsigned int i = 0; i < acceleratingBodyIterator->second.size( ); i++ )
                {
                    torqueModelMap[ acceleratedBodyIterator->first ][ acceleratingBodyIterator->first ].push_back(
                            createTorqueModel( bodies.at( acceleratedBodyIterator->first ),
                                               bodies.at( acceleratingBodyIterator->first ),
                                               acceleratingBodyIterator->second.at( i ),
                                               acceleratedBodyIterator->first,
                                               acceleratingBodyIterator->first,
                                               bodies ) );
                }
            }
        }
