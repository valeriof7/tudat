        ...
        const bool isCentralBody )
{
    ...
    switch( accelerationSettings->accelerationType_ )
    {
        case point_mass_gravity:
            accelerationModel = createCentralGravityAcceleratioModel( bodyUndergoingAcceleration,
        ...
        case spherical_harmonic_gravity:
            accelerationModel = createSphericalHarmonicsGravityAcceleration( bodyUndergoingAcceleration,
                                                                             bodyExertingAcceleration,
                                                                             nameOfBodyUndergoingAcceleration,
                                                                             nameOfBodyExertingAcceleration,
        ...
        case mutual_spherical_harmonic_gravity:
            accelerationModel = createMutualSphericalHarmonicsGravityAcceleration( bodyUndergoingAcceleration,
                                                                                   bodyExertingAcceleration,
                                                                                   nameOfBodyUndergoingAcceleration,
                                                                                   nameOfBodyExertingAcceleration,
                                                                                   accelerationSettings,
        ...
        case polyhedron_gravity:
            ...
            break;
        case ring_gravity:
            accelerationModel = createRingGravityAcceleration( bodyUndergoingAcceleration,
                                                               bodyExertingAcceleration,
                                                               nameOfBodyUndergoingAcceleration,
                                                               nameOfBodyExertingAcceleration,
                                                               sumGravitationalParameters );
        ...
        const bool useCentralBodyFixedFrame )
{
    ...
    {
        throw std::runtime_error( std::string( "Error, gravity field model not set when making central " ) +
...
std::shared_ptr< gravitation::SphericalHarmonicsGravitationalAccelerationModel > createSphericalHarmonicsGravityAcceleration(
        const std::shared_ptr< Body > bodyUndergoingAcceleration,
        const std::shared_ptr< Body > bodyExertingAcceleration,
        const std::string& nameOfBodyUndergoingAcceleration,
        const std::string& nameOfBodyExertingAcceleration,
        ...
        const bool useDegreeZeroTerm )
{
    ...
    {
        throw std::runtime_error( std::string( "Error, acceleration settings inconsistent " ) +
                                  " making sh gravitational acceleration of " + nameOfBodyExertingAcceleration + " on " +
                                  nameOfBodyUndergoingAcceleration );
    }
    else
    {
        ...
        else
        {
            ...
            if( rotationalEphemeris->getTargetFrameOrientation( ) != sphericalHarmonicsGravityField->getFixedReferenceFrame( ) )
            {
                throw std::runtime_error( "Warning when making spherical harmonic acceleration on body " +
                                          nameOfBodyUndergoingAcceleration + ", rotation model found for " +
                                          nameOfBodyExertingAcceleration +
                                          " is incompatible, frames are: " + rotationalEphemeris->getTargetFrameOrientation( ) + " and " +
            ...
            {
                cosineCoefficientFunction = std::bind( &setDegreeAndOrderCoefficientToZero, originalCosineCoefficientFunction );
            }
            else
            {
...
std::shared_ptr< gravitation::MutualSphericalHarmonicsGravitationalAccelerationModel > createMutualSphericalHarmonicsGravityAcceleration(
        const std::shared_ptr< Body > bodyUndergoingAcceleration,
        const std::shared_ptr< Body > bodyExertingAcceleration,
        const std::string& nameOfBodyUndergoingAcceleration,
        const std::string& nameOfBodyExertingAcceleration,
        ...
        const bool acceleratedBodyIsCentralBody )
{
    ...
    else
    {
        ...
        std::shared_ptr< SphericalHarmonicsGravityField > sphericalHarmonicsGravityFieldOfBodyUndergoingAcceleration =
                std::dynamic_pointer_cast< SphericalHarmonicsGravityField >( bodyUndergoingAcceleration->getGravityFieldModel( ) );
        ...
        if( sphericalHarmonicsGravityFieldOfBodyExertingAcceleration == nullptr )
        {
            std::string errorMessage = "Error " + nameOfBodyExertingAcceleration + " does not have a spherical harmonics gravity field " +
                    "when making mutual spherical harmonics gravity acceleration on " + nameOfBodyUndergoingAcceleration;
            throw std::runtime_error( errorMessage );
        }
...
std::shared_ptr< gravitation::PolyhedronGravitationalAccelerationModel > createPolyhedronGravityAcceleration(
        const std::shared_ptr< Body > bodyUndergoingAcceleration,
        const std::shared_ptr< Body > bodyExertingAcceleration,
        const std::string& nameOfBodyUndergoingAcceleration,
        const std::string& nameOfBodyExertingAcceleration,
        const bool useCentralBodyFixedFrame )
{
    ...
    {
        throw std::runtime_error( std::string( "Error, polyhedron gravity field model not set when " ) +
                                  " making polyhedron gravitational acceleration of " + nameOfBodyExertingAcceleration + " on " +
                                  nameOfBodyUndergoingAcceleration );
    }
    else
    {
        if( rotationalEphemeris == nullptr )
        {
            throw std::runtime_error( "Warning when making polyhedron acceleration on body " + nameOfBodyUndergoingAcceleration +
        ...
        const bool useCentralBodyFixedFrame )
{
    ...
    {
        throw std::runtime_error( std::string( "Error, ring gravity field model not set when " ) +
                                  " making ring gravitational acceleration of " + nameOfBodyExertingAcceleration + " on " +
                                  nameOfBodyUndergoingAcceleration );
    }
    else
    {
        if( rotationalEphemeris == nullptr )
        {
            throw std::runtime_error( "Warning when making ring acceleration on body " + nameOfBodyUndergoingAcceleration +
                                      ", no rotation model found for " + nameOfBodyExertingAcceleration );
        }
    ...
    accelerationModelPointer = std::make_shared< ThirdBodyCentralGravityAcceleration >(
            std::dynamic_pointer_cast< CentralGravitationalAccelerationModel3d >(
                    createCentralGravityAcceleratioModel( bodyUndergoingAcceleration,
                                                          ...
                                                          nameOfBodyExertingAcceleration,
                                                          0 ) ),
            std::dynamic_pointer_cast< CentralGravitationalAccelerationModel3d >( createCentralGravityAcceleratioModel(
                    centralBody, bodyExertingAcceleration, nameOfCentralBody, nameOfBodyExertingAcceleration, 0 ) ),
            nameOfCentralBody );
                                                          ...
                                                          const std::shared_ptr< AccelerationSettings > accelerationSettings )
{
    ...
    else
    {
        ...
        {
            std::string errorMessage = "Error " + nameOfBodyExertingAcceleration + " does not have a spherical harmonics gravity field " +
                    "when making third body spherical harmonics gravity acceleration on " + nameOfBodyUndergoingAcceleration;
            throw std::runtime_error( errorMessage );
        }
        else
        {
            accelerationModel = std::make_shared< ThirdBodySphericalHarmonicsGravitationalAccelerationModel >(
                    std::dynamic_pointer_cast< SphericalHarmonicsGravitationalAccelerationModel >(
                            createSphericalHarmonicsGravityAcceleration( bodyUndergoingAcceleration,
                                                                         bodyExertingAcceleration,
                                                                         nameOfBodyUndergoingAcceleration,
                                                                         nameOfBodyExertingAcceleration,
                                                                         ...
                                                                         0 ) ),
                    std::dynamic_pointer_cast< SphericalHarmonicsGravitationalAccelerationModel >(
                            createSphericalHarmonicsGravityAcceleration( centralBody,
                                                                         bodyExertingAcceleration,
                                                                         nameOfCentralBody,
                                                                         nameOfBodyExertingAcceleration,
                                                                         sphericalHarmonicsSettings,
                                                                ...
                                                                const std::shared_ptr< AccelerationSettings > accelerationSettings )
{
    ...
    else
    {
        ...
        else
        {
            ...
            accelerationModel = std::make_shared< ThirdBodyMutualSphericalHarmonicsGravitationalAccelerationModel >(
                    std::dynamic_pointer_cast< MutualSphericalHarmonicsGravitationalAccelerationModel >(
                            createMutualSphericalHarmonicsGravityAcceleration( bodyUndergoingAcceleration,
                                                                               bodyExertingAcceleration,
                                                                               nameOfBodyUndergoingAcceleration,
                                                                               nameOfBodyExertingAcceleration,
                                                                               ...
                                                                               0 ) ),
                    std::dynamic_pointer_cast< MutualSphericalHarmonicsGravitationalAccelerationModel >(
                            createMutualSphericalHarmonicsGravityAcceleration( centralBody,
                                                                               bodyExertingAcceleration,
                                                                               nameOfCentralBody,
                                                                               nameOfBodyExertingAcceleration,
                                                                               accelerationSettingsForCentralBodyAcceleration,
                                                                               0,
                                                                               1 ) ),
                    nameOfCentralBody );
        }
        ...
        const std::string& nameOfCentralBody )
{
    ...
    if( polyhedronGravityField == nullptr )
    {
        std::string errorMessage = "Error " + nameOfBodyExertingAcceleration + " does not have a polyhedron gravity field " +
                "when making third body polyhedron gravity acceleration on " + nameOfBodyUndergoingAcceleration;
        throw std::runtime_error( errorMessage );
    }
    else
    {
        accelerationModel = std::make_shared< ThirdBodyPolyhedronGravitationalAccelerationModel >(
                std::dynamic_pointer_cast< PolyhedronGravitationalAccelerationModel >(
                        createPolyhedronGravityAcceleration( bodyUndergoingAcceleration,
                                                             bodyExertingAcceleration,
                                                             nameOfBodyUndergoingAcceleration,
                                                             nameOfBodyExertingAcceleration,
                                                             0 ) ),
                std::dynamic_pointer_cast< PolyhedronGravitationalAccelerationModel >( createPolyhedronGravityAcceleration(
                        centralBody, bodyExertingAcceleration, nameOfCentralBody, nameOfBodyExertingAcceleration, 0 ) ),
                nameOfCentralBody );
    }
        ...
        const std::string& nameOfCentralBody )
{
    ...
    if( ringGravityField == nullptr )
    {
        std::string errorMessage = "Error " + nameOfBodyExertingAcceleration + " does not have a ring gravity field " +
                "when making third body ring gravity acceleration on " + nameOfBodyUndergoingAcceleration;
        throw std::runtime_error( errorMessage );
    }
    else
    {
        accelerationModel = std::make_shared< ThirdBodyRingGravitationalAccelerationModel >(
                std::dynamic_pointer_cast< RingGravitationalAccelerationModel >(
                        createPolyhedronGravityAcceleration( bodyUndergoingAcceleration,
                                                             ...
                                                             nameOfBodyExertingAcceleration,
                                                             0 ) ),
                std::dynamic_pointer_cast< RingGravitationalAccelerationModel >( createPolyhedronGravityAcceleration(
                        centralBody, bodyExertingAcceleration, nameOfCentralBody, nameOfBodyExertingAcceleration, 0 ) ),
                nameOfCentralBody );
    }
        ...
        const std::string& nameOfBodyExertingAcceleration )
{
    ...
    {
        throw std::runtime_error( "Error when making aerodynamic acceleration, body " + nameOfBodyUndergoingAcceleration +
    ...
    auto panelledAerodynamicCoefficientInterface = std::dynamic_pointer_cast< PanelledAerodynamicCoefficientInterface >( aerodynamicCoefficients );
        ...
        const std::shared_ptr< AccelerationSettings > accelerationSettings )
{
    ...
    {
        throw std::runtime_error( "Error when making radiation pressure acceleration, body " + sourceName +
    ...
    {
        throw std::runtime_error( "Error when making radiation pressure acceleration, body " + targetName +
                                  " has no radiation pressure target models." );
    }
    else
    {
        targetModel = getRadiationPressureTargetModelOfType(
                target, targetModelType, " when making radiation pressure acceleration due to " + sourceName + " " );
    }

    // Cast source and target models for type checks
    auto isotropicPointRadiationSourceModel =
            std::dynamic_pointer_cast< electromagnetism::IsotropicPointRadiationSourceModel >( source->getRadiationSourceModel( ) );
    auto paneledRadiationSourceModel =
            std::dynamic_pointer_cast< electromagnetism::PaneledRadiationSourceModel >( source->getRadiationSourceModel( ) );
    ...
    for( auto& occultingBodyName: sourceToTargetOccultingBodies )
    {
        if( occultingBodyName == sourceName )
        {
            throw std::runtime_error(
                    "Error when making radiation pressure acceleration, source body cannot "
                    "act as occulting body." );
        }
        if( occultingBodyName == targetName )
        {
            throw std::runtime_error(
                    "Error when making radiation pressure acceleration, target body cannot "
                    "act as occulting body." );
        }
    }
    ...
    {
        throw std::runtime_error(
                "Error when making radiation pressure acceleration, radiation source model "
                "for body " +
                sourceName + " is not supported." );
    }
}
        ...
        const std::shared_ptr< AccelerationSettings > accelerationSettings )
{
    ...
    std::shared_ptr< simulation_setup::YarkovskyAccelerationSettings > yarkovskySettings =
            std::dynamic_pointer_cast< simulation_setup::YarkovskyAccelerationSettings >( accelerationSettings );
        ...
        const std::string& nameOfBodyUndergoingAcceleration )
{
    std::shared_ptr< CustomAccelerationSettings > customAccelerationSettings =
            std::dynamic_pointer_cast< CustomAccelerationSettings >( accelerationSettings );
...
std::shared_ptr< EmpiricalAcceleration > createEmpiricalAcceleration( const std::shared_ptr< Body > bodyUndergoingAcceleration,
                                                                      const std::shared_ptr< Body > bodyExertingAcceleration,
                                                                      const std::string& nameOfBodyUndergoingAcceleration,
                                                                      const std::string& nameOfBodyExertingAcceleration,
                                                                      const std::shared_ptr< AccelerationSettings > accelerationSettings )
...
std::shared_ptr< propulsion::ThrustAcceleration > createThrustAcceleratioModel(
        const std::shared_ptr< AccelerationSettings > accelerationSettings,
        const SystemOfBodies& bodies,
        const std::string& nameOfBodyUndergoingThrust )
{
    ...
    else
    {
        for( unsigned int i = 0; i < thrustAccelerationSettings->engineIds_.size( ); i++ )
        {
            ...
            {
                engineModelsForAcceleration.push_back( engineModels.at( thrustAccelerationSettings->engineIds_.at( i ) ) );
            }
        }
    }
        ...
        const std::shared_ptr< AccelerationSettings > accelerationSettings )
{
    ...
    if( tidalAccelerationSettings->useTideRaisedOnPlanet_ )
    {
        if( std::dynamic_pointer_cast< gravitation::SphericalHarmonicsGravityField >( bodyExertingAcceleration->getGravityFieldModel( ) ) ==
            nullptr )
        {
    ...
    if( tidalAccelerationSettings->useTideRaisedOnPlanet_ )
    {
        ...
        else
        {
            return std::make_shared< DirectTidalDissipationAcceleration >( std::bind( &Body::getState, bodyUndergoingAcceleration ),
                                                                           ...
                                                                           planetAngularVelocityVectorFunction,
                                                                           tidalAccelerationSettings->k2LoveNumber_,
                                                                           tidalAccelerationSettings->inverseTidalQualityFactor_,
                                                                           tidalAccelerationSettings->tidalPeriod_,
                                                                           referenceRadius,
                                                                           tidalAccelerationSettings->includeDirectRadialComponent_ );
        }
    }
        ...
        const std::shared_ptr< AccelerationSettings > accelerationSettings )
{
    ...
    {
        throw std::runtime_error(
                "Error when creating momentum wheel desaturation acceleration, exerting and undergoing bodies are not the same" );
    }
        ...
        const SystemOfBodies& bodies )
{
    ...
    switch( accelerationSettings->accelerationType_ )
    {
        ...
        case ring_gravity:
            ...
            break;
        case aerodynamic:
            accelerationModelPointer = createAerodynamicAcceleratioModel( bodyUndergoingAcceleration,
                                                                          bodyExertingAcceleration,
                                                                          nameOfBodyUndergoingAcceleration,
                                                                          nameOfBodyExertingAcceleration );
            break;
        ...
        case relativistic_correction_acceleration:
            accelerationModelPointer = createRelativisticCorrectionAcceleration( bodyUndergoingAcceleration,
                                                                                 bodyExertingAcceleration,
                                                                                 nameOfBodyUndergoingAcceleration,
                                                                                 nameOfBodyExertingAcceleration,
                                                                                 accelerationSettings,
        ...
        case direct_tidal_dissipation_in_central_body_acceleration:
            accelerationModelPointer = createDirectTidalDissipationAcceleration( bodyUndergoingAcceleration,
                                                                                 bodyExertingAcceleration,
                                                                                 nameOfBodyUndergoingAcceleration,
                                                                                 nameOfBodyExertingAcceleration,
                                                                                 accelerationSettings );
        ...
        case momentum_wheel_desaturation_acceleration:
            accelerationModelPointer = createMomentumWheelDesaturationAcceleration( bodyUndergoingAcceleration,
                                                                                    bodyExertingAcceleration,
                                                                                    nameOfBodyUndergoingAcceleration,
                                                                                    nameOfBodyExertingAcceleration,
                                                                                    accelerationSettings );
        ...
        case yarkovsky_acceleration:
            accelerationModelPointer = createYarkovskyAcceleration( bodyUndergoingAcceleration,
                                                                    bodyExertingAcceleration,
                                                                    nameOfBodyUndergoingAcceleration,
                                                                    nameOfBodyExertingAcceleration,
                                                                    accelerationSettings );
        ...
        case custom_acceleration:
            ...
            break;
        default:
            throw std::runtime_error( std::string( "Error, acceleration model " ) +
                                      std::to_string( accelerationSettings->accelerationType_ ) +
                                      " not recognized when making acceleration model of" + nameOfBodyExertingAcceleration + " on " +
                                      nameOfBodyUndergoingAcceleration );
            break;
    }
    return accelerationModelPointer;
...
void addEihAccelerations( const SystemOfBodies& bodies,
                          ...
                          basic_astrodynamics::AccelerationMap& accelerationMap )
{
    ...
    for( auto it: centralBodies )
    {
        ...
        {
            throw std::runtime_error(
                    "Error when creating EIH accelerations, implementation currently requires central body equal to SSB" );
        }
    }
    if( bodies.getFrameOrigin( ) != "SSB" )
    {
        throw std::runtime_error(
                "Error when creating EIH accelerations, implementation currently requires global frame origin to be equal to SSB" );
    }
    ...
    bool duplicateEntriesExist =
            std::adjacent_find( firstBodyInvolvedBodies.begin( ), firstBodyInvolvedBodies.end( ) ) != firstBodyInvolvedBodies.end( );
    if( duplicateEntriesExist )
    {
        throw std::runtime_error( "Error when creating EIH equations, acceleration settings for " + eihUndergoingBodies.at( 0 ) +
                                  " duplicate entries found for involved bodies" );
    }
    ...
    for( unsigned int i = 1; i < eihBodyListToCheck.size( ); i++ )
    {
        ...
        {
            throw std::runtime_error( "Error when creating EIH equations, acceleration settings for " + eihUndergoingBodies.at( 0 ) +
                                      " and " + eihUndergoingBodies.at( i ) +
                                      " are incompatible (different number of bodies exerting EIH acceleration found)" );
        }
        ...
        for( unsigned int j = 0; j < firstBodyInvolvedBodies.size( ); j++ )
        {
            ...
            {
                throw std::runtime_error( "Error when creating EIH equations, acceleration settings for " + eihExertingBodies.at( 0 ) +
                                          " and " + eihExertingBodies.at( i ) +
                                          " are incompatible (ordered list of involved bodies are different)" );
            }
        }
    }
    ...
    for( unsigned int j = 0; j < firstBodyInvolvedBodies.size( ); j++ )
    {
        if( std::find( eihExertingBodies.begin( ), eihExertingBodies.end( ), firstBodyInvolvedBodies.at( j ) ) == eihExertingBodies.end( ) )
        {
    ...
    for( unsigned int i = 0; i < eihUndergoingBodies.size( ); i++ )
    {
        ...
        std::vector< std::string >::iterator entryToRemove =
                std::find( currentEihExertingAccelerations.begin( ), currentEihExertingAccelerations.end( ), eihUndergoingBodies.at( i ) );
...
SelectedAccelerationList orderSelectedAccelerationMap( const SelectedAccelerationMap& selectedAccelerationsPerBody )
{
    ...
    for( SelectedAccelerationMap::const_iterator bodyIterator = selectedAccelerationsPerBody.begin( );
         bodyIterator != selectedAccelerationsPerBody.end( );
         bodyIterator++ )
    {
        ...
        // current body.
        std::map< std::string, std::vector< std::shared_ptr< AccelerationSettings > > > accelerationsForBody = bodyIterator->second;
        ...
        std::vector< std::pair< std::string, std::shared_ptr< AccelerationSettings > > > currentBodyAccelerations;
        int counter = 0;
        // Iterate over all bodies exerting an acceleration
        for( std::map< std::string, std::vector< std::shared_ptr< AccelerationSettings > > >::iterator body2Iterator =
                     accelerationsForBody.begin( );
             body2Iterator != accelerationsForBody.end( );
             body2Iterator++ )
        {
        ...
        if( thrustAccelerationIndices.size( ) > 0 && aerodynamicAccelerationIndices.size( ) > 0 )
        {
            ...
            {
                std::vector< std::pair< std::string, std::shared_ptr< AccelerationSettings > > > orderedAccelerationSettings =
...
inline basic_astrodynamics::AccelerationMap createAccelerationModelsMap( const SystemOfBodies& bodies,
                                                                         ...
                                                                         const std::map< std::string, std::string >& centralBodies )
{
    ...
    SelectedAccelerationList orderedAccelerationPerBody = orderSelectedAccelerationMap( selectedAccelerationPerBody );

    // Iterate over all bodies which are undergoing acceleration
    for( SelectedAccelerationList::const_iterator bodyIterator = orderedAccelerationPerBody.begin( );
         bodyIterator != orderedAccelerationPerBody.end( );
         bodyIterator++ )
    {
        ...
        if( !ephemerides::isFrameInertial( currentCentralBodyName ) )
        {
            ...
            {
                throw std::runtime_error( std::string( "Error, could not find non-inertial central body " ) + currentCentralBodyName +
                                          " of " + bodyUndergoingAcceleration + " when making acceleration model." );
            }
            else
            {
        ...
        if( bodies.count( bodyUndergoingAcceleration ) == 0 )
        {
            throw std::runtime_error( std::string( "Error when making acceleration models, requested forces" ) + "acting on body " +
                                      bodyUndergoingAcceleration + ", but no such body found in map of bodies" );
        }
        ...
        // Retrieve list of required acceleration model types and bodies exerting accelerationd on
        // current body.
        std::vector< std::pair< std::string, std::shared_ptr< AccelerationSettings > > > accelerationsForBody = bodyIterator->second;
        ...
        for( unsigned int i = 0; i < accelerationsForBody.size( ); i++ )
        {
            ...
            else if( accelerationsForBody.at( i ).second->accelerationType_ == basic_astrodynamics::einstein_infeld_hoffmann_acceleration )
            {
                if( orderedEihBodies.count( bodyUndergoingAcceleration ) > 0 )
                {
                    if( std::find( orderedEihBodies.at( bodyUndergoingAcceleration ).begin( ),
                                   ...
                                   bodyExertingAcceleration ) != orderedEihBodies.at( bodyUndergoingAcceleration ).end( ) )
                    {
            ...
            else
            {
                ...
                // Create acceleration model.
                mapOfAccelerationsForBody[ bodyExertingAcceleration ].push_back( currentAcceleration );
            }
        }
        ...
        for( unsigned int i = 0; i < thrustAccelerationSettings.size( ); i++ )
        {
            ...
            // Create acceleration model.
            mapOfAccelerationsForBody[ thrustAccelerationSettings.at( i ).first ].push_back( currentAcceleration );
        }
    ...
    {
        addEihAccelerations( bodies, orderedEihBodies, centralBodies, accelerationModelMap );
    }
...
basic_astrodynamics::AccelerationMap createAccelerationModelsMap( const SystemOfBodies& bodies,
                                                                  const SelectedAccelerationMap& selectedAccelerationPerBody,
