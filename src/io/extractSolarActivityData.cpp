...
#include "tudat/io/parseSolarActivityData.h"  // For the FieldTypes
        ...
        tudat::input_output::parsed_data_vector_utilities::ParsedDataLineMapPtr data )
{
    ...
    solarActivityContainer->bartelsSolarRotationNumber = getField< unsigned int >( data, bartelsSolarRotationNumber );
    solarActivityContainer->dayOfBartelsCycle = getField< unsigned int >( data, dayOfBartelsCycle );
    solarActivityContainer->solarRadioFlux107Adjusted = getField< double >( data, solarRadioFlux107Adjusted );
    solarActivityContainer->centered81DaySolarRadioFlux107Adjusted = getField< double >( data, centered81DaySolarRadioFlux107Adjusted );
    solarActivityContainer->last81DaySolarRadioFlux107Adjusted = getField< double >( data, last81DaySolarRadioFlux107Adjusted );
    solarActivityContainer->solarRadioFlux107Observed = getField< double >( data, solarRadioFlux107Observed );
    solarActivityContainer->centered81DaySolarRadioFlux107Observed = getField< double >( data, centered81DaySolarRadioFlux107Observed );
    solarActivityContainer->last81DaySolarRadioFlux107Observed = getField< double >( data, last81DaySolarRadioFlux107Observed );
    try
    {
    ...
    if( !data->find( planetaryRangeIndex0to3 )->second->getRaw( ).empty( ) )  // check if string is empty
    {
        ...
        solarActivityContainer->planetaryRangeIndexVector << getField< unsigned int >( data, planetaryRangeIndex0to3 ),
                getField< unsigned int >( data, planetaryRangeIndex3to6 ), getField< unsigned int >( data, planetaryRangeIndex6to9 ),
                getField< unsigned int >( data, planetaryRangeIndex9to12 ), getField< unsigned int >( data, planetaryRangeIndex12to15 ),
    ...
    {
        solarActivityContainer->planetaryDailyCharacterFigure = getField< double >( data, planetaryDailyCharacterFigure );
        solarActivityContainer->planetaryDailyCharacterFigureConverted =
                getField< unsigned int >( data, planetaryDailyCharacterFigureConverted );
    }

    if( !data->find( internationalSunspotNumber )->second->getRaw( ).empty( ) )
    {
        solarActivityContainer->internationalSunspotNumber = getField< unsigned int >( data, internationalSunspotNumber );
    }
