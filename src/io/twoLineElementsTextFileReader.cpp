 ...
 *          - applied to SSN TLE catalog data and including astro and
...
void TwoLineElementsTextFileReader::openFile( )
{
    ...
    if( !dataFile_ )
    {
        throw std::runtime_error( "Data file could not be opened: " + absoluteFilePath_ );
    }
}
...
void TwoLineElementsTextFileReader::readAndStoreData( )
{
    ...
    while( !dataFile_.eof( ) )
    {
        ...
        else
        {
            ...
            if( ( ( !startingCharacter_.empty( ) && stringOfData_.substr( 0, 1 ).compare( startingCharacter_ ) != 0 ) ||
                  ( !skipKeyword_.empty( ) && stringOfData_.find( skipKeyword_ ) == std::string::npos ) ||
...
void TwoLineElementsTextFileReader::stripEndOfLineCharacters( LineBasedStringDataMap& containerOfLinesOfData )
{
    ...
    for( LineBasedStringDataMap::iterator iteratorContainerOfDataFromFile_ = containerOfLinesOfData.begin( );
         ...
         iteratorContainerOfDataFromFile_++ )
    {
             ...
             iteratorString_++ )
        {
            ...
            if( static_cast< int >( *iteratorString_ ) == 10 || static_cast< int >( *iteratorString_ ) == 13 )
            {
...
void TwoLineElementsTextFileReader::storeTwoLineElementData( )
{
    ...
    for( unsigned int i = 1; i < lineCounter_; i += numberOfLinesPerTwoLineElementDatum_ )
    {
        ...
        if( numberOfLinesPerTwoLineElementDatum_ == 3 )
        {
            ...
            twoLineElementData_[ objectNumberCounter_ ].objectNameString = twoLineElementString_.at( 0 );
        }
        ...
        twoLineElementData_[ objectNumberCounter_ ].lineNumberLine1 =
                std::stoul( trim_copy( twoLineElementString_.at( 1 ).substr( 0, 1 ) ) );
        ...
        twoLineElementData_[ objectNumberCounter_ ].objectIdentificationNumber =
                std::stoul( trim_copy( twoLineElementString_.at( 1 ).substr( 2, 5 ) ) );

        // Get classification character from string.
        twoLineElementData_[ objectNumberCounter_ ].tleClassification = twoLineElementString_.at( 1 )[ 7 ];

        // Get launch year integer from string.
        twoLineElementData_[ objectNumberCounter_ ].launchYear = std::stoul( trim_copy( twoLineElementString_.at( 1 ).substr( 9, 2 ) ) );

        // Calculate four-digit launch year from the above.
        if( twoLineElementData_[ objectNumberCounter_ ].launchYear > 56 )
        {
            twoLineElementData_[ objectNumberCounter_ ].fourDigitlaunchYear = twoLineElementData_[ objectNumberCounter_ ].launchYear + 1900;
        }

        else
        {
            twoLineElementData_[ objectNumberCounter_ ].fourDigitlaunchYear = twoLineElementData_[ objectNumberCounter_ ].launchYear + 2000;
        }

        // Get launch number integer from string.
        twoLineElementData_[ objectNumberCounter_ ].launchNumber = std::stoul( trim_copy( twoLineElementString_.at( 1 ).substr( 11, 3 ) ) );

        // Get launch part string from string.
        twoLineElementData_[ objectNumberCounter_ ].launchPart = twoLineElementString_.at( 1 ).substr( 14, 3 );

        // Get epoch year integer from string.
        twoLineElementData_[ objectNumberCounter_ ].epochYear = std::stoul( trim_copy( twoLineElementString_.at( 1 ).substr( 18, 2 ) ) );

        // Calculate four-digit epoch year from the above.
        if( twoLineElementData_[ objectNumberCounter_ ].epochYear > 56 )
        {
            twoLineElementData_[ objectNumberCounter_ ].fourDigitEpochYear = twoLineElementData_[ objectNumberCounter_ ].epochYear + 1900;
        }

        else
        {
            twoLineElementData_[ objectNumberCounter_ ].fourDigitEpochYear = twoLineElementData_[ objectNumberCounter_ ].epochYear + 2000;
        }

        // Get epoch day double from string.
        twoLineElementData_[ objectNumberCounter_ ].epochDay = std::stod( trim_copy( twoLineElementString_.at( 1 ).substr( 20, 12 ) ) );
        ...
        twoLineElementData_[ objectNumberCounter_ ].firstDerivativeOfMeanMotionDividedByTwo =
                std::stod( trim_copy( twoLineElementString_.at( 1 ).substr( 33, 10 ) ) );
        ...
        twoLineElementData_[ objectNumberCounter_ ].coefficientOfSecondDerivativeOfMeanMotionDividedBySix =
                std::stod( trim_copy( twoLineElementString_.at( 1 ).substr( 44, 6 ) ) ) / 100000.0;

        // Get exponent of scientific notation of "second-derivative of mean motion divided
        // by six" integer from string.
        twoLineElementData_[ objectNumberCounter_ ].exponentOfSecondDerivativeOfMeanMotionDividedBySix =
                std::stod( trim_copy( twoLineElementString_.at( 1 ).substr( 50, 2 ) ) );
        ...
        twoLineElementData_[ objectNumberCounter_ ].secondDerivativeOfMeanMotionDividedBySix =
                twoLineElementData_[ objectNumberCounter_ ].coefficientOfSecondDerivativeOfMeanMotionDividedBySix *
                pow( 10, twoLineElementData_[ objectNumberCounter_ ].exponentOfSecondDerivativeOfMeanMotionDividedBySix );
        ...
        twoLineElementData_[ objectNumberCounter_ ].coefficientOfBStar =
                std::stod( trim_copy( twoLineElementString_.at( 1 ).substr( 53, 6 ) ) ) / 100000.0;

        // Get exponent of scientific notation of B* integer from string
        twoLineElementData_[ objectNumberCounter_ ].exponentOfBStar =
                std::stoi( trim_copy( twoLineElementString_.at( 1 ).substr( 59, 2 ) ) );

        // Calculate B* double from the above two.
        twoLineElementData_[ objectNumberCounter_ ].bStar = twoLineElementData_[ objectNumberCounter_ ].coefficientOfBStar *
                pow( 10.0, twoLineElementData_[ objectNumberCounter_ ].exponentOfBStar );

        // Get orbital model integer from string.
        twoLineElementData_[ objectNumberCounter_ ].orbitalModel = std::stoul( trim_copy( twoLineElementString_.at( 1 ).substr( 62, 1 ) ) );

        // Get TLE number integer from string.
        twoLineElementData_[ objectNumberCounter_ ].tleNumber = std::stoul( trim_copy( twoLineElementString_.at( 1 ).substr( 64, 4 ) ) );

        // Get modulo-10 checksum integer from string.
        twoLineElementData_[ objectNumberCounter_ ].modulo10CheckSumLine1 =
                std::stoul( trim_copy( twoLineElementString_.at( 1 ).substr( 68, 1 ) ) );
        ...
        line2StringStream_ >> twoLineElementData_[ objectNumberCounter_ ].objectIdentificationNumberLine2;

        // Get inclination double from stringstream.
        line2StringStream_ >> inclination_;
        twoLineElementData_[ objectNumberCounter_ ].TLEKeplerianElements( orbital_element_conversions::inclinationIndex ) = inclination_;

        // Get right ascension of ascending node double from stringstream.
        line2StringStream_ >> rightAscensionOfAscendingNode_;
        twoLineElementData_[ objectNumberCounter_ ].TLEKeplerianElements( orbital_element_conversions::longitudeOfAscendingNodeIndex ) =
        ...
        twoLineElementData_[ objectNumberCounter_ ].TLEKeplerianElements( orbital_element_conversions::eccentricityIndex ) = eccentricity_;

        // Get argument of perigee double from stringstream
        line2StringStream_ >> argumentOfPerigee_;
        twoLineElementData_[ objectNumberCounter_ ].TLEKeplerianElements( orbital_element_conversions::argumentOfPeriapsisIndex ) =
                argumentOfPerigee_;
        ...
        twoLineElementData_[ objectNumberCounter_ ].meanMotionInRevolutionsPerDay =
                std::stod( trim_copy( twoLineElementString_[ 2 ].substr( 52, 11 ) ) );

        // Get revolution number integer from line-2 string.
        twoLineElementData_[ objectNumberCounter_ ].revolutionNumber = std::stoi( trim_copy( twoLineElementString_[ 2 ].substr( 63, 5 ) ) );
        ...
        twoLineElementData_[ objectNumberCounter_ ].modulo10CheckSumLine2 =
                std::stoul( trim_copy( twoLineElementString_[ 2 ].substr( 68, 1 ) ) );
        ...
        approximateNumberOfRevolutions_ = twoLineElementData_[ objectNumberCounter_ ].meanMotionInRevolutionsPerDay *
        ...
        lostNumberOfRevolutions_ = approximateNumberOfRevolutions_ - approximateNumberOfRevolutionsRemainder_;

        // Check if the counter has been reset after passing 99999.
        if( ( twoLineElementData_[ objectNumberCounter_ ].revolutionNumber - approximateNumberOfRevolutionsRemainder_ ) <=
            ( approximateNumberOfRevolutionsRemainder_ + 100000 - twoLineElementData_[ objectNumberCounter_ ].revolutionNumber ) )
        {
            twoLineElementData_[ objectNumberCounter_ ].totalRevolutionNumber =
                    lostNumberOfRevolutions_ + twoLineElementData_[ objectNumberCounter_ ].revolutionNumber;
        }

        else
        {
            twoLineElementData_[ objectNumberCounter_ ].totalRevolutionNumber =
                    lostNumberOfRevolutions_ - 100000 + twoLineElementData_[ objectNumberCounter_ ].revolutionNumber;
        }
        ...
        meanMotion_ = twoLineElementData_[ objectNumberCounter_ ].meanMotionInRevolutionsPerDay * 2.0 * PI / physical_constants::JULIAN_DAY;
        twoLineElementData_[ objectNumberCounter_ ].TLEKeplerianElements( orbital_element_conversions::semiMajorAxisIndex ) =
        ...
        twoLineElementData_[ objectNumberCounter_ ].perigee =
                twoLineElementData_[ objectNumberCounter_ ].TLEKeplerianElements( orbital_element_conversions::semiMajorAxisIndex ) *
                ( 1.0 -
                  twoLineElementData_[ objectNumberCounter_ ].TLEKeplerianElements( orbital_element_conversions::eccentricityIndex ) );
...
std::multimap< int, std::string > TwoLineElementsTextFileReader::checkTwoLineElementsFileIntegrity( )
{
    ...
    for( unsigned int i = 0; i < numberOfObjects_; i++ )
    {
        ...
        {
            corruptedTwoLineElementDataErrors_.insert( std::pair< int, std::string >( i, "Incorrect line-1 leading integer." ) );
        ...
        {
            corruptedTwoLineElementDataErrors_.insert( std::pair< int, std::string >( i, "Incorrect line-2 leading integer." ) );

            isObjectErroneous = true;
        }

        if( twoLineElementData_.at( i ).tleClassification != 'U' && twoLineElementData_.at( i ).tleClassification != 'C' )
        {
            corruptedTwoLineElementDataErrors_.insert( std::pair< int, std::string >( i, "Invalid TLE classification." ) );
        ...
        {
            corruptedTwoLineElementDataErrors_.insert( std::pair< int, std::string >( i, "Incorrect orbital model." ) );
        ...
        for( unsigned int j = 0; j < 68; j++ )
        {
            ...
            if( twoLineElementString_.at( 1 )[ j ] != ' ' && twoLineElementString_.at( 1 )[ j ] != '.' &&
                twoLineElementString_.at( 1 )[ j ] != '+' && twoLineElementString_.at( 1 )[ j ] != '-' && j != 7 && j != 14 && j != 15 &&
                j != 16 )
            {
                // Add int to checksum.
                line1Modulo10Sum_ += std::stoi( twoLineElementString_.at( 1 ).substr( j, 1 ) );
            }
        ...
        {
            corruptedTwoLineElementDataErrors_.insert( std::pair< int, std::string >( i, "Incorrect line-1 modulo-10 checksum." ) );
        ...
        for( unsigned int k = 0; k < 68; k++ )
        {
            ...
            if( twoLineElementString_.at( 2 )[ k ] != ' ' && twoLineElementString_.at( 2 )[ k ] != '.' )
            {
                // Add int to checksum.
                line2Modulo10Sum_ += std::stoi( twoLineElementString_[ 2 ].substr( k, 1 ) );
            }
        }
        ...
        {
            corruptedTwoLineElementDataErrors_.insert( std::pair< int, std::string >( i, "Incorrect line-2 modulo-10 checksum." ) );
        ...
        if( twoLineElementData_.at( i ).objectIdentificationNumber != twoLineElementData_.at( i ).objectIdentificationNumberLine2 )
        {
            corruptedTwoLineElementDataErrors_.insert(
                    std::pair< int, std::string >( i, "Line-1 and line-2 object idenfitication number mismatch." ) );
    ...
    {
        twoLineElementData_.erase( twoLineElementData_.begin( ) + ( corruptedTwoLineElementDataPositions_.at( q ) - q ) );
        numberOfObjects_--;
    }
