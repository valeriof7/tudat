...
double AtmosphericCorrectionCspCommand::convertTime( std::string yearMonthDay, std::string hoursMinutesSeconds )
{
    ...
    boost::algorithm::split(
            vectorOfIndividualStrings, yearMonthDay, boost::algorithm::is_any_of( "/" ), boost::algorithm::token_compress_on );
    ...
    boost::algorithm::split(
            vectorOfIndividualStrings, hoursMinutesSeconds, boost::algorithm::is_any_of( ":" ), boost::algorithm::token_compress_on );
    ...
    return basic_astrodynamics::convertCalendarDateToJulianDaysSinceEpoch(
                   year, month, day, hours, minutes, seconds, basic_astrodynamics::JULIAN_DAY_ON_J2000 ) *
...
void CspRawFile::parseCspCommands( const std::vector< std::string >& cspCommandsVector )
{
    for( unsigned int i = 0; i < cspCommandsVector.size( ); ++i )
    {
        ...
        boost::algorithm::split( cspVectorOfIndividualStrings,
                                 cspCommandsVector.at( i ),
        ...
        for( unsigned int j = 0; j < cspVectorOfIndividualStrings.size( ); ++j )
        {
            ...
            {
                if( cspVectorOfIndividualStrings.at( j + 1 ) == "DRY NUPART" || cspVectorOfIndividualStrings.at( j + 1 ) == "WET NUPART" ||
...
{
    std::shared_ptr< AtmosphericCorrectionCspCommand > cspCommand1 =
            std::dynamic_pointer_cast< AtmosphericCorrectionCspCommand >( rawCspData1->getCspCommands( ).at( 0 ) );
    std::shared_ptr< AtmosphericCorrectionCspCommand > cspCommand2 =
            std::dynamic_pointer_cast< AtmosphericCorrectionCspCommand >( rawCspData2->getCspCommands( ).at( 0 ) );
    ...
    else if( sourceSpecifier == "SCID" )
    {
        ...
        else
        {
            ...
            throw std::runtime_error(
                    "Error when retrieving source name from CSP atmosphere file (troposphere/ionosphere corrections):"
    ...
    for( unsigned int i = 0; i < 6; ++i )
    {
        ...
        switch( i )
        {
            ...
            case 2:
                ...
                command->computationCoefficients_ = { 31557600., 0.1149, 0.0255, 0.0020, 0.0010, 0.0026, 0.0036, -0.0001, 0.0007, 0.0012 };
                break;
            case 3:
                command->modelIdentifier_ = "DRY NUPART";
                command->groundStationsId_ = "C40";
            ...
            case 4:
                ...
                command->computationCoefficients_ = { 31557600., 0.1255,  -0.0284, -0.0273, -0.0094,
                                                      0.0005,    -0.0031, -0.0003, -0.0034, -0.0013 };
                break;
            case 5:
                command->modelIdentifier_ = "DRY NUPART";
                command->groundStationsId_ = "C60";
