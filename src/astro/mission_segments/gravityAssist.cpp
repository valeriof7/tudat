...
double calculateUnpoweredGravityAssistMaximumBendingAngle( const double smallestPeriapsisDistance,
                                                           ...
                                                           const double centralBodyGravitationalParameter )
{
    ...
    return std::asin( 1.0 /
                      ( 1.0 +
                        ( smallestPeriapsisDistance * absoluteIncomingExcessVelocity * absoluteIncomingExcessVelocity /
                          centralBodyGravitationalParameter ) ) ) +
            std::asin( 1.0 /
                       ( 1.0 +
                         ( smallestPeriapsisDistance * absoluteOutgoingExcessVelocity * absoluteOutgoingExcessVelocity /
                           centralBodyGravitationalParameter ) ) );
}

double calculateUnpoweredGravityAssistPericenter( const double absoluteIncomingSemiMajorAxis,
                                                  ...
                                                  const RootFinderPointer rootFinder )
{
    ...
    PericenterFindingFunctions pericenterFindingFunctions( absoluteIncomingSemiMajorAxis, absoluteOutgoingSemiMajorAxis, bendingAngle );
...
double calculateGravityAssistDeltaVThroughPericenter( const double centralBodyGravitationalParameter,
                                                      const double absoluteIncomingExcessVelocity,
                                                      const double absoluteOutgoingExcessVelocity,
                                                      const double bendingAngle,
                                                      const double initialGuess,
                                                      const RootFinderPointer rootFinder )
{
    ...
    const double absoluteIncomingSemiMajorAxis =
            1.0 * centralBodyGravitationalParameter / absoluteIncomingExcessVelocity / absoluteIncomingExcessVelocity;
    const double absoluteOutgoingSemiMajorAxis =
            1.0 * centralBodyGravitationalParameter / absoluteOutgoingExcessVelocity / absoluteOutgoingExcessVelocity;

    // Calculate pericenter radius
    const double pericenterRadius = calculateUnpoweredGravityAssistPericenter(
            absoluteIncomingSemiMajorAxis, absoluteOutgoingSemiMajorAxis, bendingAngle, initialGuess, rootFinder );
    ...
    const double incomingVelocityAtPeriapsis =
            absoluteIncomingExcessVelocity * std::sqrt( ( incomingEccentricity + 1.0 ) / ( incomingEccentricity - 1.0 ) );
    const double outgoingVelocityAtPeriapsis =
            absoluteOutgoingExcessVelocity * std::sqrt( ( outgoingEccentricity + 1.0 ) / ( outgoingEccentricity - 1.0 ) );

    // Compute necessary delta-V due to velocity-effect.
    return std::fabs( incomingVelocityAtPeriapsis - outgoingVelocityAtPeriapsis );
}

double calculateGravityAssistDeltaVThroughEccentricity( const double centralBodyGravitationalParameter,
                                                        const double absoluteIncomingExcessVelocity,
                                                        const double absoluteOutgoingExcessVelocity,
                                                        const double bendingAngle,
                                                        const RootFinderPointer rootFinder )
{
    ...
    if( ( absoluteOutgoingExcessVelocity / absoluteIncomingExcessVelocity ) < 100.0 )
    {
        ...
        {
            root_finders::RootFinderPointer rootFinder_temp = std::make_shared< root_finders::Bisection<> >( 1.0e-12, 1000 );
                                     ...
                                     RootFinderPointer rootFinder )
{
    ...
    const Eigen::Vector3d incomingHyperbolicExcessVelocity = incomingVelocity - centralBodyVelocity;
    const Eigen::Vector3d outgoingHyperbolicExcessVelocity = outgoingVelocity - centralBodyVelocity;
    ...
    double bendingAngle = linear_algebra::computeAngleBetweenVectors( incomingHyperbolicExcessVelocity, outgoingHyperbolicExcessVelocity );

    // Compute maximum achievable bending angle.
    const double maximumBendingAngle = calculateUnpoweredGravityAssistMaximumBendingAngle(
            smallestPeriapsisDistance, absoluteIncomingExcessVelocity, absoluteOutgoingExcessVelocity, centralBodyGravitationalParameter );
    ...
    if( bendingAngle > maximumBendingAngle )
    {
        ...
        bendingEffectDeltaV =
                2.0 * std::min( absoluteIncomingExcessVelocity, absoluteOutgoingExcessVelocity ) * std::sin( extraBendingAngle / 2.0 );
        ...
        // Compute semi-major axis of hyperbolic legs.
        const double incomingSemiMajorAxis =
                -centralBodyGravitationalParameter / ( absoluteIncomingExcessVelocity * absoluteIncomingExcessVelocity );
        const double outgoingSemiMajorAxis =
                -centralBodyGravitationalParameter / ( absoluteOutgoingExcessVelocity * absoluteOutgoingExcessVelocity );
    ...
    else if( ( std::fabs( absoluteIncomingExcessVelocity - absoluteOutgoingExcessVelocity ) <= speedTolerance ) )
    {
    ...
    else if( useEccentricityInsteadOfPericenter )
    {
        ...
        if( ( absoluteOutgoingExcessVelocity / absoluteIncomingExcessVelocity ) < 100.0 )
        {
            ...
            catch( std::runtime_error& )
            {
                root_finders::RootFinderPointer rootFinder_temp =
                        root_finders::createRootFinder( root_finders::bisectionRootFinderSettings( TUDAT_NAN, 10E-12, TUDAT_NAN, 1000 ) );
        ...
        // Compute necessary delta-V due to velocity-effect.
        velocityEffectDeltaV = calculateGravityAssistDeltaVThroughEccentricity( centralBodyGravitationalParameter,
    ...
    {
        // Compute necessary delta-V due to velocity-effect.
        velocityEffectDeltaV = calculateGravityAssistDeltaVThroughPericenter( centralBodyGravitationalParameter,
                                                                              absoluteIncomingExcessVelocity,
                                                                              absoluteOutgoingExcessVelocity,
                                                                              bendingAngle,
                                                                 ...
                                                                 const double pericenterRadius )
{
    ...
    const double eccentricity = 1.0 +
            pericenterRadius / centralBodyGravitationalParameter * absoluteRelativeIncomingVelocity * absoluteRelativeIncomingVelocity;
    const double bendingAngle = 2.0 * std::asin( 1.0 / eccentricity );

    // Calculate the unit vectors.
    const Eigen::Vector3d unitVector1 = relativeIncomingVelocity / absoluteRelativeIncomingVelocity;
    ...
    const Eigen::Vector3d relativeOutgoingVelocity = absoluteRelativeIncomingVelocity *
            ( std::cos( bendingAngle ) * unitVector1 + std::sin( bendingAngle ) * std::cos( outgoingVelocityRotationAngle ) * unitVector2 +
              std::sin( bendingAngle ) * std::sin( outgoingVelocityRotationAngle ) * unitVector3 );
                                                               ...
                                                               const double deltaV )
{
    ...
    const double incomingEccentricity = 1.0 +
            pericenterRadius / centralBodyGravitationalParameter * absoluteRelativeIncomingVelocity * absoluteRelativeIncomingVelocity;
    ...
    {
        incomingPericenterVelocity = std::sqrt( absoluteRelativeIncomingVelocity * absoluteRelativeIncomingVelocity *
    ...
    const double outgoingBendingAngle = std::asin( 1.0 /
                                                   ( 1.0 +
                                                     absoluteRelativeOutgoingVelocity * absoluteRelativeOutgoingVelocity *
                                                             pericenterRadius / centralBodyGravitationalParameter ) );
                                                               ...
                                                               const double deltaV )
{
    ...
    incomingVelocity = calculatePoweredGravityAssistOutgoingVelocity( centralBodyGravitationalParameter,
                                                                      centralBodyVelocity,
                                                                      outgoingVelocity,
                                                                      incomingVelocityRotationAngle,
                                                                      pericenterRadius,
                                                                      -deltaV );
...
{
    return std::asin( absoluteIncomingSemiMajorAxis_ / ( absoluteIncomingSemiMajorAxis_ + pericenterRadius ) ) +
            std::asin( absoluteOutgoingSemiMajorAxis_ / ( absoluteOutgoingSemiMajorAxis_ + pericenterRadius ) ) - bendingAngle_;
}

//! Compute first-derivative of the pericenter radius function.
double PericenterFindingFunctions::computeFirstDerivativePericenterRadiusFunction( const double pericenterRadius )
{
    return -absoluteIncomingSemiMajorAxis_ / ( absoluteIncomingSemiMajorAxis_ + pericenterRadius ) /
            std::sqrt( ( pericenterRadius + 2.0 * absoluteIncomingSemiMajorAxis_ ) * pericenterRadius ) -
            absoluteOutgoingSemiMajorAxis_ / ( absoluteOutgoingSemiMajorAxis_ + pericenterRadius ) /
            std::sqrt( ( pericenterRadius + 2.0 * absoluteOutgoingSemiMajorAxis_ ) * pericenterRadius );
}

//! Compute incoming eccentricity function.
double EccentricityFindingFunctions::computeIncomingEccentricityFunction( const double incomingEccentricity )
{
    return std::asin( 1.0 / incomingEccentricity ) +
            std::asin( 1.0 / ( 1.0 - incomingSemiMajorAxis_ / outgoingSemiMajorAxis_ * ( 1.0 - incomingEccentricity ) ) ) - bendingAngle_;
}

//! Compute first-derivative of the incoming eccentricity function.
double EccentricityFindingFunctions::computeFirstDerivativeIncomingEccentricityFunction( const double incomingEccentricity )
{
