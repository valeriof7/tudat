...
// Created using theory from PyKEP toolbox and LambertTargeterIzzo class
{
    ...
    velocityNormalizingValue = std::sqrt( gravitationalParameter / distanceNormalizingValue );
    ...
    // Chord.
    normalizedChord = std::sqrt( 1.0 + normalizedRadiusAtArrival * ( normalizedRadiusAtArrival - 2.0 * cosineOfTransferAngle ) );
    ...
    // Lambda parameter.
    lambdaParameter = std::sqrt( normalizedRadiusAtArrival ) * std::cos( transferAngle / 2.0 ) / normalizedSemiPerimeter;
...
// Created using theory from PyKEP toolbox and LambertTargeterIzzo class
{
    ...
    while( ( rootFindingError > convergenceTolerance ) && ( y1 != y2 ) && ( iterator < maximumNumberOfIterations ) )
    {
        ...
        yNew = std::log( computeTimeOfFlight( std::exp( xNew ) - 1.0 ) ) - logarithmOfTheSpecifiedTimeOfFlight;
...
// Created using theory from PyKEP toolbox and LambertTargeterIzzo class
{
    ...
    if( xParameter < 1.0 )
    {
        ...
        // Beta parameter in Lagrange's equation (no explanation available).
        double betaParameter = 2.0 * std::asin( std::sqrt( ( normalizedSemiPerimeter - normalizedChord ) / ( 2.0 * semiMajorAxis ) ) );
        ...
        etaParameterSquared = 2.0 * semiMajorAxis * std::sin( psiParameter ) * std::sin( psiParameter ) / normalizedSemiPerimeter;
    ...
    else
    {
        ...
        // Eta parameter in Izzo's approach (no explanation available).
        etaParameterSquared = -2.0 * semiMajorAxis * std::sinh( psiParameter ) * std::sinh( psiParameter ) / normalizedSemiPerimeter;
        etaParameter = std::sqrt( etaParameterSquared );
    }

    // Determine semi-latus rectum, p.
    const double semiLatusRectum = ( normalizedRadiusAtArrival / ( normalizedMinimumEnergySemiMajorAxis * etaParameterSquared ) ) *
            std::sin( transferAngle / 2.0 ) * std::sin( transferAngle / 2.0 );

    // Velocity components at departure.
    const double radialVelocityAtDeparture = ( 1.0 / ( etaParameter * std::sqrt( normalizedMinimumEnergySemiMajorAxis ) ) ) *
            ( 2.0 * lambdaParameter * normalizedMinimumEnergySemiMajorAxis - ( lambdaParameter + xParameter * etaParameter ) );
