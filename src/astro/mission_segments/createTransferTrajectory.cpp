...
std::shared_ptr< TransferLegSettings > sphericalShapingLeg( const std::shared_ptr< root_finders::RootFinderSettings >& rootFinderSettings,
...
{
    return std::make_shared< SphericalShapingLegSetting >( rootFinderSettings,
                                                           lowerBoundFreeCoefficient,
                                                           upperBoundFreeCoefficient,
                                                           initialValueFreeCoefficient,
                                                           timeToAzimuthInterpolatorStepSize );
}
...
std::shared_ptr< TransferNodeSettings > swingbyNode( const double minimumPeriapsisDistance )
{
...
{
    return std::make_shared< CaptureAndInsertionNodeSettings >( captureSemiMajorAxis, captureEccentricity );
}
                               ...
                               const std::string engineName )
{
    ...
    {
        std::dynamic_pointer_cast< propulsion::CustomThrustMagnitudeWrapper >( engineModel->getThrustMagnitudeWrapper( ) )
                ->resetThrustMagnitudeFunction( thrustMagnitudeFunction );
    }

    std::shared_ptr< ephemerides::RotationalEphemeris > rotationModel = bodyMap.getBody( bodyName )->getRotationalEphemeris( );
    ...
    else
    {
        ...
        {
            throw std::runtime_error( "Error, inertial thrust direction model inconsistent when setting low-thrust acceleration." );
        }
        else
        {
            std::dynamic_pointer_cast< ephemerides::CustomBodyFixedDirectionCalculator >(
                    directionBasedRotationalEphemeris->getInertialBodyAxisDirectionCalculator( ) )
                    ->resetInertialBodyAxisDirectionFunction( thrustDirectionFunction );
        }
    }
}
                                                  ...
                                                  const std::shared_ptr< TransferNode > arrivalNode )
{
    ...
    const double centralBodyGravitationalParameter = bodyMap.at( centralBodyName )->getGravityFieldModel( )->getGravitationalParameter( );
    const std::shared_ptr< ephemerides::Ephemeris > departureBodyEphemeris = bodyMap.at( departureBodyName )->getEphemeris( );
    const std::shared_ptr< ephemerides::Ephemeris > arrivalBodyEphemeris = bodyMap.at( arrivalBodyName )->getEphemeris( );
                                                    ...
                                                    const bool nodeComputesOutgoingVelocity )
{
    ...
    const double centralBodyGravitationalParameter = bodyMap.at( nodeBodyName )->getGravityFieldModel( )->getGravitationalParameter( );
    const std::shared_ptr< ephemerides::Ephemeris > centralBodyEphemeris = bodyMap.at( nodeBodyName )->getEphemeris( );
    ...
    switch( nodeSettings->nodeType_ )
    {
        case swingby: {
            ...
            {
                std::shared_ptr< SwingbyNodeSettings > swingbySettings = std::dynamic_pointer_cast< SwingbyNodeSettings >( nodeSettings );
                ...
                transferNode = std::make_shared< SwingbyWithFixedIncomingFixedOutgoingVelocity >( centralBodyEphemeris,
                                                                                                  centralBodyGravitationalParameter,
                                                                                                  swingbySettings->minimumPeriapsisRadius_,
            ...
            {
                transferNode = std::make_shared< SwingbyWithFreeIncomingFreeOutgoingVelocity >( centralBodyEphemeris,
                                                                                                centralBodyGravitationalParameter );
            }
            else
            {
        ...
        case escape_and_departure: {
            ...
            else
            {
                ...
                transferNode =
                        std::make_shared< DepartureWithFreeOutgoingVelocityNode >( centralBodyEphemeris,
                                                                                   centralBodyGravitationalParameter,
                                                                                   escapeAndDepartureSettings->departureSemiMajorAxis_,
                                                                                   escapeAndDepartureSettings->departureEccentricity_ );
            }
        ...
        case capture_and_insertion: {
            ...
            {
                transferNode = std::make_shared< CaptureWithFreeIncomingVelocityNode >( centralBodyEphemeris,
                                                                                        centralBodyGravitationalParameter,
                                                                                        captureAndInsertionSettings->captureSemiMajorAxis_,
                                                                                        captureAndInsertionSettings->captureEccentricity_ );
            }
                                  ...
                                  const std::map< std::string, double > minimumPericenterRadii )
{
    ...
    getMgaTransferTrajectorySettings( transferLegSettings,
                                      transferNodeSettings,
                                      fullBodiesList,
                                      identicalTransferLegSettingsConstructor,
                                      departureOrbit,
                                      arrivalOrbit,
        ...
        const std::map< std::string, double > minimumPericenterRadii )
{
    ...
    if( arrivalOrbit.first != arrivalOrbit.first )
    {
        ...
        {
            throw std::runtime_error( "Error when making MGA settings, no pericenter radius provided for " +
                                      fullBodiesList.at( numberOfNodes - 1 ) );
        }
...
void getMgaTransferTrajectorySettingsWithoutDsm( std::vector< std::shared_ptr< TransferLegSettings > >& legSettings,
                                            ...
                                            const std::map< std::string, double > minimumPericenterRadii )
{
    ...
    std::vector< std::shared_ptr< TransferNodeSettings > > transferNodeSettings;
    getMgaTransferTrajectorySettingsWithoutDsm(
            transferLegSettings, transferNodeSettings, fullBodiesList, departureOrbit, arrivalOrbit, minimumPericenterRadii );
    return std::make_pair( transferLegSettings, transferNodeSettings );
}
                                                 ...
                                                 const std::map< std::string, double > minimumPericenterRadii )
{
    ...
    getMgaTransferTrajectorySettings( legSettings,
                                      nodeSettings,
                                      fullBodiesList,
                                      unpoweredLegSettingsConstructor,
                                      departureOrbit,
                                      arrivalOrbit,
                                      minimumPericenterRadii );
}

void getMgaTransferTrajectorySettingsWithPositionBasedDsm( std::vector< std::shared_ptr< TransferLegSettings > >& legSettings,
                                                           std::vector< std::shared_ptr< TransferNodeSettings > >& nodeSettings,
                                                           const std::string& departureBody,
                                                           const std::string& arrivalBody,
                                                           const std::vector< std::string >& flybyBodies,
...
std::pair< std::vector< std::shared_ptr< TransferLegSettings > >, std::vector< std::shared_ptr< TransferNodeSettings > > >
getMgaTransferTrajectorySettingsWithPositionBasedDsm( const std::vector< std::string >& fullBodiesList,
                                                      const std::pair< double, double > departureOrbit,
                                                      const std::pair< double, double > arrivalOrbit,
                                                      const std::map< std::string, double > minimumPericenterRadii )
{
                                                           ...
                                                           const std::map< std::string, double > minimumPericenterRadii )
{
    ...
    getMgaTransferTrajectorySettings( legSettings,
                                      nodeSettings,
                                      fullBodiesList,
                                      dsmPositionBasedLegSettingsConstructor,
                                      departureOrbit,
                                      arrivalOrbit,
                                      minimumPericenterRadii );
}

void getMgaTransferTrajectorySettingsWithVelocityBasedDsm( std::vector< std::shared_ptr< TransferLegSettings > >& legSettings,
                                                           std::vector< std::shared_ptr< TransferNodeSettings > >& nodeSettings,
                                                           const std::string& departureBody,
                                                           const std::string& arrivalBody,
                                                           const std::vector< std::string >& flybyBodies,
...
std::pair< std::vector< std::shared_ptr< TransferLegSettings > >, std::vector< std::shared_ptr< TransferNodeSettings > > >
getMgaTransferTrajectorySettingsWithVelocityBasedDsm( const std::vector< std::string >& fullBodiesList,
                                                      const std::pair< double, double > departureOrbit,
                                                      const std::pair< double, double > arrivalOrbit,
                                                      const std::map< std::string, double > minimumPericenterRadii )
{
                                                           ...
                                                           const std::map< std::string, double > minimumPericenterRadii )
{
    ...
    getMgaTransferTrajectorySettings( legSettings,
                                      nodeSettings,
                                      fullBodiesList,
                                      dsmVelocityBasedLegSettingsConstructor,
                                      departureOrbit,
                                      arrivalOrbit,
                                      minimumPericenterRadii );
}
        ...
        const std::map< std::string, double > minimumPericenterRadii )
{
    ...
    getMgaTransferTrajectorySettingsWithSphericalShapingThrust( legSettings,
                                                                nodeSettings,
                                                                fullBodiesList,
                                                                rootFinderSettings,
                                                                departureOrbit,
...
std::pair< std::vector< std::shared_ptr< TransferLegSettings > >, std::vector< std::shared_ptr< TransferNodeSettings > > >
getMgaTransferTrajectorySettingsWithSphericalShapingThrust( const std::vector< std::string >& fullBodiesList,
                                                            const std::shared_ptr< root_finders::RootFinderSettings > rootFinderSettings,
                                                            const std::pair< double, double > departureOrbit,
                                                            const std::pair< double, double > arrivalOrbit,
                                                            const double lowerBoundFreeCoefficient,
                                                            ...
                                                            const std::map< std::string, double > minimumPericenterRadii )
{
    std::vector< std::shared_ptr< TransferLegSettings > > transferLegSettings;
    std::vector< std::shared_ptr< TransferNodeSettings > > transferNodeSettings;
    getMgaTransferTrajectorySettingsWithSphericalShapingThrust( transferLegSettings,
                                                                transferNodeSettings,
                                                                fullBodiesList,
                                                                rootFinderSettings,
                                                                departureOrbit,
        ...
        const std::map< std::string, double > minimumPericenterRadii )
{
    ...
    return getMgaTransferTrajectorySettings( legSettings,
                                             nodeSettings,
                                             fullBodiesList,
                                             sphericalShapingLegSettingsConstructor,
                                             departureOrbit,
                                             arrivalOrbit,
                                             minimumPericenterRadii );
}
        ...
        const std::map< std::string, double > minimumPericenterRadii )
{
    ...
    getMgaTransferTrajectorySettings( legSettings,
                                      nodeSettings,
                                      fullBodiesList,
                                      hodographicShapingLegSettingsConstructor,
                                      departureOrbit,
                                      arrivalOrbit,
                                      minimumPericenterRadii );
}
        ...
        const std::map< std::string, double > minimumPericenterRadii )
{
    ...
    fullBodiesList.insert( fullBodiesList.end( ), flybyBodies.begin( ), flybyBodies.end( ) );
    fullBodiesList.push_back( arrivalBody );
    getMgaTransferTrajectorySettingsWithHodographicShapingThrust( legSettings,
                                                                  nodeSettings,
                                                                  fullBodiesList,
                                                                  radialVelocityFunctionComponentsPerLeg,
                                                                  normalVelocityFunctionComponentsPerLeg,
                                                                  axialVelocityFunctionComponentsPerLeg,
                                                                  departureOrbit,
                                                                  arrivalOrbit,
                                                                  minimumPericenterRadii );
}
        ...
        const std::map< std::string, double > minimumPericenterRadii )
{
    std::vector< std::shared_ptr< TransferLegSettings > > transferLegSettings;
    std::vector< std::shared_ptr< TransferNodeSettings > > transferNodeSettings;
    getMgaTransferTrajectorySettingsWithHodographicShapingThrust( transferLegSettings,
                                                                  transferNodeSettings,
                                                                  fullBodiesList,
                                                                  radialVelocityFunctionComponentsPerLeg,
                                                                  normalVelocityFunctionComponentsPerLeg,
...
std::pair< std::vector< std::shared_ptr< TransferLegSettings > >, std::vector< std::shared_ptr< TransferNodeSettings > > >
getMgaTransferTrajectorySettingsWithHodographicShapingThrust( const std::vector< std::string >& fullBodiesList,
                                                              ...
                                                              const std::map< std::string, double > minimumPericenterRadii )
{
    ...
    {
        throw std::runtime_error( "Error when creating settings for transfer with hodographic shaping legs: number of legs (" +
    ...
    for( unsigned int i = 0; i < fullBodiesList.size( ) - 1; ++i )
    {
        ...
        axialVelocityFunctionComponentsPerLeg.push_back( shape_based_methods::getRecommendedAxialVelocityBaseFunctions(
                timeOfFlightPerLeg.at( i ), numberOfRevolutionsPerLeg.at( i ) ) );
    }

    return getMgaTransferTrajectorySettingsWithHodographicShapingThrust( fullBodiesList,
                                                                         radialVelocityFunctionComponentsPerLeg,
                                                                         normalVelocityFunctionComponentsPerLeg,
                                                                         axialVelocityFunctionComponentsPerLeg,
                                                                         departureOrbit,
                                                                ...
                                                                const std::string& centralBody )
{
    ...
    {
        throw std::runtime_error( "Error when making transfer trajectory, number of legs ( " + std::to_string( legSettings.size( ) ) +
                                  " ) and number of nodes ( " + std::to_string( nodeSettings.size( ) ) + " ) are incompatible" );
    }

    if( nodeIds.size( ) != nodeSettings.size( ) )
    {
        throw std::runtime_error( "Error when making transfer trajectory, number of nodes ( " + std::to_string( nodeSettings.size( ) ) +
                                  " ) and number of node names ( " + std::to_string( nodeIds.size( ) ) + " ) are incompatible" );
    }
    ...
    while( ( std::find( legs.begin( ), legs.end( ), nullptr ) != legs.end( ) ) ||
           ( std::find( nodes.begin( ), nodes.end( ), nullptr ) != nodes.end( ) ) )
    {
        ++iteration;
        ...
        if( nodes.at( 0 ) == nullptr )
        {
            ...
            {
                nodes.at( 0 ) = createTransferNode( bodyMap, nodeSettings.at( 0 ), nodeIds.at( 0 ), nullptr, nullptr, false, true );
            }
            // If node requires input from following leg but following leg is already created, then create node
            else if( legs.at( 0 ) != nullptr )
            {
                nodes.at( 0 ) = createTransferNode( bodyMap, nodeSettings.at( 0 ), nodeIds.at( 0 ), nullptr, legs.at( 0 ), false, false );
            }
        }
        ...
        for( unsigned int i = 0; i < legSettings.size( ); i++ )
        {
            ...
            if( ( legs.at( i ) == nullptr ) &&
                !( legRequiresInputFromPreviousNode.at( legSettings.at( i )->legType_ ) && nodes.at( i ) == nullptr ) &&
                !( legRequiresInputFromFollowingNode.at( legSettings.at( i )->legType_ ) && nodes.at( i + 1 ) == nullptr ) )
            {
                legs.at( i ) = createTransferLeg(
                        bodyMap, legSettings.at( i ), nodeIds.at( i ), nodeIds.at( i + 1 ), centralBody, nodes.at( i ), nodes.at( i + 1 ) );
            }
            ...
            if( i < legSettings.size( ) - 1 )
            {
                ...
                {
                    nodes.at( i + 1 ) = createTransferNode( bodyMap,
                                                            nodeSettings.at( i + 1 ),
                                                            nodeIds.at( i + 1 ),
                                                            legs.at( i ),
                                                            legs.at( i + 1 ),
                                                            !nodeRequiresInputFromPreviousLeg,
                                                            !nodeRequiresInputFromFollowingLeg );
                }
            }
        }
        ...
        if( nodes.at( legSettings.size( ) ) == nullptr )
        {
            ...
            if( legRequiresInputFromFollowingNode.at( legSettings.at( legSettings.size( ) - 1 )->legType_ ) )
            {
                nodes.at( legSettings.size( ) ) = createTransferNode(
                        bodyMap, nodeSettings.at( legSettings.size( ) ), nodeIds.at( legSettings.size( ) ), nullptr, nullptr, true, true );
            }
            ...
            {
                nodes.at( legSettings.size( ) ) = createTransferNode( bodyMap,
                                                                      nodeSettings.at( legSettings.size( ) ),
                                                                      nodeIds.at( legSettings.size( ) ),
                                                                      legs.at( legSettings.size( ) - 1 ),
                                                                      nullptr,
                                                                      false,
                                                                      true );
            }
        ...
        switch( nodeSettings.at( i )->nodeType_ )
        {
            case swingby:
                if( legRequiresInputFromFollowingNode.at( legSettings.at( i - 1 )->legType_ ) &&
                    legRequiresInputFromPreviousNode.at( legSettings.at( i )->legType_ ) )
                {
                    nodeParameterIndices.push_back( std::make_pair( currentParameterIndex, 6 ) );
                    currentParameterIndex += 6;
                }
                else if( !legRequiresInputFromFollowingNode.at( legSettings.at( i - 1 )->legType_ ) &&
                         !legRequiresInputFromPreviousNode.at( legSettings.at( i )->legType_ ) )
                {
                ...
                case hodographic_low_thrust_leg: {
                    ...
                    if( hodographicShapingLegSettings == nullptr )
                    {
                        throw std::runtime_error(
                                "Error when decomposing parameter indices, hodographic shaping settings type is invalid " );
                    }
