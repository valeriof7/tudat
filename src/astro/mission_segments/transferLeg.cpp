...
TransferLeg::TransferLeg( const std::shared_ptr< ephemerides::Ephemeris > departureBodyEphemeris,
...
void TransferLeg::updateDepartureAndArrivalBodies( const double departureTime, const double arrivalTime )
{
...
UnpoweredUnperturbedTransferLeg::UnpoweredUnperturbedTransferLeg( const std::shared_ptr< ephemerides::Ephemeris > departureBodyEphemeris,
                                                                  const std::shared_ptr< ephemerides::Ephemeris > arrivalBodyEphemeris,
...
void UnpoweredUnperturbedTransferLeg::getStateAlongTrajectory( Eigen::Vector6d& stateAlongTrajectory, const double time )
{
...
void UnpoweredUnperturbedTransferLeg::computeTransfer( )
{
    ...
    mission_segments::solveLambertProblemIzzo( departureBodyState_.segment( 0, 3 ),
                                               arrivalBodyState_.segment( 0, 3 ),
                                               legParameters_( 1 ) - legParameters_( 0 ),
                                               centralBodyGravitationalParameter_,
                                               departureVelocity_,
                                               arrivalVelocity_ );
...
void DsmTransferLeg::calculateKeplerianElements( )
{
    ...
    constantKeplerianStateBeforeDsm_ =
            orbital_element_conversions::convertCartesianToKeplerianElements( initialState, centralBodyGravitationalParameter_ );
    ...
    constantKeplerianStateAfterDsm_ =
            orbital_element_conversions::convertCartesianToKeplerianElements( stateAfterDsm, centralBodyGravitationalParameter_ );
}

void DsmTransferLeg::getStateAlongTrajectory( Eigen::Vector6d& stateAlongTrajectory, const double time )
{
    if( time < departureTime_ || time > arrivalTime_ )
    {
        throw std::runtime_error( "Error when requesting state along unpowered unperturbed leg, requested time is outside bounds" );
    ...
    {
        stateAlongTrajectory = orbital_element_conversions::convertKeplerianToCartesianElements(
                orbital_element_conversions::propagateKeplerOrbit( constantKeplerianStateAfterDsm_,
                                                                   time - trajectoryManeuver_.getManeuverTime( ),
                                                                   centralBodyGravitationalParameter_ ),
                centralBodyGravitationalParameter_ );
    }
}

DsmPositionBasedTransferLeg::DsmPositionBasedTransferLeg( const std::shared_ptr< ephemerides::Ephemeris > departureBodyEphemeris,
                                                          const std::shared_ptr< ephemerides::Ephemeris > arrivalBodyEphemeris,
                                                          const double centralBodyGravitationalParameter ):
                                                   ...
                                                   const double outOfPlaneAngle )
{
    ...
    const Eigen::Vector3d unitVector3 = ( unitVector1.cross( Eigen::Vector3d( departureBodyState.segment( 3, 3 ) ) ) ).normalized( );
    ...
    return std::cos( inPlaneAngle ) * std::cos( outOfPlaneAngle ) * absoluteRadiusDsm * unitVector1 +
            std::sin( inPlaneAngle ) * std::cos( outOfPlaneAngle ) * absoluteRadiusDsm * unitVector2 +
...
void DsmPositionBasedTransferLeg::computeTransfer( )
{
    ...
    Eigen::Vector3d dsmLocation =
            calculatePositionBasedDsmLocation( departureBodyState_, dimensionlessRadiusDsm_, inPlaneAngle_, outOfPlaneAngle_ );
    ...
    mission_segments::solveLambertProblemIzzo( departureBodyState_.segment( 0, 3 ),
                                               ...
                                               departureVelocity_,
                                               velocityBeforeDsm_ );
    mission_segments::solveLambertProblemIzzo( dsmLocation,
                                               arrivalBodyState_.segment( 0, 3 ),
                                               timeOfFlight_ - dsmTime,
                                               centralBodyGravitationalParameter_,
                                               velocityAfterDsm_,
                                               arrivalVelocity_ );
...
void computeVelocityBasedDsmState( const Eigen::Vector6d& departureCartesianElements,
                                   const double centralBodyGravitationalParameter,
                                   ...
                                   Eigen::Vector3d& velocityBeforeDsm )
{
    ...
    Eigen::Vector6d dsmArrivalKeplerianElements =
            orbital_element_conversions::propagateKeplerOrbit( departureKeplerianElements, dsmTime, centralBodyGravitationalParameter );
...
DsmVelocityBasedTransferLeg::DsmVelocityBasedTransferLeg( const std::shared_ptr< ephemerides::Ephemeris > departureBodyEphemeris,
                                                          const std::shared_ptr< ephemerides::Ephemeris > arrivalBodyEphemeris,
                                                          const double centralBodyGravitationalParameter,
...
void DsmVelocityBasedTransferLeg::computeTransfer( )
{
    ...
    computeVelocityBasedDsmState(
            departureCartesianElements, centralBodyGravitationalParameter_, dsmTime, dsmLocation, velocityBeforeDsm_ );
