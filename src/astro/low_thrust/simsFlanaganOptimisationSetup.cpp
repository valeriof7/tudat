...
SimsFlanaganProblem::SimsFlanaganProblem( const Eigen::Vector6d &stateAtDeparture,
    ...
    maximumThrust_( maximumThrust ), specificImpulseFunction_( specificImpulseFunction ), numberSegments_( numberSegments ),
    timeOfFlight_( timeOfFlight ), initialGuessThrustModel_( initialGuessThrustModel ),
    relativeToleranceConstraints_( relativeToleranceConstraints )
{
    ...
    if( ( relativeMarginWrtInitialGuess_ != TUDAT_NAN ) &&
        ( ( relativeMarginWrtInitialGuess_ < 0.0 ) || ( relativeMarginWrtInitialGuess_ > 1.0 ) ) )
    {
        std::string errorMessage =
                "Error when using initial guess to create Sims-Flanagan optimisation problem, the relative margin w.r.t. the "
    ...
    if( initialGuessThrottles_.size( ) != 0 )
    {
        if( initialGuessThrottles_.size( ) != static_cast< unsigned int >( 3 * numberSegments_ ) )
        {
            throw std::runtime_error(
                    "Error when providing an initial guess for Sims-Flanagan, vector size unconsistent with"
...
std::vector< double > SimsFlanaganProblem::fitness( const std::vector< double > &designVariables ) const
{
    ...
    for( int i = 0; i < numberSegments_; i++ )
    {
        throttles.push_back( ( Eigen::Vector3d( ) << designVariables[ i * 3 ], designVariables[ i * 3 + 1 ], designVariables[ i * 3 + 2 ] )
                                     .finished( ) );
    }
    ...
    Eigen::VectorXd c;
    c.resize( equalityConstraints.size( ) + inequalityConstraints.size( ) );
    Eigen::VectorXd r;
    r.resize( equalityConstraints.size( ) + inequalityConstraints.size( ) );
    Eigen::VectorXd epsilon;
    epsilon.resize( equalityConstraints.size( ) + inequalityConstraints.size( ) );
    ...
    for( unsigned int i = 0; i < inequalityConstraints.size( ); i++ )
    {
        ...
        epsilon[ equalityConstraints.size( ) + i ] =
                inequalityConstraints[ i ] * c[ equalityConstraints.size( ) + i ] + r[ equalityConstraints.size( ) + i ];
    }
