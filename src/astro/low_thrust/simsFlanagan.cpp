                                                        ...
                                                        const int numberSegmentsBackwardPropagation )
{
    ...
    for( int i = 0; i < numberSegmentsBackwardPropagation; i++ )
    {
        Eigen::Vector3d currentThrustVector =
                thrustModelWrtTime( timeOfFlight / 2.0 + segmentDurationBackwardPropagation * ( i + 1.0 / 2.0 ) ) / maximumThrust;
        for( int j = 0; j < 3; j++ )
        {
            simsFlanaganThrustModel.push_back( currentThrustVector[ j ] );
        }
        ...
        std::shared_ptr< numerical_integrators::IntegratorSettings< double > > integratorSettings )
{
    ...
    {
        thrustVectorPerSegment.push_back(
                ( thrustProfileShapedTrajectory[ timesAtNodes[ i ] ] + thrustProfileShapedTrajectory[ timesAtNodes[ i + 1 ] ] ) / 2.0 );
    }
...
std::pair< std::vector< double >, std::vector< double > > SimsFlanagan::performOptimisation( )
{
    ...
    problem prob{ SimsFlanaganProblem( stateAtDeparture_,
                                       stateAtArrival_,
    ...
    for( int i = 0; i < optimisationSettings_->numberOfGenerations_; i++ )
    {
        ...
        while( islUnconstrained.status( ) != pagmo::evolve_status::idle && islUnconstrained.status( ) != pagmo::evolve_status::idle_error )
        {
...
void SimsFlanagan::getThrustAccelerationProfile( std::vector< double >& epochsVector,
    ...
    for( unsigned int i = 0; i < epochsVector.size( ); i++ )
    {
        if( ( i > 0 ) && ( epochsVector[ i ] < epochsVector[ i - 1 ] ) )
        {
            throw std::runtime_error(
                    "Error when retrieving the thrust profile of a low-thrust trajectory, "
                    "epochs are not provided in increasing order." );
        }

        thrustAccelerationProfile[ epochsVector[ i ] ] = thrustProfile[ epochsVector[ i ] ] / massProfile[ epochsVector[ i ] ][ 0 ];
    }
}
...
{
    basic_astrodynamics::AccelerationMap SimsFlanaganAccelerationMap =
            simsFlanaganModel_->getLowThrustTrajectoryAccelerationMap( bodies, bodyToPropagate, centralBody );
        ...
        const std::shared_ptr< propagators::DependentVariableSaveSettings > dependentVariablesToSave )
{
    ...
    Eigen::Vector6d stateHalfOfTimeOfFlight =
            1.0 / 2.0 * ( stateHalfOfTimeOfFlightForwardPropagation + stateHalfOfTimeOfFlightBackwardPropagation );
    ...
    translationalStatePropagatorSettings.first =
            std::make_shared< propagators::TranslationalStatePropagatorSettings< double > >( std::vector< std::string >{ centralBody },
                                                                                             accelerationModelMap,
                                                                                             std::vector< std::string >{ bodyToPropagate },
                                                                                             stateHalfOfTimeOfFlightForwardPropagation,
                                                                                             terminationConditions.first,
                                                                                             propagators::cowell,
                                                                                             dependentVariablesToSave );

    // Define forward translational state propagation settings.
    translationalStatePropagatorSettings.second =
            std::make_shared< propagators::TranslationalStatePropagatorSettings< double > >( std::vector< std::string >{ centralBody },
                                                                                             accelerationModelMap,
                                                                                             std::vector< std::string >{ bodyToPropagate },
                                                                                             stateHalfOfTimeOfFlightBackwardPropagation,
