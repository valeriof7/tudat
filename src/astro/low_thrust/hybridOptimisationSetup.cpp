...
HybridMethodProblem::HybridMethodProblem( const Eigen::Vector6d &stateAtDeparture,
    ...
    specificImpulse_( specificImpulse ), timeOfFlight_( timeOfFlight ), bodies_( bodies ), bodyToPropagate_( bodyToPropagate ),
    centralBody_( centralBody ), integratorSettings_( integratorSettings ), initialGuessThrustModel_( initialGuessThrustModel ),
    initialAndFinalMEEcostatesBounds_( initialAndFinalMEEcostatesBounds ), relativeToleranceConstraints_( relativeToleranceConstraints )
{
        ...
        ( ( relativeMarginWrtInitialGuess_ < 0.0 ) || ( relativeMarginWrtInitialGuess_ > 1.0 ) ) )
    {
        std::string errorMessage =
                "Error when using initial guess to create hybrid method optimisation problem, the relative margin w.r.t. the "
    ...
    if( guessInitialAndFinalCostates_.size( ) != 0 )
    {
        if( guessInitialAndFinalCostates_.size( ) != 10 )
        {
            throw std::runtime_error(
                    "Error when providing an initial guess for hybrid method, size of the vector unconsistent"
...
std::vector< double > HybridMethodProblem::fitness( const std::vector< double > &designVariables ) const
{
    ...
    {
        throw std::runtime_error(
                "Error, size of the design variables vector unconsistent with initial and final "
                "MEE costates sizes." );
    }
    ...
    low_thrust_trajectories::HybridMethodModel currentLeg = low_thrust_trajectories::HybridMethodModel( stateAtDeparture_,
                                                                                                        stateAtArrival_,
                                                                                                        initialCostates,
                                                                                                        finalCostates,
                                                                                                        maximumThrust_,
