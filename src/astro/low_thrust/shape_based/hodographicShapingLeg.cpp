...
HodographicShapingLeg::HodographicShapingLeg( const std::shared_ptr< ephemerides::Ephemeris > departureBodyEphemeris,
    ...
    arrivalVelocityFunction_( arrivalVelocityFunction ), numberOfFreeRadialCoefficients_( radialVelocityFunctionComponents.size( ) - 3 ),
    ...
    numberOfFreeAxialCoefficients_( axialVelocityFunctionComponents.size( ) - 3 )
{
    if( numberOfFreeRadialCoefficients_ < 0 || numberOfFreeNormalCoefficients_ < 0 || numberOfFreeAxialCoefficients_ < 0 )
    {
        throw std::runtime_error(
                "Error, input of number of free coefficients for hodographic shaping is smaller than 0. Each direction must have at least "
                "three components. Total number of coefficients is: " +
...
void HodographicShapingLeg::computeTransfer( )
{
    ...
    {
        throw std::runtime_error(
                "Error when updating hodographic shaping object, number of revolutions should be equal to or larger than 0" );
    }
...
void HodographicShapingLeg::updateFreeCoefficients( )
{
    ...
    fullCoefficientsRadialVelocityFunction_.segment( 3, numberOfFreeRadialCoefficients_ ) =
            legParameters_.segment( 3, numberOfFreeRadialCoefficients_ );
    ...
    fullCoefficientsNormalVelocityFunction_.segment( 3, numberOfFreeNormalCoefficients_ ) =
            legParameters_.segment( 3 + numberOfFreeRadialCoefficients_, numberOfFreeNormalCoefficients_ );
    ...
    fullCoefficientsAxialVelocityFunction_.segment( 3, numberOfFreeAxialCoefficients_ ) =
            legParameters_.segment( 3 + numberOfFreeRadialCoefficients_ + numberOfFreeNormalCoefficients_, numberOfFreeAxialCoefficients_ );
        ...
        std::shared_ptr< CompositeFunctionHodographicShaping > velocityFunction )
{
    ...
    matrixBoundaryValues << velocityFunction->getComponentFunctionIntegralCurrentValue( 0, timeOfFlight_ ) -
            ...
            velocityFunction->getComponentFunctionCurrentValue( 0, 0.0 ), velocityFunction->getComponentFunctionCurrentValue( 1, 0.0 ),
...
void HodographicShapingLeg::satisfyRadialBoundaryConditions( const Eigen::VectorXd& freeCoefficients )
{
    ...
    {
        vectorBoundaryConditionsRadial[ 0 ] -= freeCoefficients( i ) *
                ( radialVelocityFunction_->getComponentFunctionIntegralCurrentValue( i + 3, timeOfFlight_ ) -
                  radialVelocityFunction_->getComponentFunctionIntegralCurrentValue( i + 3, 0.0 ) );
        vectorBoundaryConditionsRadial[ 1 ] -=
                freeCoefficients( i ) * radialVelocityFunction_->getComponentFunctionCurrentValue( i + 3, 0.0 );
        vectorBoundaryConditionsRadial[ 2 ] -=
                freeCoefficients( i ) * radialVelocityFunction_->getComponentFunctionCurrentValue( i + 3, timeOfFlight_ );
    }
...
void HodographicShapingLeg::satisfyAxialBoundaryConditions( const Eigen::VectorXd& freeCoefficients )
{
    ...
    for( int i = 0; i < numberOfFreeAxialCoefficients_; i++ )
    {
        ...
        vectorBoundaryConditionsAxial[ 1 ] -=
                freeCoefficients( i ) * axialVelocityFunction_->getComponentFunctionCurrentValue( i + 3, 0.0 );
        vectorBoundaryConditionsAxial[ 2 ] -=
                freeCoefficients( i ) * axialVelocityFunction_->getComponentFunctionCurrentValue( i + 3, timeOfFlight_ );
    }
...
void HodographicShapingLeg::satisfyNormalBoundaryConditions( const Eigen::VectorXd& freeCoefficients )
{
    ...
    {
        vectorBoundaryConditionsNormal[ 0 ] -=
                freeCoefficients[ i ] * normalVelocityFunction_->getComponentFunctionCurrentValue( i + 3, 0.0 );
        vectorBoundaryConditionsNormal[ 1 ] -=
                freeCoefficients[ i ] * normalVelocityFunction_->getComponentFunctionCurrentValue( i + 3, timeOfFlight_ );
    }
...
double HodographicShapingLeg::computeDerivativePolarAngleDueToThirdComponent( const double timeSinceDeparture,
                                                                              const Eigen::Vector2d& matrixK )
{
...
double HodographicShapingLeg::computeDerivativePolarAngleDueToOtherComponents( const double timeSinceDeparture,
                                                                               ...
                                                                               const Eigen::VectorXd& freeCoefficients )
{
    ...
    for( int j = 0; j < numberOfFreeNormalCoefficients_; j++ )
    {
        angularVelocityDueToFreeCoefficients +=
                freeCoefficients( j ) * normalVelocityFunction_->getComponentFunctionCurrentValue( j + 3, timeSinceDeparture );
    }
...
Eigen::Vector6d HodographicShapingLeg::computeStateVectorInCylindricalCoordinates( const double timeSinceDeparture )
{
    ...
    Eigen::Vector6d cylindricalState = ( Eigen::Vector6d( ) << computeCurrentRadialDistance( timeSinceDeparture ),
                                         ...
                                         velocityCylindricalCoordinates[ 2 ] )
                                               .finished( );
...
Eigen::Vector3d HodographicShapingLeg::computeThrustAccelerationInCylindricalCoordinates( const double timeSinceDeparture )
{
    ...
    thrustAccelerationComponents[ 0 ] = radialVelocityFunction_->evaluateCompositeFunctionDerivativeCurrentValue( timeSinceDeparture ) -
            angularVelocity * normalVelocity +
            centralBodyGravitationalParameter_ / std::pow( distanceFromCentralBody, 3.0 ) * radialDistance;

    // Compute normal thrust acceleration.
    thrustAccelerationComponents[ 1 ] = normalVelocityFunction_->evaluateCompositeFunctionDerivativeCurrentValue( timeSinceDeparture ) +
    ...
    thrustAccelerationComponents[ 2 ] = axialVelocityFunction_->evaluateCompositeFunctionDerivativeCurrentValue( timeSinceDeparture ) +
...
double HodographicShapingLeg::computeThrustAccelerationMagnitude( const double timeSinceDeparture )
{
    // Return total thrust acceleration.
...
Eigen::Vector3d HodographicShapingLeg::computeThrustAcceleration( const double timeSinceDeparture )
{
    ...
    if( thrustAccelerationVectorCache_.count( timeSinceDeparture ) == 0 )
    {
        ...
        cartesianAcceleration[ 0 ] =
                ( 1.0 / ( cartesianPosition[ 0 ] + ( std::pow( cartesianPosition[ 1 ], 2 ) / cartesianPosition[ 0 ] ) ) ) *
        ...
        cartesianAcceleration[ 1 ] =
                ( cylindricalPosition[ 0 ] * cylindricalAcceleration[ 1 ] + cartesianPosition[ 1 ] * cartesianAcceleration[ 0 ] ) /
                cartesianPosition[ 0 ];
...
Eigen::Vector3d HodographicShapingLeg::computeThrustAccelerationDirection( const double timeSinceDeparture )
{
