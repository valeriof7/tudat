...
void CompositeRadialFunctionSphericalShaping::resetCompositeFunctionCoefficients( const Eigen::VectorXd& compositeFunctionCoefficients )
{
...
double CompositeRadialFunctionSphericalShaping::evaluateCompositeFunctionFirstDerivative( const double independentVariable )
{
    ...
    for( unsigned int i = 0; i < compositeFunctionComponents_.size( ); i++ )
    {
        sumComponentsFirstDerivativeValue +=
                compositeFunctionCoefficients_[ i ] * compositeFunctionComponents_[ i ]->evaluateFirstDerivative( independentVariable );
    }
...
double CompositeRadialFunctionSphericalShaping::evaluateCompositeFunctionSecondDerivative( const double independentVariable )
{
    ...
    for( unsigned int i = 0; i < compositeFunctionComponents_.size( ); i++ )
    {
        sumComponentsSecondDerivativeValue +=
                compositeFunctionCoefficients_[ i ] * compositeFunctionComponents_[ i ]->evaluateSecondDerivative( independentVariable );
    }
...
double CompositeRadialFunctionSphericalShaping::evaluateCompositeFunctionThirdDerivative( const double independentVariable )
{
    ...
    for( unsigned int i = 0; i < compositeFunctionComponents_.size( ); i++ )
    {
        sumComponentsThirdDerivativeValue +=
                compositeFunctionCoefficients_[ i ] * compositeFunctionComponents_[ i ]->evaluateThirdDerivative( independentVariable );
    }
...
double CompositeRadialFunctionSphericalShaping::getComponentFunctionCurrentValue( const int componentIndex, const double currentTime )
{
    return compositeFunctionComponents_[ componentIndex ]->evaluateFunction( currentTime );
}

double CompositeRadialFunctionSphericalShaping::getComponentFunctionFirstDerivative( const int componentIndex, const double currentTime )
{
    return compositeFunctionComponents_[ componentIndex ]->evaluateFirstDerivative( currentTime );
}

double CompositeRadialFunctionSphericalShaping::getComponentFunctionSecondDerivative( const int componentIndex, const double currentTime )
{
    return compositeFunctionComponents_[ componentIndex ]->evaluateSecondDerivative( currentTime );
}

double CompositeRadialFunctionSphericalShaping::getComponentFunctionThirdDerivative( const int componentIndex, const double currentTime )
{
    return compositeFunctionComponents_[ componentIndex ]->evaluateThirdDerivative( currentTime );
}

void CompositeElevationFunctionSphericalShaping::resetCompositeFunctionCoefficients( const Eigen::VectorXd& compositeFunctionCoefficients )
{
    // Check whether the size is correct.
    if( compositeFunctionCoefficients.size( ) == static_cast< int >( compositeFunctionComponents_.size( ) ) )
    {
...
double CompositeElevationFunctionSphericalShaping::getComponentFunctionCurrentValue( const int componentIndex, const double currentTime )
{
    return compositeFunctionComponents_[ componentIndex ]->evaluateFunction( currentTime );
}

double CompositeElevationFunctionSphericalShaping::getComponentFunctionFirstDerivative( const int componentIndex, const double currentTime )
{
    return compositeFunctionComponents_[ componentIndex ]->evaluateFirstDerivative( currentTime );
}

double CompositeElevationFunctionSphericalShaping::getComponentFunctionSecondDerivative( const int componentIndex,
                                                                                         const double currentTime )
{
    return compositeFunctionComponents_[ componentIndex ]->evaluateSecondDerivative( currentTime );
}

double CompositeElevationFunctionSphericalShaping::getComponentFunctionThirdDerivative( const int componentIndex, const double currentTime )
{
    return compositeFunctionComponents_[ componentIndex ]->evaluateThirdDerivative( currentTime );
}
