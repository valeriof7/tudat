...
SphericalShapingLeg::SphericalShapingLeg( const std::shared_ptr< ephemerides::Ephemeris > departureBodyEphemeris,
    ...
    centralBodyGravitationalParameter_( centralBodyGravitationalParameter ), rootFinderSettings_( rootFinderSettings ),
    initialValueFreeCoefficient_( initialValueFreeCoefficient ), lowerBoundFreeCoefficient_( lowerBoundFreeCoefficient ),
    upperBoundFreeCoefficient_( upperBoundFreeCoefficient ), timeToAzimuthInterpolatorStepSize_( timeToAzimuthInterpolatorStepSize )
{
    ...
    radialDistanceCompositeFunction_ =
            std::make_shared< CompositeRadialFunctionSphericalShaping >( Eigen::VectorXd( coefficientsRadialDistanceFunction_ ) );
    elevationAngleCompositeFunction_ =
            std::make_shared< CompositeElevationFunctionSphericalShaping >( Eigen::VectorXd( coefficientsElevationAngleFunction_ ) );
...
SphericalShapingLeg::SphericalShapingLeg( const std::shared_ptr< ephemerides::Ephemeris > departureBodyEphemeris,
                                          ...
                                          const double timeToAzimuthInterpolatorStepSize ):
    mission_segments::TransferLeg( departureBodyEphemeris, arrivalBodyEphemeris, mission_segments::spherical_shaping_low_thrust_leg ),
    centralBodyGravitationalParameter_( centralBodyGravitationalParameter ), departureVelocityFunction_( departureVelocityFunction ),
    arrivalVelocityFunction_( arrivalVelocityFunction ), rootFinderSettings_( rootFinderSettings ),
    initialValueFreeCoefficient_( initialValueFreeCoefficient ), lowerBoundFreeCoefficient_( lowerBoundFreeCoefficient ),
    upperBoundFreeCoefficient_( upperBoundFreeCoefficient ), timeToAzimuthInterpolatorStepSize_( timeToAzimuthInterpolatorStepSize )
{
    // Normalize the gravitational parameter of the central body.
...
void SphericalShapingLeg::computeTransfer( )
{
    ...
    if( legParameters_( 2 ) < 0 )
    {
        throw std::runtime_error(
                "Error when updating spherical shaping object, number of revolutions should be equal to or larger than 0" );
    }
    ...
    normalizedDepartureBodyState.segment( 0, 3 ) = departureBodyState_.segment( 0, 3 ) / physical_constants::ASTRONOMICAL_UNIT;
    ...
    normalizedArrivalBodyState.segment( 0, 3 ) = arrivalBodyState_.segment( 0, 3 ) / physical_constants::ASTRONOMICAL_UNIT;
    normalizedArrivalBodyState.segment( 3, 3 ) = arrivalVelocity_ * physical_constants::JULIAN_YEAR / physical_constants::ASTRONOMICAL_UNIT;
    ...
    if( initialStateSphericalCoordinates_( 1 ) < 0.0 ) initialStateSphericalCoordinates_( 1 ) += 2.0 * mathematical_constants::PI;
    finalStateSphericalCoordinates_ = coordinate_conversions::convertCartesianToSphericalState( normalizedArrivalBodyState );
    if( finalStateSphericalCoordinates_( 1 ) < 0.0 ) finalStateSphericalCoordinates_( 1 ) += 2.0 * mathematical_constants::PI;
    ...
    ( ( finalStateSphericalCoordinates_( 1 ) - initialStateSphericalCoordinates_( 1 ) ) < 0.0 )
            ? ( finalAzimuthAngle_ =
                        finalStateSphericalCoordinates_( 1 ) + 2.0 * mathematical_constants::PI * ( numberOfRevolutions_ + 1.0 ) )
    ...
    initialStateParametrizedByAzimuthAngle_ = ( Eigen::Vector6d( ) << initialStateSphericalCoordinates_[ 0 ],
                                                ...
                                                initialStateSphericalCoordinates_[ 5 ] / initialDerivativeAzimuthAngle )
                                                      .finished( );
    finalStateParametrizedByAzimuthAngle_ = ( Eigen::Vector6d( ) << finalStateSphericalCoordinates_[ 0 ],
                                              ...
                                              finalStateSphericalCoordinates_[ 5 ] / finalDerivativeAzimuthAngle )
                                                    .finished( );
    ...
    iterateToMatchRequiredTimeOfFlight(
            rootFinderSettings_, lowerBoundFreeCoefficient_, upperBoundFreeCoefficient_, initialValueFreeCoefficient_ );

    // Vector of azimuth angles at which the time should be computed.
    Eigen::VectorXd azimuthAnglesToComputeAssociatedEpochs = Eigen::VectorXd::LinSpaced(
            std::ceil( computeNormalizedTimeOfFlight( ) * physical_constants::JULIAN_YEAR / timeToAzimuthInterpolatorStepSize_ ),
            initialAzimuthAngle_,
            finalAzimuthAngle_ );
...
double SphericalShapingLeg::convertAzimuthToTime( const double currentAzimuthAngle )
{
    ...
    {
        throw std::runtime_error(
                "Error in spherical shaping, converting azimuth to time resulted in NaN value, this could be a result of poorly defined "
                "ephemerides or gravitational parameter." );
    }
...
Eigen::MatrixXd SphericalShapingLeg::computeInverseMatrixBoundaryConditions( )
{
    ...
    for( int i = 0; i < 6; i++ )
    {
        ...
        matrixBoundaryConditions( 0, i ) =
                radialDistanceCompositeFunction_->getComponentFunctionCurrentValue( index, initialAzimuthAngle_ );
        matrixBoundaryConditions( 1, i ) = radialDistanceCompositeFunction_->getComponentFunctionCurrentValue( index, finalAzimuthAngle_ );
        matrixBoundaryConditions( 2, i ) =
                radialDistanceCompositeFunction_->getComponentFunctionFirstDerivative( index, initialAzimuthAngle_ );
        matrixBoundaryConditions( 3, i ) =
                radialDistanceCompositeFunction_->getComponentFunctionFirstDerivative( index, finalAzimuthAngle_ );
    ...
    for( int i = 0; i < 4; i++ )
    {
        ...
        matrixBoundaryConditions( 6, i + 6 ) =
                elevationAngleCompositeFunction_->getComponentFunctionCurrentValue( i, initialAzimuthAngle_ );
        matrixBoundaryConditions( 7, i + 6 ) = elevationAngleCompositeFunction_->getComponentFunctionCurrentValue( i, finalAzimuthAngle_ );
        matrixBoundaryConditions( 8, i + 6 ) =
                elevationAngleCompositeFunction_->getComponentFunctionFirstDerivative( i, initialAzimuthAngle_ );
        matrixBoundaryConditions( 9, i + 6 ) =
                elevationAngleCompositeFunction_->getComponentFunctionFirstDerivative( i, finalAzimuthAngle_ );
    }
                                                   ...
                                                   Eigen::Vector6d stateSphericalCoordinates )
{
    ...
    double derivativeOfTimeWrtAzimuthAngle =
            ( stateParametrizedByAzimuthAngle[ 0 ] * std::cos( stateParametrizedByAzimuthAngle[ 2 ] ) ) / stateSphericalCoordinates[ 4 ];
...
void SphericalShapingLeg::satisfyBoundaryConditions( double freeCoefficient )
{
    ...
    vectorBoundaryValues[ 2 ] =
            -initialStateParametrizedByAzimuthAngle_[ 3 ] / std::pow( initialStateParametrizedByAzimuthAngle_[ 0 ], 2.0 );
    vectorBoundaryValues[ 3 ] = -finalStateParametrizedByAzimuthAngle_[ 3 ] / std::pow( finalStateParametrizedByAzimuthAngle_[ 0 ], 2.0 );
    vectorBoundaryValues[ 4 ] = computeValueConstantC( initialStateParametrizedByAzimuthAngle_, initialStateSphericalCoordinates_ ) -
            2.0 * std::pow( initialStateParametrizedByAzimuthAngle_[ 3 ], 2.0 ) / initialStateParametrizedByAzimuthAngle_[ 0 ];
    vectorBoundaryValues[ 5 ] = computeValueConstantC( finalStateParametrizedByAzimuthAngle_, finalStateSphericalCoordinates_ ) -
    ...
    vectorSecondComponentContribution[ 2 ] =
            radialDistanceCompositeFunction_->getComponentFunctionFirstDerivative( 2, initialAzimuthAngle_ );
    ...
    Eigen::MatrixXd compositeFunctionCoefficients =
            inverseMatrixBoundaryConditions * ( vectorBoundaryValues - vectorSecondComponentContribution );
...
double SphericalShapingLeg::computeScalarFunctionD( double currentAzimuthAngle )
{
    ...
    double firstDerivativeRadialFunction =
            radialDistanceCompositeFunction_->evaluateCompositeFunctionFirstDerivative( currentAzimuthAngle );
    double secondDerivativeRadialFunction =
            radialDistanceCompositeFunction_->evaluateCompositeFunctionSecondDerivative( currentAzimuthAngle );

    double elevationFunctionValue = elevationAngleCompositeFunction_->evaluateCompositeFunction( currentAzimuthAngle );
    double firstDerivativeElevationFunction =
            elevationAngleCompositeFunction_->evaluateCompositeFunctionFirstDerivative( currentAzimuthAngle );
    double secondDerivativeElevationFunction =
            elevationAngleCompositeFunction_->evaluateCompositeFunctionSecondDerivative( currentAzimuthAngle );
...
double SphericalShapingLeg::computeDerivativeScalarFunctionD( double currentAzimuthAngle )
{
    ...
    double secondDerivativeRadialFunction =
            radialDistanceCompositeFunction_->evaluateCompositeFunctionSecondDerivative( currentAzimuthAngle );
    double thirdDerivativeRadialFunction =
            radialDistanceCompositeFunction_->evaluateCompositeFunctionThirdDerivative( currentAzimuthAngle );
    ...
    double secondDerivativeElevationFunction =
            elevationAngleCompositeFunction_->evaluateCompositeFunctionSecondDerivative( currentAzimuthAngle );
    double thirdDerivativeElevationFunction =
            elevationAngleCompositeFunction_->evaluateCompositeFunctionThirdDerivative( currentAzimuthAngle );
                                                              ...
                                                              const double initialGuess )
{
    ...
    std::shared_ptr< root_finders::RootFinder< double > > rootFinder =
            root_finders::createRootFinder( rootFinderSettings, lowerBound, upperBound, initialGuess );
...
double SphericalShapingLeg::computeSecondDerivativeAzimuthAngleWrtTime( const double currentAzimuthAngle )
{
    ...
    double radialDistance = radialDistanceCompositeFunction_->evaluateCompositeFunction( currentAzimuthAngle );
    double firstDerivativeRadialDistance =
            radialDistanceCompositeFunction_->evaluateCompositeFunctionFirstDerivative( currentAzimuthAngle );

    return -std::pow( firstDerivativeAzimuthAngle, 2.0 ) *
            ( derivativeScalarFunctionTimeEquation / ( 2.0 * scalarFunctionTimeEquation ) +
              firstDerivativeRadialDistance / radialDistance );
}
...
Eigen::Vector6d SphericalShapingLeg::computeNormalizedStateInSphericalCoordinates( const double currentAzimuthAngle )
{
    ...
    currentSphericalState.segment( 0, 3 ) =
            ( Eigen::Vector3d( ) << radialDistanceCompositeFunction_->evaluateCompositeFunction( currentAzimuthAngle ),
              currentAzimuthAngle,
              elevationAngleCompositeFunction_->evaluateCompositeFunction( currentAzimuthAngle ) )
                    .finished( );
    ...
    currentSphericalState.segment( 3, 3 ) =
            derivativeAzimuthAngle * computeNormalizedVelocityParametrizedByAzimuthAngle( currentAzimuthAngle );
...
Eigen::Vector6d SphericalShapingLeg::computeStateFromAzimuth( const double currentAzimuthAngle )
{
    ...
    Eigen::Vector6d normalizedStateVector =
            coordinate_conversions::convertSphericalToCartesianState( computeNormalizedStateInSphericalCoordinates( currentAzimuthAngle ) );

    Eigen::Vector6d dimensionalStateVector;
    dimensionalStateVector.segment( 0, 3 ) = normalizedStateVector.segment( 0, 3 ) * physical_constants::ASTRONOMICAL_UNIT;
    dimensionalStateVector.segment( 3, 3 ) =
            normalizedStateVector.segment( 3, 3 ) * physical_constants::ASTRONOMICAL_UNIT / physical_constants::JULIAN_YEAR;
...
Eigen::Vector3d SphericalShapingLeg::computeNormalizedVelocityParametrizedByAzimuthAngle( const double currentAzimuthAngle )
{
    ...
    return ( Eigen::Vector3d( ) << derivativeRadialDistance,
             ...
             radialDistance * derivativeElevationAngle )
            .finished( );
}
...
Eigen::Vector3d SphericalShapingLeg::computeNormalizedThrustAccelerationParametrizedByAzimuthAngle( const double currentAzimuthAngle )
{
    ...
    double firstDerivativeRadialDistance =
            radialDistanceCompositeFunction_->evaluateCompositeFunctionFirstDerivative( currentAzimuthAngle );
    double secondDerivativeRadialDistance =
            radialDistanceCompositeFunction_->evaluateCompositeFunctionSecondDerivative( currentAzimuthAngle );
    double elevationAngle = elevationAngleCompositeFunction_->evaluateCompositeFunction( currentAzimuthAngle );
    double firstDerivativeElevationAngle =
            elevationAngleCompositeFunction_->evaluateCompositeFunctionFirstDerivative( currentAzimuthAngle );
    double secondDerivativeElevationAngle =
            elevationAngleCompositeFunction_->evaluateCompositeFunctionSecondDerivative( currentAzimuthAngle );
...
Eigen::Vector3d SphericalShapingLeg::computeNormalizedThrustAccelerationInSphericalCoordinates( const double currentAzimuthAngle )
{
    ...
    Eigen::Vector3d velocityParametrizedByAzimuthAngle = computeNormalizedVelocityParametrizedByAzimuthAngle( currentAzimuthAngle );

    // Compute acceleration vector parametrized by azimuth angle theta.
    Eigen::Vector3d accelerationParametrizedByAzimuthAngle =
            computeNormalizedThrustAccelerationParametrizedByAzimuthAngle( currentAzimuthAngle );
    ...
    return std::pow( firstDerivativeAzimuthAngleWrtTime, 2.0 ) * accelerationParametrizedByAzimuthAngle +
            ...
            centralBodyGravitationalParameter_ / std::pow( radialDistance, 3.0 ) *
            ( Eigen::Vector3d( ) << radialDistance, 0.0, 0.0 ).finished( );
}
    ...
    if( thrustAccelerationVectorCache_.count( currentAzimuthAngle ) == 0 )
    {
        ...
        sphericalStateToBeConverted.segment( 0, 3 ) =
                ( Eigen::Vector3d( ) << radialDistanceCompositeFunction_->evaluateCompositeFunction( currentAzimuthAngle ),
                  ...
                  elevationAngleCompositeFunction_->evaluateCompositeFunction( currentAzimuthAngle ) )
                        .finished( );
        sphericalStateToBeConverted.segment( 3, 3 ) = computeNormalizedThrustAccelerationInSphericalCoordinates( currentAzimuthAngle );
...
double SphericalShapingLeg::computeDeltaV( )
{
    ...
    std::function< double( const double ) > derivativeFunctionDeltaV = [ & ]( const double currentAzimuthAngle ) {
        ...
        double derivativeOfTimeWithRespectToAzimuth =
                std::sqrt( computeScalarFunctionD( currentAzimuthAngle ) *
                           std::pow( radialDistanceCompositeFunction_->evaluateCompositeFunction( currentAzimuthAngle ), 2.0 ) /
                           centralBodyGravitationalParameter_ );
