...
std::pair< std::vector< double >, std::vector< double > > HybridMethod::performOptimisation( )
{
    ...
    problem prob{ HybridMethodProblem( stateAtDeparture_,
                                       stateAtArrival_,
                                       maximumThrust_,
                                       specificImpulse_,
                                       timeOfFlight_,
                                       initialGuessThrustModel_,
                                       initialAndFinalMEEcostatesBounds_,
    ...
    {
        island.evolve( );
        while( island.status( ) != pagmo::evolve_status::idle && island.status( ) != pagmo::evolve_status::idle_error )
        {
            island.wait( );
        }
        ...
        std::shared_ptr< numerical_integrators::IntegratorSettings< double > > integratorSettings )
{
    ...
    propulsion::MeeCostatesBangBangThrustMagnitudeWrapper thrustMagnitudeWrapper =
            propulsion::MeeCostatesBangBangThrustMagnitudeWrapper( thrustingBodyStateFunction,
                                                                   ...
                                                                   hybridMethodModel_->getCostatesFunction_( ),
                                                                   maximumThrust_,
                                                                   specificImpulseFunction,
                                                                   thrustingBodyMassFunction );

    propulsion::MeeCostateBasedThrustGuidance thrustGuidance =
            propulsion::MeeCostateBasedThrustGuidance( thrustingBodyStateFunction,
                                                       centralBodyStateFunction,
                                                       centralBodyGravitationalParameterFunction,
                                                       hybridMethodModel_->getCostatesFunction_( ),
...
void HybridMethod::getThrustForceProfile( std::vector< double >& epochsVector,
    ...
    for( unsigned int i = 0; i < epochsVector.size( ); i++ )
    {
        if( ( i > 0 ) && ( epochsVector[ i ] < epochsVector[ i - 1 ] ) )
        {
            throw std::runtime_error(
                    "Error when retrieving the thrust profile of a hybrid method trajectory, "
        ...
        thrustProfile[ epochsVector[ i ] ] =
                thrustMagnitudeWrapper.getCurrentThrustMagnitude( ) * thrustGuidance.getCurrentForceDirectionInPropagationFrame( );
    }
}
...
void HybridMethod::getThrustAccelerationProfile( std::vector< double >& epochsVector,
