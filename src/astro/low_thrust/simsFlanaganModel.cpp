                                                                                        ...
                                                                                        const std::string& centralBody )
{
    ...
    accelerationsSettings[ centralBody ].push_back(
...
void SimsFlanaganModel::propagateMassToSegments( )
{
    ...
    for( int currentSegment = 0; currentSegment < numberSegments_; currentSegment++ )
    {
        ...
        double currentTime =
                timesAtNodes_[ currentSegment ] + ( timesAtNodes_[ currentSegment + 1 ] - timesAtNodes_[ currentSegment ] ) / 2.0;
...
void SimsFlanaganModel::propagateForwardFromDepartureToMatchPoint( )
{
    ...
    currentState =
            propagateTrajectoryForward( 0.0, timeAtMatchPoint_, stateAtDeparture_, timeAtMatchPoint_ / numberSegmentsForwardPropagation_ );
...
Eigen::Vector6d SimsFlanaganModel::propagateInsideForwardSegment( double initialTime,
                                                                  double finalTime,
                                                                  double segmentDuration,
                                                                  Eigen::Vector6d initialState )
{
    ...
    else
    {
        ...
        if( ( timesAtNodes_[ currentSegment ] + segmentDuration / 2.0 ) - initialTime > 0.0 )
        {
            propagatedState = orbital_element_conversions::convertKeplerianToCartesianElements(
                    orbital_element_conversions::propagateKeplerOrbit(
                            orbital_element_conversions::convertCartesianToKeplerianElements( propagatedState,
                                                                                              centralBodyGravitationalParameter_ ),
        ...
        {
            deltaVvector[ i ] = maximumThrust_ / segmentMasses_[ currentSegment ] * segmentDuration * throttles_[ currentSegment ][ i ];
        }
...
//! Propagate the trajectory inside one segment.
Eigen::Vector6d SimsFlanaganModel::propagateInsideBackwardSegment( double initialTime,
                                                                   double finalTime,
                                                                   double segmentDuration,
                                                                   Eigen::Vector6d initialState )
{
...
Eigen::Vector6d SimsFlanaganModel::propagateTrajectoryForward( double initialTime,
                                                               double finalTime,
                                                               Eigen::Vector6d initialState,
                                                               double segmentDuration )
{
    ...
    else
    {
        ...
        propagatedState =
                propagateInsideForwardSegment( initialTime, timesAtNodes_[ initialSegment + 1 ], segmentDuration, propagatedState );
        ...
        for( int i = 0; i < propagatedSegments; i++ )
        {
            propagatedState = propagateInsideForwardSegment(
                    timesAtNodes_[ currentSegment ], timesAtNodes_[ currentSegment + 1 ], segmentDuration, propagatedState );
            currentSegment++;
        }
...
Eigen::Vector6d SimsFlanaganModel::propagateTrajectoryBackward( double initialTime,
                                                                double finalTime,
                                                                Eigen::Vector6d initialState,
                                                                double segmentDuration )
{
    ...
    else
    {
        ...
        for( int i = 0; i < propagatedSegments; i++ )
        {
            propagatedState = propagateInsideBackwardSegment(
                    timesAtNodes_[ currentSegment + 1 ], timesAtNodes_[ currentSegment ], segmentDuration, propagatedState );
            currentSegment--;
        }
        ...
        {
            propagatedState =
                    propagateInsideBackwardSegment( timesAtNodes_[ currentSegment + 1 ], finalTime, segmentDuration, propagatedState );
            currentSegment--;
        }
    }
        ...
        double segmentDuration )
{
    ...
    for( unsigned int epochIndex = 0; epochIndex < epochs.size( ); epochIndex++ )
    {
        ...
        if( epochIndex > 0 )
        {
            if( currentTime < epochs[ epochIndex - 1 ] )
            {
                throw std::runtime_error(
                        "Error when propagating trajectory in Sims-Flanagan, epochs at which the trajectory should be computed are not in "
                        "increasing order." );
            }
        }
        ...
        {
            throw std::runtime_error(
                    "Error when propagating trajectory in Sims-Flanagan, epochs at which the trajectory should be computed are not "
                    "constrained between 0.0 and timeOfFlight." );
        }
        ...
        {
            propagatedState = propagateTrajectoryForward( epochs[ epochIndex - 1 ], currentTime, propagatedState, segmentDuration );
            propagatedTrajectory[ currentTime ] = propagatedState;
        }
    }
        ...
        double segmentDuration )
{
    ...
    for( unsigned int epochIndex = 0; epochIndex < epochs.size( ); epochIndex++ )
    {
        ...
        if( epochIndex > 0 )
        {
            ...
            {
                throw std::runtime_error(
                        "Error when propagating trajectory backward in Sims-Flanagan, epochs at which the trajectory should be "
...
//! Propagate the trajectory to set of epochs.
void SimsFlanaganModel::propagateTrajectory( std::vector< double > epochs, std::map< double, Eigen::Vector6d >& propagatedTrajectory )
{
    ...
    for( unsigned int epochIndex = 0; epochIndex < epochs.size( ); epochIndex++ )
    {
        ...
        {
            throw std::runtime_error(
                    "Error when propagating trajectory in Sims-Flanagan, epochs at which the trajectory should be "
        ...
        if( ( epochIndex != epochs.size( ) - 1 ) &&
            ( ( currentTime <= timeOfFlight_ / 2.0 ) && ( epochs[ epochIndex + 1 ] >= timeOfFlight_ / 2.0 ) ) )
        {
            index = epochIndex;
        }
    }
    ...
    propagatedTrajectory =
            propagateTrajectoryForward( epochsVectorForwardPropagation, propagatedTrajectory, segmentDurationForwardPropagation_ );

    // Propagate the backward part of the trajectory.
    propagatedTrajectory =
            propagateTrajectoryBackward( epochsVectorBackwardPropagation, propagatedTrajectory, segmentDurationBackwardPropagation_ );
}
