...
double computePrandtlMeyerFunction( double machNumber, double ratioOfSpecificHeats )
{
    ...
    return sqrt( ( ratioOfSpecificHeats + 1.0 ) / ( ratioOfSpecificHeats - 1.0 ) ) *
            atan( sqrt( ( ratioOfSpecificHeats - 1.0 ) / ( ratioOfSpecificHeats + 1.0 ) * ( machNumberSquared_ - 1.0 ) ) ) -
...
double computeStagnationPressure( double machNumber, double ratioOfSpecificHeats )
{
    ...
    return 2.0 / ( ratioOfSpecificHeats * machNumberSquared_ ) *
            ( pow( pow( ( ratioOfSpecificHeats + 1.0 ) * machNumber, 2.0 ) /
                           ( 4.0 * ratioOfSpecificHeats * machNumberSquared_ - 2.0 * ( ratioOfSpecificHeats - 1.0 ) ),
                   ratioOfSpecificHeats / ( ratioOfSpecificHeats - 1.0 ) ) *
                      ( ( 1.0 - ratioOfSpecificHeats + 2.0 * ratioOfSpecificHeats * machNumberSquared_ ) /
...
double computeEmpiricalTangentWedgePressureCoefficient( double inclinationAngle, double machNumber )
{
    ...
    return ( pow( 1.2 * machNumberSine_ + exp( -0.6 * machNumberSine_ ), 2.0 ) - 1.0 ) / ( 0.6 * pow( machNumber, 2.0 ) );
}
...
double computeEmpiricalTangentConePressureCoefficient( double inclinationAngle, double machNumber )
{
    ...
    // Set local variables.
    machNumberSine_ = machNumber * sin( inclinationAngle );
    temporaryValue_ = pow( ( 1.090909 * machNumberSine_ + exp( -0.5454545 * machNumberSine_ ) ), 2.0 );

    // Return pressure coefficient approximation.
    return ( 48.0 * temporaryValue_ * pow( sin( inclinationAngle ), 2.0 ) ) / ( 23.0 * temporaryValue_ - 5.0 );
}
...
double computeModifiedDahlemBuckPressureCoefficient( double inclinationAngle, double machNumber )
{
    ...
    {
        pressureCoefficient_ = computeNewtonianPressureCoefficient( inclinationAngle );
    }

    // Else use Dahlem-Buck method.
    else
    {
        pressureCoefficient_ = ( 1.0 + sin( 4.0 * pow( inclinationAngle, 0.75 ) ) ) /
                ( pow( 4.0 * cos( inclinationAngle ) * cos( 2.0 * inclinationAngle ), 0.75 ) ) * pow( sin( inclinationAngle ), 1.25 );
    }
...
double computeHankeyFlatSurfacePressureCoefficient( double inclinationAngle, double machNumber )
{
    ...
    {
        stagnationPressureCoefficient_ = ( 0.195 + 0.222594 / pow( machNumber, 0.3 ) - 0.4 ) * inclinationAngle * 180.0 / PI + 4.0;
    }
    // Calculate 'effective' stagnation pressure coefficient for other
    // inclination angle.
    else
    {
        stagnationPressureCoefficient_ = 1.95 + 0.3925 / ( pow( machNumber, 0.3 ) * tan( inclinationAngle ) );
    }

    // Return pressure coefficient using 'effective' stagnation pressure
    // coefficient.
    return computeModifiedNewtonianPressureCoefficient( inclinationAngle, stagnationPressureCoefficient_ );
}
...
double computeSmythDeltaWingPressureCoefficient( double inclinationAngle, double machNumber )
{
    ...
    // Return pressure coefficient.
    return 1.66667 * ( pow( 1.09 * machNumberSine_ + exp( -0.49 * machNumberSine_ ), 2.0 ) - 1.0 ) / pow( machNumber, 2.0 );
}
...
double computeVanDykeUnifiedPressureCoefficient( double inclinationAngle, double machNumber, double ratioOfSpecificHeats, int type )
{
    ...
    if( inclinationAngle >= 0.0 && type == 1 )
    {
        pressureCoefficient_ = pow( inclinationAngle, 2.0 ) *
                ( ratioOfSpecificHeatsTerm_ +
                  sqrt( pow( ratioOfSpecificHeatsTerm_, 2.0 ) + 4.0 / ( pow( inclinationAngle * machNumberTerm_, 2.0 ) ) ) );
    }
    ...
    else if( inclinationAngle < 0.0 && type == -1 )
    {
        ...
        double vacuumPressureCoefficient_ = computeVacuumPressureCoefficient( machNumber, ratioOfSpecificHeats );

        // Check to see if pressure coefficient will be lower than vacuum case,
        // set to vacuum if so.
        if( -1.0 * inclinationAngle * machNumberTerm_ > 2.0 / ( ratioOfSpecificHeats - 1.0 ) )
        {
            pressureCoefficient_ = vacuumPressureCoefficient_;
        }
        else
        {
            pressureCoefficient_ = 2.0 / ( ratioOfSpecificHeats * pow( machNumberTerm_, 2.0 ) ) *
                    ( pow( 1.0 - ( ratioOfSpecificHeats - 1.0 ) / 2.0 * -1.0 * inclinationAngle * machNumberTerm_, exponent_ ) - 1.0 );
                                                         ...
                                                         double freestreamPrandtlMeyerFunction )
{
    ...
    {
        pressureCoefficient_ = computeVacuumPressureCoefficient( machNumber, ratioOfSpecificHeats );
    }

    else
    {
        // Determine local mach number.
        double localMachNumber_ = computeInversePrandtlMeyerFunction( prandtlMeyerFunction_ );

        // Determine local to freestream pressure ratio.
        double pressureRatio_ = computeLocalToStaticPressureRatio( localMachNumber_, ratioOfSpecificHeats ) /
                computeLocalToStaticPressureRatio( machNumber, ratioOfSpecificHeats );

        // Form pressure coefficient.
        pressureCoefficient_ = 2.0 / ( ratioOfSpecificHeats * pow( machNumber, 2.0 ) ) * ( pressureRatio_ - 1.0 );
    }
...
double computeAcmEmpiricalPressureCoefficient( double inclinationAngle, double machNumber )
{
    ...
    preliminaryPressureCoefficient_ = 180.0 / PI * inclinationAngle / ( 16.0 * pow( machNumber, 2.0 ) );
...
double computeInversePrandtlMeyerFunction( double prandtlMeyerFunctionValue )
{
    ...
    inputVariableForCorrelation_ = pow( prandtlMeyerFunctionValue / maximumPrandtlMeyerFunctionValue, 2.0 / 3.0 );

    // Calculate Mach number.
    machNumber_ = ( 1.0 +
                    inputVariableForCorrelation_ *
                            ( PrandtlMeyerParameter1 +
                              inputVariableForCorrelation_ *
                                      ( PrandtlMeyerParameter2 + inputVariableForCorrelation_ * PrandtlMeyerParameter3 ) ) ) /
            ( 1.0 + inputVariableForCorrelation_ * ( PrandtlMeyerParameter4 + inputVariableForCorrelation_ * PrandtlMeyerParameter5 ) );
...
double computeShockPressureRatio( double normalMachNumber, double ratioOfSpecificHeats )
{
    ...
    return 1.0 + 2.0 * ratioOfSpecificHeats / ( ratioOfSpecificHeats + 1.0 ) * ( normalMachNumber * normalMachNumber - 1.0 );
}
...
double computeShockDensityRatio( double normalMachNumber, double ratioOfSpecificHeats )
{
    ...
    return ( ratioOfSpecificHeats + 1.0 ) * machNumberSquared_ / ( 2.0 + ( ratioOfSpecificHeats - 1.0 ) * machNumberSquared_ );
}
...
double computeShockEntropyJump( double normalMachNumber, double ratioOfSpecificHeats, double specificGasConstant )
{
    ...
    specificHeatConstantPressure_ = ratioOfSpecificHeats * specificGasConstant / ( ratioOfSpecificHeats - 1.0 );

    // Return entropy jump from temperature and pressure ratio.
    return specificHeatConstantPressure_ * log( computeShockTemperatureRatio( normalMachNumber, ratioOfSpecificHeats ) ) -
            specificGasConstant * log( computeShockPressureRatio( normalMachNumber, ratioOfSpecificHeats ) );
}
...
double computeShockTotalPressureRatio( double normalMachNumber, double ratioOfSpecificHeats, double specificGasConstant )
{
    ...
    return exp( -1.0 * computeShockEntropyJump( normalMachNumber, ratioOfSpecificHeats, specificGasConstant ) / specificGasConstant );
}
...
double computeShockDeflectionAngle( double shockAngle, double machNumber, double ratioOfSpecificHeats )
{
    ...
    tangentOfDeflectionAngle_ = 2.0 * ( pow( machNumber * sin( shockAngle ), 2.0 ) - 1.0 ) /
            ( tan( shockAngle ) * ( pow( machNumber, 2.0 ) * ( ratioOfSpecificHeats + cos( 2.0 * shockAngle ) ) + 2.0 ) );
...
{
    return 1.0 /
            ( std::sqrt( 2.0 ) * mathematical_constants::PI * weightedAverageCollisionDiameter * weightedAverageCollisionDiameter *
              averageNumberDensity );
}
...
{
    return computeAerodynamicLoadFromAcceleration( 0.5 * airDensity * airSpeed * airSpeed * referenceArea * aerodynamicForceCoefficients /
                                                   vehicleMass );
}
...
{
    return heatTransferFunction( computeEquilibiumWallTemperature( heatTransferFunction, wallEmmisivity, adiabaticWallTemperature ) );
}
...
double computeAdiabaticWallTemperature( const double airTemperature,
                                        const double machNumber,
                                        const double ratioSpecificHeats,
                                        const double recoveryFactor )
{
    double totalTemperature = airTemperature * ( 1 + 0.5 * ( ratioSpecificHeats - 1 ) * machNumber * machNumber );
