...
Eigen::Vector6d ApproximateJplEphemeris::getCartesianState( const double secondsSinceEpoch )
{
    // Convert planet elements in Keplerian elements to Cartesian elements.
    return orbital_element_conversions::convertKeplerianToCartesianElements( getKeplerianStateFromEphemeris( secondsSinceEpoch ),
                                                                             sunGravitationalParameter_ + planetGravitationalParameter_ );
}

//! Get keplerian state from ephemeris.
Eigen::Vector6d ApproximateJplEphemeris::getKeplerianStateFromEphemeris( const double secondsSinceEpoch )
{
    using std::cos;
    using std::pow;
    using std::sin;
    ...
    // Set Julian date.
    julianDate_ = basic_astrodynamics::convertSecondsSinceEpochToJulianDay( secondsSinceEpoch, basic_astrodynamics::JULIAN_DAY_ON_J2000 );
    ...
    planetKeplerianElementsAtGivenJulianDate_( semiMajorAxisIndex ) = approximatePlanetPositionsDataContainer_.semiMajorAxis_ +
            ( approximatePlanetPositionsDataContainer_.rateOfChangeOfSemiMajorAxis_ * numberOfCenturiesPastJ2000_ );

    // Compute and set eccentricity of planet at given Julian date.
    planetKeplerianElementsAtGivenJulianDate_( eccentricityIndex ) = approximatePlanetPositionsDataContainer_.eccentricity_ +
            ( approximatePlanetPositionsDataContainer_.rateOfChangeOfEccentricity_ * numberOfCenturiesPastJ2000_ );

    // Compute and set inclination of planet at given Julian date.
    planetKeplerianElementsAtGivenJulianDate_( inclinationIndex ) = approximatePlanetPositionsDataContainer_.inclination_ +
            ( approximatePlanetPositionsDataContainer_.rateOfChangeOfInclination_ * numberOfCenturiesPastJ2000_ );
    ...
    planetKeplerianElementsAtGivenJulianDate_( longitudeOfAscendingNodeIndex ) =
            ...
            ( approximatePlanetPositionsDataContainer_.rateOfChangeOfLongitudeOfAscendingNode_ * numberOfCenturiesPastJ2000_ );

    // Compute longitude of perihelion of planet at given Julian date.
    longitudeOfPerihelionAtGivenJulianDate_ = approximatePlanetPositionsDataContainer_.longitudeOfPerihelion_ +
            ( approximatePlanetPositionsDataContainer_.rateOfChangeOfLongitudeOfPerihelion_ * numberOfCenturiesPastJ2000_ );
    ...
    planetKeplerianElementsAtGivenJulianDate_( argumentOfPeriapsisIndex ) =
            longitudeOfPerihelionAtGivenJulianDate_ - planetKeplerianElementsAtGivenJulianDate_( longitudeOfAscendingNodeIndex );
    ...
    meanLongitudeAtGivenJulianDate_ = approximatePlanetPositionsDataContainer_.meanLongitude_ +
            ( approximatePlanetPositionsDataContainer_.rateOfChangeOfMeanLongitude_ * numberOfCenturiesPastJ2000_ );

    // Compute mean anomaly of planet at given Julian date.
    meanAnomalyAtGivenJulianDate_ = meanLongitudeAtGivenJulianDate_ - longitudeOfPerihelionAtGivenJulianDate_ +
            ( approximatePlanetPositionsDataContainer_.additionalTermB_ * pow( numberOfCenturiesPastJ2000_, 2.0 ) ) +
    ...
    meanAnomalyAtGivenJulianDate_ = basic_mathematics::computeModulo( meanAnomalyAtGivenJulianDate_, 360.0 );
    ...
    eccentricAnomalyAtGivenJulianDate_ =
            convertMeanAnomalyToEccentricAnomaly( planetKeplerianElementsAtGivenJulianDate_( eccentricityIndex ),
    ...
    trueAnomalyAtGivenJulianData_ = orbital_element_conversions::convertEccentricAnomalyToTrueAnomaly(
            eccentricAnomalyAtGivenJulianDate_, planetKeplerianElementsAtGivenJulianDate_( eccentricityIndex ) );

    planetKeplerianElementsAtGivenJulianDate_( trueAnomalyIndex ) = trueAnomalyAtGivenJulianData_;
    ...
    planetKeplerianElementsAtGivenJulianDate_( semiMajorAxisIndex ) =
            unit_conversions::convertAstronomicalUnitsToMeters( planetKeplerianElementsAtGivenJulianDate_( semiMajorAxisIndex ) );

    // Convert inclination from degrees to radians.
    planetKeplerianElementsAtGivenJulianDate_( inclinationIndex ) =
            unit_conversions::convertDegreesToRadians( planetKeplerianElementsAtGivenJulianDate_( inclinationIndex ) );

    // Convert longitude of ascending node from degrees to radians.
    planetKeplerianElementsAtGivenJulianDate_( longitudeOfAscendingNodeIndex ) =
            unit_conversions::convertDegreesToRadians( planetKeplerianElementsAtGivenJulianDate_( longitudeOfAscendingNodeIndex ) );

    // Convert argument of periapsis from degrees to radians.
    planetKeplerianElementsAtGivenJulianDate_( argumentOfPeriapsisIndex ) =
            unit_conversions::convertDegreesToRadians( planetKeplerianElementsAtGivenJulianDate_( argumentOfPeriapsisIndex ) );

    return planetKeplerianElementsAtGivenJulianDate_;
}

ApproximateGtopEphemeris::ApproximateGtopEphemeris( const std::string& bodyName ): Ephemeris( "Sun", "ECLIPJ2000" )
{
...
Eigen::Vector6d getGtopCartesianElements( const double daysSinceMjd2000, const int planet )
{
    ...
    switch( planet )
    {
        ...
        case( 3 ):  // Earth
            ...
            keplerElements( 3 ) =
                    ( 1.01220833333333333e+2 + 1.7191750 * T + 4.52777777777777778e-4 * T * T + 3.33333333333333333e-6 * T * T * T );
        ...
        case( 4 ):  // Mars
            ...
            keplerElements( 4 ) = ( 4.87864416666666667e+1 + 7.70991666666666667e-1 * T - 1.38888888888888889e-6 * T * T -
                                    5.33333333333333333e-6 * T * T * T );
            keplerElements( 3 ) =
                    ( 2.85431761111111111e+2 + 1.069766666666666670 * T + 1.3125e-4 * T * T + 4.13888888888888889e-6 * T * T * T );
        ...
        case( 5 ):  // Jupiter
            ...
            keplerElements( 4 ) =
                    ( 9.94433861111111111e+1 + 1.010530 * T + 3.52222222222222222e-4 * T * T - 8.51111111111111111e-6 * T * T * T );
            keplerElements( 3 ) =
                    ( 2.73277541666666667e+2 + 5.99431666666666667e-1 * T + 7.0405e-4 * T * T + 5.07777777777777778e-6 * T * T * T );
        ...
        case( 6 ):  // Saturn
            ...
            keplerElements( 2 ) = ( 2.492519444444444440 - 3.91888888888888889e-3 * T - 1.54888888888888889e-5 * T * T +
                                    4.44444444444444444e-8 * T * T * T );
            keplerElements( 4 ) = ( 1.12790388888888889e+2 + 8.73195138888888889e-1 * T - 1.52180555555555556e-4 * T * T -
                                    5.30555555555555556e-6 * T * T * T );
            keplerElements( 3 ) = ( 3.38307772222222222e+2 + 1.085220694444444440 * T + 9.78541666666666667e-4 * T * T +
        ...
        case( 7 ):  // Uranus
            ...
            keplerElements( 3 ) =
                    ( 9.80715527777777778e+1 + 9.85765e-1 * T - 1.07447222222222222e-3 * T * T - 6.05555555555555556e-7 * T * T * T );
        ...
        case( 8 ):  // Neptune
            ...
            keplerElements( 4 ) =
                    ( 1.30681358333333333e+2 + 1.0989350 * T + 2.49866666666666667e-4 * T * T - 4.71777777777777778e-6 * T * T * T );
            keplerElements( 3 ) =
                    ( 2.76045966666666667e+2 + 3.25639444444444444e-1 * T + 1.4095e-4 * T * T + 4.11333333333333333e-6 * T * T * T );
        ...
        case( 9 ):  // Pluto
            ...
            keplerElements( 0 ) = ( 39.34041961252520 + 4.33305138120726 * T - 22.93749932403733 * T * T + 48.76336720791873 * T * T * T -
                                    45.52494862462379 * T * T * T * T + 15.55134951783384 * T * T * T * T * T );
            keplerElements( 1 ) = ( 0.24617365396517 + 0.09198001742190 * T - 0.57262288991447 * T * T + 1.39163022881098 * T * T * T -
                                    1.46948451587683 * T * T * T * T + 0.56164158721620 * T * T * T * T * T );
            keplerElements( 2 ) = ( 17.16690003784702 - 0.49770248790479 * T + 2.73751901890829 * T * T - 6.26973695197547 * T * T * T +
                                    6.36276927397430 * T * T * T * T - 2.37006911673031 * T * T * T * T * T );
            keplerElements( 4 ) = ( 110.222019291707 + 1.551579150048 * T - 9.701771291171 * T * T + 25.730756810615 * T * T * T -
                                    30.140401383522 * T * T * T * T + 12.796598193159 * T * T * T * T * T );
            keplerElements( 3 ) = ( 113.368933916592 + 9.436835192183 * T - 35.762300003726 * T * T + 48.966118351549 * T * T * T -
                                    19.384576636609 * T * T * T * T - 3.362714022614 * T * T * T * T * T );
            keplerElements( 5 ) = ( 15.17008631634665 + 137.023166578486 * T + 28.362805871736 * T * T - 29.677368415909 * T * T * T -
                                    3.585159909117 * T * T * T * T + 13.406844652829 * T * T * T * T * T );
            break;
        default:
    ...
    return tudat::orbital_element_conversions::convertKeplerianToCartesianElements( keplerElements, gtopSunGravitationalParameter );
}
}  // namespace ephemerides
}  // namespace tudat
