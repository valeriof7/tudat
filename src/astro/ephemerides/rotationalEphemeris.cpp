...
{
    Eigen::Matrix3d crossProductMatrix = rotationMatrixToGlobalFrameDerivative * rotationToTargetFrame;
    return ( Eigen::Vector3d( ) << crossProductMatrix( 2, 1 ), crossProductMatrix( 0, 2 ), crossProductMatrix( 1, 0 ) ).finished( );
}
...
{
    return linear_algebra::getCrossProductMatrix( -rotationToTargetFrame * rotationalVelocityVectorOfTargetFrameInBaseFrame ) *
            rotationToTargetFrame;
}

//! Get rotation quaternion from target frame to base frame.
template<>
Eigen::Quaterniond RotationalEphemeris::getRotationToBaseFrameTemplated< double >( const double timeSinceEpoch )
{
...
//! Get rotation quaternion from target frame to base frame.
template<>
Eigen::Quaterniond RotationalEphemeris::getRotationToBaseFrameTemplated< Time >( const Time timeSinceEpoch )
{
...
template<>
Eigen::Quaterniond RotationalEphemeris::getRotationToTargetFrameTemplated< double >( const double secondsSinceEpoch )
{
...
//! Get rotation quaternion to target frame from base frame.
template<>
Eigen::Quaterniond RotationalEphemeris::getRotationToTargetFrameTemplated< Time >( const Time secondsSinceEpoch )
{
...
template<>
Eigen::Matrix3d RotationalEphemeris::getDerivativeOfRotationToBaseFrameTemplated< double >( const double timeSinceEpoch )
{
...
//! Function to calculate the derivative of the rotation matrix from target frame to original frame.
template<>
Eigen::Matrix3d RotationalEphemeris::getDerivativeOfRotationToBaseFrameTemplated< Time >( const Time timeSinceEpoch )
{
...
template<>
Eigen::Matrix3d RotationalEphemeris::getDerivativeOfRotationToTargetFrameTemplated< double >( const double secondsSinceEpoch )
{
...
//! Function to calculate the derivative of the rotation matrix to target frame from original frame.
template<>
Eigen::Matrix3d RotationalEphemeris::getDerivativeOfRotationToTargetFrameTemplated< Time >( const Time secondsSinceEpoch )
{
        ...
        const double timeSinceEpoch )
{
    getFullRotationalQuantitiesToTargetFrame(
            currentRotationToLocalFrame, currentRotationToLocalFrameDerivative, currentAngularVelocityVectorInGlobalFrame, timeSinceEpoch );
}
