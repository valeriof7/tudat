...
void PanelledRadiationPressurePartial::update( const double currentTime )
{
    if( !( currentTime_ == currentTime ) )
    {
        ...
        Eigen::Vector3d bodyFixedUnitVectorToSource =
                -( radiationPressureAcceleration_->getTargetRotationFromGlobalToLocalFrame( ) * inertialVectorFromSource.normalized( ) );
        ...
        if( currentRadiationPressure > 0.0 )
        {
            ...
            for( int i = 0; i < panelledTargetModel_->getTotalNumberOfPanels( ); i++ )
            {
                ...
                {
                    currentPanelPartialContribution = panelledTargetModel_->getPanelForces( acceleratingBody_ ).at( i ).norm( ) *
                            rotationToInertialFrame.transpose( ) *
                            ( Eigen::Matrix3d::Identity( ) / distanceToSource -
                              inertialVectorFromSource * inertialVectorFromSource.transpose( ) /
                                      ( distanceToSource * distanceToSource * distanceToSource ) ) *
...
void PanelledRadiationPressurePartial::wrtDiffuseReflectivity( Eigen::MatrixXd& partial, const std::string& panelTypeId )
{
...
void PanelledRadiationPressurePartial::wrtSpecularReflectivity( Eigen::MatrixXd& partial, const std::string& panelTypeId )
{
    Eigen::Vector3d partial_temp = Eigen::Vector3d::Zero( );
    partial = partial_temp;
