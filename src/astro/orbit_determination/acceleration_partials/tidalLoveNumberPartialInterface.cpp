...
TidalLoveNumberPartialInterface::calculateSphericalHarmonicCoefficientsPartialWrtComplexTidalLoveNumbers(
        const int degree,
        ...
        const int maximumOrder )
{
    ...
    return calculateSphericalHarmonicCoefficientPartialMatrix( realCoefficientPartials, complexLoveNumberScaler_ );
}

//! Function to calculate the partial of spherical harmonic acceleration wrt real tidal love numbers.
std::vector< Eigen::Matrix< double, 2, Eigen::Dynamic > >
TidalLoveNumberPartialInterface::calculateSphericalHarmonicCoefficientsPartialWrtRealTidalLoveNumbers(
        const int degree,
        const std::vector< int >& orders,
        const std::vector< int >& deformingBodyIndices,
        const int maximumDegree,
        const int maximumOrder )
{
    ...
    return calculateSphericalHarmonicCoefficientPartialMatrix( realCoefficientPartials, realLoveNumberScaler_ );
}

std::vector< Eigen::Matrix< double, 2, Eigen::Dynamic > >
TidalLoveNumberPartialInterface::calculateSphericalHarmonicCoefficientsPartialWrtModeCoupledTidalLoveNumbers(
        ...
        const int maximumOrder )
{
    ...
    {
        partials.push_back(
                partialsPerDegree.at( parameterDegreeAndOrderIndices.at( i ).first ).at( parameterDegreeAndOrderIndices.at( i ).second ) );
    }
    return partials;
}
    ...
    if( parameter->getParameterName( ).second.first == deformedBody_ )
    {
        switch( parameter->getParameterName( ).first )
        {
            case full_degree_tidal_love_number: {
                ...
                if( coefficientsParameter == nullptr )
                {
                    throw std::runtime_error(
                            "Error when setting partil function of full_degree_tidal_love_number in TidalLoveNumberPartialInterface, input "
                            "is inconsistent" );
                }
            ...
            case single_degree_variable_tidal_love_number: {
                ...
                {
                    throw std::runtime_error(
                            "Error when setting partil function of single_degree_variable_tidal_love_number in "
                            "TidalLoveNumberPartialInterface, input is inconsistent" );
                }
            ...
            case mode_coupled_tidal_love_numbers: {
                ...
                // Retrieve the maximum degree and order that are to be used for this model
                getMaximumUsedDegreeAndOrder( maximumDegree,
                                              maximumOrder,
...
{
    {
        ...
        relativeDeformingBodyPosition_ =
                rotationToTidallyDeformedBody_ * ( positionsOfDeformingBodies_[ body ] - positionOfTidallyDeformedBody_ );
        relativeDeformingBodySphericalPosition_ = coordinate_conversions::convertCartesianToSpherical( relativeDeformingBodyPosition_ );
...
std::vector< Eigen::Matrix< double, 2, Eigen::Dynamic > >
TidalLoveNumberPartialInterface::calculateSphericalHarmonicCoefficientPartialMatrix(
                ...
                coefficientPartialScalers )
{
    ...
    {
        throw std::runtime_error(
                "Error when making spherical harmonic coefficient partial from coefficient partial in TidalLoveNumberPartialInterface, "
                "scale matrices not of same size." );
    }

    // COmpute scaled partials
    for( unsigned int m = 0; m < coefficientPartialsPerOrder.size( ); m++ )
    {
        coefficientsPartial.push_back( ( coefficientPartialScalers.first * coefficientPartialsPerOrder[ m ].x( ) +
