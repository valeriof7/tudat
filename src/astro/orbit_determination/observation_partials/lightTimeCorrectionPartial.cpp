    ...
    switch( lightTimeCorrectionPartial->getCorrectionType( ) )
    {
        ...
        case observation_models::first_order_relativistic: {
            ...
            if( currentLightTimeCorrectorPartial == nullptr )
            {
                std::string errorMessage = "Error when getting light time correction partial function, type " +
                        std::to_string( lightTimeCorrectionPartial->getCorrectionType( ) ) + "is inconsistent.";
            ...
            if( parameterId.first == estimatable_parameters::gravitational_parameter )
            {
                ...
                std::vector< std::string >::iterator findIterator =
                        std::find( perturbingBodies.begin( ), perturbingBodies.end( ), parameterId.second.first );
        ...
        default:
            std::string errorMessage = "Error, light time correction type " +
                    std::to_string( lightTimeCorrectionPartial->getCorrectionType( ) ) + "not found when creating partial ";
            throw std::runtime_error( errorMessage );
    }
