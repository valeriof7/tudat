                                             ...
                                             const Eigen::VectorXd currentObservation )
{
    if( fixedLinkEnd != observation_models::receiver )
    {
        throw std::runtime_error(
                "Error when updating a relative angular position scaling object, fixed link end time different from receiver." );
    }
    ...
    referenceLightTimeCorrectionScalingFirstTransmitter_ = scalingFactorFirstTransmitter_ * linkEndStates[ 0 ].segment( 3, 3 ) /
            ( physical_constants::SPEED_OF_LIGHT -
              linkEndStates[ 0 ].segment( 3, 3 ).dot( normalizedRelativeRangeVectorFirstTransmitter ) );
    referenceScalingFactorFirstTransmitter_ = scalingFactorFirstTransmitter_ *
            ( Eigen::Matrix3d::Identity( ) +
              linkEndStates[ 0 ].segment( 3, 3 ) * normalizedRelativeRangeVectorFirstTransmitter.transpose( ) /
                      ( physical_constants::SPEED_OF_LIGHT -
                        linkEndStates[ 0 ].segment( 3, 3 ).dot( normalizedRelativeRangeVectorFirstTransmitter ) ) );

    referenceLightTimeCorrectionScalingSecondTransmitter_ = scalingFactorSecondTransmitter_ * linkEndStates[ 1 ].segment( 3, 3 ) /
            ( physical_constants::SPEED_OF_LIGHT -
              linkEndStates[ 1 ].segment( 3, 3 ).dot( normalizedRelativeRangeVectorSecondTransmitter ) );
    referenceScalingFactorSecondTransmitter_ = scalingFactorSecondTransmitter_ *
            ( Eigen::Matrix3d::Identity( ) +
              linkEndStates[ 1 ].segment( 3, 3 ) * normalizedRelativeRangeVectorSecondTransmitter.transpose( ) /
                      ( physical_constants::SPEED_OF_LIGHT -
                        linkEndStates[ 1 ].segment( 3, 3 ).dot( normalizedRelativeRangeVectorSecondTransmitter ) ) );
    //    }
