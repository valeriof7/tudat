...
{
    Eigen::Matrix3d scalingMatrix = linear_algebra::getCrossProductMatrix( bodyFixedRelativePosition ) * inertiaTensor -
            linear_algebra::getCrossProductMatrix( inertiaTensor * bodyFixedRelativePosition );
        ...
        switch( parameter->getParameterName( ).first )
        {
                ...
                if( c20Index >= 0 || c21Index >= 0 || c22Index >= 0 )
                {
                    if( ( getInertiaTensorNormalizationFactor_ == nullptr ) )
                    {
                        throw std::runtime_error(
                                "Error when getting partial of 2nd degree grac torque w.r.t. cosine sh parameters, inertia tensor "
            ...
            case spherical_harmonics_sine_coefficient_block: {
                ...
                if( s21Index >= 0 || s22Index >= 0 )
                {
                    ...
                    {
                        throw std::runtime_error(
                                "Error when getting partial of 2nd degree grac torque w.r.t. sine sh parameters, inertia tensor "
                                "normalization function not found." );
                    }
                    partialFunction = std::make_pair(
...
void SecondDegreeGravitationalTorquePartial::update( const double currentTime )
{
    if( !( currentTime_ == currentTime ) )
    {
        ...
        currentQuaternionVector_ =
                linear_algebra::convertQuaternionToVectorFormat( ( torqueModel_->getCurrentRotationToBodyFixedFrame( ) ).inverse( ) );
...
void SecondDegreeGravitationalTorquePartial::wrtGravitationalParameterOfCentralBody( Eigen::MatrixXd& gravitationalParameterPartial )
{
    ...
    gravitationalParameterPartial = torqueModel_->getTorque( ) / torqueModel_->getCurrentGravitationalParameterOfAttractingBody( );
}
        ...
        const int c22Index )
{
    ...
    if( c20Index >= 0 )
    {
        sphericalHarmonicCoefficientPartial.block( 0, c20Index, 3, 1 ) =
                basic_mathematics::calculateLegendreGeodesyNormalizationFactor( 2, 0 ) * getInertiaTensorNormalizationFactor_( ) *
                currentCoefficientPartialPremultiplier_ * UNSCALED_INERTIAL_TENSOR_PARTIAL_WRT_C20 * currentBodyFixedRelativePosition_;
    }

    if( c21Index >= 0 )
    {
        sphericalHarmonicCoefficientPartial.block( 0, c21Index, 3, 1 ) =
                basic_mathematics::calculateLegendreGeodesyNormalizationFactor( 2, 1 ) * getInertiaTensorNormalizationFactor_( ) *
                currentCoefficientPartialPremultiplier_ * UNSCALED_INERTIAL_TENSOR_PARTIAL_WRT_C21 * currentBodyFixedRelativePosition_;
    }

    if( c22Index >= 0 )
    {
        sphericalHarmonicCoefficientPartial.block( 0, c22Index, 3, 1 ) =
                basic_mathematics::calculateLegendreGeodesyNormalizationFactor( 2, 2 ) * getInertiaTensorNormalizationFactor_( ) *
                currentCoefficientPartialPremultiplier_ * UNSCALED_INERTIAL_TENSOR_PARTIAL_WRT_C22 * currentBodyFixedRelativePosition_;
    }
}
        ...
        const int s22Index )
{
    ...
    if( s21Index >= 0 )
    {
        sphericalHarmonicCoefficientPartial.block( 0, s21Index, 3, 1 ) =
                basic_mathematics::calculateLegendreGeodesyNormalizationFactor( 2, 1 ) * getInertiaTensorNormalizationFactor_( ) *
                currentCoefficientPartialPremultiplier_ * UNSCALED_INERTIAL_TENSOR_PARTIAL_WRT_S21 * currentBodyFixedRelativePosition_;
    }
    ...
    if( s22Index >= 0 )
    {
        sphericalHarmonicCoefficientPartial.block( 0, s22Index, 3, 1 ) =
                basic_mathematics::calculateLegendreGeodesyNormalizationFactor( 2, 2 ) * getInertiaTensorNormalizationFactor_( ) *
                currentCoefficientPartialPremultiplier_ * UNSCALED_INERTIAL_TENSOR_PARTIAL_WRT_S22 * currentBodyFixedRelativePosition_;
    }
}
