...
{
    const auto shadowFunction = mission_geometry::computeShadowFunction( occultedSourcePosition,
...
double SingleOccultingBodyOccultationModel::evaluateReceivedFractionFromPointSource( const Eigen::Vector3d& occultedSourcePosition,
                                                                                     const Eigen::Vector3d& targetPosition ) const
{
    auto isVisible = evaluatePointToPointVisibilityWithOccultation(
            occultedSourcePosition, occultingBodyPosition, occultingBodyShapeModel_->getAverageRadius( ), targetPosition );
        ...
        const Eigen::Vector3d& targetPosition ) const
{
    ...
    {
        shadowFunctionOfBody = mission_geometry::computeShadowFunction( occultedSourcePosition,
                                                                        occultedSourceShapeModel->getAverageRadius( ),
...
double SimpleMultipleOccultingBodyOccultationModel::evaluateReceivedFractionFromPointSource( const Eigen::Vector3d& occultedSourcePosition,
                                                                                             const Eigen::Vector3d& targetPosition ) const
{
    bool isVisible = true;
    for( unsigned int i = 0; i < getNumberOfOccultingBodies( ); i++ )
    {
        ...
        // visibilities
        isVisible = isVisible &&
                evaluatePointToPointVisibilityWithOccultation( occultedSourcePosition,
                                                               occultingBodyPositions[ i ],
                                                               occultingBodyShapeModels_[ i ]->getAverageRadius( ),
                                                               targetPosition );
    }
...
bool evaluatePointToPointVisibilityWithOccultation( const Eigen::Vector3d& occultedSourcePosition,
