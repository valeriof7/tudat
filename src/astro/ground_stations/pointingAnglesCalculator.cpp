...
double PointingAnglesCalculator::calculateElevationAngle( const Eigen::Vector3d& vectorInTopoCentricFrame )
{
    ...
    return mathematical_constants::PI / 2.0 -
            linear_algebra::computeAngleBetweenVectors( vectorInTopoCentricFrame, Eigen::Vector3d::UnitZ( ) );
}

//! Function to calculate the elevation angle from body-fixed point to given point.
double PointingAnglesCalculator::calculateElevationAngleFromInertialVector( const Eigen::Vector3d& inertialVectorAwayFromStation,
                                                                            const double time )
{
    // Transform vector to local topocentric frame.
    Eigen::Vector3d vectorInTopoCentricFrame = convertVectorFromInertialToTopocentricFrame( inertialVectorAwayFromStation, time );
...
double PointingAnglesCalculator::calculateAzimuthAngleFromInertialVector( const Eigen::Vector3d& inertialVectorAwayFromStation,
                                                                          const double time )
{
    // Transform vector to local topocentric frame.
    Eigen::Vector3d vectorInTopoCentricFrame = convertVectorFromInertialToTopocentricFrame( inertialVectorAwayFromStation, time );
...
std::pair< double, double > PointingAnglesCalculator::calculatePointingAngles( const Eigen::Vector3d inertialVectorAwayFromStation,
                                                                               const double time )
{
    // Transform vector to local topocentric frame.
    Eigen::Vector3d vectorInTopoCentricFrame = convertVectorFromInertialToTopocentricFrame( inertialVectorAwayFromStation, time );
                                             ...
                                             const std::pair< int, int >& linkEndIndices )
{
    ...
    Eigen::Vector3d targetRelativeState =
            ( linkEndStates.at( linkEndIndices.second ) - linkEndStates.at( linkEndIndices.first ) ).segment( 0, 3 );
