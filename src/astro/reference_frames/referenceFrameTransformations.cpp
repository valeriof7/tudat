        ...
        const Eigen::Matrix3d& rotationMatrixFromInertialToPlanetFixedFrame )
{
    ...
    rotationAngles.x( ) = basic_mathematics::computeModulo(
            std::atan2( rotationMatrixFromInertialToPlanetFixedFrame( 2, 0 ), -rotationMatrixFromInertialToPlanetFixedFrame( 2, 1 ) ) -
                    mathematical_constants::PI / 2.0,
            2.0 * mathematical_constants::PI );  // right ascension
    rotationAngles.y( ) =
            -std::acos( rotationMatrixFromInertialToPlanetFixedFrame( 2, 2 ) ) + mathematical_constants::PI / 2.0;  // declination
...
//! Wrapper function to transform a vector to a different frame from a single rotation function.
void transformVectorReferenceFromQuaternionFunction( Eigen::Vector3d& transformedVector,
...
Eigen::Matrix3d getRotatingPlanetocentricToInertialFrameTransformationMatrix( const double angleFromXItoXR )
{
    ...
    // Set local matrix.
    localMatrix_ = reference_frames::getInertialToPlanetocentricFrameTransformationMatrix( angleFromXItoXR );
...
Eigen::Quaterniond getRotatingPlanetocentricToInertialFrameTransformationQuaternion( const double angleFromXItoXR )
{
    ...
    Eigen::AngleAxisd eigenRotationObject = Eigen::AngleAxisd( -1.0 * -angleFromXItoXR, Eigen::Vector3d::UnitZ( ) );
                                                                                     ...
                                                                                     const double longitudeOfPrimeMeridian )
{
    // Compute transformation quaternion.
    // Note the sign change, because how angleAxisd is defined.
    Eigen::AngleAxisd firstRotationAroundZaxis = Eigen::AngleAxisd( longitudeOfPrimeMeridian, Eigen::Vector3d::UnitZ( ) );
    Eigen::AngleAxisd rotationAroundXaxis =
            Eigen::AngleAxisd( ( mathematical_constants::PI / 2.0 - declinationOfPole ), Eigen::Vector3d::UnitX( ) );
    Eigen::AngleAxisd secondRotationAroundZaxis =
            Eigen::AngleAxisd( rightAscensionOfPole + mathematical_constants::PI / 2.0, Eigen::Vector3d::UnitZ( ) );
    Eigen::Quaterniond frameTransformationQuaternion =
            Eigen::Quaterniond( ( secondRotationAroundZaxis * rotationAroundXaxis * firstRotationAroundZaxis ) );
    return frameTransformationQuaternion;
}
                                          ...
                                          const bool doesNaxisPointAwayFromCentralBody )
{
    ...
    Eigen::Vector3d unitW =
            ( ( ( doesNaxisPointAwayFromCentralBody ) ? -1.0 : 1.0 ) * ( vehicleRadius.cross( vehicleVelocity ) ).normalized( ) );

    Eigen::Vector3d unitN = ( unitW.cross( unitT ) ).normalized( );

    Eigen::Matrix3d transformationMatrix;
    transformationMatrix << unitT( 0 ), unitN( 0 ), unitW( 0 ), unitT( 1 ), unitN( 1 ), unitW( 1 ), unitT( 2 ), unitN( 2 ), unitW( 2 );

    return transformationMatrix;
}

Eigen::Matrix3d getTnwToInertialRotation( const Eigen::Vector6d& vehicleInertialState, const bool doesNaxisPointAwayFromCentralBody )
{
    return getTnwToInertialRotation( vehicleInertialState, Eigen::Vector6d::Zero( ), doesNaxisPointAwayFromCentralBody );
}

Eigen::Matrix3d getInertialToTnwRotation( const Eigen::Vector6d& vehicleInertialState, const bool doesNaxisPointAwayFromCentralBody )
{
    return getTnwToInertialRotation( vehicleInertialState, Eigen::Vector6d::Zero( ), doesNaxisPointAwayFromCentralBody ).transpose( );
}
...
Eigen::Matrix3d getTnwToInertialRotationFromFunctions( const std::function< Eigen::Vector6d( ) >& vehicleStateFunction,
                                                       ...
                                                       const bool doesNaxisPointAwayFromCentralBody )
{
    return getTnwToInertialRotation( vehicleStateFunction( ), centralBodyStateFunction( ), doesNaxisPointAwayFromCentralBody );
}
...
Eigen::Quaterniond getTnwToPlanetocentricRotationKeplerian( const Eigen::Matrix< double, 6, 1 > spacecraftKeplerianState )
{
    ...
    double flightPathAngle = std::atan( ( eccentricity * std::sin( trueAnomaly ) ) / ( 1.0 + eccentricity * std::cos( trueAnomaly ) ) );

    // Compute first rotation around Z axis.
    Eigen::AngleAxisd firstRotationAroundZaxis(
            -( -mathematical_constants::PI * 0.5 + flightPathAngle - ( trueAnomaly + argumentOfPeriapsis ) ), Eigen::Vector3d::UnitZ( ) );
...
Eigen::Matrix3d getInertialToRswSatelliteCenteredFrameRotationMatrix( const Eigen::Vector6d& bodyState )
{
    ...
    Eigen::Matrix3d transformationMatrix;
    transformationMatrix << unitR( 0 ), unitR( 1 ), unitR( 2 ), unitS( 0 ), unitS( 1 ), unitS( 2 ), unitW( 0 ), unitW( 1 ), unitW( 2 );
    return transformationMatrix;
}
...
{
    Eigen::Matrix3d rotationInerialToRsw = getInertialToRswSatelliteCenteredFrameRotationMatrix( bodyState );
    double trueAnomaly = orbital_element_conversions::convertCartesianToKeplerianElements( bodyState, gravitationalParameter )( 5 );
    return Eigen::AngleAxisd( trueAnomaly, Eigen::Vector3d::UnitZ( ) ).toRotationMatrix( ) * rotationInerialToRsw;
}
...
{
    Eigen::Matrix3d rotationInerialToRsw = getInertialToRswSatelliteCenteredFrameRotationMatrix( bodyState );
    Eigen::Vector6d keplerElements = orbital_element_conversions::convertCartesianToKeplerianElements( bodyState, gravitationalParameter );
    Eigen::Matrix3d intermediateRotation =
            Eigen::AngleAxisd( -keplerElements( orbital_element_conversions::longitudeOfAscendingNodeIndex ), Eigen::Vector3d::UnitZ( ) )
                    .toRotationMatrix( );
    ...
    if( keplerElements( orbital_element_conversions::inclinationIndex ) > mathematical_constants::PI / 2.0 )
    {
        return intermediateRotation.transpose( ) *
                ...
                intermediateRotation * rotationInerialToRsw;
    }
    else
    {
        return intermediateRotation *
                Eigen::AngleAxisd( orbital_element_conversions::inclinationIndex, Eigen::Vector3d::UnitX( ) ).toRotationMatrix( ) *
                intermediateRotation.transpose( ) * rotationInerialToRsw;
    }
}
                                                                             ...
                                                                             const double longitudeOfPrimeMeridian )
{
    // Compute transformation quaternion.
    // Note the sign change, because how angleAxisd is defined.
    Eigen::AngleAxisd secondRotationAroundZaxis = Eigen::AngleAxisd( -longitudeOfPrimeMeridian, Eigen::Vector3d::UnitZ( ) );
    Eigen::AngleAxisd rotationAroundXaxis =
            Eigen::AngleAxisd( -( mathematical_constants::PI / 2.0 - declinationOfPole ), Eigen::Vector3d::UnitX( ) );
    Eigen::AngleAxisd firstRotationAroundZaxis =
            Eigen::AngleAxisd( -( rightAscensionOfPole + mathematical_constants::PI / 2.0 ), Eigen::Vector3d::UnitZ( ) );
                                                                      ...
                                                                      const double longitudeOfPrimeMeridian )
{
    return Eigen::Matrix3d(
            getInertialToPlanetocentricFrameTransformationQuaternion( declinationOfPole, rightAscensionOfPole, longitudeOfPrimeMeridian ) );
}
...
Eigen::Quaterniond getQuaternionObjectFromQuaternionValues( const Eigen::Vector4d& vectorWithQuaternion )
{
    ...
    Eigen::Quaterniond frameTransformationQuaternion = Eigen::Quaterniond(
            vectorWithQuaternion( 0 ), vectorWithQuaternion( 1 ), vectorWithQuaternion( 2 ), vectorWithQuaternion( 3 ) );
...
{
    return getRotatingPlanetocentricToLocalVerticalFrameTransformationQuaternion( longitude, latitude ).toRotationMatrix( );
}
...
{
    return getRotatingPlanetocentricToLocalVerticalFrameTransformationMatrix( longitude, latitude ).transpose( );
}
...
{
    return getTrajectoryToLocalVerticalFrameTransformationQuaternion( flightPathAngle, headingAngle ).toRotationMatrix( );
}
...
Eigen::Quaterniond getTrajectoryToLocalVerticalFrameTransformationQuaternion( const double flightPathAngle, const double headingAngle )
{
    // Compute transformation quaternion.
    // Note the sign change, because how angleAxisd is defined.
    Eigen::AngleAxisd rotationAroundZaxis = Eigen::AngleAxisd( -1.0 * -headingAngle, Eigen::Vector3d::UnitZ( ) );
    Eigen::AngleAxisd rotationAroundYaxis = Eigen::AngleAxisd( -1.0 * -flightPathAngle, Eigen::Vector3d::UnitY( ) );
...
{
    return getTrajectoryToLocalVerticalFrameTransformationMatrix( flightPathAngle, headingAngle ).transpose( );
}
...
{
    return getTrajectoryToLocalVerticalFrameTransformationQuaternion( flightPathAngle, headingAngle ).inverse( );
}
...
{
    return getTrajectoryToAerodynamicFrameTransformationQuaternion( bankAngle ).toRotationMatrix( );
}
...
Eigen::Quaterniond getTrajectoryToAerodynamicFrameTransformationQuaternion( const double bankAngle )
{
    // Compute transformation quaternion.
    // Note the sign change, because how angleAxisd is defined.
    Eigen::AngleAxisd rotationAroundXaxis = Eigen::AngleAxisd( bankAngle, Eigen::Vector3d::UnitX( ) );
...
{
    return getBodyToAirspeedBasedAerodynamicFrameTransformationQuaternion( angleOfAttack, angleOfSideslip ).toRotationMatrix( );
}
                                                                                   ...
                                                                                   const double angleOfSideslip )
{
    // Compute transformation quaternion.
    // Note the sign change, because how angleAxisd is defined.
    Eigen::AngleAxisd rotationAroundZaxis = Eigen::AngleAxisd( -1.0 * angleOfSideslip, Eigen::Vector3d::UnitZ( ) );
    Eigen::AngleAxisd rotationAroundYaxis = Eigen::AngleAxisd( -1.0 * -angleOfAttack, Eigen::Vector3d::UnitY( ) );
...
{
    return getBodyToAirspeedBasedAerodynamicFrameTransformationMatrix( angleOfAttack, angleOfSideslip ).transpose( );
}
...
Eigen::Quaterniond getAirspeedBasedAerodynamicToBodyFrameTransformationQuaternion( const double angleOfAttack,
                                                                                   const double angleOfSideslip )
{
    return getBodyToAirspeedBasedAerodynamicFrameTransformationQuaternion( angleOfAttack, angleOfSideslip ).inverse( );
}
...
Eigen::Quaterniond getRotatingPlanetocentricToEnuLocalVerticalFrameTransformationQuaternion( double longitude, double latitude )
{
    return getEnuLocalVerticalToRotatingPlanetocentricFrameTransformationQuaternion( longitude, latitude ).inverse( );
}

//! Get transformation quaternion between V-frame and ECEF
Eigen::Quaterniond getEnuLocalVerticalToRotatingPlanetocentricFrameTransformationQuaternion( double longitude, double latitude )
{
    // Compute transformation quaternion.
    ...
    Eigen::AngleAxisd RotationAroundZaxis = Eigen::AngleAxisd( longitude + mathematical_constants::PI / 2.0, Eigen::Vector3d::UnitZ( ) );
    Eigen::AngleAxisd RotationAroundXaxis = Eigen::AngleAxisd( ( mathematical_constants::PI / 2.0 - latitude ), Eigen::Vector3d::UnitX( ) );
    Eigen::Quaterniond frameTransformationQuaternion = Eigen::Quaterniond( ( RotationAroundZaxis * RotationAroundXaxis ) );
...
{
    return ( Eigen::Matrix3d( ) << 1, 0, 0, 0, 0.9174820620691818, 0.3977771559319137, 0, -0.3977771559319137, 0.9174820620691818 )
            .finished( );
}
...
{
    return ( Eigen::Matrix3d( ) << 0.0, 0.0, 0.0, 0.0, -std::sin( angle ), std::cos( angle ), 0.0, -std::cos( angle ), -std::sin( angle ) )
            .finished( );
}
...
{
    return ( Eigen::Matrix3d( ) << -std::sin( angle ), 0.0, -std::cos( angle ), 0.0, 0.0, 0.0, -std::cos( angle ), 0.0, std::sin( angle ) )
            .finished( );
}
...
{
    return ( Eigen::Matrix3d( ) << -std::sin( angle ), std::cos( angle ), 0.0, -std::cos( angle ), -std::sin( angle ), 0.0, 0.0, 0.0, 0.0 )
            .finished( );
}
...
{
    return orientationFunctionOfCentralBody( ) * ( positionFunctionOfRelativeBody( ) - positionFunctionOfCentralBody( ) );
}
...
Eigen::Vector3d getBodyFixedSphericalPosition( const std::function< Eigen::Vector3d( ) > positionFunctionOfCentralBody,
                                               ...
                                               const std::function< Eigen::Quaterniond( ) > orientationFunctionOfCentralBody )
{
    Eigen::Vector3d sphericalPosition = coordinate_conversions::convertCartesianToSpherical( getBodyFixedCartesianPosition(
                                                   ...
                                                   const SatelliteReferenceFrames targetFrame )
{
    ...
    {
        rotationMatrix = getRotationBetweenSatelliteFrames( relativeInertialCartesianState, originalFrame, global_reference_frame ) *
                getRotationBetweenSatelliteFrames( relativeInertialCartesianState, global_reference_frame, targetFrame );
    }
    else if( targetFrame == global_reference_frame )
    {
        rotationMatrix = getRotationBetweenSatelliteFrames( relativeInertialCartesianState, targetFrame, originalFrame ).transpose( );
    }
    else if( originalFrame == global_reference_frame )
    {
        switch( targetFrame )
        {
            ...
            case rsw_reference_frame:
                rotationMatrix = getInertialToRswSatelliteCenteredFrameRotationMatrix( relativeInertialCartesianState );
                break;
            case tnw_reference_frame:
                rotationMatrix = getInertialToTnwRotation( relativeInertialCartesianState, true );
                break;
            default:
...
Eigen::Matrix3d getItrf2014ToArbitraryItrfRotationMatrix( const std::string& targetFrame )
{
    ...
    {
        throw std::runtime_error( "Error when converting between ITRF frames: the selected frame (" + targetFrame + ") is not valid." );
    }
                                                                  ...
                                                                  const std::string& targetFrame )
{
    ...
    groundStationItrf2014StateAtEpoch.segment( 0, 3 ) -=
            groundStationItrf2014StateAtEpoch.segment( 3, 3 ) * ( itrf2014ReferenceEpoch - epoch );
                                                            ...
                                                            const std::string& targetFrame )
{
    ...
    {
        groundStationStateTargetFrame = convertGroundStationStateItrf2014ToArbitraryItrf( groundStationStateAtEpoch, epoch, targetFrame );
    }
    else if( targetFrame == "ITRF2014" )
    {
        groundStationStateTargetFrame = convertGroundStationStateArbitraryItrfToItrf2014( groundStationStateAtEpoch, epoch, baseFrame );
    }
    else
    {
        groundStationStateTargetFrame = convertGroundStationStateArbitraryItrfToItrf2014( groundStationStateAtEpoch, epoch, baseFrame );
        groundStationStateTargetFrame =
                convertGroundStationStateItrf2014ToArbitraryItrf( groundStationStateTargetFrame, epoch, targetFrame );
    }
