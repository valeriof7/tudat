...
double PolyhedronBodyShapeModel::getAltitude( const Eigen::Vector3d& bodyFixedPosition )
{
    ...
    if( computeAltitudeWithSign_ )
    {
        ...
        // Compute Laplacian
        double perFacetFactorsSum = -basic_mathematics::calculatePolyhedronLaplacianOfGravitationalPotential( 1.0, perFacetFactor );
...
double PolyhedronBodyShapeModel::computeDistanceToClosestVertex( const Eigen::Vector3d& bodyFixedPosition, unsigned int& closestVertexId )
{
...
double PolyhedronBodyShapeModel::computeDistanceToClosestFacet( const Eigen::Vector3d& bodyFixedPosition,
...
double PolyhedronBodyShapeModel::computeDistanceToClosestEdge( const Eigen::Vector3d& bodyFixedPosition,
...
void PolyhedronBodyShapeModel::computeVerticesDefiningEachEdge( )
{
    ...
    for( unsigned int facet = 0; facet < numberOfFacets; ++facet )
    {
        ...
        for( unsigned int edge = 0; ( edge < numberOfInsertedEdges && !edgesToInsert.empty( ) ); ++edge )
        {
            ...
            {
                if( ( edgesToInsert.at( i ).at( 0 ) == verticesDefiningEachEdge_( edge, 0 ) &&
                      edgesToInsert.at( i ).at( 1 ) == verticesDefiningEachEdge_( edge, 1 ) ) ||
                    ( edgesToInsert.at( i ).at( 0 ) == verticesDefiningEachEdge_( edge, 1 ) &&
                      edgesToInsert.at( i ).at( 1 ) == verticesDefiningEachEdge_( edge, 0 ) ) )
                {
