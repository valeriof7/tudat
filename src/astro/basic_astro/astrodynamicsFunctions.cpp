...
{
    return 2.0 * PI *
            std::sqrt( std::pow( semiMajorAxis, 3.0 ) /
                       ( ( physical_constants::GRAVITATIONAL_CONSTANT * massOfOrbitingBody ) + gravitationalParameterOfCentralBody ) );
}
...
double computeKeplerOrbitalVelocity( const double semiMajorAxis,
                                     ...
                                     const double massOfOrbitingBody )
{
    return std::sqrt( ( ( physical_constants::GRAVITATIONAL_CONSTANT * massOfOrbitingBody ) + gravitationalParameterOfCentralBody ) *
                      ( 2.0 / computeKeplerRadialDistance( semiMajorAxis, eccentricity, trueAnomaly ) - 1.0 / semiMajorAxis ) );
}
...
double computeKeplerAngularMomentum( const double semiMajorAxis,
                                     ...
                                     const double massOfOrbitingBody )
{
    return massOfOrbitingBody * std::sqrt( gravitationalParameterOfCentralBody * semiMajorAxis * ( 1.0 - std::pow( eccentricity, 2.0 ) ) );
}
...
{
    return std::sqrt( ( ( physical_constants::GRAVITATIONAL_CONSTANT * massOfOrbitingBody ) + gravitationalParameterOfCentralBody ) /
                      std::pow( semiMajorAxis, 3.0 ) );
}

//! Compute Kepler orbital energy.
double computeKeplerEnergy( const double semiMajorAxis, const double gravitationalParameterOfCentralBody, const double massOfOrbitingBody )
{
...
{
    return state( orbital_element_conversions::semiMajorAxisIndex ) * ( 1.0 - state( orbital_element_conversions::eccentricityIndex ) ) -
            centralBodyRadius;
}
                                                   ...
                                                   const double centralBodyRadius )
{
    return computePeriapsisAltitudeFromKeplerianState(
            orbital_element_conversions::convertCartesianToKeplerianElements( state, centralBodyGravitationalParameter ),
            centralBodyRadius );
}
