                                            ...
                                            const double referenceOrbitRadius )
{
    ...
    const double meanAngularMotion =
            sqrt( centralBodyGravitationalParameter / ( referenceOrbitRadius * referenceOrbitRadius * referenceOrbitRadius ) );

    // Calculate integration constants of Clohessy-Wiltshire equations.
    const double integrationConstant1 = 4.0 * initialState( 0 ) + 2.0 * initialState( 4 ) / meanAngularMotion;
    const double integrationConstant2 = initialState( 1 ) - 2.0 * initialState( 3 ) / meanAngularMotion;
    const double integrationConstant3 = 3.0 * initialState( 0 ) + 2.0 * initialState( 4 ) / meanAngularMotion;
    ...
    finalState( 0 ) = integrationConstant1 - integrationConstant3 * cosineTerm + integrationConstant4 * sineTerm;
    finalState( 1 ) = integrationConstant2 - 1.5 * integrationConstant1 * meanAngularMotion * propagationDuration +
            2.0 * integrationConstant3 * sineTerm + 2.0 * integrationConstant4 * cosineTerm;
    ...
    // by Vallado [2001, p. 382].
    finalState( 3 ) = meanAngularMotion * integrationConstant3 * sineTerm + meanAngularMotion * integrationConstant4 * cosineTerm;
    finalState( 4 ) = -1.5 * meanAngularMotion * integrationConstant1 + 2.0 * meanAngularMotion * integrationConstant3 * cosineTerm -
            2.0 * meanAngularMotion * integrationConstant4 * sineTerm;
    finalState( 5 ) = -meanAngularMotion * integrationConstant5 * sineTerm + meanAngularMotion * integrationConstant6 * cosineTerm;
