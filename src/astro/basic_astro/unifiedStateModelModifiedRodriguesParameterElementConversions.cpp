...
Eigen::Vector7d convertKeplerianToUnifiedStateModelModifiedRodriguesParameterElements( const Eigen::Vector6d& keplerianElements,
                                                                                       const double centralBodyGravitationalParameter )
{
    ...
    double RHodographElement = keplerianElements( eccentricityIndex ) * convertedUnifiedStateModelElements( CHodographUSM6Index );

    // Compute the Rf1 hodograph element of the unified state model
    convertedUnifiedStateModelElements( Rf1HodographUSM6Index ) = -RHodographElement *
    ...
    convertedUnifiedStateModelElements( Rf2HodographUSM6Index ) = RHodographElement *
    ...
    double denominator = 1.0 + std::cos( 0.5 * keplerianElements( inclinationIndex ) ) * std::cos( 0.5 * rightAscensionOfLatitude );
    ...
    convertedUnifiedStateModelElements( sigma1USM6Index ) = std::sin( 0.5 * keplerianElements( inclinationIndex ) ) *
    ...
    convertedUnifiedStateModelElements( sigma2USM6Index ) = std::sin( 0.5 * keplerianElements( inclinationIndex ) ) *
    ...
    convertedUnifiedStateModelElements( sigma3USM6Index ) =
            std::cos( 0.5 * keplerianElements( inclinationIndex ) ) * std::sin( 0.5 * rightAscensionOfLatitude ) / denominator;
...
Eigen::Vector6d convertUnifiedStateModelModifiedRodriguesParametersToKeplerianElements( const Eigen::Vector7d& unifiedStateModelElements,
                                                                                        const double centralBodyGravitationalParameter )
{
    using mathematical_constants::PI;
    ...
    double modifiedRodriguesParametersMagnitudeSquared = modifiedRodriguesParametersMagnitude * modifiedRodriguesParametersMagnitude;
    double onePlusModifiedRodriguesParametersMagnitudeSquaredSquared = std::pow( 1.0 + modifiedRodriguesParametersMagnitudeSquared, 2 );
    double oneMinusModifiedRodriguesParametersMagnitudeSquared = shadowFlag ? ( modifiedRodriguesParametersMagnitudeSquared - 1.0 )
                                                                            :  // inverse definition for SMRP
            ( 1.0 - modifiedRodriguesParametersMagnitudeSquared );
    double oneMinusModifiedRodriguesParametersMagnitudeSquaredSquared = std::pow( oneMinusModifiedRodriguesParametersMagnitudeSquared, 2 );
    ...
    double rightAscensionOfLatitude = std::atan2(
            4.0 * modifiedRodriguesParametersVector( 2 ) * ( 1.0 - modifiedRodriguesParametersMagnitudeSquared ) / denominator,
            ( oneMinusModifiedRodriguesParametersMagnitudeSquaredSquared - 4.0 * std::pow( modifiedRodriguesParametersVector( 2 ), 2 ) ) /
    ...
    double RHodographElement =
            std::sqrt( unifiedStateModelElements( Rf1HodographUSM6Index ) * unifiedStateModelElements( Rf1HodographUSM6Index ) +
                       unifiedStateModelElements( Rf2HodographUSM6Index ) * unifiedStateModelElements( Rf2HodographUSM6Index ) );

    // Compute eccentricity
    convertedKeplerianElements( eccentricityIndex ) = RHodographElement / unifiedStateModelElements( CHodographUSM6Index );
    ...
    // parabolic orbit -> semi-major axis is not defined. Use semi-latus rectum instead.
    {
        convertedKeplerianElements( semiLatusRectumIndex ) = centralBodyGravitationalParameter /
                ( unifiedStateModelElements( CHodographUSM6Index ) * unifiedStateModelElements( CHodographUSM6Index ) );
    }
    else
    {
    ...
    // Compute inclination
    double arccosineArgument =
            ( 4.0 *
                      ( std::pow( modifiedRodriguesParametersVector( 2 ), 2 ) - std::pow( modifiedRodriguesParametersVector( 0 ), 2 ) -
    ...
    if( ( ( std::fabs( arccosineArgument ) - 1.0 ) > 0.0 ) || ( std::abs( std::fabs( arccosineArgument ) - 1.0 ) < singularityTolerance ) )
    {
    ...
    double sineOmega = 8.0 * modifiedRodriguesParametersVector( 0 ) * modifiedRodriguesParametersVector( 2 ) +
            signDirectionCosineMatrix * 4.0 * modifiedRodriguesParametersVector( 1 ) * oneMinusModifiedRodriguesParametersMagnitudeSquared;
    double cosineOmega = -8.0 * modifiedRodriguesParametersVector( 1 ) * modifiedRodriguesParametersVector( 2 ) +
            signDirectionCosineMatrix * 4.0 * modifiedRodriguesParametersVector( 0 ) * oneMinusModifiedRodriguesParametersMagnitudeSquared;
    denominator = std::sqrt( cosineOmega * cosineOmega + sineOmega * sineOmega );  // overwrite
    ...
    else
    {
        convertedKeplerianElements( trueAnomalyIndex ) =
                std::atan2( ( auxiliaryParameter1 / RHodographElement ),
                            ( ( auxiliaryParameter2 - unifiedStateModelElements( CHodographUSM6Index ) ) / RHodographElement ) );
...
Eigen::Vector7d convertCartesianToUnifiedStateModelModifiedRodriguesParameterElements( const Eigen::Vector6d& cartesianElements,
                                                                                       const double centralBodyGravitationalParameter )
{
    // Declaring eventual output vector.
        ...
        const double centralBodyGravitationalParameter )
{
    ...
    unifiedStateModelQuaternionElements.segment( etaUSM7Index, 4 ) = convertModifiedRodriguesParametersToQuaternionElements(
