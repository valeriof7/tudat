...
{
    Eigen::Vector3d initialCentroid = computePolyhedronCentroidPosition( verticesCoordinates, verticesDefiningEachFacet );
            ...
            for( unsigned int col = row; col < 3; ++col )
            {
                inertiaProducts( row, col ) += density * volumeOfTetrahedron / 20.0 *
                        ( 12 * vertex0( row ) * vertex0( col ) + 2 * edge0( row ) * edge0( col ) + 2 * edge1( row ) * edge1( col ) +
                          ...
                          4 * ( vertex0( row ) * edge1( col ) + vertex0( col ) * edge1( row ) ) + edge0( col ) * edge1( row ) +
                          edge0( row ) * edge1( col ) );
            }
        }
    }
...
{
    double density =
            gravitationalParameter / ( computePolyhedronVolume( verticesCoordinates, verticesDefiningEachFacet ) * gravitationalConstant );
