...
Eigen::Vector7d convertKeplerianToUnifiedStateModelQuaternionsElements( const Eigen::Vector6d& keplerianElements,
                                                                        const double centralBodyGravitationalParameter )
{
    ...
    double RHodographElement = keplerianElements( eccentricityIndex ) * convertedUnifiedStateModelElements( CHodographUSM7Index );

    // Compute the Rf1 hodograph element of the unified state model
    convertedUnifiedStateModelElements( Rf1HodographUSM7Index ) = -RHodographElement *
    ...
    convertedUnifiedStateModelElements( Rf2HodographUSM7Index ) = RHodographElement *
    ...
    double argumentOfLatitude = keplerianElements( argumentOfPeriapsisIndex ) + keplerianElements( trueAnomalyIndex );

    // Compute the eta quaternion of the unified state model
    convertedUnifiedStateModelElements( etaUSM7Index ) = std::cos( 0.5 * keplerianElements( inclinationIndex ) ) *
    ...
    convertedUnifiedStateModelElements( epsilon1USM7Index ) = std::sin( 0.5 * keplerianElements( inclinationIndex ) ) *
            std::cos( 0.5 * ( keplerianElements( longitudeOfAscendingNodeIndex ) - argumentOfLatitude ) );

    // Compute the epsilon2 quaternion of the unified state model
    convertedUnifiedStateModelElements( epsilon2USM7Index ) = std::sin( 0.5 * keplerianElements( inclinationIndex ) ) *
    ...
    convertedUnifiedStateModelElements( epsilon3USM7Index ) = std::cos( 0.5 * keplerianElements( inclinationIndex ) ) *
            std::sin( 0.5 * ( keplerianElements( longitudeOfAscendingNodeIndex ) + argumentOfLatitude ) );
...
Eigen::Vector6d convertUnifiedStateModelQuaternionsToKeplerianElements( const Eigen::Vector7d& unifiedStateModelElements,
                                                                        const double centralBodyGravitationalParameter,
                                                                        const bool forceQuaternionNormalization )
{
    ...
    double denominator = epsilon3QuaternionParameter * epsilon3QuaternionParameter + etaQuaternionParameter * etaQuaternionParameter;
    double cosineLambda =
            ( etaQuaternionParameter * etaQuaternionParameter - epsilon3QuaternionParameter * epsilon3QuaternionParameter ) / denominator;
    double sineLambda = ( 2.0 * epsilon3QuaternionParameter * etaQuaternionParameter ) / denominator;
    ...
    double RHodographElement =
            std::sqrt( unifiedStateModelElements( Rf1HodographUSM7Index ) * unifiedStateModelElements( Rf1HodographUSM7Index ) +
                       unifiedStateModelElements( Rf2HodographUSM7Index ) * unifiedStateModelElements( Rf2HodographUSM7Index ) );

    // Compute eccentricity
    convertedKeplerianElements( eccentricityIndex ) = RHodographElement / unifiedStateModelElements( CHodographUSM7Index );
    ...
    // Compute inclination
    convertedKeplerianElements( inclinationIndex ) = std::acos( 1.0 -
                                                                2.0 *
                                                                        ( epsilon1QuaternionParameter * epsilon1QuaternionParameter +
    ...
    double sineOmega = epsilon1QuaternionParameter * epsilon3QuaternionParameter + epsilon2QuaternionParameter * etaQuaternionParameter;
    double cosineOmega = epsilon1QuaternionParameter * etaQuaternionParameter - epsilon2QuaternionParameter * epsilon3QuaternionParameter;
    denominator = std::sqrt( cosineOmega * cosineOmega + sineOmega * sineOmega );  // overwrite old denominator
    ...
    else
    {
        convertedKeplerianElements( trueAnomalyIndex ) =
                std::atan2( ( auxiliaryParameter1 / RHodographElement ),
                            ( ( auxiliaryParameter2 - unifiedStateModelElements( CHodographUSM7Index ) ) / RHodographElement ) );
...
Eigen::Vector7d convertCartesianToUnifiedStateModelQuaternionsElements( const Eigen::Vector6d& cartesianElements,
                                                                        const double centralBodyGravitationalParameter )
{
    using mathematical_constants::PI;
    ...
    convertedUnifiedStateModelElements( CHodographUSM7Index ) = centralBodyGravitationalParameter / angularMomentumMagnitude;
    ...
    directionCosineMatrix.block( 1, 0, 1, 3 ) = ( angularMomentumVector.normalized( ).cross( positionVector.normalized( ) ) ).transpose( );
    ...
    {
        quaternionsSquaredVector( i ) = ( 1.0 - traceDirectionCosineMatrix + 2.0 * directionCosineMatrix( i, i ) ) / 4.0;
    }
    ...
    double denominator = convertedUnifiedStateModelElements( epsilon3USM7Index ) * convertedUnifiedStateModelElements( epsilon3USM7Index ) +
            convertedUnifiedStateModelElements( etaUSM7Index ) * convertedUnifiedStateModelElements( etaUSM7Index );
    double cosineLambda =
            ( convertedUnifiedStateModelElements( etaUSM7Index ) * convertedUnifiedStateModelElements( etaUSM7Index ) -
              convertedUnifiedStateModelElements( epsilon3USM7Index ) * convertedUnifiedStateModelElements( epsilon3USM7Index ) ) /
            denominator;
    double sineLambda =
            ( 2.0 * convertedUnifiedStateModelElements( epsilon3USM7Index ) * convertedUnifiedStateModelElements( etaUSM7Index ) ) /
            denominator;
    ...
    double auxiliaryParameter1 =
            ( ( radialVelocity >= 0 ) ? 1.0 : -1.0 ) * auxiliaryParameter3.norm( );  // take norm now that vector value has been used
...
Eigen::Vector6d convertUnifiedStateModelQuaternionsToCartesianElements( const Eigen::Vector7d& unifiedStateModelElements,
                                                                        const double centralBodyGravitationalParameter,
                                                                        const bool forceQuaternionNormalization )
{
    using mathematical_constants::PI;
    ...
    {
        double denominator = epsilon3QuaternionParameter * epsilon3QuaternionParameter + etaQuaternionParameter * etaQuaternionParameter;
        cosineLambda = ( etaQuaternionParameter * etaQuaternionParameter - epsilon3QuaternionParameter * epsilon3QuaternionParameter ) /
                denominator;
        sineLambda = ( 2.0 * epsilon3QuaternionParameter * etaQuaternionParameter ) / denominator;
    ...
    Eigen::Matrix3d inverseDirectionCosineMatrix = linear_algebra::convertVectorToQuaternionFormat( quaternionsVector ).toRotationMatrix( );

    // Get Cartesian position vector
    convertedCartesianElements.segment( xCartesianPositionIndex, 3 ) = centralBodyGravitationalParameter /
            unifiedStateModelElements( CHodographUSM7Index ) / auxiliaryParameter2 *
            inverseDirectionCosineMatrix.block( 0, 0, 3, 1 );  // take first column of matrix

    // Get Cartesian velocity vector
    convertedCartesianElements.segment( xCartesianVelocityIndex, 3 ) = inverseDirectionCosineMatrix.block( 0, 0, 3, 2 ) * auxiliaryVector1;
