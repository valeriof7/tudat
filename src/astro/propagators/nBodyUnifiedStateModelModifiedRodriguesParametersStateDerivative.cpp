        ...
        const Eigen::Vector3d& pAuxiliaryVector )
{
    ...
    stateDerivative.segment( 3, 4 ) =
            calculateModifiedRodriguesParametersDerivative( currentUnifiedStateModelElements.segment( 3, 4 ), rotationalVelocityVector );
        ...
        const double centralBodyGravitationalParameter )
{
    ...
    double oneMinusModifiedRodriguesParametersMagnitudeSquaredSquared = std::pow( 1.0 - modifiedRodriguesParametersMagnitudeSquared, 2 );
    ...
    double sineLambda = 4.0 * modifiedRodriguesParametersVector( 2 ) * ( 1.0 - modifiedRodriguesParametersMagnitudeSquared ) / denominator;
    double cosineLambda =
            ( oneMinusModifiedRodriguesParametersMagnitudeSquaredSquared - 4.0 * sigma3ModifiedRodriguesParametersSquared ) / denominator;
    double gammaParameter = 2.0 *
            ( modifiedRodriguesParametersVector( 1 ) * ( modifiedRodriguesParametersMagnitudeSquared - 1.0 ) +
            ...
            ( std::pow( sigma1ModifiedRodriguesParametersSquared, 2 ) + std::pow( sigma2ModifiedRodriguesParametersSquared, 2 ) +
              std::pow( sigma3ModifiedRodriguesParametersSquared, 2 ) +
              2.0 *
                      ( sigma1ModifiedRodriguesParametersSquared * sigma2ModifiedRodriguesParametersSquared +
                        ...
                        modifiedRodriguesParametersMagnitudeSquared + 2.0 * sigma3ModifiedRodriguesParametersSquared ) +
              1.0 );
    ...
    rotationalVelocityVector( 2 ) = std::pow( velocityHodographParameter, 2 ) * currentUnifiedStateModelElements( CHodographUSM6Index ) /
    ...
    // Evaluate USM6 equations
    return computeStateDerivativeForUnifiedStateModelModifiedRodriguesParameters( currentUnifiedStateModelElements,
                                                                                  accelerationsInRswFrame,
                                                                                  sineLambda,
                                                                                  cosineLambda,
                                                                                  gammaParameter,
        ...
        const double centralBodyGravitationalParameter )
{
    return computeStateDerivativeForUnifiedStateModelModifiedRodriguesParameters(
            ...
            reference_frames::getInertialToRswSatelliteCenteredFrameRotationMatrix( currentCartesianState ) * accelerationsInInertialFrame,
            centralBodyGravitationalParameter );
}
