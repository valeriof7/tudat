                                                                                    ...
                                                                                    const std::string& varianceType )
{
    ...
    for( std::map< int, double >::const_iterator iter = allanVarianceNodes.begin( ); iter != std::prev( allanVarianceNodes.end( ) );
         iter++ )
    {
    ...
    int counter = 0;
    double Bi = 0.;
    for( std::map< int, double >::const_iterator iter = std::next( allanVarianceNodes.begin( ) ); iter != allanVarianceNodes.end( );
         iter++ )
    {
        if( counter == 0 )
        {
                                                                              ...
                                                                              const bool isInverseSquareTermFlickerPhaseNoise )
{
    ...
    for( std::map< int, double >::const_iterator amplitudeIterator = allanVarianceAmplitudes.begin( );
         amplitudeIterator != allanVarianceAmplitudes.end( );
         amplitudeIterator++ )
    {
                ...
                case -2:
                    if( isInverseSquareTermFlickerPhaseNoise )
                    {
                        phaseNoiseAmplitudes[ -1 ] = amplitudeIterator->second * 1.0 /
                                ( 3.0 *
                                          ( mathematical_constants::EULER_MASCHERONI_CONSTANT +
                                            2.0 * std::log( 2.0 * mathematical_constants::PI * frequencyDomainCutoffFrequency ) ) -
                                  std::log( 2 ) );
                    }
                    else
                    {
                    ...
                    break;
                case -1:
                    phaseNoiseAmplitudes[ -2 ] =
                            amplitudeIterator->second / ( 2.0 * mathematical_constants::PI * mathematical_constants::PI );
                    break;
                case 0:
                    phaseNoiseAmplitudes[ -3 ] =
                            amplitudeIterator->second / ( 8.0 * mathematical_constants::PI * mathematical_constants::PI * std::log( 2.0 ) );
                    break;
                case 1:
 ...
 * all frequencies leads to a divergent solution, i.e. infinite power) \param isInverseSquareTermFlickerPhaseNoise Boolean determining
 * whether inverse square time term is flicker phase noise (freq.^(-1)) or white phase modulation (freq.^(0)). Note that for allan variance
 * of flicker phase noise, the logarithmic term will be neglected when using this function. \return Pair containing first: Map with phase
 * noise amplitudes, powers of frequency as keys and ampliutdes as values, second: frequency_nodes
 */
        ...
        const std::string& varianceType )
{
    ...
    {
        throw std::runtime_error( "Unknown varianceType" );
    }
    ...
    for( std::map< int, std::pair< double, double > >::const_iterator iter = allanVarianceAmplitudes.begin( );
         iter != allanVarianceAmplitudes.end( );
         iter++ )
    {
                                                               ...
                                                               const double seed )
{
    ...
    return generatePowerLawNoise( maximumFrequency, numberOfFrequencySteps, doublePhaseNoiseAmplitudes, seed );
}
                                                                      ...
                                                                      const double seed )
{
    ...
    std::map< int, std::pair< double, double > > allanVarianceAmplitudes =
            convertAllanVarianceNodesToAmplitudes( allanVarianceNodes, varianceType );

    // Convert time domain amplitudes to frequency domain amplitudes.
    std::pair< std::map< double, double >, std::vector< double > > dummy =
            convertAllanVarianceAmplitudesToApproximatedPhaseNoiseSpectrum( allanVarianceAmplitudes, varianceType );
    std::map< double, double > phaseNoiseAmplitudes = dummy.first;
    std::vector< double > frequencyNodes = dummy.second;

    // return clock noise with time step used.
    return generatePowerLawNoise( maximumFrequency, numberOfFrequencySteps, phaseNoiseAmplitudes, frequencyNodes, seed );
}
                                                         ...
                                                         const double seed )
{
    ...
    std::pair< std::vector< double >, double > clockNoise =
            generateColoredClockNoise( allanVarianceNodes, varianceType, startTime, endTime, numberOfTimeSteps, seed );
    std::map< double, double > clockNoiseMap;
                            ...
                            const double clockNoiseTimeStep )
{
    ...
    {
        throw std::runtime_error( "Error when making timing system, inconsistent polynomial error vector size" +
...
double TimingSystem::getCompleteClockError( const Time trueTime )
{
    ...
    return getDeterministicClockError( timeIntoArcAndIndex.first, timeIntoArcAndIndex.second ) +
            clockErrorFunction_[ timeIntoArcAndIndex.second ]( static_cast< double >( trueTime ) );
}
...
void TimingSystem::setSingleArcPolynomialClockCorrections( const std::map< int, double >& newCorrections, const int arcIndex )
{
    for( std::map< int, double >::const_iterator correctionIterator = newCorrections.begin( ); correctionIterator != newCorrections.end( );
         correctionIterator++ )
    {
