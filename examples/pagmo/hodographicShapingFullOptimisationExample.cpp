...
std::vector< std::vector< std::shared_ptr< BaseFunctionHodographicShaping > > > getShapingBasisFunctions( const double timeOfFlight,
                                                                                                          const int numberOfRevolutions )
{
    ...
    getRecommendedRadialVelocityBaseFunctions( radialVelocityFunctionComponents, dummyVector, timeOfFlight );
    ...
    getRecommendedNormalAxialBaseFunctions( normalVelocityFunctionComponents, dummyVector, timeOfFlight );
    ...
    getRecommendedAxialVelocityBaseFunctions( axialVelocityFunctionComponents, dummyVector, timeOfFlight, numberOfRevolutions );
...
int main( )
{
    ...
    for( int useMultiObjective = 0; useMultiObjective < 2; useMultiObjective++ )
    {
        for( int revolutions = 2; revolutions < 3; revolutions++ )
        {
            ...
            problem prob{ HodographicShapingOptimisationProblem( departureStateFunction,
                                                                 arrivalStateFunction,
                                                                 spice_interface::getBodyGravitationalParameter( "Sun" ),
                                                                 revolutions,
                                                                 std::bind( &getShapingBasisFunctions, std::placeholders::_1, revolutions ),
                                                                 bounds,
                                                                 useMultiObjective,
                                                                 2000.0 ) };
            ...
            for( int i = 0; i < 101; i++ )
            {
                ...
                if( i % 25 == 0 )
                {
                    if( !useMultiObjective )
                    {
                        std::cout << "Iteration: " << " " << i << "; Best Delta V: " << isl.get_population( ).champion_f( ).at( 0 )
                                  << std::endl;

                        printPopulationToFile(
                                isl.get_population( ).get_x( ), "hodograph_single_objective_" + std::to_string( i / 25 ), false );
                        printPopulationToFile(
                                isl.get_population( ).get_f( ), "hodograph_single_objective_" + std::to_string( i / 25 ), true );
                    }
                    else
                    {
                        ...
                        printPopulationToFile(
                                isl.get_population( ).get_x( ), "hodograph_multi_objective_" + std::to_string( i / 25 ), false );
                        printPopulationToFile(
                                isl.get_population( ).get_f( ), "hodograph_multi_objective_" + std::to_string( i / 25 ), true );
                    }
                }
            }
            ...
            if( !useMultiObjective )
            {
                ...
                std::vector< std::vector< std::shared_ptr< BaseFunctionHodographicShaping > > > shapingFunctions =
                        getShapingBasisFunctions( timeOfFlight, 2 );

                std::shared_ptr< HodographicShaping > hodographicShaping =
                        std::make_shared< HodographicShaping >( departureStateFunction( derpartureTime ),
                                                                arrivalStateFunction( arrivalTime ),
                                                                timeOfFlight,
                                                                spice_interface::getBodyGravitationalParameter( "Sun" ),
                                                                2,
                                                                shapingFunctions.at( 0 ),
                                                                shapingFunctions.at( 1 ),
                                                                shapingFunctions.at( 2 ),
                                                                radialFreeParameters,
                                                                normalFreeParameters,
                                                                axialFreeParameters,
                                                                initialMass );
