    ...
    for( unsigned int j = 0; j < 3; j++ )
    {
        ...
        for( int i = 0; i < 100; i++ )
        {
            ...
            printPopulationToFile(
                    isl.get_population( ).get_x( ), "mo_EarthMars_" + std::to_string( i ) + "_" + std::to_string( j ), false );
            printPopulationToFile(
                    isl.get_population( ).get_f( ), "mo_EarthMars_" + std::to_string( i ) + "_" + std::to_string( j ), true );
