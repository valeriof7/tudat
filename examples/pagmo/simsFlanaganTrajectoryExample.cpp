...
int main( )
{
    ...
    std::shared_ptr< shape_based_methods::HodographicShaping > hodographicShaping =
            std::make_shared< shape_based_methods::HodographicShaping >( cartesianStateDepartureBody,
    ...
    hodographicShaping->getThrustProfile(
            epochsToSaveResults, hodographicShapingThrustProfile, specificImpulseFunction, integratorSettings );
    hodographicShaping->getThrustAccelerationProfile(
            epochsToSaveResults, hodographicShapingThrustAcceleration, specificImpulseFunction, integratorSettings );
    ...
    std::vector< double > initialGuessVector = convertToSimsFlanaganThrustModel(
            initialGuessThrustFromShaping, maximumThrust, timeOfFlight, ( numberSegments + 1 ) / 2, numberSegments / 2 );
    ...
    for( unsigned int i = 0; i < epochsToSaveResults.size( ); i++ )
    {
        ...
        {
            initialGuessMassProfile[ currentTime ] =
                    ( Eigen::Vector1d( ) << initialGuessMassProfile[ epochsToSaveResults[ i - 1 ] ][ 0 ] -
                              initialGuessThrustProfile[ currentTime ].norm( ) /
                                      ( specificImpulse * physical_constants::SEA_LEVEL_GRAVITATIONAL_ACCELERATION ) * stepSize )
                            .finished( );
        }

        initialGuessThrustAccelerationProfile[ currentTime ] =
                initialGuessThrustProfile[ currentTime ] / initialGuessMassProfile[ currentTime ][ 0 ];
    }
    ...
    std::shared_ptr< OptimisationSettings > optimisationSettings =
            std::make_shared< OptimisationSettings >( optimisationAlgorithm, 10, 1024, 1.0e-6, std::make_pair( initialGuessVector, 0.3 ) );
    ...
    simsFlanagan.getThrustAccelerationProfile(
            epochsToSaveResults, SimsFlanaganThrustAcceleration, specificImpulseFunction, integratorSettings );
