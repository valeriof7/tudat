...
int main( )
{
    ...
    Eigen::MatrixXd MikhailData = readMatrixFromFile(
            "/home/dominic/Software/numericalAstrodynamicsTudatBundle/tudatBundle/tudatExampleApplications/satellitePropagatorExamples/"
            "SatellitePropagatorExamples/MikhailData.txt",
            " ",
            "" );
    ...
    double InitialutcSecondsSinceJ2000 = convertCalendarDateToJulianDaysSinceEpoch< double >(
    ...
    double FinalutcSecondsSinceJ2000 = convertCalendarDateToJulianDaysSinceEpoch< double >(
                                               year, months, days, hours, minutes, seconds, getJulianDayOnJ2000< double >( ) ) *
            physical_constants::getJulianDay< double >( );
    ...
    createGroundStation( bodies.at( "Earth" ),
                         "Station1M",
                         ( Eigen::Vector3d( ) << 2892607.149, 1311813.079, 5512598.659 ).finished( ),
                         cartesian_position );
    ...
    Eigen::Matrix< double, 6, 1 > RAInitialKeplerianState =
            convertCartesianToKeplerianElements( systemInitialState, earthGravitationalParameter );
    ...
    for( unsigned int i = 0; i < ArcInitialTimes.size( ); i++ )
    {
        ...
        std::cout << "Distance arc " << i + 1 << ": "
                  << sqrt( pow( currentArcInitialState( 0 ), 2 ) + pow( currentArcInitialState( 1 ), 2 ) +
                           pow( currentArcInitialState( 2 ), 2 ) )
                  << std::endl;
    ...
    //    std::shared_ptr< PropagatorSettings< double > > propagatorSettings = std::make_shared< TranslationalStatePropagatorSettings<
    //    double > >
    ...
    parameterNames.push_back(
            std::make_shared< ArcWiseRadiationPressureCoefficientEstimatableParameterSettings >( "RA", ArcInitialTimes ) );
    //    parameterNames.push_back(
    ...
    parameterNames.push_back(
            std::make_shared< EstimatableParameterSettings >( "Earth", equivalence_principle_lpi_violation_parameter, "" ) );
         ...
         linkEndIterator++ )
    {
        ...
        for( unsigned int i = 0; i < currentLinkEndsList.size( ); i++ )
        {
            ...
            std::shared_ptr< DopplerProperTimeRateSettings > properTimeRate =
            ...
            if( currentObservable == one_way_doppler )
            {
                ...
                observationSettingsMap.insert( std::make_pair( currentLinkEndsList.at( i ), OneWayDopplerSettings ) );
            }
            else if( currentObservable == two_way_doppler )
            {
                ...
                std::shared_ptr< OneWayDopplerObservationSettings > uplinkDopplerSettings =
                        std::make_shared< OneWayDopplerObservationSettings >(
                                std::shared_ptr< LightTimeCorrectionSettings >( ), properTimeRate, properTimeRate, biasSettings );

                std::shared_ptr< OneWayDopplerObservationSettings > downlinkDopplerSettings =
                        std::make_shared< OneWayDopplerObservationSettings >(
                                std::shared_ptr< LightTimeCorrectionSettings >( ), properTimeRate, properTimeRate, biasSettings );
    ...
    observationViabilitySettings.push_back( std::make_shared< ObservationViabilitySettings >(
            minimum_elevation_angle, std::make_pair( "Earth", "" ), "", 5.0 * mathematical_constants::PI / 180.0 ) );
    PerObservableObservationViabilityCalculatorList viabilityCalculators =
            createObservationViabilityCalculators( bodies, linkEndsPerObservable, observationViabilitySettings );
    ...
    noiseFunctions[ one_way_doppler ] = std::bind( &utilities::evaluateFunctionWithoutInputArgumentDependency< double, const double >,
                                                   createBoostContinuousRandomVariableGeneratorFunction(
                                                           normal_boost_distribution, std::assign::list_of( 0.0 )( dopplerNoise ), 0.0 ),
                                                   _1 );

    noiseFunctions[ two_way_doppler ] = std::bind( &utilities::evaluateFunctionWithoutInputArgumentDependency< double, const double >,
                                                   createBoostContinuousRandomVariableGeneratorFunction(
                                                           normal_boost_distribution, std::assign::list_of( 0.0 )( dopplerNoise ), 0.0 ),
                                                   _1 );
    ...
    input_output::writeMatrixToFile( podOutput->normalizedInformationMatrix_,
                                     "RAEstimationInformationMatrix.dat",
                                     16,
                                     tudat_applications::getOutputPath( ) + outputSubFolder );
    input_output::writeMatrixToFile( podOutput->informationMatrixTransformationDiagonal_,
                                     "RAEstimationInformationMatrixNormalization.dat",
                                     16,
                                     tudat_applications::getOutputPath( ) + outputSubFolder );
    input_output::writeMatrixToFile( podOutput->weightsMatrixDiagonal_,
                                     "RAEstimationWeightsDiagonal.dat",
                                     16,
                                     tudat_applications::getOutputPath( ) + outputSubFolder );
    input_output::writeMatrixToFile(
            podOutput->residuals_, "RAEstimationResiduals.dat", 16, tudat_applications::getOutputPath( ) + outputSubFolder );
    input_output::writeMatrixToFile( podOutput->getCorrelationMatrix( ),
                                     "RAEstimationCorrelations.dat",
                                     16,
                                     tudat_applications::getOutputPath( ) + outputSubFolder );
    input_output::writeMatrixToFile( podOutput->getUnnormalizedInverseCovarianceMatrix( ),
                                     "RAEstimationInverseCovarianceMatrix.dat",
                                     16,
                                     tudat_applications::getOutputPath( ) + outputSubFolder );
    input_output::writeMatrixToFile(
            podOutput->getResidualHistoryMatrix( ), "RAResidualHistory.dat", 16, tudat_applications::getOutputPath( ) + outputSubFolder );
    input_output::writeMatrixToFile(
            podOutput->getParameterHistoryMatrix( ), "RAParameterHistory.dat", 16, tudat_applications::getOutputPath( ) + outputSubFolder );
    input_output::writeMatrixToFile( getConcatenatedMeasurementVector( podInput->getObservationsAndTimes( ) ),
                                     "earthOrbitObservationMeasurements.dat",
                                     16,
                                     tudat_applications::getOutputPath( ) + outputSubFolder );
    input_output::writeMatrixToFile(
            utilities::convertStlVectorToEigenVector( getConcatenatedTimeVector( podInput->getObservationsAndTimes( ) ) ),
            "RAObservationTimes.dat",
            16,
            tudat_applications::getOutputPath( ) + outputSubFolder );
    input_output::writeMatrixToFile(
            utilities::convertStlVectorToEigenVector( getConcatenatedGroundStationIndex( podInput->getObservationsAndTimes( ) ).first ),
            "RAObservationLinkEnds.dat",
            16,
            tudat_applications::getOutputPath( ) + outputSubFolder );
    input_output::writeMatrixToFile(
            utilities::convertStlVectorToEigenVector( getConcatenatedObservableTypes( podInput->getObservationsAndTimes( ) ) ),
            "RAObservationObservableTypes.dat",
            16,
            tudat_applications::getOutputPath( ) + outputSubFolder );
    input_output::writeMatrixToFile( getConcatenatedMeasurementVector( podInput->getObservationsAndTimes( ) ),
                                     "RAObservationMeasurements.dat",
                                     16,
                                     tudat_applications::getOutputPath( ) + outputSubFolder );
    input_output::writeMatrixToFile(
            estimationError, "RAObservationTrueEstimationError.dat", 16, tudat_applications::getOutputPath( ) + outputSubFolder );
    input_output::writeMatrixToFile( podOutput->getFormalErrorVector( ),
                                     "RAObservationFormalEstimationError.dat",
                                     16,
                                     tudat_applications::getOutputPath( ) + outputSubFolder );
    input_output::writeMatrixToFile( AprioriCov, "RAaprioriCov.dat", 16, tudat_applications::getOutputPath( ) + outputSubFolder );
