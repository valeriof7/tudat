...
int main( )
{
    ...
    std::map< std::string, std::shared_ptr< BodySettings > > bodySettings =
            getDefaultBodySettings( { "Earth" }, simulationStartEpoch - 300, simulationEndEpoch + 300 );
    ...
    basic_astrodynamics::AccelerationMap accelerationModelMap_1 =
            createAccelerationModelsMap( bodies_1, accelerationMap_1, bodiesToPropagate_1, centralBodies_1 );
    ...
    targetInitialStateInKeplerianElements( argumentOfPeriapsisIndex ) = convertDegreesToRadians( 235.7 );
    targetInitialStateInKeplerianElements( longitudeOfAscendingNodeIndex ) = convertDegreesToRadians( 23.4 );
    ...
    chaserInitialStateInKeplerianElements( argumentOfPeriapsisIndex ) = convertDegreesToRadians( 84.1 );
    chaserInitialStateInKeplerianElements( longitudeOfAscendingNodeIndex ) = convertDegreesToRadians( 23.4 );
    ...
    const Eigen::Vector6d targetInitialState =
            convertKeplerianToCartesianElements( targetInitialStateInKeplerianElements, earthGravitationalParameter );

    // Convert chaser state from Keplerian elements to Cartesian elements.
    const Eigen::Vector6d chaserInitialState =
            convertKeplerianToCartesianElements( chaserInitialStateInKeplerianElements, earthGravitationalParameter );
    ...
    double meanOrbitalMotion =
            pow( earthGravitationalParameter / pow( targetInitialStateInKeplerianElements( semiMajorAxisIndex ), 3 ), 0.5 );
    ...
    double estimatedBurnTime = specificImpulse * physical_constants::SEA_LEVEL_GRAVITATIONAL_ACCELERATION * chaserMass *
            ( 1 - 1 / massRatio ) / thrustMagnitude;
    ...
    std::shared_ptr< MassRateModelSettings > massRateModelSettings = std::make_shared< FromThrustMassModelSettings >( 1 );
    std::map< std::string, std::shared_ptr< basic_astrodynamics::MassRateModel > > massRateModels;
    massRateModels[ "Chaser" ] = createMassRateModel( "Chaser", massRateModelSettings, bodies_1, accelerationModelMap_1 );
    ...
    decisionVariableList.push_back( std::make_shared< optimization::SingleDecisionVariableSettings >(
            optimization::simulation_time_decision_variable, estimatedBurnTime - 10.0, estimatedBurnTime + 10.0 ) );
    decisionVariableList.push_back( std::make_shared< optimization::SingleCartesianComponentDecisionVariableSettings >(
            orbital_elements::xCartesianVelocity, chaserInitialState( 3 ) - 10.0, chaserInitialState( 3 ) + 10.0, "Chaser" ) );
    decisionVariableList.push_back( std::make_shared< optimization::SingleCartesianComponentDecisionVariableSettings >(
            orbital_elements::yCartesianVelocity, chaserInitialState( 4 ) - 10.0, chaserInitialState( 4 ) + 10.0, "Chaser" ) );
    decisionVariableList.push_back( std::make_shared< optimization::SingleCartesianComponentDecisionVariableSettings >(
            orbital_elements::zCartesianVelocity, chaserInitialState( 5 ) - 10.0, chaserInitialState( 5 ) + 10.0, "Chaser" ) );
    ...
    std::shared_ptr< optimization::DecisionVariableSettings > decisionVariableSettings =
            std::make_shared< optimization::DecisionVariableSettings >( decisionVariableList );
    ...
    basic_astrodynamics::AccelerationMap accelerationModelMap_2 =
            createAccelerationModelsMap( bodies_2, accelerationMap_2, bodiesToPropagate_2, centralBodies_2 );
    ...
    dependentVariableSaveSettingsVector.push_back(
            std::make_shared< SingleDependentVariableSaveSettings >( relative_distance_dependent_variable, "Chaser", "Target" ) );
    ...
    multiTerminationSettings.push_back( std::make_shared< propagators::PropagationDependentVariableTerminationSettings >(
    ...
    std::shared_ptr< optimization::ObjectiveFunctionFromMinOrMaxDependentVariableSettings > objectiveFunction =
            std::make_shared< optimization::ObjectiveFunctionFromMinOrMaxDependentVariableSettings >(
                    dependentVariableSaveSettingsVector[ 0 ], objectiveValue, 0, true, minimumSeparation, maxNumberOfEvolutions );
    ...
    std::shared_ptr< optimization::MissionSegmentSettings > missionSegment_2 =
            std::make_shared< optimization::MissionSegmentSettings >( dynamicsSimulator_2, objectiveFunction );
    ...
    optimization::MissionLinker missionLinker(
            missionSegments,
            std::make_shared< optimization::OptimizationSettings >( optimization::global_optimization, 16, true, 1 ),
            false );
    ...
    for( std::map< double, Eigen::VectorXd >::const_iterator stateIterator = integrationResult_1.begin( );
         stateIterator != integrationResult_1.end( );
         stateIterator++ )
    {
    ...
    for( std::map< double, Eigen::VectorXd >::const_iterator stateIterator = integrationResult_2.begin( );
         stateIterator != integrationResult_2.end( );
         stateIterator++ )
    {
