...
int main( )
{
    ...
    for( auto itr: patchedConicsTrajectory )
    {
        ...
        std::cout << "Departure: "
                  << fullProblemTrajectory[ itr.first ].begin( )->second - patchedConicsTrajectory[ itr.first ].begin( )->second << "\n\n";
        std::cout << "Arrival: "
                  << fullProblemTrajectory[ itr.first ].rbegin( )->second - patchedConicsTrajectory[ itr.first ].rbegin( )->second
                  << "\n\n";
    }
    ...
    bodyToPropagateAccelerations[ "Sun" ].push_back(
    ...
    bodyToPropagateAccelerations[ "Venus" ].push_back(
            std::make_shared< simulation_setup::AccelerationSettings >( basic_astrodynamics::central_gravity ) );
    bodyToPropagateAccelerations[ "Mercury" ].push_back(
            std::make_shared< simulation_setup::AccelerationSettings >( basic_astrodynamics::central_gravity ) );
    ...
    basic_astrodynamics::AccelerationMap accelerationModelMapPerturbedCase =
            simulation_setup::createAccelerationModelsMap( bodies, accelerationMapPerturbedCase, { bodyToPropagate }, { centralBody } );
    ...
    propagators::fullPropagationPatchedConicsTrajectory( bodies,
                                                         accelerationMapVectorPerturbedCase,
                                                         transferBodyTrajectory,
                                                         centralBody[ 0 ],
                                                         bodyToPropagate,
                                                         legTypeVector,
                                                         ...
                                                         eccentricities,
                                                         integratorSettings,
                                                         patchedConicsTrajectoryPerturbedCase,
                                                         fullProblemTrajectoryPerturbedCase,
    ...
    for( auto itr: patchedConicsTrajectoryPerturbedCase )
    {
        std::cout << "Leg " << itr.first << "\n\n";
        std::cout << "Departure: "
                  << fullProblemTrajectoryPerturbedCase[ itr.first ].begin( )->second -
                        patchedConicsTrajectoryPerturbedCase[ itr.first ].begin( )->second
                  << "\n\n";
        std::cout << "Arrival: "
                  << fullProblemTrajectoryPerturbedCase[ itr.first ].rbegin( )->second -
    ...
    for( std::map< int, std::map< double, Eigen::Vector6d > >::iterator itr = patchedConicsTrajectory.begin( );
         itr != patchedConicsTrajectory.end( );
         itr++ )
    {
