...
{
public:
    SingleSatelliteThrust( const double initial_epoch = 0,
                           ...
                           const std::vector< std::string > &spice_bodies = { "Sun", "Earth", "Moon" } )
    {
        ...
        // Create body objects.
        std::map< std::string, std::shared_ptr< BodySettings > > body_settings = getDefaultBodySettings( spice_bodies );
    ...
    void step( std::vector< double > &action, double time_step )
    {
        ...
        basic_astrodynamics::AccelerationMap acceleration_model_map =
                createAccelerationModelsMap( this->_body_system, acceleration_map, this->_bodies_to_propagate, this->_central_bodies );

        // Define settings for propagation of translational dynamics.
        std::shared_ptr< TranslationalStatePropagatorSettings< double > > translationalPropagatorSettings =
                std::make_shared< TranslationalStatePropagatorSettings< double > >( this->_central_bodies,
        ...
        massRateModels[ this->_vehicle_name ] =
                createMassRateModel( this->_vehicle_name, massRateModelSettings, this->_body_system, acceleration_model_map );
