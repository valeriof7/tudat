...
std::shared_ptr< propagators::SingleArcSimulationResults<> > createAndRunSimulation( const SystemOfBodies&,
...
SystemOfBodies createSimulationBodies( )
{
    ...
    bodySettings.at( "Mercury" )->rotationModelSettings = spiceRotationModelSettings( globalFrameOrientation, mercuryFrame, mercuryFrame );
    ...
    bodySettings.at( "Mercury" )->gravityFieldSettings =
            sphericalHarmonicsGravitySettings( mercuryGravitationalParameter, 2439e3, cosineCoefficients, sineCoefficients, mercuryFrame );
...
AccelerationMap createSimulationAccelerations( const SystemOfBodies& bodies )
{
    SelectedAccelerationMap accelerationMap{
        { "MPO",
          {
                  { "Mercury", { sphericalHarmonicAcceleration( 2, 2 ), radiationPressureAcceleration( ) } },
                  { "Sun", { pointMassGravityAcceleration( ), radiationPressureAcceleration( ) } },
          } }
    };
...
Eigen::VectorXd createSimulationInitialState( )
{
    ...
    initialStateInMercuryKeplerianElements( argumentOfPeriapsisIndex ) = convertDegreesToRadians( 0.7 );
    initialStateInMercuryKeplerianElements( longitudeOfAscendingNodeIndex ) = convertDegreesToRadians( 180 );
    initialStateInMercuryKeplerianElements( trueAnomalyIndex ) = convertDegreesToRadians( 0 );

    const Eigen::Vector6d initialStateInMercuryCartesianElements =
            convertKeplerianToCartesianElements( initialStateInMercuryKeplerianElements, mercuryGravitationalParameter );

    const Eigen::Matrix6d rotationFromMercuryToGlobalFrame =
            spice_interface::computeStateRotationMatrixBetweenFrames( mercuryFrame, globalFrameOrientation, simulationStartEpoch );

    const Eigen::Vector6d initialStateInGlobalCartesianElements = rotationFromMercuryToGlobalFrame * initialStateInMercuryCartesianElements;
