...
BOOST_AUTO_TEST_CASE( test_linearInterpolation_vector )
{
    ...
    // Test 1: Test vector data with expected result of 0.0.
    {
        ...
        const double interpolatedValue = interpolators::computeLinearInterpolation(
                sortedIndependentVariables, associatedDependentVariables, targetIndependentVariableValue );

        // Verify that interpolated value corresponds to expected value (0.0).
        BOOST_CHECK_SMALL( std::fabs( interpolatedValue - 0.0 ), std::numeric_limits< double >::min( ) );
    }
    ...
    BOOST_CHECK_SMALL( std::fabs( interpolatedValue - 20.5 ), std::numeric_limits< double >::min( ) );
}
...
BOOST_AUTO_TEST_CASE( test_linearInterpolation_map )
{
    ...
    // Compute interpolation.
    Eigen::Vector3d interpolatedVector =
            interpolators::computeLinearInterpolation( sortedIndepedentAndDependentVariables, targetIndependentVariableValue );

    // Check that interpolated values correspond to expected elements [25, 27.5, 85].
    BOOST_CHECK_SMALL( std::fabs( interpolatedVector( 0 ) - 25.0 ), std::numeric_limits< double >::min( ) );

    BOOST_CHECK_SMALL( std::fabs( interpolatedVector( 1 ) - 27.5 ), std::numeric_limits< double >::min( ) );

    BOOST_CHECK_SMALL( std::fabs( interpolatedVector( 2 ) - 85.0 ), std::numeric_limits< double >::min( ) );
}
...
BOOST_AUTO_TEST_CASE( test_linearInterpolation_matlab_compare )
{
    ...
    LinearInterpolatorDouble linearInterpolator( independentVariableValues, dependentVariableValues, huntingAlgorithm );

    // Load points at which interpolator is to be evaluated and data generated by Matlab.
    Eigen::MatrixXd benchmarkData =
            input_output::readMatrixFromFile( paths::getTudatTestDataPath( ) + "/linear_interpolator_test_output_data.dat", "," );
    ...
    linearInterpolator = LinearInterpolatorDouble( independentVariableValues, dependentVariableValues, binarySearch );
...
BOOST_AUTO_TEST_CASE( test_linearInterpolation_boundary_case )
{
    ...
    for( unsigned int i = 0; i < 9; i++ )
    {
        ...
        {
            interpolatedValue = linearInterpolator.interpolate( valueBelowMinimumValue );
            expectedValue = dependentVariableValues.at( 0 ) -
                    valueOffset * ( dependentVariableValues.at( 1 ) - dependentVariableValues.at( 0 ) ) /
            ...
            interpolatedValue = linearInterpolator.interpolate( valueAboveMaximumValue );
            expectedValue = dependentVariableValues.at( inputData.rows( ) - 1 ) +
                    valueOffset *
                            ( dependentVariableValues.at( inputData.rows( ) - 1 ) - dependentVariableValues.at( inputData.rows( ) - 2 ) ) /
                            ( independentVariableValues.at( inputData.rows( ) - 1 ) -
                              independentVariableValues.at( inputData.rows( ) - 2 ) );
            BOOST_CHECK_CLOSE_FRACTION( interpolatedValue, expectedValue, 1.0E-15 );
        }
...
BOOST_AUTO_TEST_CASE( test_linearInterpolation_boundary_case_extrapolation_user_value )
{
    ...
    // Test with long double
    {
        ...
        for( unsigned int i = 5; i < 7; i++ )
        {
            LinearInterpolator< double, long double > linearInterpolator( independentVariableValues,
    ...
    // Test with Eigen::Vector3d
    {
        ...
        for( unsigned int i = 5; i < 7; i++ )
        {
            LinearInterpolator< double, Eigen::Vector3d > linearInterpolator( independentVariableValues,
                                                                              dependentVariableValues,
                                                                              huntingAlgorithm,
                                                                              static_cast< BoundaryInterpolationType >( i ),
                                                                              extrapolationValue );
    ...
    // Test with Eigen::Matrix3d
    {
        ...
        for( unsigned int i = 5; i < 7; i++ )
        {
            LinearInterpolator< double, Eigen::Matrix3d > linearInterpolator( independentVariableValues,
                                                                              dependentVariableValues,
                                                                              huntingAlgorithm,
                                                                              static_cast< BoundaryInterpolationType >( i ),
                                                                              extrapolationValue );
