...
BOOST_AUTO_TEST_CASE( test_RungeKuttaFehlberg78_Integrator_Fehlberg_Benchmark )
{
    ...
    // Setup integrator
    numerical_integrators::RungeKuttaVariableStepSizeIntegratorXd integrator78( coeff78,
...
BOOST_AUTO_TEST_CASE( testRungeKuttaFehlberg78IntegratorUsingMatlabData )
{
    ...
    const std::string pathToForwardIntegrationOutputFile = paths::getTudatTestDataPath( ) + "/matlabOutputRungeKuttaFehlberg78Forward.txt";
    const std::string pathToDiscreteEventIntegrationOutputFile =
            paths::getTudatTestDataPath( ) + "/matlabOutputRungeKuttaFehlberg78DiscreteEvent.txt";
