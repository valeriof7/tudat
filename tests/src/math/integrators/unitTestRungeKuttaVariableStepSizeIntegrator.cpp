...
BOOST_AUTO_TEST_CASE( testCompilerErrors )
{
    ...
    {
        RungeKuttaVariableStepSizeIntegratorXd integrator( RungeKuttaCoefficients( ),
        ...
        BOOST_CHECK_EQUAL( std::string( typeid( integrator.getCurrentIndependentVariable( ) ).name( ) ),
    ...
    {
        RungeKuttaVariableStepSizeIntegrator< double, Eigen::Vector3d, Eigen::VectorXd > integrator(
    ...
    {
        RungeKuttaVariableStepSizeIntegrator< double, Eigen::MatrixXd, Eigen::MatrixXd > integrator(
                RungeKuttaCoefficients( ),
                &computeZeroStateDerivative,
                0.0,
                Eigen::Vector3d::Zero( ),
...
BOOST_AUTO_TEST_CASE( testStateDerivativeRetrievalFunction )
{
    ...
    RungeKuttaVariableStepSizeIntegratorXd integrator( RungeKuttaCoefficients::get( CoefficientSets::rungeKuttaFehlberg45 ),
    ...
    RungeKuttaCoefficients rkf45Coefficients = RungeKuttaCoefficients::get( rungeKuttaFehlberg45 );
...
BOOST_AUTO_TEST_CASE( testVariableStepIntegrateToFunction )
{
    ...
    RungeKuttaVariableStepSizeIntegratorXd verificationIntegrator( RungeKuttaCoefficients::get( CoefficientSets::rungeKuttaFehlberg45 ),
                                                                   &computeSecondNonAutonomousModelStateDerivative,
                                                                   0.0,
                                                                   ( Eigen::VectorXd( 1 ) << 1.0 ).finished( ),
                                                                   0.0,
    ...
    BOOST_CHECK_EQUAL(
            ( verificationIntegrator.getCurrentIndependentVariable( ) < 0.5 * ( 1.0 - 10.0 * std::numeric_limits< double >::epsilon( ) ) ),
            true );
    ...
    RungeKutta4IntegratorXd fixedStepSizeIntegrator(
            &computeSecondNonAutonomousModelStateDerivative, 0.0, ( Eigen::VectorXd( 1 ) << 1.0 ).finished( ), 0.01 );
