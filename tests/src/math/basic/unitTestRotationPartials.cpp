...
Eigen::Matrix3d getTestRotationMatrix( const double perturbation )
{
    return ( Eigen::AngleAxisd( -( -1.2 + perturbation ), Eigen::Vector3d::UnitZ( ) ) *
             Eigen::AngleAxisd( -( 0.3 + perturbation ), Eigen::Vector3d::UnitX( ) ) *
             Eigen::AngleAxisd( -( -0.4 + perturbation ), Eigen::Vector3d::UnitZ( ) ) )
            .toRotationMatrix( );
}
...
BOOST_AUTO_TEST_CASE( testEulerAngles )
{
    ...
    Eigen::Quaterniond manualQuaternion = Eigen::Quaterniond( Eigen::AngleAxisd( -eulerAngles( 0 ), Eigen::Vector3d::UnitZ( ) ) *
...
{
    {
        ...
        Eigen::Matrix3d rotationMatrix =
                ( Eigen::AngleAxisd( -angleX, Eigen::Vector3d::UnitX( ) ) * Eigen::AngleAxisd( -angleZ, Eigen::Vector3d::UnitZ( ) ) *
                  Eigen::AngleAxisd( -angleY, Eigen::Vector3d::UnitY( ) ) )
                        .toRotationMatrix( );

        Eigen::Vector3d eulerAngles = get132EulerAnglesFromRotationMatrix( rotationMatrix );
...

    {
        ...
        Eigen::Matrix3d rotationMatrix =
                ( Eigen::AngleAxisd( -angleZ2, Eigen::Vector3d::UnitZ( ) ) * Eigen::AngleAxisd( -angleX, Eigen::Vector3d::UnitX( ) ) *
                  Eigen::AngleAxisd( -angleZ1, Eigen::Vector3d::UnitZ( ) ) )
                        .toRotationMatrix( );
        Eigen::Matrix3d manualMatrix = ( Eigen::Matrix3d( ) << std::cos( angleZ2 ),
                                         std::sin( angleZ2 ),
                                         0.0,
                                         -std::sin( angleZ2 ),
                                         std::cos( angleZ2 ),
                ...
                ( Eigen::Matrix3d( ) << 1.0,
                  ...
                  std::cos( angleX ) )
                        .finished( ) *
                ( Eigen::Matrix3d( ) << std::cos( angleZ1 ),
                  std::sin( angleZ1 ),
                  0.0,
                  -std::sin( angleZ1 ),
                  std::cos( angleZ1 ),
        ...
        Eigen::Quaterniond rotationQuaternion = Eigen::Quaterniond( manualMatrix );
        double quaternionEntryW = 0.5 * std::sqrt( 1.0 + manualMatrix( 0, 0 ) + manualMatrix( 1, 1 ) + manualMatrix( 2, 2 ) );
        Eigen::Quaterniond manualQuaternion =
                Eigen::Quaterniond( quaternionEntryW,
...
BOOST_AUTO_TEST_CASE( testEulerAnglePartials )
{
    ...
    eulerAnglePartial2 = calculateEulerAngle313WrtQuaternionPartialFromEulerAngles( get313EulerAnglesFromQuaternion( quaternion ) );
    ...
    for( int i = 1; i < 4; i++ )
    {
        ...
        manualAnglePartial.block( 0, i, 3, 1 ) =
                ( get313EulerAnglesFromQuaternion( upperturbedQuaternion ) - get313EulerAnglesFromQuaternion( downperturbedQuaternion ) ) /
                ( 2.0 * perturbation );
        manualQ0PartialContribution.block( 0, i, 3, 1 ) =
                eulerAnglePartial.block( 0, 0, 3, 1 ) * -quaternionInVectorForm( i ) / quaternionInVectorForm( 0 );
    }
    ...
    for( unsigned int i = 0; i < 3; i++ )
    {
        for( unsigned int j = 1; j < 4; j++ )
        {
            ...
            BOOST_CHECK_LT( std::fabs( eulerAnglePartial( i, j ) - ( manualAnglePartial( i, j ) - manualQ0PartialContribution( i, j ) ) ),
                            1.0E-9 );
        }
