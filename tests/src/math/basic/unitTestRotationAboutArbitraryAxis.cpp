...
BOOST_AUTO_TEST_CASE( test_RotationAboutArbitraryAxis_PointRotationWithCommonOrigin )
{
    ...
    const Eigen::Vector3d computedRotatedPosition = basic_mathematics::computeRotationOfPointAboutArbitraryAxis(
    ...
    TUDAT_CHECK_MATRIX_CLOSE_FRACTION(
            computedRotatedPosition.segment( 1, 2 ), expectedRotatedPosition.segment( 1, 2 ), std::numeric_limits< double >::epsilon( ) );
}
...
BOOST_AUTO_TEST_CASE( test_RotationAboutArbitraryAxis_PointRotationWithDifferentOrigins )
{
    ...
    // Set expected rotated vector.
    const Eigen::Vector3d expectedRotatedPosition = Eigen::Vector3d( 3.156561876696307, -5.97145870839039, -4.418680390057799 );
    // Set origin of rotation.
    ...
    TUDAT_CHECK_MATRIX_CLOSE_FRACTION( computedRotatedPosition, expectedRotatedPosition, std::numeric_limits< double >::epsilon( ) );
}
...
BOOST_AUTO_TEST_CASE( test_RotationAboutArbitraryAxis_PointRotationWithDifferentOrigins2 )
{
    ...
    // Set expected rotated vector.
    const Eigen::Vector3d expectedRotatedPosition = Eigen::Vector3d( 15.71267522236938, -0.9723168350942417, 9.449538267504582 );
    // Set origin of rotation.
...
BOOST_AUTO_TEST_CASE( test_RotationAboutArbitraryAxis_VectorRotationWithCommonOrigin )
{
    ...
    // Set expected rotated vector.
    const Eigen::Vector3d expectedRotatedVector = Eigen::Vector3d( -5.480598288892924, 0.532794739754852, 6.138336269794405 );
    // Set origin of rotation.
    const Eigen::Vector3d originOfRotation = Eigen::Vector3d( 0.0, 0.0, 0.0 );
    ...
    // Compute rotated position.
    const Eigen::Vector3d computedRotatedVector = basic_mathematics::computeRotationOfVectorAboutArbitraryAxis(
            originOfRotation, angleOfRotation, axisOfRotation, initialPositionOfVectorTail, initialVector );
...
BOOST_AUTO_TEST_CASE( test_RotationAboutArbitraryAxis_VectorRotationWithDifferentOrigins )
{
    ...
    // Set expected rotated vector.
    const Eigen::Vector3d expectedRotatedVector = Eigen::Vector3d( -7.15233485964669, -2.4444137866784175, 8.308967883857736 );
    // Set origin of rotation.
    const Eigen::Vector3d originOfRotation = Eigen::Vector3d( 1.5, 3.8, 12.0 );
