...
BOOST_AUTO_TEST_CASE( testSphericalToCartesianConversion )
{
    ...
    // Test 1: test conversion of: ( 0.0, 0.0, 0.0 ).
    {
        ...
        BOOST_CHECK_SMALL( cartesianCoordinates.coeff( row, col ), std::numeric_limits< double >::min( ) );
    }
    ...
    // Test 2: test conversion of: ( 1.0, pi/6, pi/6 ), from Stewart (2003), Exercise 12.7.15.
    {
        ...
        // Expected cartesian coordinates.
        Eigen::Vector3d expectedCartesianCoordinates( sqrt( 3.0 ) / 4.0, 1.0 / 4.0, sqrt( 3.0 ) / 2.0 );

        // Check if converted Cartesian coordinates are correct.
        TUDAT_CHECK_MATRIX_CLOSE_FRACTION( cartesianCoordinates, expectedCartesianCoordinates, 1.0e-15 );
    }
    ...
    // Test 3: test conversion of: ( 2.0, pi/4, pi/3 ), from Stewart (2003), Exercise 12.7.17.
    {
        ...
        // Expected cartesian coordinates.
        Eigen::Vector3d expectedCartesianCoordinates( 0.5 * sqrt( 2.0 ), 0.5 * sqrt( 6.0 ), sqrt( 2.0 ) );

        // Check if converted Cartesian coordinates are correct.
        TUDAT_CHECK_MATRIX_CLOSE_FRACTION( cartesianCoordinates, expectedCartesianCoordinates, std::numeric_limits< double >::epsilon( ) );
    }
...
BOOST_AUTO_TEST_CASE( testCartesianToSphericalConversion )
{
    ...
    // Test 1: Test conversion of: ( 0.0, 0.0, 0.0 ).
    {
        ...
        TUDAT_CHECK_MATRIX_CLOSE_FRACTION( sphericalCoordinates, expectedSphericalCoordinates, std::numeric_limits< double >::epsilon( ) );
    }
...
BOOST_AUTO_TEST_CASE( testCylindricalToCartesianPositionCoordinateConversion )
{
    ...
    // Test 1: test conversion of ( 0.0, pi, 1.2 ).
    {
        ...
        const Eigen::Vector3d computedCartesianCoordinates =
                coordinate_conversions::convertCylindricalToCartesian( cylindricalCoordinates );

        // Check if computed Cartesian coordinates match expected values.
        BOOST_CHECK_SMALL( computedCartesianCoordinates( xCartesianCoordinateIndex ), std::numeric_limits< double >::epsilon( ) );

        BOOST_CHECK_SMALL( computedCartesianCoordinates( yCartesianCoordinateIndex ), std::numeric_limits< double >::epsilon( ) );
    ...
    // Test 2: test conversion of ( 2.3, -pi/2, -3.5 ).
    {
        ...
        // Set expected Cartesian coordinates.
        const Eigen::Vector3d expectedCartesianCoordinates( 2.3 * std::cos( -PI / 2.0 ), 2.3 * std::sin( -PI / 2.0 ), -3.5 );
        ...
        BOOST_CHECK_SMALL( convertedCartesianCoordinates( xCartesianCoordinateIndex ), std::numeric_limits< double >::epsilon( ) );

        TUDAT_CHECK_MATRIX_CLOSE_FRACTION( expectedCartesianCoordinates.segment( yCartesianCoordinateIndex, 2 ),
...
BOOST_AUTO_TEST_CASE( testCylindricalToCartesianPositionAndVelocityCoordinateConversion )
{
    ...
    // Test 1: test conversion of (2.1, pi/2.0, 1.2, 5.4, 4.5, -3.9).
    {
        ...
        const Eigen::Vector6d cylindricalState = ( Eigen::Vector6d( ) << 2.1, PI / 2.0, 1.2, 5.4, 4.5, -3.9 ).finished( );

        // Set expected Cartesian state (x, y, z, xdot, ydot, zdot).
        const Eigen::Vector6d expectedCartesianState = ( Eigen::Vector6d( ) << 2.1 * std::cos( PI / 2.0 ),
                                                         2.1 * std::sin( PI / 2.0 ),
                                                         1.2,
        ...
        const Eigen::Vector6d convertedCartesianState = coordinate_conversions::convertCylindricalToCartesianState( cylindricalState );

        // Check if converted Cartesian state match expected state.
        BOOST_CHECK_SMALL( convertedCartesianState( xCartesianCoordinateIndex ), std::numeric_limits< double >::epsilon( ) );

        TUDAT_CHECK_MATRIX_CLOSE_FRACTION( expectedCartesianState.segment( yCartesianCoordinateIndex, 5 ),
    ...
    {
        // Set Cylindrical state (r, theta, z, Vr, Vtheta, Vz).
        const Eigen::Vector6d cylindricalState = ( Eigen::Vector6d( ) << 0.0, 8.2 * PI / 3.0, -2.5, -5.8, 0.0, 1.7 ).finished( );

        // Set expected Cartesian state (x, y, z, xdot, ydot, zdot).
        const Eigen::Vector6d expectedCartesianState =
                ( Eigen::Vector6d( ) << 0.0, 0.0, -2.5, -5.8 * std::cos( 8.2 * PI / 3.0 ), -5.8 * std::sin( 8.2 * PI / 3.0 ), 1.7 )
                        .finished( );
        ...
        BOOST_CHECK_SMALL( convertedCartesianState( yCartesianCoordinateIndex ), std::numeric_limits< double >::epsilon( ) );

        TUDAT_CHECK_MATRIX_CLOSE_FRACTION( expectedCartesianState.segment( zCartesianCoordinateIndex, 4 ),
...
BOOST_AUTO_TEST_CASE( testCartesianToCylindricalPositionCoordinateConversion )
{
    ...
    // Test 1: test conversion of ( 0.0, 0.0, 1.0 ).
    {
        ...
        const Eigen::Vector3d convertedCylindricalCoordinates =
                coordinate_conversions::convertCartesianToCylindrical( cartesianCoordinates );

        // Check if converted cylindrical coordinates match expected values.
        BOOST_CHECK_SMALL( convertedCylindricalCoordinates( rCylindricalCoordinateIndex ), std::numeric_limits< double >::epsilon( ) );

        BOOST_CHECK_SMALL( convertedCylindricalCoordinates( thetaCylindricalCoordinateIndex ), std::numeric_limits< double >::epsilon( ) );
    ...
    // Test 2: test conversion of ( 0.0, 2.0, 1.0 ).
    {
        ...
        TUDAT_CHECK_MATRIX_CLOSE_FRACTION(
                expectedCylindricalCoordinates, convertedCylindricalCoordinates, std::numeric_limits< double >::epsilon( ) );
    }
    ...
    // Test 4: test conversion of ( -5.0, -8.0, 5.0 ).
    {
        ...
        // Set expected cylindrical coordinates (r, theta, z).
        const Eigen::Vector3d expectedCylindricalCoordinates( std::sqrt( 25.0 + 64.0 ), std::atan2( -8.0, -5.0 ) + 2.0 * PI, 5.0 );
...
BOOST_AUTO_TEST_CASE( testCartesianToCylindricalPositionAndVelocityCoordinateConversion )
{
    ...
    // Test 1: test conversion of ( 0.0, 0.0, 1.0, 5.0, 6.0, -9.0 ).
    {
        ...
        const Eigen::Vector6d cartesianState = ( Eigen::Vector6d( ) << 0.0, 0.0, 1.0, 5.0, 6.0, -9.0 ).finished( );

        // Set expected cylindrical state (r, theta, z, Vr, Vtheta, Vz).
        const Eigen::Vector6d expectedCylindricalState =
                ( Eigen::Vector6d( ) << 0.0, 0.0, 1.0, std::sqrt( 25.0 + 36.0 ), 0.0, -9.0 ).finished( );

        // Convert Cartesian to cylindrical state (r, theta, z, Vr, Vtheta, Vz).
        const Eigen::Vector6d convertedCylindricalState = coordinate_conversions::convertCartesianToCylindricalState( cartesianState );

        // Check that converted cylindrical state matches expected state.
        BOOST_CHECK_SMALL( convertedCylindricalState( rCylindricalCoordinateIndex ), std::numeric_limits< double >::epsilon( ) );

        BOOST_CHECK_SMALL( convertedCylindricalState( thetaCylindricalCoordinateIndex ), std::numeric_limits< double >::epsilon( ) );
        ...
        BOOST_CHECK_SMALL( convertedCylindricalState( vThetaCylindricalCoordinateIndex ), std::numeric_limits< double >::epsilon( ) );
    ...
    {
        // Set Cartesian state (x,y,z,xdot,ydot,zdot).
        const Eigen::Vector6d cartesianState = ( Eigen::Vector6d( ) << 2.0, 0.0, -5.0, -4.0, 6.0, -6.0 ).finished( );

        // Set expected cylindrical state (r, theta, z, Vr, Vtheta, Vz).
        const Eigen::Vector6d expectedCylindricalState = ( Eigen::Vector6d( ) << 2.0, 0.0, -5.0, -4.0, 6.0, -6.0 ).finished( );
        ...
        TUDAT_CHECK_MATRIX_CLOSE_FRACTION( expectedCylindricalState.segment( zCylindricalCoordinateIndex, 4 ),
    ...
    {
        // Set Cartesian state (x,y,z,xdot,ydot,zdot).
        const Eigen::Vector6d cartesianState = ( Eigen::Vector6d( ) << -7.0, -4.0, 3.0, 5.0, -3.0, 7.0 ).finished( );

        // Set expected cylindrical state (r, theta, z, Vr, Vtheta, Vz).
        const Eigen::Vector6d expectedCylindricalState = ( Eigen::Vector6d( ) << std::sqrt( 49.0 + 16.0 ),
                                                           std::atan2( -4.0, -7.0 ) + 2.0 * PI,
                                                           3.0,
                                                           ( -7.0 * 5.0 + ( -4.0 ) * -3.0 ) / std::sqrt( 49.0 + 16.0 ),
                                                           ( -7.0 * -3.0 - ( -4.0 ) * 5.0 ) / sqrt( 49.0 + 16.0 ),
                                                           7.0 )
                                                                 .finished( );
        ...
        // Check that converted cylindrical state matches expected state.
        TUDAT_CHECK_MATRIX_CLOSE_FRACTION( expectedCylindricalState, convertedCylindricalState, std::numeric_limits< double >::epsilon( ) );
    }
}
...
BOOST_AUTO_TEST_CASE( test_SphericalToCartesianGradientConversion )
{
    ...
    const Eigen::Vector3d sphericalGradient( -2.438146967844150, 1.103964186650749e4, 5.932496087870976e1 );
    ...
    const Eigen::Vector3d cartesianGradient =
            coordinate_conversions::convertSphericalToCartesianGradient( sphericalGradient, cartesianCoordinates );
    ...
    const Eigen::Vector3d expectedCartesianGradient( 1.334464111786447, -1.620429182163669, 1.240151677089496 );
...
BOOST_AUTO_TEST_CASE( test_SphericalToCartesianGradientPartialDerivatives )
{
    ...
    for( unsigned int i = 0; i < 3; i++ )
    {
        ...
        matrixPartials[ i ] = coordinate_conversions::getSphericalToCartesianGradientMatrix( perturbedCartesianCoordinates );
...
BOOST_AUTO_TEST_CASE( testCartesianToSphericalStateConversion )
{
    ...
    // Test 1: test conversion of ( 0.0, 0.0, 1.0, 5.0, 6.0, -9.0 ).
    {
        ...
        const Eigen::Vector6d expectedSphericalState = ( Eigen::Vector6d( ) << 1.0, 0.0, PI / 2.0, -9.0, 6.0, -5.0 ).finished( );

        // Convert Cartesian to spherical state.
        const Eigen::Vector6d convertedSphericalState = convertCartesianToSphericalState( cartesianState );
        ...
        BOOST_CHECK_SMALL( convertedSphericalState( azimuthSphericalCoordinateIndex ), std::numeric_limits< double >::epsilon( ) );
        ...
        BOOST_CHECK_CLOSE_FRACTION( expectedSphericalState( radialVelocitySphericalCoordinateIndex ),
                                    convertedSphericalState( radialVelocitySphericalCoordinateIndex ),
                                    std::numeric_limits< double >::epsilon( ) );

        BOOST_CHECK_CLOSE_FRACTION( expectedSphericalState( azimuthVelocitySphericalCoordinateIndex ),
                                    convertedSphericalState( azimuthVelocitySphericalCoordinateIndex ),
                                    std::numeric_limits< double >::epsilon( ) );

        BOOST_CHECK_CLOSE_FRACTION( expectedSphericalState( elevationVelocitySphericalCoordinateIndex ),
                                    convertedSphericalState( elevationVelocitySphericalCoordinateIndex ),
                                    std::numeric_limits< double >::epsilon( ) );
    }
    ...
    {
        // Set Cartesian state (x, y , z, xdot, ydot, zdot).
        const Eigen::Vector6d cartesianState = ( Eigen::Vector6d( ) << 2.0, 0.0, 0.0, -4.0, 6.0, -6.0 ).finished( );

        // Set expected spherical state (r, theta, phi, Vr, Vtheta, Vphi).
        const Eigen::Vector6d expectedSphericalState = ( Eigen::Vector6d( ) << 2.0, 0.0, 0.0, -4.0, 6.0, -6.0 ).finished( );
        ...
        BOOST_CHECK_SMALL( convertedSphericalState( elevationSphericalCoordinateIndex ), std::numeric_limits< double >::epsilon( ) );
    ...
    {
        // Set Cartesian state ( x, y, z, xdot, ydot, zdot).
        const Eigen::Vector6d cartesianState = ( Eigen::Vector6d( ) << -1.0, -1.0, sqrt( 2.0 ), -1.0, 1.0, sqrt( 2.0 ) ).finished( );

        // Set expected spherical state (r, theta, phi, Vr, Vtheta, Vphi).
        const Eigen::Vector6d expectedSphericalState =
                ( Eigen::Vector6d( ) << 2.0, -PI * 3.0 / 4.0, PI / 4.0, 1.0, -sqrt( 2.0 ), 1.0 ).finished( );

        // Convert Cartesian to spherical state (r, theta, phi, Vr, Vtheta, Vphi).
        const Eigen::Vector6d convertedSphericalState = convertCartesianToSphericalState( cartesianState );

        // Check that converted spherical state matches the expected spherical state.
        TUDAT_CHECK_MATRIX_CLOSE_FRACTION( expectedSphericalState, convertedSphericalState, std::numeric_limits< double >::epsilon( ) );
    }
}
...
BOOST_AUTO_TEST_CASE( testSphericalToCartesianStateConversion )
{
    ...
    // Test 1: test conversion of ( 1.0, 0.0, pi/2, -9.0, 6.0, -5.0 ).
    {
        ...
        const Eigen::Vector6d sphericalState = ( Eigen::Vector6d( ) << 1.0, 0.0, PI / 2.0, -9.0, 6.0, -5.0 ).finished( );

        // Set expected Cartesian state (x, y, z, xdot, ydot, zdot).
        const Eigen::Vector6d expectedCartesianState = ( Eigen::Vector6d( ) << 0.0, 0.0, 1.0, 5.0, 6.0, -9.0 ).finished( );

        // Convert spherical to Cartesian state.
        const Eigen::Vector6d convertedCartesianState = convertSphericalToCartesianState( sphericalState );
    ...
    {
        // Set spherical state (r, theta, phi, Vr, Vtheta, Vphi).
        const Eigen::Vector6d sphericalState = ( Eigen::Vector6d( ) << 2.0, 0.0, 0.0, -4.0, 6.0, -6.0 ).finished( );

        // Set expected Cartesian state ( x, y, z, xdot, ydot, zdot).
        const Eigen::Vector6d expectedCartesianState = ( Eigen::Vector6d( ) << 2.0, 0.0, 0.0, -4.0, 6.0, -6.0 ).finished( );
        ...
        BOOST_CHECK_SMALL( convertedCartesianState( zCartesianCoordinateIndex ), std::numeric_limits< double >::epsilon( ) );
    ...
    {
        // Set spherical state (r, theta, phi, Vr, Vtheta, Vphi).
        const Eigen::Vector6d sphericalState = ( Eigen::Vector6d( ) << 2.0, -PI * 3.0 / 4.0, PI / 4.0, 1.0, -sqrt( 2.0 ), 1.0 ).finished( );

        // Set expected Cartesian state (x, y, z, xdot, ydot, zdot).
        const Eigen::Vector6d expectedCartesianState =
                ( Eigen::Vector6d( ) << -1.0, -1.0, sqrt( 2.0 ), -1.0, 1.0, sqrt( 2.0 ) ).finished( );

        // Convert spherical to Cartesian state.
        const Eigen::Vector6d convertedCartesianState = convertSphericalToCartesianState( sphericalState );

        // Check that converted Cartesian state matches the expected state.
        TUDAT_CHECK_MATRIX_CLOSE_FRACTION( expectedCartesianState, convertedCartesianState, 1.0e-15 );
    }
}
