...
BOOST_AUTO_TEST_CASE( testRadiationPressureCalculation )
{
    ...
    double totalSolarPower =
            1367.0 * 4.0 * mathematical_constants::PI * physical_constants::ASTRONOMICAL_UNIT * physical_constants::ASTRONOMICAL_UNIT;

    // Calculate radiation pressure at 1 AU.
    double calculatedRadiationPressure =
            electromagnetism::calculateRadiationPressure( totalSolarPower, physical_constants::ASTRONOMICAL_UNIT );
    ...
    std::shared_ptr< electromagnetism::RadiationPressureInterface > radiationPressureInterface =
            std::make_shared< electromagnetism::RadiationPressureInterface >(
                    ...
                    [ & ]( ) {
                        return ( Eigen::Vector3d( ) << 0.0, physical_constants::ASTRONOMICAL_UNIT / std::sqrt( 2.0 ), 0.0 ).finished( );
                    },
                    [ & ]( ) {
                        return ( Eigen::Vector3d( ) << physical_constants::ASTRONOMICAL_UNIT / std::sqrt( 2.0 ), 0.0, 0.0 ).finished( );
                    },
                    1.0,
                    1.0 );
    radiationPressureInterface->updateInterface( );
    double classCalculatedRadiationPressure = radiationPressureInterface->getCurrentRadiationPressure( );

    BOOST_CHECK_CLOSE_FRACTION( classCalculatedRadiationPressure, expectedRadiationPressure, 1.0E-4 );
    BOOST_CHECK_CLOSE_FRACTION(
            classCalculatedRadiationPressure, calculatedRadiationPressure, 2.0 * std::numeric_limits< double >::epsilon( ) );
}
...
BOOST_AUTO_TEST_CASE( testShadowFunctionLink )
{
    ...
    std::shared_ptr< electromagnetism::RadiationPressureInterface > occultedRadiationPressureInterface =
            std::make_shared< electromagnetism::RadiationPressureInterface >( [ & ]( ) { return totalSolarPower; },
                                                                              [ & ]( ) { return occultedBodyPosition; },
                                                                              [ & ]( ) { return satellitePosition; },
                                                                              1.0,
                                                                              1.0,
                                                                              occultingBodyPositionFunctions,
                                                                              occultingBodyRadii,
                                                                              occultedBodyRadius );
    occultedRadiationPressureInterface->updateInterface( );

    // Create radiation pressure interface without occultation.
    std::shared_ptr< electromagnetism::RadiationPressureInterface > unoccultedRadiationPressureInterface =
            std::make_shared< electromagnetism::RadiationPressureInterface >( [ & ]( ) { return totalSolarPower; },
                                                                              [ & ]( ) { return occultedBodyPosition; },
                                                                              [ & ]( ) { return satellitePosition; },
                                                                              1.0,
    ...
    BOOST_CHECK_CLOSE_FRACTION( occultedRadiationPressureInterface->getCurrentRadiationPressure( ) /
                                        unoccultedRadiationPressureInterface->getCurrentRadiationPressure( ),
                                0.4547,
                                1.0E-4 );
}
