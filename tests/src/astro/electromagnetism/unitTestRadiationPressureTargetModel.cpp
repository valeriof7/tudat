...
BOOST_AUTO_TEST_CASE( testCannonballRadiationPressureTargetModelAtAU )
{
    ...
    const auto actualForce = targetModel.updateAndGetRadiationPressureForce( 1367, Eigen::Vector3d::UnitX( ), true );
...
BOOST_AUTO_TEST_CASE( testCannonballRadiationPressureTargetModelGaneff )
{
    ...
    const auto actualForce = targetModel.updateAndGetRadiationPressureForce( sourceIrradiance, sourceToTargetDirection, true );
...
BOOST_AUTO_TEST_CASE( testRadiationPressureTargetModel_EquivalentSinglePanel )
{
    ...
    const auto cannonballForce = cannonballModel.updateAndGetRadiationPressureForce( sourceIrradiance, sourceToTargetDirection, true );
    const auto paneledForce = paneledModel.updateAndGetRadiationPressureForce( sourceIrradiance, sourceToTargetDirection, true );

    TUDAT_CHECK_MATRIX_CLOSE( cannonballForce, paneledForce, 1e-10 );

    const auto cannonballTorque = cannonballModel.updateAndGetRadiationPressureTorque( sourceIrradiance, sourceToTargetDirection, true );
    const auto paneledTorque = paneledModel.updateAndGetRadiationPressureTorque( sourceIrradiance, sourceToTargetDirection, true );
...
BOOST_AUTO_TEST_CASE( testRadiationPressureTargetModel_EquivalentSphere )
{
    ...
    for( unsigned int i = 0; i < numberOfPanels; ++i )
    {
        ...
        const Eigen::Vector3d relativeCenter =
                coordinate_conversions::convertSphericalToCartesian( Eigen::Vector3d( radius, polarAngle, azimuthAngle ) );
    ...
    const Eigen::Vector3d cannonballTorque =
            cannonballModel.updateAndGetRadiationPressureTorque( sourceIrradiance, sourceToTargetDirection, true );
    const Eigen::Vector3d paneledTorque =
            paneledModel.updateAndGetRadiationPressureTorque( sourceIrradiance, sourceToTargetDirection, true );
    const Eigen::Vector3d expectedPaneledTorque = -centerOfMass.cross( paneledForce );
