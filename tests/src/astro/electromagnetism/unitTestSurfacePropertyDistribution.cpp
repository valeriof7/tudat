...
BOOST_AUTO_TEST_CASE( testSphericalHarmonicsSurfacePropertyDistribution )
{
    // Order 0, degree 0 (identical to constant distribution)
    {
        ...
        SphericalHarmonicsSurfacePropertyDistribution distributionModel( cosineCoefficients, sineCoefficients );
        distributionModel.updateMembers( TUDAT_NAN );
    ...
    // Order 1, degree 1
    {
        ...
        cosineCoefficients << 0.1, 0.0, 0.2, 0.3;

        Eigen::MatrixXd sineCoefficients( 2, 2 );
        sineCoefficients << 0.0, 0.0, 0.0, 0.4;
...
BOOST_AUTO_TEST_CASE( testSecondDegreeZonalPeriodicSurfacePropertyDistribution_Zonality )
{
    ...
    const double referenceEpoch = spice_interface::convertDateStringToEphemerisTime( "1981 DEC 22" );
    ...
    for( double latitude: { -PI / 2, 0., 1.403, PI / 2 } )
    {
        ...
        {
            values.push_back( distributionModel.getValue( latitude, unit_conversions::convertDegreesToRadians( longitude ) ) );
        }
