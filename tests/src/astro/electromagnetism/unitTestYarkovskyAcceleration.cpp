 ...
 *
 *    Reference: Pérez-Hernández, J. A., & Benet, L. (2022). Non-zero Yarkovsky acceleration for near-Earth asteroid (99942) Apophis.
...
double computeExpectedSemiMajorAxisDrift( const Eigen::Vector6d& keplerElements, const double mu )
{
    ...
    const double semiMajorAxisDrift =
            2.0 * yarkovskyParameter * ( 1.0 - eccentricity * eccentricity ) * AU * AU / ( meanMotion * semiLatusRectum * semiLatusRectum );
...
BOOST_AUTO_TEST_CASE( testYarkovskyAccelerationVerySimple )
{
    ...
    const Eigen::Vector3d computedYarkovskyAcceleration = electromagnetism::computeYarkovskyAcceleration( yarkovskyParameter, state );
    const Eigen::Vector3d expectedYarkovskyAcceleration = computeExpectedYarkovskyAcceleration( yarkovskyParameter, state );

    TUDAT_CHECK_MATRIX_CLOSE_FRACTION(
            computedYarkovskyAcceleration, expectedYarkovskyAcceleration, ( 10.0 * std::numeric_limits< double >::epsilon( ) ) );
}
...
{
    electromagnetism::YarkovskyAccelerationPointer yarkovskyAccelerationModel =
            std::make_shared< electromagnetism::YarkovskyAcceleration >( yarkovskyParameter, &getBodyState, &getCentralBodyState );

    yarkovskyAccelerationModel->updateMembers( 0.0 );
    const Eigen::Vector3d expectedYarkovskyAcceleration = computeExpectedYarkovskyAcceleration( yarkovskyParameter, bodyState );
...
BOOST_AUTO_TEST_CASE( testYarkovskyAccelerationCircular )
{
    ...
    // Initial State
    Eigen::VectorXd initialState = orbital_element_conversions::convertKeplerianToCartesianElements( initialSemiMajorAxis,
                                                                                                     initialEccentricity,
                                                                                                     initialInclination,
                                                                                                     initialRAAN,
                                                                                                     initialArgOfPeri,
    ...
    std::vector< std::shared_ptr< propagators::SingleDependentVariableSaveSettings > > dependentVariablesToSave = {
        propagators::singleAccelerationDependentVariable( basic_astrodynamics::yarkovsky_acceleration, "Asteroid", "Sun" ),
    ...
    std::shared_ptr< numerical_integrators::IntegratorSettings< double > > integratorSettings =
            numerical_integrators::rungeKutta4Settings( fixedStepSize );

    // Propagator Settings
    std::shared_ptr< propagators::TranslationalStatePropagatorSettings< double, double > > propagatorSettings =
            propagators::translationalStatePropagatorSettings( centralBodies,
                                                               accelerationModels,
                                                               bodiesToPropagate,
                                                               initialState,
...

    {
        ...
        for( const auto& pair: stateHistory )
        {
            if( pair.first > time + timeBetweenChecks )
            {
                ...
                BOOST_CHECK_CLOSE_FRACTION( expectedYarkovskyAcceleration.norm( ),
