...
BOOST_AUTO_TEST_CASE( testEarthOrientationRotationSetupAgainstSofa )
{
    ...
    for( unsigned int i = 0; i < 3; i++ )
    {
        ...
        {
            BOOST_CHECK_SMALL( std::fabs( expectedCirsToGcrsRotation.transpose( )( i, j ) - cirsToGcrsRotation( i, j ) ), 1.0E-15 );
        }
    }
    ...
    Eigen::Matrix3d itrsToGcrsRotationDirect = Eigen::Matrix3d( calculateRotationFromItrsToGcrs(
            X, Y, s, ut1Long, xPole, yPole, getApproximateTioLocator( terrestrialTimeSecondsSinceJ2000 ) ) );
    for( unsigned int i = 0; i < 3; i++ )
    {
        for( unsigned int j = 0; j < 3; j++ )
        {
            BOOST_CHECK_SMALL( std::fabs( itrsToGcrsRotationDirect( i, j ) - itrsToGcrsRotation( i, j ) ), 1.0E-14 );
        }
    }
    ...
    Eigen::Matrix3d itrsToGcrsRotationDirectInaccurate = Eigen::Matrix3d(
            calculateRotationFromItrsToGcrs( X, Y, s, ut1, xPole, yPole, getApproximateTioLocator( terrestrialTimeSecondsSinceJ2000 ) ) );

    for( unsigned int i = 0; i < 3; i++ )
    {
        for( unsigned int j = 0; j < 3; j++ )
        {
            BOOST_CHECK_SMALL( std::fabs( itrsToGcrsRotationDirectInaccurate( i, j ) - itrsToGcrsRotation( i, j ) ), 1.0E-12 );
        }
    }

    // Compare Tudat GCRS -> TIRS against Sofa
    Eigen::Matrix3d expectedGcrsToTirs;
    expectedGcrsToTirs << 0.9731043175731277, 0.2303638262477064, -0.0007033328188453794, -0.2303637988041795, 0.9731045707355742,
            0.0001208885495858678, 0.0007122647295989105, 4.438525042571229e-05, 0.9999997453544198;
    for( unsigned int i = 0; i < 3; i++ )
    {
        for( unsigned int j = 0; j < 3; j++ )
        {
            BOOST_CHECK_SMALL( std::fabs( expectedGcrsToTirs.transpose( )( i, j ) - ( cirsToGcrsRotation * tirsToCirsRotation )( i, j ) ),
                               1.0E-15 );
        }
    }
    ...
    for( unsigned int i = 0; i < 3; i++ )
    {
        for( unsigned int j = 0; j < 3; j++ )
        {
            BOOST_CHECK_SMALL( expectedGcrsToItrs.transpose( )( i, j ) - itrsToGcrsRotation( i, j ), 1.0E-15 );
        }
    }
}
...
BOOST_AUTO_TEST_CASE( testEarthOrientationAngleFunctionsAgainstSofa )
{
    ...
    double terrestrialTimeSecondsSinceJ2000Inaccruate =
            ( terrestrialTimeDaysSinceMjd0 - ( basic_astrodynamics::JULIAN_DAY_ON_J2000 - basic_astrodynamics::JULIAN_DAY_AT_0_MJD ) ) *
            physical_constants::JULIAN_DAY;
    ...
    Time ut1TimeSecondsSinceJ2000 =
            tudat::Time( ( ut1JulianDay - basic_astrodynamics::JULIAN_DAY_ON_J2000 ) * 24 * 3600 / TIME_NORMALIZATION_INTEGER_TERM,
                         ut1FractionOfDay * physical_constants::JULIAN_DAY_LONG );
    ...
    Eigen::Vector3d positionOfCipInGcrs = sofa_interface::getPositionOfCipInGcrs(
            terrestrialTimeSecondsSinceJ2000Inaccruate, basic_astrodynamics::JULIAN_DAY_ON_J2000, basic_astrodynamics::iau_2006 );
    ...
    Eigen::Vector2d tudatXYCorrection =
            createStandardEarthOrientationCalculator( )->getPrecessionNutationCalculator( )->getDailyCorrectionInterpolator( )->interpolate(
    ...
    BOOST_CHECK_SMALL(
            std::fabs( era - tudat::sofa_interface::calculateEarthRotationAngleTemplated< double >( rotationAnglesInaccurate.second ) ),
            1.0E-9 );
    ...
    std::pair< Eigen::Vector5d, Time > rotationAngles =
            earthOrientationCalculator->getRotationAnglesFromItrsToGcrs< Time >( ut1TimeSecondsSinceJ2000, basic_astrodynamics::ut1_scale );
