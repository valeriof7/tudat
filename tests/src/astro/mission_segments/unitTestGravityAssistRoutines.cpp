...
BOOST_AUTO_TEST_CASE( testPoweredGravityAssistBackwardPropagationConsistencyCheck )
{
    ...
    // Perform the gravity assist.
    const Eigen::Vector3d outgoingVelocity = mission_segments::calculatePoweredGravityAssistOutgoingVelocity(
            venusGravitationalParameter, venusVelocity, incomingVelocity, outgoingVelocityRotationAngle, pericenterRadius, deltaV );

    const Eigen::Vector3d calculatedIncomingVelocity = mission_segments::calculatePoweredGravityAssistIncomingVelocity(
            venusGravitationalParameter, venusVelocity, outgoingVelocity, incomingVelocityRotationAngle, pericenterRadius, deltaV );
    ...
    const double calculatedDeltaV = mission_segments::calculateGravityAssistDeltaV(
            venusGravitationalParameter, venusVelocity, calculatedIncomingVelocity, outgoingVelocity, pericenterRadius, useEccentricity );
    ...
    const double incomingEccentricity =
            1.0 + pericenterRadius / venusGravitationalParameter * absoluteRelativeIncomingVelocity * absoluteRelativeIncomingVelocity;
    ...
    const double absoluteRelativeOutgoingVelocity =
            std::sqrt( outgoingPericenterVelocity * outgoingPericenterVelocity - 2.0 * venusGravitationalParameter / pericenterRadius );
    const double outgoingBendingAngle = std::asin(
            1.0 /
            ( 1.0 +
              absoluteRelativeOutgoingVelocity * absoluteRelativeOutgoingVelocity * pericenterRadius / venusGravitationalParameter ) );
    ...
    const double calculatedBendingAngle =
            std::acos( planetocentricOutgoingVelocity.dot( planetocentricCalculatedIncomingVelocity ) /
                       planetocentricOutgoingVelocity.norm( ) / planetocentricCalculatedIncomingVelocity.norm( ) );
