...
BOOST_AUTO_TEST_CASE( testVelocities )
{
    ...
    const Eigen::Vector3d expectedArrivalVelocity( 17969.3166254715, -23543.6915939142, 6.38384671663485 );
    ...
    Eigen::VectorXd nodeFreeParameters =
            ( Eigen::VectorXd( 4 ) << 0.0, excessVelocityMagnitude, excessVelocityInPlaneAngle, excessVelocityOutOfPlaneAngle ).finished( );

    using namespace mission_segments;
    DepartureWithFreeOutgoingVelocityNode transferNode( constantEphemeris1, earthGravitationalParameter, semiMajorAxis, eccentricity );
    ...
    Eigen::VectorXd legFreeParameters = ( Eigen::VectorXd( 3 ) << 0.0, timeOfFlight, dsmTimeOfFlightFraction ).finished( );

    Eigen::Vector3d departureVelocity = transferNode.getOutgoingVelocity( );
    DsmVelocityBasedTransferLeg transferLeg(
            constantEphemeris1, constantEphemeris2, sunGravitationalParameter, [ = ]( ) { return departureVelocity; } );
