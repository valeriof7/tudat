...
BOOST_AUTO_TEST_CASE( testAerodynamicCoefficientsFromFile )
{
    ...
    for( unsigned int i = 0; i < 3; i++ )
    {
        ...
        forceCoefficientFiles[ 0 ] = tudat::paths::getTudatTestDataPath( ) + "/aurora_CD.txt";
        forceCoefficientFiles[ 2 ] = tudat::paths::getTudatTestDataPath( ) + "/aurora_CL.txt";
        std::map< int, std::string > momentCoefficientFiles;
        momentCoefficientFiles[ 1 ] = tudat::paths::getTudatTestDataPath( ) + "/aurora_Cm.txt";
        std::map< int, std::string > controlSurfaceForceCoefficientFiles;
        controlSurfaceForceCoefficientFiles[ 0 ] = tudat::paths::getTudatTestDataPath( ) + "/dCDwTest.txt";
        std::map< int, std::string > controlSurfaceMomentCoefficientFiles;
        controlSurfaceMomentCoefficientFiles[ 1 ] = tudat::paths::getTudatTestDataPath( ) + "/dCDwTest.txt";
        ...
        std::shared_ptr< ephemerides::RotationalEphemeris > vehicleRotationModel = createRotationModel(
                std::make_shared< PitchTrimRotationSettings >( "Earth", "J2000", "VehicleFixed" ), "SpacePlane", bodies );
        ...
        addEngineModel( "SpacePlane",
                        "MainEngine",
                        std::make_shared< ParameterizedThrustMagnitudeSettings >(
                                thrustMagnitudeInterpolator, thrustDependencies, specificImpulseInterpolator, thrustDependencies ),
                        bodies );
        ...
        accelerationsOfSpacePlane[ "SpacePlane" ].push_back( std::make_shared< ThrustAccelerationSettings >( "MainEngine" ) );
        ...
        const Eigen::Vector6d systemInitialState = convertSphericalOrbitalToCartesianState( SpacePlaneInitialState );
        ...
        dependentVariables.push_back(
                std::make_shared< BodyAerodynamicAngleVariableSaveSettings >( "SpacePlane", reference_frames::angle_of_attack ) );
        dependentVariables.push_back(
                std::make_shared< BodyAerodynamicAngleVariableSaveSettings >( "SpacePlane", reference_frames::angle_of_sideslip ) );
        dependentVariables.push_back(
                std::make_shared< BodyAerodynamicAngleVariableSaveSettings >( "SpacePlane", reference_frames::bank_angle ) );
        if( i > 0 )
        {
        ...
        propagationTerminationSettingsList.push_back( std::make_shared< PropagationDependentVariableTerminationSettings >(
                std::make_shared< SingleDependentVariableSaveSettings >( altitude_dependent_variable, "SpacePlane", "Earth" ),
                120.0E3,
                false ) );
        propagationTerminationSettingsList.push_back( std::make_shared< PropagationDependentVariableTerminationSettings >(
                std::make_shared< SingleDependentVariableSaveSettings >( altitude_dependent_variable, "SpacePlane", "Earth" ),
                1.0E3,
                true ) );
        propagationTerminationSettingsList.push_back( std::make_shared< PropagationTimeTerminationSettings >( simulationEndEpoch ) );
        ...
        std::shared_ptr< interpolators::Interpolator< double, double > > cdInterpolator =
                std::make_shared< interpolators::MultiLinearInterpolator< double, double, 2 > >( cdMuliArray.second, cdMuliArray.first );
        ...
        std::shared_ptr< interpolators::Interpolator< double, double > > clInterpolator =
                std::make_shared< interpolators::MultiLinearInterpolator< double, double, 2 > >( clMuliArray.second, clMuliArray.first );
        ...
        std::shared_ptr< interpolators::Interpolator< double, double > > cmInterpolator =
                std::make_shared< interpolators::MultiLinearInterpolator< double, double, 2 > >( cmMuliArray.second, cmMuliArray.first );
        ...
        std::shared_ptr< interpolators::Interpolator< double, double > > controlSurfaceIncrementinterpolator =
                std::make_shared< interpolators::MultiLinearInterpolator< double, double, 3 > >( controlSurfaceIncrementMuliArray.second,
                                                                                                 controlSurfaceIncrementMuliArray.first );
             ...
             variableIterator++ )
        {
            ...
            apolloCoefficientInterface->updateFullCurrentCoefficients( aerodynamicCoefficientInput, controlSurfaceCoefficientInput );
            ...
            {
                BOOST_CHECK_SMALL( std::fabs( computedForceCoefficients( j ) - forceCoefficients( j ) ),
                                   6.0 * std::numeric_limits< double >::epsilon( ) );
                BOOST_CHECK_SMALL( std::fabs( computedMomentCoefficients( j ) - momentCoefficients( j ) ),
                                   6.0 * std::numeric_limits< double >::epsilon( ) );
            }
            ...
            apolloCoefficientInterface->updateFullCurrentCoefficients( perturbedAerodynamicCoefficientInput,
