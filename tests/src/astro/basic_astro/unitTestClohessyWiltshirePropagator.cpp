 ...
 *
 *    References
 *      Vallado, D.A. Fundamentals of astro and Applications website.
 *        URL http://www.smad.com/vallado, 2001. Last accessed: 25-07-2012.
 *      Wakker, K.F. astro I, reader of course AE4874 I. Delft University of Technology,
 *        2007.
 *
 */
...
BOOST_AUTO_TEST_CASE( test_ClohessyWiltshirePropagation_fullState )
{
    ...
    const Eigen::Vector6d initialState1 = ( Eigen::Vector6d( ) << 45.0, 37.0, 12.0, 0.08, 0.03, 0.01 ).finished( );

    // Calculate final state according to Tudat function.
    const Eigen::Vector6d computedFinalState1 = basic_astrodynamics::propagateClohessyWiltshire(
            initialState1, propagationDuration1, centralBodyGravitationalParameter1, referenceOrbitRadius1 );

    // Set final state according to the MATLAB routine "hillsr" from Vallado [2001].
    const Eigen::Vector6d expectedFinalState1 = ( Eigen::Vector6d( ) << 3.806450080201250e2,
                                                  -5.437424675454679e2,
                                                  2.509547637285142,
...
BOOST_AUTO_TEST_CASE( test_ClohessyWiltshirePropagation_harmonicMotion )
{
    ...
    const double meanAngularMotion =
            std::sqrt( centralBodyGravitationalParameter2 / ( referenceOrbitRadius2 * referenceOrbitRadius2 * referenceOrbitRadius2 ) );

    // Set propagation duration [s].
    // In this case: orbital period of the reference orbit.
    const double propagationDuration2 = 2.0 * mathematical_constants::PI / meanAngularMotion;
    ...
    const Eigen::Vector6d initialState2 =
            ( Eigen::Vector6d( ) << 34.0, 49.0, 17.0, 0.5 * meanAngularMotion * 49.0, -2.0 * meanAngularMotion * 34.0, 0.04 ).finished( );

    // Calculate final state according to Tudat function.
    const Eigen::Vector6d computedFinalState2 = basic_astrodynamics::propagateClohessyWiltshire(
            initialState2, propagationDuration2, centralBodyGravitationalParameter2, referenceOrbitRadius2 );
