...
void writeErrorsToFile( std::vector< double > eccentricities, std::vector< double > meanAnomalies, std::string testName )
{
    ...
    const std::string outputFileName = paths::getTudatTestDataPath( ) + "/ErrorReportConversionMeanToHyperbolicEccentricAnomaly" +
            testName + "RunAt" + boost::posix_time::to_iso_string( now ) + ".txt";
    ...
    errorFile << "This error report was generated because the unit test for the" << std::endl
              ...
              << "into it and the code can be improved." << std::endl
              << std::endl
...
BOOST_AUTO_TEST_CASE( test_convertMeanAnomalyToHyperbolicEccentricAnomaly_range )
{
    ...
    const double arrayOfExpectedHyperbolicAnomalies[ 6 ] = { 1.9132897042137,   2.60400218106873,  0.478057581067141,
                                                             -1.50971422579796, 0.529595060186511, 0.0253214157050963 };
    ...
    for( int counter = 0; counter < 6; counter++ )
    {
        ...
        const double hyperbolicEccentricAnomaly =
                convertMeanAnomalyToHyperbolicEccentricAnomaly( arrayOfTestEccentricities[ counter ], arrayOfTestMeanAnomalies[ counter ] );

        // Check if computed eccentric anomaly is less than error tolerance.
        BOOST_CHECK_CLOSE_FRACTION( arrayOfExpectedHyperbolicAnomalies[ counter ], hyperbolicEccentricAnomaly, 1.0E-14 );
    }
}
...
BOOST_AUTO_TEST_CASE( test_convertMeanAnomalyToHyperbolicEccentricAnomaly_TooLow )
{
    ...
    BOOST_CHECK_THROW( convertMeanAnomalyToHyperbolicEccentricAnomaly( testEccentricity, testMeanAnomaly ), std::runtime_error );
}
...
BOOST_AUTO_TEST_CASE( test_convertMeanAnomalyToHyperbolicEccentricAnomaly_nearParabolic )
{
    ...
    // the mean anomalies were converted to cartesian positions and compared to the same conversion
    // using Tudat methods. (GTOP also does not use true anomaly as explicit step)
    const double arrayOfExpectedHyperbolicAnomalies[ 4 ] = { -3.280887528670698, -1.913052492643601, 1.396250871565077, 3.062027761338891 };
    ...
    for( int counter = 0; counter < 4; counter++ )
    {
        // Compute the hyperbolic eccentric anomaly.
        const double hyperbolicEccentricAnomaly =
                convertMeanAnomalyToHyperbolicEccentricAnomaly( testEccentricity, arrayOfTestMeanAnomalies[ counter ] );
...
void testMeanToHyperbolicEccentricAnomalyConversions( const std::string& caseId,
                                                      ...
                                                      const int numberOfSamples = 1E5 )
{
    ...
    boost::random::uniform_real_distribution< ScalarType > meanAnomalyDistibution( -meanAnomalyLimit, meanAnomalyLimit );
    boost::variate_generator< boost::mt19937&, boost::random::uniform_real_distribution< ScalarType > > generateMeanAnomaly(
            randomNumbergenerator, meanAnomalyDistibution );
    ...
    for( int counter = 0; counter < numberOfSamples; counter++ )
    {
        ...
        {
            testMeanAnomaly = testMeanAnomaly * std::pow( getFloatingInteger< ScalarType >( 10 ), generateMeanAnomaly( ) );
        }
        ...
        if( !useConstantEccentricity )
        {
            ...
            {
                testEccentricity =
                        getFloatingInteger< ScalarType >( 1 ) + std::pow( getFloatingInteger< ScalarType >( 10 ), testEccentricity );
            }
        }
        ...
        try
        {
            ...
            eccentricAnomaly = convertMeanAnomalyToHyperbolicEccentricAnomaly< ScalarType >( testEccentricity, testMeanAnomaly );
        }
        ...
        reverseCalculatedMeanAnomaly = convertHyperbolicEccentricAnomalyToMeanAnomaly< ScalarType >( eccentricAnomaly, testEccentricity );
        ...
        {
            if( ( ( !( std::abs( testMeanAnomaly - reverseCalculatedMeanAnomaly ) / testMeanAnomaly < testTolerance ) ) &&
                  !( testMeanAnomaly == getFloatingInteger< ScalarType >( 0 ) ||
                     reverseCalculatedMeanAnomaly == getFloatingInteger< ScalarType >( 0 ) ) ) &&
                !aRuntimeErrorOccurred )
            {
...
BOOST_AUTO_TEST_CASE( test_convertMeanAnomalyToEccentricAnomaly_nearParabolic_random_double )
{
    ...
    testMeanToHyperbolicEccentricAnomalyConversions< long double >(
            "LongDoubleParabolic", 1.0E-13L * ratioOfPrecision, 20.0L, 1, 0, TUDAT_NAN, TUDAT_NAN, 1.0L + 1.0e-15L * ratioOfPrecision );

    // Test random conversions for random orbits
    testMeanToHyperbolicEccentricAnomalyConversions< double >( "DoubleTypical", 1.0E-13, 20.0, 0, 0, 1.0 + 1.0e-15, 10.0, TUDAT_NAN );
    testMeanToHyperbolicEccentricAnomalyConversions< long double >(
            "LongDoubleTypical", 1.0E-13L, 20.0L, 0, 0, 1.0L + 1.0e-15L * ratioOfPrecision, 10.0L, TUDAT_NAN );

    // Test random conversions for extreme values of eccentricity and mean anomaly.
    testMeanToHyperbolicEccentricAnomalyConversions< double >( "DoubleHighlyEccentric", 1.0E-14, 12.0, 0, 1, 0.0, 15.0, TUDAT_NAN );
    testMeanToHyperbolicEccentricAnomalyConversions< long double >(
...
BOOST_AUTO_TEST_CASE( test_convertMeanAnomalyToHyperbolicEccentricAnomaly_specificInitialGuess )
{
    ...
    // Compute eccentric anomaly.
    const double hyperbolicEccentricAnomaly =
            convertMeanAnomalyToHyperbolicEccentricAnomaly( testEccentricity, testHyperbolicMeanAnomaly, false, initialGuess );

    // Check if computed eccentric anomaly is NaN for invalid eccentricity.
    BOOST_CHECK_CLOSE_FRACTION( expectedHyperbolicEccentricAnomaly, hyperbolicEccentricAnomaly, 1.0E-14 );
}
