...
BOOST_AUTO_TEST_CASE( testPolyhedronUtilities )
{
    ...
    double computedArea = basic_astrodynamics::computePolyhedronSurfaceArea( verticesCoordinates, verticesDefiningEachFacet );
    BOOST_CHECK_CLOSE_FRACTION( expectedArea, computedArea, tolerance );

    // Computation of volume
    double expectedVolume = l * w * h;
    double computedVolume = basic_astrodynamics::computePolyhedronVolume( verticesCoordinates, verticesDefiningEachFacet );
    ...
    double computedRadius = basic_astrodynamics::computePolyhedronMeanRadius( verticesCoordinates, verticesDefiningEachFacet );
    ...
    Eigen::Vector3d computedCentroid =
            basic_astrodynamics::computePolyhedronCentroidPosition( verticesCoordinates, verticesDefiningEachFacet );
    ...
    Eigen::MatrixXd correctedVerticesCoordinates =
            basic_astrodynamics::modifyPolyhedronCentroidPosition( verticesCoordinates, verticesDefiningEachFacet, desiredCentroid );
    computedCentroid = basic_astrodynamics::computePolyhedronCentroidPosition( correctedVerticesCoordinates, verticesDefiningEachFacet );
    ...
    Eigen::Matrix3d expectedInertiaTensor = inertiaTensorWrtPrincipalAxes +
            mass *
                    ( Eigen::Matrix3d( ) << std::pow( expectedCentroid( 1 ), 2 ) + std::pow( expectedCentroid( 2 ), 2 ),
                      ...
                      -expectedCentroid( 0 ) * expectedCentroid( 2 ),
                      -expectedCentroid( 1 ) * expectedCentroid( 2 ),
                      std::pow( expectedCentroid( 0 ), 2 ) + std::pow( expectedCentroid( 1 ), 2 ) )
                            .finished( );

    Eigen::Matrix3d computedInertiaTensor =
            basic_astrodynamics::computePolyhedronInertiaTensor( verticesCoordinates, verticesDefiningEachFacet, density );
