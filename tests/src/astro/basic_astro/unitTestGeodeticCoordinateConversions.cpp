...
BOOST_AUTO_TEST_CASE( testGeodeticCoordinateConversions )
{
    ...
    // Test conversion to geodetic coordinates.
    {
        ...
        const Eigen::Vector3d calculatedGeodeticPosition =
                convertCartesianToGeodeticCoordinates( testCartesianPosition, equatorialRadius, flattening, 1.0E-4 );
    ...
    // Test separate functions for altitude and geodetic latitude.
    {
        ...
        const double directGeodeticLatitude = calculateGeodeticLatitude( testCartesianPosition, equatorialRadius, flattening, 1.0E-4 );
    ...
    // Test conversions from geodetic coordinates to cartesian position.
    {
        const Eigen::Vector3d calculateCartesianPosition =
                convertGeodeticToCartesianCoordinates( testGeodeticPosition, equatorialRadius, flattening );

        TUDAT_CHECK_MATRIX_CLOSE_FRACTION( calculateCartesianPosition, testCartesianPosition, 1.0E-9 );
    }
}
