 ...
 */
void writeErrorsToFile( std::vector< double > eccentricities, std::vector< double > meanAnomalies, std::string testName )
{
    ...
    const std::string outputFileName = paths::getTudatTestDataPath( ) + "/ErrorReportConversionMeanToEccentricAnomaly" + testName +
            "RunAt" + boost::posix_time::to_iso_string( now ) + ".txt";
    ...
    {
        errorFile << std::setw( 25 ) << eccentricities[ counter ] << std::setw( 25 ) << meanAnomalies[ counter ] << std::endl;
    }
    errorFile.close( );
    ...
    std::cerr << "One or multiple errors occurred during random sampling. " << std::endl
              << "The values leading to these errors have been written to the following file: " << std::endl
              << outputFileName;
}
...
BOOST_AUTO_TEST_CASE( test_convertMeanAnomalyToEccentricAnomaly_circular )
{
    ...
    const double eccentricAnomaly = convertMeanAnomalyToEccentricAnomaly( testEccentricity, testMeanAnomaly );

    // Check if computed eccentric anomaly is less than error tolerance.
    BOOST_CHECK_CLOSE_FRACTION( eccentricAnomaly, referenceEccentricAnomaly, 1.0E-13 );
}
...
BOOST_AUTO_TEST_CASE( test_convertMeanAnomalyToEccentricAnomaly_range )
{
    ...
    const double arrayOfTestMeanAnomalies[ 4 ] = {
        convertDegreesToRadians( 60.0 ), convertDegreesToRadians( 90.0 ), convertDegreesToRadians( 120.0 ), convertDegreesToRadians( 220.0 )
    };

    // Set reference values for eccentric anomaly. These were obtained using GTOP.
    const double arrayOfReferenceEccentricAnomalies[ 4 ] = { 1.06178920406832, 1.97200731113253, 2.5392410896466, 3.51006218528448 };
    ...
    for( int i = 0; i < 4; i++ )
    {
        ...
        const double eccentricAnomaly =
                convertMeanAnomalyToEccentricAnomaly( arrayOfTestEccentricities[ i ], arrayOfTestMeanAnomalies[ i ] );

        // Check if computed eccentric anomaly is less than error tolerance.
        BOOST_CHECK_CLOSE_FRACTION( eccentricAnomaly, arrayOfReferenceEccentricAnomalies[ i ], 1.0E-13 );
    }
}
...
BOOST_AUTO_TEST_CASE( test_convertMeanAnomalyToEccentricAnomaly_negative )
{
    ...
    BOOST_CHECK_THROW( convertMeanAnomalyToEccentricAnomaly( testEccentricity, testMeanAnomaly ), std::runtime_error );
}
...
BOOST_AUTO_TEST_CASE( test_convertMeanAnomalyToEccentricAnomaly_nearParabolic )
{
    ...
    // values.
    const double arrayOfTestMeanAnomalies[ 17 ] = {
        0.0,     1.0e-10,      0.5, PI / 2 - 1.0e-10,         PI / 2,         PI / 2 + 1.0e-10,         2.5, PI - 1.0e-10,
        PI,      PI + 1.0e-10, 4.0, 3.0 / 2.0 * PI - 1.0e-10, 3.0 / 2.0 * PI, 3.0 / 2.0 * PI + 1.0e-10, 5.5, 2.0 * PI - 1.0e-10,
        2.0 * PI
    };
    ...
    double eccentricAnomaly = convertMeanAnomalyToEccentricAnomaly( testEccentricity, arrayOfTestMeanAnomalies[ 0 ] );
    BOOST_CHECK_SMALL( eccentricAnomaly, 10.0 * std::numeric_limits< double >::epsilon( ) );
    eccentricAnomaly = convertMeanAnomalyToEccentricAnomaly( testEccentricity, arrayOfTestMeanAnomalies[ 16 ] );
    BOOST_CHECK_SMALL( eccentricAnomaly, 10.0 * std::numeric_limits< double >::epsilon( ) );
    ...
    {
        // Compute eccentric anomaly.
        eccentricAnomaly = convertMeanAnomalyToEccentricAnomaly( testEccentricity, arrayOfTestMeanAnomalies[ counter ] );

        // Check whether the expected value is set to the eccentric anomaly.
        BOOST_CHECK_CLOSE_FRACTION( eccentricAnomaly, arrayOfExpectedEccentricAnomalies[ counter ], 1.0E-9 );
    }
}
...
void testMeanToEccentricAnomalyConversions( const std::string& caseId,
                                            ...
                                            const int numberOfSamples = 1E5 )
{
    ...
    {
        eccentricityDistribution = boost::random::uniform_real_distribution<>( minimumEccentricity, maximumEccentricity );
    }

    boost::variate_generator< boost::mt19937&, boost::random::uniform_real_distribution<> > eccentricityGenerator(
    ...
    boost::random::uniform_real_distribution< ScalarType > distribution0To2Pi(
            getFloatingInteger< ScalarType >( 0 ), getFloatingInteger< ScalarType >( 2 ) * getPi< ScalarType >( ) );
    boost::variate_generator< boost::mt19937&, boost::random::uniform_real_distribution< ScalarType > > generateRandomNumber0To2Pi(
            randomNumbergenerator, distribution0To2Pi );
    ...
    for( int counter = 0; counter < numberOfSamples; counter++ )
    {
        ...
        {
            // Compute eccentric anomaly.
            eccentricAnomaly = convertMeanAnomalyToEccentricAnomaly< ScalarType >( testEccentricity, testMeanAnomaly );
        }
        ...
        reverseCalculatedMeanAnomaly = convertEccentricAnomalyToMeanAnomaly< ScalarType >( eccentricAnomaly, testEccentricity );
...
{
    long double ratioOfPrecision = std::numeric_limits< long double >::epsilon( ) / std::numeric_limits< double >::epsilon( );

    // Test random conversions for near-parabolic orbits
    testMeanToEccentricAnomalyConversions< double >( "DoubleParabolic", 1.0E-13, 1, TUDAT_NAN, TUDAT_NAN, 1.0 - 1.0e-15 );
    testMeanToEccentricAnomalyConversions< long double >(
            "LongDoubleParabolic", 1.0E-13L * ratioOfPrecision, 1, TUDAT_NAN, TUDAT_NAN, 1.0L - 1.0e-15L * ratioOfPrecision );

    // Test random conversions for random orbits
    testMeanToEccentricAnomalyConversions< double >( "DoubleEccentric", 1.0E-15, 0, 0.0, 1.0 - 1.0e-11 );
    testMeanToEccentricAnomalyConversions< long double >(
            "LongDoubleEccentric", 1.0E-15L * ratioOfPrecision, 0, 0.0, 1.0L - 1.0e-11L * ratioOfPrecision );
}
...
BOOST_AUTO_TEST_CASE( test_convertMeanAnomalyToEccentricAnomaly_specificInitialGuess )
{
    ...
    // Compute eccentric anomaly.
    const double eccentricAnomaly =
            convertMeanAnomalyToEccentricAnomaly( testEccentricity, testMeanAnomaly, useDefaultGuess, initialGuess );

    // Check if computed eccentric anomaly is less than error tolerance.
    BOOST_CHECK_CLOSE( eccentricAnomaly, referenceEccentricAnomaly, 1.0E-13 );
}
