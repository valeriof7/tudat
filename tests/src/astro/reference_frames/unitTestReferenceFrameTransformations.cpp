...
BOOST_AUTO_TEST_CASE( testRotatingPlanetocentricFrameTransformations )
{
    // Test 1: Test Inertial to rotating planetocentric frame transformation.
    {
        ...
        double horizontalStartLocationSize = sqrt( std::pow( startLocation( 0 ), 2.0 ) + std::pow( startLocation( 1 ), 2.0 ) );
        ...
        transformedLocation = reference_frames::getInertialToPlanetocentricFrameTransformationMatrix( angleInTime ) * startLocation;

        // Check whether both vectors are equal within tolerances.
        TUDAT_CHECK_MATRIX_CLOSE_FRACTION( transformedLocation, expectedLocation, std::numeric_limits< double >::epsilon( ) );
    }
    ...
    // Compute the error in the calculation.
    {
        ...
        Eigen::Vector3d transformedLocation;
        transformedLocation = reference_frames::getRotatingPlanetocentricToInertialFrameTransformationMatrix( angleInTime ) *
                reference_frames::getInertialToPlanetocentricFrameTransformationMatrix( angleInTime ) * startLocation;

        // Check whether both vectors are equal within tolerances.
        TUDAT_CHECK_MATRIX_CLOSE_FRACTION( transformedLocation, startLocation, std::numeric_limits< double >::epsilon( ) );
    }
}
...
BOOST_AUTO_TEST_CASE( testRotatingPlanetocentricFrameTransformationQuaternion )
{
    ...
    // Compute location of the point in the Rotating frame subject to the transformation matrix.
    {
        ...
        // Compute location of the point in the rotating frame subject to the transformation matrix.
        Eigen::Vector3d transformedLocation;
        transformedLocation = reference_frames::getInertialToPlanetocentricFrameTransformationQuaternion( angleInTime ) * startLocation;
    ...
    // Compute the error in the calculation.
    {
        ...
        // Compute location of the point in the rotating frame subject to the transformation matrix.
        Eigen::Vector3d transformedLocation;
        transformedLocation = reference_frames::getRotatingPlanetocentricToInertialFrameTransformationQuaternion( angleInTime ) *
                reference_frames::getInertialToPlanetocentricFrameTransformationQuaternion( angleInTime ) * startLocation;
...
BOOST_AUTO_TEST_CASE( testAirspeedBasedAerodynamicToBodyFrameTransformation )
{
    ...
    // Declare and initialize the start location and angles.
    {
        ...
        Eigen::Vector3d transformedLocation;
        transformedLocation =
                reference_frames::getAirspeedBasedAerodynamicToBodyFrameTransformationMatrix( angleOfAttack, angleOfSideslip ) *
                startLocation;
...
BOOST_AUTO_TEST_CASE( testRotatingPlanetocentricToLocalVerticalFrameTransformationQuaternion )
{
    // Test 6: Test Rotating planetocentric to local vertical frame transformation quaternion.
    {
        ...
        expectedLocation( 0 ) = -std::cos( longitude ) * std::sin( latitude ) * startLocation( 0 ) -
                std::sin( longitude ) * std::sin( latitude ) * startLocation( 1 ) + std::cos( latitude ) * startLocation( 2 );
        expectedLocation( 1 ) = -std::sin( longitude ) * startLocation( 0 ) + std::cos( longitude ) * startLocation( 1 ) + 0.0;
        expectedLocation( 2 ) = -std::cos( longitude ) * std::cos( latitude ) * startLocation( 0 ) -
                std::sin( longitude ) * std::cos( latitude ) * startLocation( 1 ) - std::sin( latitude ) * startLocation( 2 );

        // Compute location of the point in the rotating frame subject to the transformation matrix.
        Eigen::Vector3d transformedLocation;
        transformedLocation =
                reference_frames::getRotatingPlanetocentricToLocalVerticalFrameTransformationQuaternion( longitude, latitude ) *
                startLocation;
...
BOOST_AUTO_TEST_CASE( testRotatingPlanetocentricWithEquatorChangeFrameTransformations )
{
    ...
    spiceInitialRotationToTargetFrameMatrix << -0.9548214974296336, 0.2665104385944917, 0.1314841974018291, -0.296591573568662,
            -0.882413772579987, -0.3652114078848295, 0.01869081416890202, -0.3877088083617989, 0.9215923900425705;

    // Calculate given rotation to Venus-fixed frame.
    Eigen::Matrix3d calculatedTransformationMatrix = Eigen::Matrix3d( getInertialToPlanetocentricFrameTransformationQuaternion(
            venusPoleDeclination, venusPoleRightAscension, venusPrimeMeridianAtJ2000 ) );

    // Compare calculated with Spice result.
    TUDAT_CHECK_MATRIX_CLOSE_FRACTION( spiceInitialRotationToTargetFrameMatrix, calculatedTransformationMatrix, 1.0E-15 );

    // Calculate inverse rotation.
    calculatedTransformationMatrix = Eigen::Matrix3d( getRotatingPlanetocentricToInertialFrameTransformationQuaternion(
            venusPoleDeclination, venusPoleRightAscension, venusPrimeMeridianAtJ2000 ) );
    ...
    Eigen::Matrix3d matrixToCheck = spiceInitialRotationToTargetFrameMatrix * calculatedTransformationMatrix;
...
BOOST_AUTO_TEST_CASE( testTrajectoryToLocalVerticalFrameTransformations )
{
    // Test 8: Test trajectory to local vertical frame and inverse transformations.
    {
        ...
        for( double headingAngle = convertDegreesToRadians( 0.0 ); headingAngle < convertDegreesToRadians( 360.0 );
             headingAngle += convertDegreesToRadians( 30.0 ) )
        {
            for( double flightPathAngle = convertDegreesToRadians( -90.0 ); flightPathAngle < convertDegreesToRadians( 90.0 );
                 flightPathAngle += convertDegreesToRadians( 30.0 ) )
            {
                // Declare the expected location of the point in the V-frame (per Mooij 1997).
                Eigen::Vector3d expectedLocation;
                Eigen::Matrix3d rotation;
                rotation << std::cos( headingAngle ) * std::cos( flightPathAngle ), -std::sin( headingAngle ),
                        std::cos( headingAngle ) * std::sin( flightPathAngle ), std::sin( headingAngle ) * std::cos( flightPathAngle ),
                        std::cos( headingAngle ), std::sin( headingAngle ) * std::sin( flightPathAngle ), -std::sin( flightPathAngle ), 0.0,
                ...
                Eigen::Vector3d transformedLocationQuat =
                        reference_frames::getTrajectoryToLocalVerticalFrameTransformationQuaternion( flightPathAngle, headingAngle ) *
                        startLocation;
                Eigen::Vector3d transformedLocationMat =
                        reference_frames::getTrajectoryToLocalVerticalFrameTransformationMatrix( flightPathAngle, headingAngle ) *
                        startLocation;

                // Compute product of inverse rotations.
                Eigen::Vector3d inverseRotationProductMat =
                        reference_frames::getTrajectoryToLocalVerticalFrameTransformationMatrix( flightPathAngle, headingAngle ) *
                        reference_frames::getLocalVerticalFrameToTrajectoryTransformationMatrix( flightPathAngle, headingAngle ) *
                        startLocation;
                ...
                TUDAT_CHECK_MATRIX_CLOSE_FRACTION( transformedLocationQuat, expectedLocation, 1.0e-14 );
                TUDAT_CHECK_MATRIX_CLOSE_FRACTION( transformedLocationMat, expectedLocation, 2.0e-14 );
                ...
                TUDAT_CHECK_MATRIX_CLOSE_FRACTION( inverseRotationProductQuat, startLocation, 1.0e-14 );
            }
        }
    }
}
...
BOOST_AUTO_TEST_CASE( testTrajectoryToAerodynamicFrameTransformations )
{
    // Test 9: Test trajectory to aerodynamic and inverse transformations.
    {
        ...
        // Declare rotation angles.
        for( double bankAngle = convertDegreesToRadians( 0.0 ); bankAngle < convertDegreesToRadians( 360.0 );
             bankAngle += convertDegreesToRadians( 30.0 ) )
        {
            ...
            Eigen::Vector3d expectedLocation;
            Eigen::Matrix3d rotation;
            rotation << 1.0, 0.0, 0.0, 0.0, std::cos( bankAngle ), -std::sin( bankAngle ), 0.0, std::sin( bankAngle ),
                    std::cos( bankAngle );
            expectedLocation = rotation * startLocation;
            ...
            Eigen::Vector3d transformedLocationQuat =
                    reference_frames::getTrajectoryToAerodynamicFrameTransformationQuaternion( bankAngle ) * startLocation;
            Eigen::Vector3d transformedLocationMat =
                    reference_frames::getTrajectoryToAerodynamicFrameTransformationMatrix( bankAngle ) * startLocation;

            // Compute product of inverse rotations.
            Eigen::Vector3d inverseRotationProductMat = reference_frames::getAerodynamicToTrajectoryFrameTransformationMatrix( bankAngle ) *
                    reference_frames::getTrajectoryToAerodynamicFrameTransformationMatrix( bankAngle ) * startLocation;
            Eigen::Vector3d inverseRotationProductQuat =
                    reference_frames::getAerodynamicToTrajectoryFrameTransformationQuaternion( bankAngle ) *
                    reference_frames::getTrajectoryToAerodynamicFrameTransformationQuaternion( bankAngle ) * startLocation;
...
BOOST_AUTO_TEST_CASE( testAerodynamicToBodyFrameTransformations )
{
    // Test 10: Test body to airspeed-based aerodynamic frame and inverse transformations.
    {
        ...
        // Declare rotation angles.
        for( double angleOfAttack = convertDegreesToRadians( -180.0 ); angleOfAttack < convertDegreesToRadians( 180.0 );
             angleOfAttack += convertDegreesToRadians( 30.0 ) )
        {
            for( double angleOfSideslip = convertDegreesToRadians( -90.0 ); angleOfSideslip < convertDegreesToRadians( 90.0 );
                 angleOfSideslip += convertDegreesToRadians( 30.0 ) )
            {
                ...
                Eigen::Vector3d expectedLocation;
                Eigen::Matrix3d rotation;
                rotation << std::cos( angleOfAttack ) * std::cos( angleOfSideslip ), std::sin( angleOfSideslip ),
                        std::sin( angleOfAttack ) * std::cos( angleOfSideslip ), -std::cos( angleOfAttack ) * std::sin( angleOfSideslip ),
                        std::cos( angleOfSideslip ), -std::sin( angleOfAttack ) * std::sin( angleOfSideslip ), -std::sin( angleOfAttack ),
                        0.0, std::cos( angleOfAttack );
                expectedLocation = rotation * startLocation;

                // Compute location of the point in the AA-frame using the tested function.
                Eigen::Vector3d transformedLocationQuat =
                        reference_frames::getBodyToAirspeedBasedAerodynamicFrameTransformationQuaternion( angleOfAttack, angleOfSideslip ) *
                        startLocation;
                Eigen::Vector3d transformedLocationMat =
                        reference_frames::getBodyToAirspeedBasedAerodynamicFrameTransformationMatrix( angleOfAttack, angleOfSideslip ) *
                        startLocation;
                ...
                Eigen::Vector3d inverseRotationProductMat =
                        ...
                        startLocation;
                Eigen::Vector3d inverseRotationProductQuat =
                        reference_frames::getAirspeedBasedAerodynamicToBodyFrameTransformationQuaternion( angleOfAttack, angleOfSideslip ) *
                        reference_frames::getBodyToAirspeedBasedAerodynamicFrameTransformationQuaternion( angleOfAttack, angleOfSideslip ) *
                        startLocation;
...
void testIsRotationMatrixRightHanded( const Eigen::Matrix3d matrixToTest )
{
    Eigen::Vector3d expectedUnitVectorZ =
            ( Eigen::Vector3d( matrixToTest.block( 0, 0, 3, 1 ) ) ).cross( Eigen::Vector3d( matrixToTest.block( 0, 1, 3, 1 ) ) );
    Eigen::Vector3d expectedUnitVectorX =
            ( Eigen::Vector3d( matrixToTest.block( 0, 1, 3, 1 ) ) ).cross( Eigen::Vector3d( matrixToTest.block( 0, 2, 3, 1 ) ) );
    Eigen::Vector3d expectedUnitVectorY =
            ( Eigen::Vector3d( matrixToTest.block( 0, 2, 3, 1 ) ) ).cross( Eigen::Vector3d( matrixToTest.block( 0, 0, 3, 1 ) ) );

    for( unsigned int i = 0; i < 3; i++ )
    {
        BOOST_CHECK_CLOSE_FRACTION( expectedUnitVectorX( i ), matrixToTest( i, 0 ), 2.0 * std::numeric_limits< double >::epsilon( ) );
        BOOST_CHECK_CLOSE_FRACTION( expectedUnitVectorY( i ), matrixToTest( i, 1 ), 2.0 * std::numeric_limits< double >::epsilon( ) );
        BOOST_CHECK_CLOSE_FRACTION( expectedUnitVectorZ( i ), matrixToTest( i, 2 ), 2.0 * std::numeric_limits< double >::epsilon( ) );
    }
}
...
BOOST_AUTO_TEST_CASE( testVelocityBasedtnwFrameTransformations )
{
    ...
    // test all expected properties of rotation matrices
    {
        ...
        Eigen::Matrix3d nAxisAwayFromBodyMatrix =
                reference_frames::getTnwToInertialRotation( vehicleStateCartesian, centralBodyStateCartesian, true );
        Eigen::Matrix3d nAxisAwayTowardsBodyMatrix =
                reference_frames::getTnwToInertialRotation( vehicleStateCartesian, centralBodyStateCartesian, false );
        ...
        // Test if central body is properly processed.
        {
            ...
            for( unsigned int i = 0; i < 3; i++ )
            {
                for( unsigned int j = 0; j < 3; j++ )
                {
                    BOOST_CHECK_CLOSE_FRACTION( nAxisAwayFromBodyMatrixCentral( i, j ),
                                                nAxisAwayFromBodyMatrix( i, j ),
                                                2.0 * std::numeric_limits< double >::epsilon( ) );
                    BOOST_CHECK_CLOSE_FRACTION( nAxisAwayTowardsBodyMatrix( i, j ),
                                                nAxisAwayTowardsBodyMatrixCentral( i, j ),
                                                2.0 * std::numeric_limits< double >::epsilon( ) );
                }
            }
        }
        ...
        double positionDotProductWithNAwayFromBody =
                Eigen::Vector3d( nAxisAwayFromBodyMatrix.block( 0, 1, 3, 1 ) ).dot( relativeState.segment( 0, 3 ).normalized( ) );
        ...
        double positionDotProuctWithNTowardsBody =
                Eigen::Vector3d( nAxisAwayTowardsBodyMatrix.block( 0, 1, 3, 1 ) ).dot( relativeState.segment( 0, 3 ).normalized( ) );
        BOOST_CHECK_EQUAL( positionDotProuctWithNTowardsBody < 0, 1 );

        // Check if axes point exactly in opposite directions
        BOOST_CHECK_CLOSE_FRACTION(
                positionDotProductWithNAwayFromBody, -positionDotProuctWithNTowardsBody, std::numeric_limits< double >::epsilon( ) );
        ...
        for( unsigned int i = 0; i < 3; i++ )
        {
            for( unsigned int j = 0; j < 3; j++ )
            {
                BOOST_CHECK_CLOSE_FRACTION( nAxisAwayFromBodyMatrix( i, j ),
                                            ( j == 0 ? 1.0 : -1.0 ) * nAxisAwayTowardsBodyMatrix( i, j ),
                                            2.0 * std::numeric_limits< double >::epsilon( ) );
            }
        }
        ...
        Eigen::Vector3d inertialThrustVectorAlongVelocity = thrustScale * relativeState.segment( 3, 3 );
        {
            ...
            tnwFrameThrust1 = nAxisAwayFromBodyMatrix.transpose( ) * inertialThrustVectorAlongVelocity;
            tnwFrameThrust2 = nAxisAwayTowardsBodyMatrix.transpose( ) * inertialThrustVectorAlongVelocity;
            ...
            BOOST_CHECK_CLOSE_FRACTION( tnwFrameThrust1( 0 ), tnwFrameThrust2( 0 ), std::numeric_limits< double >::epsilon( ) );

            // Test thrust vector norms.
            BOOST_CHECK_CLOSE_FRACTION(
                    thrustMagnitudeAlongVelocityVector, tnwFrameThrust1.norm( ), 5.0 * std::numeric_limits< double >::epsilon( ) );
            BOOST_CHECK_CLOSE_FRACTION(
                    thrustMagnitudeAlongVelocityVector, tnwFrameThrust2.norm( ), 5.0 * std::numeric_limits< double >::epsilon( ) );

            // Test if entry 0 magnitude is equal to norm
            BOOST_CHECK_CLOSE_FRACTION(
                    std::fabs( tnwFrameThrust1( 0 ) ), thrustMagnitudeAlongVelocityVector, std::numeric_limits< double >::epsilon( ) );
            BOOST_CHECK_CLOSE_FRACTION(
                    std::fabs( tnwFrameThrust2( 0 ) ), thrustMagnitudeAlongVelocityVector, std::numeric_limits< double >::epsilon( ) );

            // Test if other entries are 0 (to numerical precision).
            BOOST_CHECK_SMALL( tnwFrameThrust1( 1 ), thrustMagnitudeAlongVelocityVector * std::numeric_limits< double >::epsilon( ) );
            BOOST_CHECK_SMALL( tnwFrameThrust2( 1 ), thrustMagnitudeAlongVelocityVector * std::numeric_limits< double >::epsilon( ) );
            BOOST_CHECK_SMALL( tnwFrameThrust1( 2 ), thrustMagnitudeAlongVelocityVector * std::numeric_limits< double >::epsilon( ) );
            BOOST_CHECK_SMALL( tnwFrameThrust2( 2 ), thrustMagnitudeAlongVelocityVector * std::numeric_limits< double >::epsilon( ) );
        }

        // Define semi-random thrust vector perpendicular to plane
        Eigen::Vector3d inertialThrustPerpendicularToPlane = thrustScale *
                ( Eigen::Vector3d( relativeState.segment( 0, 3 ) ).cross( Eigen::Vector3d( relativeState.segment( 3, 3 ) ) ) );
        {
            // Compute local thrust vectors
            tnwFrameThrust1 = nAxisAwayFromBodyMatrix.transpose( ) * inertialThrustPerpendicularToPlane;
            tnwFrameThrust2 = nAxisAwayTowardsBodyMatrix.transpose( ) * inertialThrustPerpendicularToPlane;
            ...
            BOOST_CHECK_CLOSE_FRACTION( tnwFrameThrust1( 2 ), -tnwFrameThrust2( 2 ), std::numeric_limits< double >::epsilon( ) );

            // Test thrust vector norms.
            BOOST_CHECK_CLOSE_FRACTION(
                    thrustMagnitudePerpendicularToPlane, tnwFrameThrust1.norm( ), 5.0 * std::numeric_limits< double >::epsilon( ) );
            BOOST_CHECK_CLOSE_FRACTION(
                    thrustMagnitudePerpendicularToPlane, tnwFrameThrust2.norm( ), 5.0 * std::numeric_limits< double >::epsilon( ) );

            // Test if entry 2 magnitude is equal to norm
            BOOST_CHECK_CLOSE_FRACTION(
                    std::fabs( tnwFrameThrust1( 2 ) ), thrustMagnitudePerpendicularToPlane, std::numeric_limits< double >::epsilon( ) );
            BOOST_CHECK_CLOSE_FRACTION(
                    std::fabs( tnwFrameThrust2( 2 ) ), thrustMagnitudePerpendicularToPlane, std::numeric_limits< double >::epsilon( ) );

            // Test if other entries are 0 (to numerical precision).
            BOOST_CHECK_SMALL( tnwFrameThrust1( 0 ), thrustMagnitudePerpendicularToPlane * std::numeric_limits< double >::epsilon( ) );
            BOOST_CHECK_SMALL( tnwFrameThrust2( 0 ), thrustMagnitudePerpendicularToPlane * std::numeric_limits< double >::epsilon( ) );
            BOOST_CHECK_SMALL( tnwFrameThrust1( 1 ), thrustMagnitudePerpendicularToPlane * std::numeric_limits< double >::epsilon( ) );
            BOOST_CHECK_SMALL( tnwFrameThrust2( 1 ), thrustMagnitudePerpendicularToPlane * std::numeric_limits< double >::epsilon( ) );
        }

        // Define semi-random thrust vector along local y-axis
        Eigen::Vector3d inertialThrustAlongLocalYAxis = inertialThrustPerpendicularToPlane.cross( inertialThrustVectorAlongVelocity );
        {
            // Compute local thrust vectors
            tnwFrameThrust1 = nAxisAwayFromBodyMatrix.transpose( ) * inertialThrustAlongLocalYAxis;
            tnwFrameThrust2 = nAxisAwayTowardsBodyMatrix.transpose( ) * inertialThrustAlongLocalYAxis;

            // Compute thrust vector magnitudes
            double thrustMagnitudeAlongYAxis = inertialThrustAlongLocalYAxis.norm( );

            // Test if entry 1 of two vectors are the same
            BOOST_CHECK_CLOSE_FRACTION( tnwFrameThrust1( 1 ), -tnwFrameThrust2( 1 ), std::numeric_limits< double >::epsilon( ) );

            // Test thrust vector norms.
            BOOST_CHECK_CLOSE_FRACTION(
                    thrustMagnitudeAlongYAxis, tnwFrameThrust1.norm( ), 5.0 * std::numeric_limits< double >::epsilon( ) );
            BOOST_CHECK_CLOSE_FRACTION(
                    thrustMagnitudeAlongYAxis, tnwFrameThrust2.norm( ), 5.0 * std::numeric_limits< double >::epsilon( ) );

            // Test if entry 1 magnitude is equal to norm
            BOOST_CHECK_CLOSE_FRACTION(
                    std::fabs( tnwFrameThrust1( 1 ) ), thrustMagnitudeAlongYAxis, std::numeric_limits< double >::epsilon( ) );
            BOOST_CHECK_CLOSE_FRACTION(
                    std::fabs( tnwFrameThrust2( 1 ) ), thrustMagnitudeAlongYAxis, std::numeric_limits< double >::epsilon( ) );
    ...
    // Test 11: Test velocity based tnw frame to inertial (I) frame transformation.
    {
        ...
        transformedThrustVector = reference_frames::getTnwToInertialRotation(
                                          vehicleStateCartesian, centralBodyStateCartesian, doesNaxisPointAwayFromCentralBody ) *
                startThrustVector;
    ...
    // Test 12: Test velocity based tnw frame to planetocentric frame transformation with Keplerian elements.
    {
        ...
        vehicleStateKeplerian << 1.0, 0.5, convertDegreesToRadians( 60.0 ), convertDegreesToRadians( 180.0 ),
                convertDegreesToRadians( 15.0 ), convertDegreesToRadians( 90.0 );
        ...
        Eigen::Matrix3d rotationMatrixFromKeplerElements =
                reference_frames::getTnwToPlanetocentricRotationKeplerian( vehicleStateKeplerian ).toRotationMatrix( );
        Eigen::Vector3d transformedThrustVector;
        transformedThrustVector = rotationMatrixFromKeplerElements * startThrustVector;

        // Check whether both vectors are equal within tolerances.
        TUDAT_CHECK_MATRIX_CLOSE_FRACTION(
                expectedThrustVector, transformedThrustVector, ( 10.0 * std::numeric_limits< double >::epsilon( ) ) );

        Eigen::Vector6d vehicleStateCartesian =
                orbital_element_conversions::convertKeplerianToCartesianElements( vehicleStateKeplerian, 3.986E14 );

        Eigen::Matrix3d rotationMatrixFromCartesianElements =
                reference_frames::getTnwToInertialRotation( vehicleStateCartesian, Eigen::Vector6d::Zero( ), false );

        for( unsigned int i = 0; i < 3; i++ )
        {
            for( unsigned int j = 0; j < 3; j++ )
            {
                BOOST_CHECK_SMALL( std::fabs( rotationMatrixFromCartesianElements( i, j ) - rotationMatrixFromKeplerElements( i, j ) ),
...
BOOST_AUTO_TEST_CASE( testAutomaticConversionFunctions )
{
    ...
    Eigen::Matrix3d manualInertialToRsw = getInertialToRswSatelliteCenteredFrameRotationMatrix( inertialCartesianState );
    Eigen::Matrix3d automaticInertialToRsw =
            getRotationBetweenSatelliteFrames( inertialCartesianState, global_reference_frame, rsw_reference_frame );
    TUDAT_CHECK_MATRIX_CLOSE_FRACTION( manualInertialToRsw, automaticInertialToRsw, std::numeric_limits< double >::epsilon( ) );

    Eigen::Matrix3d manualRswToInertial = getRswSatelliteCenteredToInertialFrameRotationMatrix( inertialCartesianState );
    Eigen::Matrix3d automaticRswToInertial =
            getRotationBetweenSatelliteFrames( inertialCartesianState, rsw_reference_frame, global_reference_frame );
    TUDAT_CHECK_MATRIX_CLOSE_FRACTION( manualRswToInertial, automaticRswToInertial, std::numeric_limits< double >::epsilon( ) );

    Eigen::Matrix3d manualInertialToTnw = getInertialToTnwRotation( inertialCartesianState );
    Eigen::Matrix3d automaticInertialToTnw =
            getRotationBetweenSatelliteFrames( inertialCartesianState, global_reference_frame, tnw_reference_frame );
    TUDAT_CHECK_MATRIX_CLOSE_FRACTION( manualInertialToTnw, automaticInertialToTnw, std::numeric_limits< double >::epsilon( ) );

    Eigen::Matrix3d manualTnwToInertial = getTnwToInertialRotation( inertialCartesianState );
    Eigen::Matrix3d automaticTnwToInertial =
            getRotationBetweenSatelliteFrames( inertialCartesianState, tnw_reference_frame, global_reference_frame );
    TUDAT_CHECK_MATRIX_CLOSE_FRACTION( manualTnwToInertial, automaticTnwToInertial, std::numeric_limits< double >::epsilon( ) );

    Eigen::Matrix3d automaticTnwToRsw =
            getRotationBetweenSatelliteFrames( inertialCartesianState, tnw_reference_frame, rsw_reference_frame );
    Eigen::Matrix3d automaticRswToTnw =
            getRotationBetweenSatelliteFrames( inertialCartesianState, rsw_reference_frame, tnw_reference_frame );
