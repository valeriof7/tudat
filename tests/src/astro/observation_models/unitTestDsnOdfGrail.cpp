...
int main( )
{
    ...
    bodies.at( "GRAIL-A" )
            ->getVehicleSystems( )
            ->setReferencePointPosition( "Antenna", ( Eigen::Vector3d( ) << -0.082, 0.152, -0.810 ).finished( ) );
    ...
    *****************************************************************************************/
...

    {
        ...
        input_output::writeMatrixToFile(
                residuals, "grailTestResiduals.dat", 16, "/home/mfayolle/Tudat/Data/GRAIL_TestResults_time_bias2/" );

        Eigen::VectorXd observationTimes =
                utilities::convertStlVectorToEigenVector( filteredObservedObservationCollection->getConcatenatedTimeVector( ) )
                        .template cast< double >( );
        input_output::writeMatrixToFile(
                observationTimes, "grailTestTimes.dat", 16, "/home/mfayolle/Tudat/Data/GRAIL_TestResults_time_bias2/" );

        Eigen::VectorXd observationLinkEndsIds =
                utilities::convertStlVectorToEigenVector( filteredObservedObservationCollection->getConcatenatedLinkEndIds( ) )
                        .template cast< double >( );
        input_output::writeMatrixToFile(
                observationLinkEndsIds, "grailTestLinkEnds.dat", 16, "/home/mfayolle/Tudat/Data/GRAIL_TestResults_time_bias2/" );
    }

    {
        ...
        parameterNames.push_back(
                std::make_shared< EmpiricalAccelerationEstimatableParameterSettings >( "GRAIL-A", "Moon", empiricalComponentsToEstimate ) );
        ...
        //                ); std::cout<<"C"<<std::endl; std::vector< double > arcTimes; arcTimes.push_back( it2.second.at( 0 ).first -
        //                3600.0 ); std::cout<<"D"<<std::endl; for( unsigned int timeIndex = 1; timeIndex < it2.second.size( ); timeIndex++
        //                )
        //                {
        ...
        for( auto it = linkEndsPerObservable.begin( ); it != linkEndsPerObservable.end( ); ++it )
        {
            for( unsigned int i = 0; i < it->second.size( ); ++i )
            {
                if( it->first == observation_models::dsn_n_way_averaged_doppler )
                {
                    ...
                    realObservationModelSettingsList.push_back( observation_models::dsnNWayAveragedDopplerObservationSettings(
                            it->second.at( i ),
                            lightTimeCorrectionSettings,
                            nullptr,  // arcWiseTimeBias( biasArcTimes, receiver ),
                            std::make_shared< LightTimeConvergenceCriteria >( true ) ) );
                }
            }
        }
        ...
        estimationInput->defineEstimationSettings( 0, 0, 0, 1, 1, 1 );
        ...
        input_output::writeMatrixToFile(
                estimationOutput->residuals_, "grailPostFitResiduals.dat", 16, "/home/mfayolle/Tudat/Data/GRAIL_TestResults_time_bias2/" );

        auto estimatedStateHistory = std::dynamic_pointer_cast< SingleArcVariationalSimulationResults< long double, Time > >(
                                             estimationOutput->getSimulationResults( ).back( ) )
                                             ->getDynamicsResults( )
        ...
        for( auto it: estimatedStateHistory )
        {
            ...
            Eigen::VectorXd stateDifference = it.second.cast< double >( ) -
                    spice_interface::getBodyCartesianStateAtEpoch( "GRAIL-A", "Moon", globalFrameOrientation, "None", it.first );
        ...
        input_output::writeMatrixToFile( estimationOutput->getUnnormalizedDesignMatrix( ),
                                         "unnormalizedDesignMatrix.dat",
                                         16,
                                         "/home/mfayolle/Tudat/Data/GRAIL_TestResults_time_bias2/" );
        input_output::writeMatrixToFile( estimationOutput->getNormalizedDesignMatrix( ),
                                         "normalizedDesignMatrix.dat",
                                         16,
                                         "/home/mfayolle/Tudat/Data/GRAIL_TestResults_time_bias2/" );
