...
BOOST_AUTO_TEST_CASE( testSeparateObservationViabilityCalculators )
{
    ...
    BodyListSettings bodySettings = getDefaultBodySettings( bodyNames, "Earth", "J2000" );
    ...
    Eigen::Vector3d earthFixedGroundStationState = bodies.at( "Earth" )
                                                           ->getGroundStation( "Station" )
                                                           ->getNominalStationState( )
                                                           ->getCartesianStateInTime( 0.0, "Earth" )
                                                           .segment( 0, 3 );
    ...
    for( unsigned int i = 0; i < 2; i++ )
    {
        ...
        std::shared_ptr< ObservationViabilityCalculator > minimumElevationCalculator =
                std::make_shared< MinimumElevationAngleCalculator >( linkEndIndices, testAngle, pointingAngleCalculator );
        std::shared_ptr< ObservationViabilityCalculator > bodyAvoidanceCalculator = std::make_shared< BodyAvoidanceAngleCalculator >(
        ...
        for( int j = 0; j < 20000; j++ )
        {
            ...
            manualGroundStationInertialPosition =
                    spice_interface::computeRotationMatrixBetweenFrames( "IAU_Earth", "J2000", linkEndTimes[ groundStationIndex ] ) *
                    earthFixedGroundStationState;
            ...
            // Test elevation angle
            {
                ...
                bool isObservationViable = minimumElevationCalculator->isObservationViable( linkEndStates, linkEndTimes );
            ...
            {
                double manualAvoidanceAngle = std::acos( ( vectorToTestBody.normalized( ) ).dot( vectorToTarget.normalized( ) ) );

                // Compute viability and check against manual calculation
                bool isObservationViable = bodyAvoidanceCalculator->isObservationViable( linkEndStates, linkEndTimes );
            ...
            // Test occultation
            {
                ...
                if( computeOccultation( manualGroundStationInertialPosition,
...
BOOST_AUTO_TEST_CASE( testStationAngleCalculations )
{
    ...
    //            bodies.at( "Earth" )->getGroundStation( "Station" )->getNominalStationState( )->getCartesianStateInTime( 0.0 ).segment( 0,
    //            3 );
...
std::vector< double > getBodyLinkElevationAngles( const LinkEnds linkEnds,
                                                  ...
                                                  const SystemOfBodies& bodies )
{
    ...
    switch( observableType )
    {
        ...
        case angular_position: {
            ...
            //                                           0 ) ) );
            //            elevationAngles.push_back( currentPointingAnglesCalculator->calculateElevationAngleFromInertialVector(
            //                                           ( linkEndStates.at( 3 ) - linkEndStates.at( 2 ) ).segment( 0, 3 ), linkEndTimes.at(
            //                                           2 ) ) );
            //
            //        }
            //        else if( linkEnds.at( receiver ).bodyName_ == referenceBody )
            ...
            //                                           1 ) ) );
            //            elevationAngles.push_back( currentPointingAnglesCalculator->calculateElevationAngleFromInertialVector(
            //                                           ( linkEndStates.at( 2 ) - linkEndStates.at( 3 ) ).segment( 0, 3 ), linkEndTimes.at(
            //                                           3 ) ) );
            //        }
            //        break;
            //    }
        case n_way_range: {
            ...
            for( LinkEnds::const_iterator linkEndIterator = linkEnds.begin( ); linkEndIterator != linkEnds.end( ); linkEndIterator++ )
            {
                if( linkEndIterator->second.bodyName_ == referenceBody )
                {
                    ...
                    {
                        elevationAngles.push_back( currentPointingAnglesCalculator->calculateElevationAngleFromInertialVector(
                                ( linkEndStates.at( 2 * ( linkEndIndex - 1 ) ) - linkEndStates.at( 2 * ( linkEndIndex - 1 ) + 1 ) )
                                        .segment( 0, 3 ),
                    ...
                    {
                        elevationAngles.push_back( currentPointingAnglesCalculator->calculateElevationAngleFromInertialVector(
                                ( linkEndStates.at( 2 * linkEndIndex + 1 ) - linkEndStates.at( 2 * linkEndIndex ) ).segment( 0, 3 ),
...
std::vector< double > getBodyCosineAvoidanceAngles( const LinkEnds linkEnds,
                                                    const ObservableType observableType,
                                                    const std::string referenceBody,
        ...
        case angular_position: {
            if( linkEnds.at( transmitter ).bodyName_ == referenceBody )
            {
                cosineAvoidanceAngles.push_back( linear_algebra::computeCosineOfAngleBetweenVectors(
                        ...
                        ( bodies.at( bodyToAvoid )
                                  ->getStateInBaseFrameFromEphemeris< double, double >( ( linkEndTimes.at( 1 ) + linkEndTimes.at( 0 ) ) /
                                                                                        2.0 )
                                  .segment( 0, 3 ) -
                ...
                if( linkEndIterator->second.bodyName_ == referenceBody )
                {
                    ...
                    if( linkEndIndex != static_cast< int >( linkEnds.size( ) ) - 1 )
                    {
                        cosineAvoidanceAngles.push_back( linear_algebra::computeCosineOfAngleBetweenVectors(
                                ...
                                ( bodies.at( bodyToAvoid )
                                          ->getStateInBaseFrameFromEphemeris< double, double >(
                                                  ( linkEndTimes.at( 2 * linkEndIndex + 1 ) + linkEndTimes.at( 2 * linkEndIndex ) ) / 2.0 )
                                          .segment( 0, 3 ) -
...
std::vector< double > getDistanceBetweenLineOfSightVectorAndPoint( const std::string bodyToAnalyze,
                                                                   const std::vector< Eigen::Vector6d > linkEndStates,
                                                                   const std::vector< double > linkEndTimes,
                                                                   const SystemOfBodies& bodies )
{
    ...
    for( unsigned int i = 0; i < linkEndStates.size( ); i += 2 )
    {
        ...
        Eigen::Vector3d pointPosition =
                bodies.at( bodyToAnalyze )
                        ->getStateInBaseFrameFromEphemeris< double, double >( ( linkEndTimes.at( i ) + linkEndTimes.at( i ) ) / 2.0 )
                        .segment( 0, 3 );
...
BOOST_AUTO_TEST_CASE( testObservationViabilityCalculators )
{
    ...
    bodySettings.at( "Mars" )->rotationModelSettings = std::make_shared< SimpleRotationModelSettings >(
            "ECLIPJ2000",
            "IAU_Mars",
            spice_interface::computeRotationQuaternionBetweenFrames( "ECLIPJ2000", "IAU_Mars", 0.0 ),
            0.0,
            2.0 * mathematical_constants::PI / ( physical_constants::JULIAN_DAY + 40.0 * 60.0 ) );
    ...
    createGroundStation( bodies.at( "Mars" ),
                         "MarsStation1",
                         ( Eigen::Vector3d( ) << 100.0, 0.2, 2.1 ).finished( ),
                         coordinate_conversions::geodetic_position );
    createGroundStation( bodies.at( "Mars" ),
                         "MarsStation2",
                         ( Eigen::Vector3d( ) << -2000.0, -0.4, 0.1 ).finished( ),
                         coordinate_conversions::geodetic_position );
    createGroundStation( bodies.at( "Earth" ),
                         "EarthStation1",
                         ( Eigen::Vector3d( ) << 800.0, 0.12, 5.3 ).finished( ),
                         coordinate_conversions::geodetic_position );
    createGroundStation( bodies.at( "Earth" ),
                         "EarthStation2",
                         ( Eigen::Vector3d( ) << 100.0, 0.15, 0.0 ).finished( ),
                         coordinate_conversions::geodetic_position );
    ...
    observationViabilitySettings.push_back( std::make_shared< ObservationViabilitySettings >(
            minimum_elevation_angle, std::make_pair< std::string, std::string >( "Earth", "" ), "", earthTestAngle ) );
    observationViabilitySettings.push_back( std::make_shared< ObservationViabilitySettings >(
            minimum_elevation_angle, std::make_pair< std::string, std::string >( "Mars", "" ), "", marsTestAngle ) );
    observationViabilitySettings.push_back( std::make_shared< ObservationViabilitySettings >(
            body_avoidance_angle, std::make_pair< std::string, std::string >( "Earth", "" ), "Sun", earthSunAvoidanceAngle ) );
    observationViabilitySettings.push_back( std::make_shared< ObservationViabilitySettings >(
            body_avoidance_angle, std::make_pair< std::string, std::string >( "Mars", "" ), "Sun", marsSunAvoidanceAngle ) );
    observationViabilitySettings.push_back( std::make_shared< ObservationViabilitySettings >(
    ...
    for( int viabilityTest = 0; viabilityTest < 6; viabilityTest++ )
    {
        ...
        for( std::map< ObservableType, std::vector< LinkEnds > >::const_iterator observableIterator = testLinkEndsList.begin( );
             observableIterator != testLinkEndsList.end( );
             observableIterator++ )
        {
            for( unsigned int i = 0; i < observableIterator->second.size( ); i++ )
            {
                ...
                observationTimeSettings.push_back( std::make_shared< TabulatedObservationSimulationSettings< double > >(
                        observableIterator->first, observableIterator->second.at( i ), unconstrainedObservationTimes, referenceLinkEnd ) );
                observationTimeSettingsConstrained.push_back(
                        std::make_shared< TabulatedObservationSimulationSettings< double > >( observableIterator->first,
                                                                                              observableIterator->second.at( i ),
                                                                                              unconstrainedObservationTimes,
                                                                                              referenceLinkEnd,
                                                                                              observationViabilitySettingsToUse ) );
            }
        }
            ...
            for( int j = 0; j < numberOfLinkEnds; j++ )
            {
                ...
                for( unsigned int k = 0; k < constrainedIndices.size( ); k++ )
                {
                    ...
                    while( unconstrainedIndex < unconstrainedNumberOfObservations )
                    {
                        ...
                        else if( unconstrainedTimesSegment.at( unconstrainedIndex ) == constrainedTimesSegment.at( constrainedIndex ) )
                        {
                        ...
                        {
                            currentObservation =
                                    std::dynamic_pointer_cast< ObservationSimulator< 2 > >( currentObservationSimulator )
                                            ->getObservationModel( currentLinkEnds )
                                            ->computeObservationsWithLinkEndData( unconstrainedObservationTimes.at( unconstrainedIndex ),
                                                                                  referenceLinkEnd,
                                                                                  linkEndTimes,
                                                                                  linkEndStates );
                        }
                        ...
                        {
                            currentObservation =
                                    std::dynamic_pointer_cast< ObservationSimulator< 1 > >( currentObservationSimulator )
                                            ->getObservationModel( currentLinkEnds )
                                            ->computeObservationsWithLinkEndData( unconstrainedObservationTimes.at( unconstrainedIndex ),
                                                                                  referenceLinkEnd,
                                                                                  linkEndTimes,
                        ...
                        {
                            isSingleViabilityConditionMet =
                                    currentViabilityCalculators.at( k )->isObservationViable( linkEndStates, linkEndTimes );
                        ...
                        if( viabilityTest == 4 || viabilityTest == 5 )
                        {
                            ...
                            std::vector< double > moonLineOfSightDistances =
                                    getDistanceBetweenLineOfSightVectorAndPoint( "Moon", linkEndStates, linkEndTimes, bodies );
...
BOOST_AUTO_TEST_CASE( testOrbiterOccultationObservationViabilityCalculators )
{
    ...
    BodyListSettings bodySettings = getDefaultBodySettings( bodyNames, "SSB", "ECLIPJ2000" );
            ...
            for( unsigned int k = 0; k < constrainedIndices.size( ); k++ )
            {
                ...
                while( unconstrainedIndex < unconstrainedNumberOfObservations )
                {
                    ...
                    for( int linksToCheck = 0; linksToCheck < numberOfLinksToCheck; linksToCheck++ )
                    {
                        ...
                        Eigen::Vector3d currentJupiterPosition = bodies.getBody( "Jupiter" )
                                                                         ->getStateInBaseFrameFromEphemeris( linkEndTimes[ jupiterIndex ] )
                                                                         .segment( 0, 3 ) -
                                bodies.getBody( "Earth" )->getStateInBaseFrameFromEphemeris( linkEndTimes[ jupiterIndex ] ).segment( 0, 3 );
                        Eigen::Vector3d currentSpacecraftPosition =
                                bodies.getBody( "Spacecraft" )
                                        ->getStateInBaseFrameFromEphemeris( linkEndTimes[ spacecraftIndex ] )
                                        .segment( 0, 3 ) -
