...
BOOST_AUTO_TEST_CASE( testInversePowerSeriesCorrectionMorley )
{
    ...
    for( unsigned int i = 0; i < sepAngle.size( ); ++i ) sepAngle.at( i ) *= mathematical_constants::PI / 180.0;

    // Convert geocentric distances to meter
    for( unsigned int i = 0; i < geocentricDistance.size( ); ++i ) geocentricDistance.at( i ) *= physical_constants::ASTRONOMICAL_UNIT;
    ...
    std::function< Eigen::Vector6d( double time ) > sunStateFunction = []( double time ) { return Eigen::Vector6d::Zero( ); };
    // Define Earth state
    ...
    for( unsigned int i = 0; i < sepAngle.size( ); ++i )
    {
        for( unsigned int j = 0; j < geocentricDistance.size( ); ++j )
        {
            ...
            double calculatedCorrectionAnalytical = coronaCorrectionAnalytical.calculateLightTimeCorrectionWithMultiLegLinkEndStates(
                                                            { earthState, spacecraftState }, { 0.0, 0.0 }, 0, dummyAncillarySettings ) *
                    SPEED_OF_LIGHT;

            double calculatedCorrectionNumerical = coronaCorrectionNumerical.calculateLightTimeCorrectionWithMultiLegLinkEndStates(
                                                           { spacecraftState, earthState }, { 0.0, 0.0 }, 0, dummyAncillarySettings ) *
                    SPEED_OF_LIGHT;
