...
std::map< double, VectorType > getStateHistoryMap( const std::shared_ptr< ephemerides::Ephemeris > originalEphemeris )
{
    ...
    {
        stateHistoryMap[ currentTime ] = originalEphemeris->getCartesianState( currentTime );
...
BOOST_AUTO_TEST_CASE( testTabulatedEphemeris )
{
    ...
    std::shared_ptr< ApproximateJplEphemeris > marsNominalEphemeris = std::make_shared< ApproximateJplEphemeris >( "Mars" );

    // Generate state history map from ephemeris
    std::map< double, Eigen::Vector6d > marsStateHistoryMap = getStateHistoryMap< Eigen::Vector6d >( marsNominalEphemeris );
    std::map< double, Eigen::VectorXd > marsDynamicSizeStateHistoryMap = getStateHistoryMap< Eigen::VectorXd >( marsNominalEphemeris );

    // Create interpolator from state history map.
    std::shared_ptr< interpolators::OneDimensionalInterpolator< double, Eigen::Vector6d > > marsStateInterpolator =
            std::make_shared< interpolators::CubicSplineInterpolator< double, Eigen::Vector6d > >( marsStateHistoryMap );
    std::shared_ptr< interpolators::OneDimensionalInterpolator< double, Eigen::VectorXd > > marsDynamicSizeStateInterpolator =
            std::make_shared< interpolators::CubicSplineInterpolator< double, Eigen::VectorXd > >( marsDynamicSizeStateHistoryMap );

    // Create tabulated epehemeris from interpolator
    std::shared_ptr< TabulatedCartesianEphemeris<> > tabulatedEphemeris =
            std::make_shared< TabulatedCartesianEphemeris<> >( marsStateInterpolator, "SSB", "J2000" );
    std::shared_ptr< TabulatedCartesianEphemeris<> > tabulatedDynamicSizeEphemeris =
            std::make_shared< TabulatedCartesianEphemeris<> >( marsDynamicSizeStateInterpolator, "SSB", "J2000" );
    ...
    Eigen::Vector6d ephemerisDynamicSizeState = tabulatedDynamicSizeEphemeris->getCartesianState( testTime );
    ...
    Eigen::Vector6d directState = marsNominalEphemeris->getCartesianState( testTime );
    ...
    directState = marsNominalEphemeris->getCartesianState( testTime );
    TUDAT_CHECK_MATRIX_CLOSE_FRACTION( directState, ephemerisState, 1.0E-10 );

    // Create new ephemeris from which table is to be generated; used to reset input to tabulated
    // ephemeris
    std::shared_ptr< ApproximateJplEphemeris > jupiterNominalEphemeris = std::make_shared< ApproximateJplEphemeris >( "Jupiter" );

    // Reset tabulated ephemeris data.
    std::map< double, Eigen::Vector6d > jupiterStateHistoryMap = getStateHistoryMap( jupiterNominalEphemeris );
    std::map< double, Eigen::VectorXd > jupiterDynamicSizeStateHistoryMap =
            getStateHistoryMap< Eigen::VectorXd >( jupiterNominalEphemeris );
    std::shared_ptr< interpolators::OneDimensionalInterpolator< double, Eigen::Vector6d > > jupiterStateInterpolator =
            std::make_shared< interpolators::CubicSplineInterpolator< double, Eigen::Vector6d > >( jupiterStateHistoryMap );
    std::shared_ptr< interpolators::OneDimensionalInterpolator< double, Eigen::VectorXd > > jupiterDynamicSizeStateInterpolator =
            std::make_shared< interpolators::CubicSplineInterpolator< double, Eigen::VectorXd > >( jupiterDynamicSizeStateHistoryMap );
    ...
    directState = jupiterNominalEphemeris->getCartesianState( testTime );
    TUDAT_CHECK_MATRIX_CLOSE_FRACTION( directState, ephemerisState, 1.0E-10 );
