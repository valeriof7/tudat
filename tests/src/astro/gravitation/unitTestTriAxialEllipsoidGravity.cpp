...
double getZonalTermForEllipsoidOfRevolution( const double axisA, const double axisC, const double referenceRadius, const int degree )
{
    ...
    if( degree % 2 == 0 )
    {
        zonalTerm = 3.0 / std::pow( referenceRadius, degree ) * std::pow( axisA * axisA - axisC * axisC, degree / 2 ) *
                std::pow( -1.0, degree / 2 ) / ( ( static_cast< double >( degree ) + 1.0 ) * ( static_cast< double >( degree ) + 3.0 ) );
    }
...
BOOST_AUTO_TEST_CASE( testTriAxialEllipsoidGravity )
{
    ...
    BOOST_CHECK_CLOSE_FRACTION( expectedC20, sphericalHarmonicCoefficients.first( 2, 0 ), 2.0 * std::numeric_limits< double >::epsilon( ) );
    BOOST_CHECK_CLOSE_FRACTION( expectedC22, sphericalHarmonicCoefficients.first( 2, 2 ), 2.0 * std::numeric_limits< double >::epsilon( ) );
    BOOST_CHECK_CLOSE_FRACTION( expectedC40, sphericalHarmonicCoefficients.first( 4, 0 ), 2.0 * std::numeric_limits< double >::epsilon( ) );
    BOOST_CHECK_CLOSE_FRACTION( expectedC42, sphericalHarmonicCoefficients.first( 4, 2 ), 2.0 * std::numeric_limits< double >::epsilon( ) );
    BOOST_CHECK_CLOSE_FRACTION( expectedC44, sphericalHarmonicCoefficients.first( 4, 4 ), 2.0 * std::numeric_limits< double >::epsilon( ) );
    ...
    {
        BOOST_CHECK_CLOSE_FRACTION( sphericalHarmonicCoefficients.first( i, 0 ),
