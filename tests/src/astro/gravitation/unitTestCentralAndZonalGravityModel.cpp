...
BOOST_AUTO_TEST_CASE( testGravitationalAcceleration )
{
    ...
    //         (Easy calculation, 2012).
    {
        ...
        const Eigen::Vector3d gravitationalAccelerationExertedAtEarthSurface =
                computeGravitationalAcceleration( positionOnEarthSurface, gravitationalParameterOfEarth, positionOfEarth );

        // Check if computed gravitational force matches expected value.
        BOOST_CHECK_CLOSE_FRACTION( 9.8, gravitationalAccelerationExertedAtEarthSurface.norm( ), 1.0e-4 );
    }
    ...
    //         (Easy calculation, 2012).
    {
        ...
        const Eigen::Vector3d gravitationalAccelerationExertedAtLunarSurface =
                computeGravitationalAcceleration( universalGravitationalConstant, positionOfLunarSurface, massOfMoon, positionOfMoon );

        // Check if computed gravitational force matches expected value.
        BOOST_CHECK_CLOSE_FRACTION( 1.63, gravitationalAccelerationExertedAtLunarSurface.norm( ), 1.0e-6 );
    }
}
...
BOOST_AUTO_TEST_CASE( testGravitationalAccelarationSumZonalMatlab )
{
    ...
    {
        for( unsigned int body1 = 0; body1 < planetData.at( planet ).body1Positions.size( ); body1++ )
        {
            for( unsigned int body2 = 0; body2 < planetData.at( planet ).body2Positions.size( ); body2++ )
            {
                // Compute central gravitational acceleration term [m s^-2].
                const Eigen::Vector3d computedCentralAcceleration =
                        computeGravitationalAcceleration( planetData.at( planet ).body2Positions.at( body2 ),
                ...
                TUDAT_CHECK_MATRIX_CLOSE_FRACTION(
                        planetData.at( planet ).expectedAcceleration[ body1 ][ body2 ][ central ], computedCentralAcceleration, 1.0e-15 );
                ...
                for( KeyIntValueDoubleMap::iterator zonalCoefficientIterator = planetData.at( planet ).zonalCoefficients.begin( );
                     zonalCoefficientIterator != planetData.at( planet ).zonalCoefficients.end( );
                     zonalCoefficientIterator++ )
                {
                    // Add current zonal coefficient to local list.
                    zonalCoefficients[ zonalCoefficientIterator->first ] = zonalCoefficientIterator->second;

                    // Compute gravitational acceleration sum [m s^-2].
                    const Eigen::Vector3d computedAccelerationSum =
                            computeGravitationalAccelerationZonalSum( planetData.at( planet ).body2Positions.at( body2 ),
                                                                      planetData.at( planet ).gravitationalParameter,
                    ...
                    TUDAT_CHECK_MATRIX_CLOSE_FRACTION(
                            planetData.at( planet ).expectedAcceleration[ body1 ][ body2 ][ zonalCoefficientIterator->first ],
        ...
        for( unsigned int body1 = 0; body1 < planetData.at( planet ).body1Positions.size( ); body1++ )
        {
            for( unsigned int body2 = 0; body2 < planetData.at( planet ).body2Positions.size( ); body2++ )
            {
                // Test central gravitational acceleration wrapper class.
                {
                    ...
                    CentralGravitationalAccelerationModel3dPointer centralGravity =
                            std::make_shared< CentralGravitationalAccelerationModel3d >(
                                    [ & ]( ) { return planetData.at( planet ).body2Positions.at( body2 ); },
                                    planetData.at( planet ).gravitationalParameter,
                                    [ & ]( ) { return planetData.at( planet ).body1Positions.at( body1 ); } );

                    // Compute central gravitational acceleration term [m s^-2].
                    const Eigen::Vector3d computedCentralAcceleration = centralGravity->getAcceleration( );

                    // Check that the computed central gravitational acceleration matches the
                    // expected values.
                    TUDAT_CHECK_MATRIX_CLOSE_FRACTION( planetData.at( planet ).expectedAcceleration[ body1 ][ body2 ][ central ],
                ...
                if( planetData.at( planet ).zonalCoefficients.rbegin( )->first == 2 )
                {
                    ...
                    // Compute gravitational acceleration sum [m s^-2].
                    const Eigen::Vector3d computedCentralJ2AccelerationSum = centralJ2Gravity->getAcceleration( );

                    // Check that computed central + J2 gravitational acceleration matches
                    // expected values.
                    TUDAT_CHECK_MATRIX_CLOSE_FRACTION( planetData.at( planet ).expectedAcceleration[ body1 ][ body2 ][ 2 ],
                ...
                if( planetData.at( planet ).zonalCoefficients.rbegin( )->first == 3 )
                {
                    ...
                    // Compute gravitational acceleration sum [m s^-2].
                    const Eigen::Vector3d computedCentralJ2J3AccelerationSum = centralJ2J3Gravity->getAcceleration( );

                    // Check that computed central + J2 + J3 gravitational acceleration matches
                    // expected values.
                    TUDAT_CHECK_MATRIX_CLOSE_FRACTION( planetData.at( planet ).expectedAcceleration[ body1 ][ body2 ][ 3 ],
                ...
                if( planetData.at( planet ).zonalCoefficients.rbegin( )->first == 4 )
                {
                    ...
                    if( planetData.at( planet ).zonalCoefficients.size( ) == 2 )
                    {
                        ...
                        centralJ2J3J4Gravity = std::make_shared< CentralJ2J3J4GravitationalAccelerationModel >(
                                [ & ]( ) { return planetData.at( planet ).body2Positions.at( body2 ); },
                                planetData.at( planet ).gravitationalParameter,
                                planetData.at( planet ).equatorialRadius,
                                planetData.at( planet ).zonalCoefficients[ 2 ],
                    ...
                    // Compute gravitational acceleration sum [m s^-2].
                    const Eigen::Vector3d computedCentralJ2J3J4AccelerationSum = centralJ2J3J4Gravity->getAcceleration( );

                    // Check that computed central + J2 gravitational acceleration matches
                    // expected values.
                    TUDAT_CHECK_MATRIX_CLOSE_FRACTION( planetData.at( planet ).expectedAcceleration[ body1 ][ body2 ][ 4 ],
...
BOOST_AUTO_TEST_CASE( testGravitationalAccelarationZonalMelman )
{
    typedef Eigen::Vector3d ( *GravitationalAccelerationPointer )(
            const Eigen::Vector3d&, const double, const double, const double, const Eigen::Vector3d& );
    ...
    std::map< int, GravitationalAccelerationPointer > zonalGravitationalAccelerationPointers = {
        { 2, &computeGravitationalAccelerationDueToJ2 },
    ...
    {
        // Loop over all available zonal gravity field coefficients.
        for( KeyIntValueDoubleMap::iterator zonalCoefficientIterator = earthData.zonalCoefficients.begin( );
             ...
             zonalCoefficientIterator++ )
        {
            ...
            const Eigen::Vector3d computedZonalGravitationalAcceleration =
                    zonalGravitationalAccelerationPointers[ zonalCoefficientIterator->first ]( earthData.body2Positions.at( body2 ),
            ...
            // Check that computed gravitational acceleration sum matches expected values.
            TUDAT_CHECK_MATRIX_CLOSE_FRACTION( earthData.expectedAcceleration[ 0 ][ body2 ][ zonalCoefficientIterator->first ],
...
BOOST_AUTO_TEST_CASE( testGravitationalAccelarationZonalRonse )
{
    ...
    {
        // Compute central gravitational acceleration term [m s^-2].
        const Eigen::Vector3d computedCentralAcceleration = computeGravitationalAcceleration(
                earthData.body2Positions.at( body2 ), earthData.gravitationalParameter, earthData.body1Positions.at( 0 ) );

        // Check that the computed central gravitational acceleration matches the expected
        // values.
        TUDAT_CHECK_MATRIX_CLOSE_FRACTION( earthData.expectedAcceleration[ 0 ][ body2 ][ central ], computedCentralAcceleration, 1.0e-15 );
