...
double computeSemiMajorAxisRateDueToTideRaisedOnPlanet( const double k2QRatio,
                                                        const double satelliteWrtPlanetMassRatio,
                                                        const double referenceRadiusWrtSemiMajorAxisRatio,
                                                        const double semiMajorAxis,
                                                        const double meanMotion )
{
    return 3.0 * k2QRatio * satelliteWrtPlanetMassRatio * std::pow( referenceRadiusWrtSemiMajorAxisRatio, 5.0 ) * semiMajorAxis *
            meanMotion;
}

double computeEccentricityRateDueToTideRaisedOnPlanet( const double k2QRatio,
                                                       const double satelliteWrtPlanetMassRatio,
                                                       const double referenceRadiusWrtSemiMajorAxisRatio,
                                                       const double eccentricity,
                                                       const double meanMotion )
{
    return 57.0 / 8.0 * k2QRatio * satelliteWrtPlanetMassRatio * std::pow( referenceRadiusWrtSemiMajorAxisRatio, 5.0 ) * eccentricity *
            meanMotion;
}

std::pair< double, double > computeKeplerElementRatesDueToDissipation( const SystemOfBodies& bodies,
                                                                       const std::string& satelliteToPropagate,
                                                                       const bool usePlanetDissipation,
                                                                       const double k2LoveNumber,
                                                                       const double tidalTimeLag,
                                                                       ...
                                                                       Eigen::Vector6d& intialKeplerElements )
{
    ...
    std::map< double, double > semiMajorAxes, eccentricities;
    {
        ...
        accelerationsOfIo[ "Jupiter" ].push_back( std::make_shared< AccelerationSettings >( basic_astrodynamics::point_mass_gravity ) );
        accelerationsOfIo[ "Jupiter" ].push_back(
                std::make_shared< DirectTidalDissipationAccelerationSettings >( k2LoveNumber, tidalTimeLag, false, usePlanetDissipation ) );
        ...
        {
            dependentVariablesToSave.push_back( std::make_shared< SingleAccelerationDependentVariableSaveSettings >(
        ...
        std::shared_ptr< TranslationalStatePropagatorSettings< double > > propagatorSettings =
                std::make_shared< TranslationalStatePropagatorSettings< double > >(
                        ...
                        bodiesToPropagate,
                        getInitialStatesOfBodies( bodiesToPropagate, centralBodies, bodies, initialTime ),
                        initialTime,
                        integratorSettings,
             ...
             mapIterator++ )
        {
            integrationResultWithDissipationKepler[ mapIterator->first ] =
                    orbital_element_conversions::convertCartesianToKeplerianElements< double >(
                            mapIterator->second,
                            getBodyGravitationalParameter( "Jupiter" ) + getBodyGravitationalParameter( satelliteToPropagate ) );
    ...
    std::vector< double > semiMajorAxisFit = linear_algebra::getLeastSquaresPolynomialFit( semiMajorAxes, { 0, 1 } );
    std::vector< double > eccentricityFit = linear_algebra::getLeastSquaresPolynomialFit( eccentricities, { 0, 1 } );
    ...
    meanMotion = basic_astrodynamics::computeKeplerMeanMotion(
            intialKeplerElements( 0 ), getBodyGravitationalParameter( "Jupiter" ) + getBodyGravitationalParameter( satelliteToPropagate ) );
...
BOOST_AUTO_TEST_CASE( testTidalDissipationInPlanetAndSatellite )
{
    ...
    // Get body settings.
    BodyListSettings bodySettings = getDefaultBodySettings( bodyNames, initialTime - 86400.0, finalTime + 86400.0 );
    ...
    {
        bodySettings.at( galileanSatellites.at( i ) )->gravityFieldSettings =
                std::make_shared< SphericalHarmonicsGravityFieldSettings >( getBodyGravitationalParameter( galileanSatellites.at( i ) ),
                                                                            getAverageRadius( galileanSatellites.at( i ) ),
                                                                            cosineCoefficients,
                                                                            sineCoefficients,
                                                                            "IAU_" + galileanSatellites.at( i ) );
    }
    ...
    bodySettings.at( "Europa" )->ephemerisSettings = std::make_shared< KeplerEphemerisSettings >(
            ( Eigen::Vector6d( ) << 671.1E6, 0.009, 0.0, 0.0, 0.0, 0.0 ).finished( ),
            0.0,
            getBodyGravitationalParameter( "Jupiter" ) + getBodyGravitationalParameter( "Europa" ),
            "Jupiter",
            "ECLIPJ2000" );
    bodySettings.at( "Ganymede" )->ephemerisSettings = std::make_shared< KeplerEphemerisSettings >(
            ( Eigen::Vector6d( ) << 1070.400E6, 0.0013, 0.0, 0.0, 0.0, 0.0 ).finished( ),
            0.0,
            getBodyGravitationalParameter( "Jupiter" ) + getBodyGravitationalParameter( "Ganymede" ),
            "Jupiter",
            "ECLIPJ2000" );
    ...
    for( unsigned int i = 0; i < galileanSatellites.size( ); i++ )
    {
        Eigen::Vector6d intialKeplerElements;
        double meanMotion;
        std::pair< double, double > elementRates = computeKeplerElementRatesDueToDissipation( bodies,
                                                                                              galileanSatellites.at( i ),
                                                                                              true,
                                                                                              jupiterLoveNumber,
                                                                                              jupiterTimeLag,
                                                                                              initialTime,
        ...
        double theoreticalSemiMajorAxisRateFromJupiterTide = computeSemiMajorAxisRateDueToTideRaisedOnPlanet(
                jupiterLoveNumber / jupiterQualityFactor,
                getBodyGravitationalParameter( galileanSatellites.at( i ) ) / getBodyGravitationalParameter( "Jupiter" ),
                getAverageRadius( "Jupiter" ) / intialKeplerElements( 0 ),
                intialKeplerElements( 0 ),
                meanMotion );
        double theoreticaEccentricityRateFromJupiterTide = computeEccentricityRateDueToTideRaisedOnPlanet(
                jupiterLoveNumber / jupiterQualityFactor,
                getBodyGravitationalParameter( galileanSatellites.at( i ) ) / getBodyGravitationalParameter( "Jupiter" ),
                getAverageRadius( "Jupiter" ) / intialKeplerElements( 0 ),
                intialKeplerElements( 1 ),
                meanMotion );

        // BOOST_CHECK_CLOSE_FRACTION( elementRates.first, theoreticalSemiMajorAxisRateFromJupiterTide, 2.0E-3 );
        BOOST_CHECK( std::fabs( elementRates.first - theoreticalSemiMajorAxisRateFromJupiterTide ) <
                     std::fabs( std::min( elementRates.first, theoreticalSemiMajorAxisRateFromJupiterTide ) * 2.0E-3 ) );
        // BOOST_CHECK_CLOSE_FRACTION( elementRates.second, theoreticaEccentricityRateFromJupiterTide, 1.0E-1 );
        BOOST_CHECK( std::fabs( elementRates.second - theoreticaEccentricityRateFromJupiterTide ) <
                     std::fabs( std::min( elementRates.second, theoreticaEccentricityRateFromJupiterTide ) * 1.0E-1 ) );
    }
    ...
    for( unsigned int i = 0; i < galileanSatellites.size( ); i++ )
    {
        ...
        double theoreticalSemiMajorAxisRateFromIoTide = -21.0 * satelliteLoveNumber / satelliteQualityFactor *
                getBodyGravitationalParameter( "Jupiter" ) / getBodyGravitationalParameter( galileanSatellites.at( i ) ) *
                std::pow( getAverageRadius( galileanSatellites.at( i ) ) / intialKeplerElements( 0 ), 5.0 ) * intialKeplerElements( 0 ) *
                meanMotion * intialKeplerElements( 1 ) * intialKeplerElements( 1 );
        double theoreticaEccentricityRateFromIoTide = -21.0 / 2.0 * satelliteLoveNumber / satelliteQualityFactor *
                getBodyGravitationalParameter( "Jupiter" ) / getBodyGravitationalParameter( galileanSatellites.at( i ) ) *
                std::pow( getAverageRadius( galileanSatellites.at( i ) ) / intialKeplerElements( 0 ), 5.0 ) * intialKeplerElements( 1 ) *
                meanMotion;
        ...
        BOOST_CHECK( std::fabs( elementRates.first - theoreticalSemiMajorAxisRateFromIoTide ) <
                     std::fabs( std::min( elementRates.first, theoreticalSemiMajorAxisRateFromIoTide ) * toleranceMultiplier * 2.0E-3 ) );
        ...
        BOOST_CHECK( std::fabs( elementRates.second - theoreticaEccentricityRateFromIoTide ) <
                     std::fabs( std::min( elementRates.second, theoreticaEccentricityRateFromIoTide ) * toleranceMultiplier * 2.0E-3 ) );
