...
BOOST_AUTO_TEST_CASE( testComputationOfThirdBodyPerturbation )
{
    ...
    gravitationalParameterOfPerturbingBodyForTestCase.push_back( 1.0 );  // Technically not a
                                                                         // perturbing body, but for corresponding indices still added.
                                                                         // Also, parameter of one to simplify manual computation.
    ...
    for( unsigned int i = 1; i < positionOfBodyForTestCase.size( ); i++ )
    {
        ...
        gravitation::CentralGravitationalAccelerationModel3dPointer directAccelerationModel =
                std::make_shared< gravitation::CentralGravitationalAccelerationModel3d >(
                        [ & ]( Eigen::Vector3d& input ) { input = positionOfBodyForTestCase[ 0 ]; },
                        gravitationalParameterOfPerturbingBodyForTestCase[ i ],
                        [ & ]( Eigen::Vector3d& input ) { input = positionOfBodyForTestCase[ i ]; } );

        gravitation::CentralGravitationalAccelerationModel3dPointer centralBodyAccelerationModel =
                std::make_shared< gravitation::CentralGravitationalAccelerationModel3d >(
        ...
        std::shared_ptr< gravitation::ThirdBodyCentralGravityAcceleration > thirdBodyAcceleration =
                std::make_shared< gravitation::ThirdBodyCentralGravityAcceleration >(
        ...
        acceleration = gravitation::computeThirdBodyPerturbingAcceleration(
                gravitationalParameterOfPerturbingBodyForTestCase[ i ], positionOfBodyForTestCase[ i ], positionOfBodyForTestCase[ 0 ] );

        // Check values with reference data.
        TUDAT_CHECK_MATRIX_CLOSE_FRACTION( expectedAccelerationForTestCase[ i ], acceleration, tolerance );

        // Compute from manual sum.
        acceleration =
                gravitation::computeGravitationalAcceleration( positionOfBodyForTestCase[ 0 ],
                                                               gravitationalParameterOfPerturbingBodyForTestCase[ i ],
                                                               positionOfBodyForTestCase[ i ] ) -
                gravitation::computeGravitationalAcceleration(
                        Eigen::Vector3d::Zero( ), gravitationalParameterOfPerturbingBodyForTestCase[ i ], positionOfBodyForTestCase[ i ] );
...
BOOST_AUTO_TEST_CASE( testRealisticThirdBodyPerturbation )
{
    ...
    Eigen::Vector3d expectedRealisticAcceleration(
            2.9394638802044552864534126789629e-6, 2.2253948636344714462843917227786e-6, 1.1680066249314399399805985890762e-6 );

    // Compute the acceleration.
    Eigen::Vector3d realisticComputedAcceleration = gravitation::computeThirdBodyPerturbingAcceleration(
            realisticGravitationalParameterOfPerturbingBody, realisticPerturberPosition, realisticTestPosition );

    // Compare differences.
    TUDAT_CHECK_MATRIX_CLOSE_FRACTION( expectedRealisticAcceleration, realisticComputedAcceleration, tolerance );

    // Compute from manual sum.
    realisticComputedAcceleration =
            gravitation::computeGravitationalAcceleration(
                    realisticTestPosition, realisticGravitationalParameterOfPerturbingBody, realisticPerturberPosition ) -
            gravitation::computeGravitationalAcceleration(
                    Eigen::Vector3d::Zero( ), realisticGravitationalParameterOfPerturbingBody, realisticPerturberPosition );
    ...
    // Compute the acceleration.
    realisticComputedAcceleration = gravitation::computeThirdBodyPerturbingAcceleration(
            realisticGravitationalParameterOfPerturbingBody, realisticPerturberPosition, realisticTestPosition, barycentricEarthPosition );
    ...
    realisticComputedAcceleration =
            gravitation::computeGravitationalAcceleration(
                    realisticTestPosition, realisticGravitationalParameterOfPerturbingBody, realisticPerturberPosition ) -
            gravitation::computeGravitationalAcceleration(
                    barycentricEarthPosition, realisticGravitationalParameterOfPerturbingBody, realisticPerturberPosition );
    ...
    gravitation::CentralGravitationalAccelerationModel3dPointer directAccelerationModel =
            std::make_shared< gravitation::CentralGravitationalAccelerationModel3d >(
                    [ & ]( Eigen::Vector3d& input ) { input = realisticTestPosition; },
                    realisticGravitationalParameterOfPerturbingBody,
                    [ & ]( Eigen::Vector3d& input ) { input = realisticPerturberPosition; } );

    gravitation::CentralGravitationalAccelerationModel3dPointer centralBodyAccelerationModel =
            std::make_shared< gravitation::CentralGravitationalAccelerationModel3d >(
                    [ & ]( Eigen::Vector3d& input ) { input = barycentricEarthPosition; },
                    realisticGravitationalParameterOfPerturbingBody,
                    [ & ]( Eigen::Vector3d& input ) { input = realisticPerturberPosition; } );
