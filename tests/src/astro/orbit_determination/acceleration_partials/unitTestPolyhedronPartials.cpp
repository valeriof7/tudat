...
BOOST_AUTO_TEST_CASE( testPolyhedronAccelerationPartialWithSynchronousRotation )
{
    ...
    simulation_setup::BodyListSettings bodySettings = simulation_setup::getDefaultBodySettings( bodyNames, "Earth", "ECLIPJ2000" );

    bodySettings.at( "Earth" )->rotationModelSettings =
            std::make_shared< simulation_setup::SynchronousRotationModelSettings >( "Moon", "ECLIPJ2000", "IAU_Earth" );
    ...
    // Create acceleration partial object.
    std::shared_ptr< acceleration_partials::PolyhedronGravityPartial > accelerationPartial =
            std::dynamic_pointer_cast< acceleration_partials::PolyhedronGravityPartial >( createAnalyticalAccelerationPartial(
