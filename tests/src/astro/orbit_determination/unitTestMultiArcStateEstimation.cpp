...
Eigen::VectorXd executeParameterEstimation( const int linkArcs )
{
    ...
    std::shared_ptr< IntegratorSettings< TimeType > > integratorSettings = rungeKutta4Settings< TimeType >( 3600.0 );
    ...
    {
        Eigen::Matrix< StateScalarType, Eigen::Dynamic, 1 > currentInitialState = getInitialStateOfBody< TimeType, StateScalarType >(
    ...
    parameterNames.push_back( std::make_shared< EstimatableParameterSettings >( "Mars", constant_rotation_rate ) );
    parameterNames.push_back( std::make_shared< EstimatableParameterSettings >( "Mars", rotation_pole_position ) );
    ...
    observationSettingsList.push_back( std::make_shared< ObservationModelSettings >( one_way_range, linkEnds2[ 0 ] ) );
    observationSettingsList.push_back( std::make_shared< ObservationModelSettings >( one_way_range, linkEnds2[ 1 ] ) );
