        ...
        SystemOfBodies& bodies )
{
    ...
    addDependentVariablesToObservationSimulationSettings(
            ...
            std::vector< std::shared_ptr< ObservationDependentVariableSettings > >( { elevationAngleSettings } ),
            bodies );
    ...
    std::map< ObservableType, std::map< LinkEnds, std::vector< std::pair< int, int > > > > obsSetsStartAndSize =
...
BOOST_AUTO_TEST_CASE( test_ObservationCollectionParser )
{
    ...
    std::shared_ptr< ObservationCollection< double, double > > simulatedObservations =
            setUpObservationCollectionToTest( startTime,
                                              numberOfDaysOfData,
    ...
    std::vector< LinkEnds > trueLinkEndsList = { stationReceiverLinkEnds[ 0 ], stationTransmitterLinkEnds[ 0 ],
    ...
    std::vector< std::shared_ptr< SingleObservationSet<> > > rangeObservationSets =
            simulatedObservations->getSingleObservationSets( observationParser( one_way_range ) );
    ...
    std::vector< LinkEnds > trueRangeLinkEnds = { stationReceiverLinkEnds[ 0 ],
    ...
    for( unsigned int k = 0; k < rangeObservationSets.size( ); k++ )
    {
        ...
        BOOST_CHECK( manuallyExtractedRangeObservationSets.at( k )->getObservationsVector( ) ==
                     rangeValuesFromObservationCollection.at( k ) );
    }

    // Retrieve observation times
    std::vector< std::vector< double > > rangeTimesFromObservationCollection =
            simulatedObservations->getObservationTimes( observationParser( one_way_range ) );
    for( unsigned int k = 0; k < rangeObservationSets.size( ); k++ )
    {
    ...
    std::vector< std::shared_ptr< SingleObservationSet<> > > station1ObservationSets =
    ...
    std::vector< std::shared_ptr< SingleObservationSet<> > > firstDayObservationSets =
            simulatedObservations->getSingleObservationSets( observationParser( std::make_pair( startTime, startTime + 86400.0 ) ) );

    // Test opposite condition
    std::vector< std::shared_ptr< SingleObservationSet<> > > afterFirstDayObservationSets =
            simulatedObservations->getSingleObservationSets( observationParser( std::make_pair( startTime, startTime + 86400.0 ), true ) );
    ...
    std::vector< std::shared_ptr< SingleObservationSet<> > > multiTypeObservationSets =
    ...
    std::vector< std::shared_ptr< SingleObservationSet<> > > multiTypeObservationSets2 =
...
BOOST_AUTO_TEST_CASE( test_ObservationsFiltering )
{
    ...
    std::vector< Eigen::Matrix< double, Eigen::Dynamic, 1 > > rangeValuesPostFiltering =
            simulatedObservations->getObservations( observationParser( one_way_range ) );
    ...
    std::vector< Eigen::Matrix< double, Eigen::Dynamic, 1 > > rangeValuesPostDefiltering =
            simulatedObservations->getObservations( observationParser( one_way_range ) );
    std::vector< std::vector< double > > rangeTimesPostDefiltering =
            simulatedObservations->getObservationTimes( observationParser( one_way_range ) );
    ...
    for( auto observableIt: obsSetsStartAndSizeAfterDefilter )
    {
        for( auto linkEndsIt: observableIt.second )
        {
            for( unsigned int k = 0; k < linkEndsIt.second.size( ); k++ )
            {
                BOOST_CHECK( obsSetsStartAndSize.at( observableIt.first ).at( linkEndsIt.first ).at( k ).first ==
                             linkEndsIt.second.at( k ).first );
                BOOST_CHECK( obsSetsStartAndSize.at( observableIt.first ).at( linkEndsIt.first ).at( k ).second ==
                             linkEndsIt.second.at( k ).second );
            }
        }
    }
    ...
    simulatedObservations->filterObservations( observationFilter( epochs_filtering, epochsToFilter, true, true ),
                                               observationParser( one_way_range ) );

    // Retrieve and check remaining observation times
    std::vector< std::vector< double > > remainingObsTimes =
            simulatedObservations->getObservationTimes( observationParser( one_way_range ) );
    ...
    // Reintroduce observations
    simulatedObservations->filterObservations( observationFilter( epochs_filtering, epochsToFilter, false, true ),
                                               observationParser( one_way_range ) );
    ...
    simulatedObservations->filterObservations( observationFilter( time_bounds_filtering, timeBounds, true, true ),
                                               observationParser( one_way_range ) );
    ...
    simulatedObservations->filterObservations( observationFilter( time_bounds_filtering, timeBounds, false, true ),
                                               observationParser( one_way_range ) );
    ...
    Eigen::Matrix< double, Eigen::Dynamic, 1 > residualsStation1 =
            ( residualCutOffValue + 0.05 ) * Eigen::Matrix< double, Eigen::Dynamic, 1 >::Ones( nbRangeStation1, 1 );
    unsigned int nbRangeStation2 = rangeObservationSets.at( 1 )->getNumberOfObservables( );
    Eigen::Matrix< double, Eigen::Dynamic, 1 > residualsStation2 =
            ( residualCutOffValue - 0.05 ) * Eigen::Matrix< double, Eigen::Dynamic, 1 >::Ones( nbRangeStation2, 1 );
    unsigned int nbRangeStation3 = rangeObservationSets.at( 2 )->getNumberOfObservables( );
    Eigen::Matrix< double, Eigen::Dynamic, 1 > residualsStation3 =
            ( residualCutOffValue - 0.05 ) * Eigen::Matrix< double, Eigen::Dynamic, 1 >::Ones( nbRangeStation3, 1 );
    ...
    std::vector< std::shared_ptr< SingleObservationSet<> > > postFilterRangeObservationSets =
            simulatedObservations->getSingleObservationSets( observationParser( one_way_range ) );
    for( unsigned int k = 0; k < rangeObservationSets.size( ); k++ )
    {
    ...
    std::vector< Eigen::MatrixXd > remainingElevationAngles =
            simulatedObservations->getDependentVariables( elevationAngleSettings, false ).first;
...
BOOST_AUTO_TEST_CASE( test_ObservationsSplitting )
{
    ...
    simulatedObservations->splitObservationSets( observationSetSplitter( time_tags_splitter, timeTags ),
                                                 observationParser( stationTransmitterLinkEnds[ 0 ] ) );

    // Check that the original observation set got split in two independent sets (before/after splitEpoch)
    std::vector< std::shared_ptr< SingleObservationSet<> > > splitObsSets =
            simulatedObservations->getSingleObservationSets( observationParser( stationTransmitterLinkEnds[ 0 ] ) );
    ...
    simulatedObservations->splitObservationSets( observationSetSplitter( time_span_splitter, maxTimeSpanObs, 10 ),
                                                 observationParser( stationTransmitterLinkEnds[ 0 ] ) );

    // Check that we now have three independent sets (out of the former two, only one set could be split while retaining enough
    // observations)
    ...
    simulatedObservations->splitObservationSets( observationSetSplitter( nb_observations_splitter, 50, 50 ),
                                                 observationParser( stationTransmitterLinkEnds[ 0 ] ) );
    ...
    simulatedObservations->splitObservationSets( observationSetSplitter( time_interval_splitter, 1500.0 ),
                                                 observationParser( stationReceiverLinkEnds[ 2 ] ) );

    // Check that the observation set associated with stationReceiverLinkEnds[ 2 ] (of type angular_position) got split into two independent
    // sets
    ...
    for( unsigned int k = 0; k < 2; k++ )
    {
        ...
        {
            BOOST_CHECK( ( splitObsSets.at( k )->getObservationTimes( ).at( i + 1 ) -
                           splitObsSets.at( k )->getObservationTimes( ).at( i ) ) <= 1500.0 );
        }
    }
