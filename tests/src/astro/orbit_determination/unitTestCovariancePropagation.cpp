...
BOOST_AUTO_TEST_CASE( test_EstimationInputAndOutput )
{
    ...
    for( int test = 0; test < 2; test++ )
    {
        ...
        auto stateTransitionMatrixHistory = std::dynamic_pointer_cast< SingleArcVariationalEquationsSolver<> >(
                                                    orbitDeterminationManager.getVariationalEquationsSolver( ) )
                                                    ->getStateTransitionMatrixSolution( );
        auto sensitivityMatrixHistory = std::dynamic_pointer_cast< SingleArcVariationalEquationsSolver<> >(
                                                orbitDeterminationManager.getVariationalEquationsSolver( ) )
                                                ->getSensitivityMatrixSolution( );

        std::map< double, Eigen::MatrixXd > propagatedCovariance;
        propagateCovariance( propagatedCovariance,
        ...
        std::map< double, Eigen::MatrixXd > rswPropagatedCovariance = convertCovarianceHistoryToFrame(
                propagatedCovariance, stateHistory, reference_frames::global_reference_frame, reference_frames::rsw_reference_frame );
        ...
        for( auto it: propagatedCovariance )
        {
            ...
            if( test == 0 )
            {
                Eigen::MatrixXd manualCovariance =
                        currentStateTransition * estimationOutput->getUnnormalizedCovarianceMatrix( ) * currentStateTransition.transpose( );
                TUDAT_CHECK_MATRIX_CLOSE_FRACTION( it.second, manualCovariance, 1.0E-10 );

                Eigen::Matrix3d rotationToRsw =
                        reference_frames::getInertialToRswSatelliteCenteredFrameRotationMatrix( stateHistory.at( it.first ) );
            ...
            else
            {
                ...
                Eigen::MatrixXd manualCovariance =
                        fullStateTransition * estimationOutput->getUnnormalizedCovarianceMatrix( ) * fullStateTransition.transpose( );
