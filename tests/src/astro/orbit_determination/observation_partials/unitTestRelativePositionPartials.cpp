...
BOOST_AUTO_TEST_CASE( testRelativePositionStatePartials )
{
    ...
    std::shared_ptr< ObservationModel< 3 > > relativePositionModel =
            observation_models::ObservationModelCreator< 3, double, double >::createObservationModel(
                    std::make_shared< observation_models::ObservationModelSettings >(
                            observation_models::relative_position_observable,
                            linkEnds,
    ...
    std::pair< std::map< std::pair< int, int >, std::shared_ptr< ObservationPartial< 3 > > >, std::shared_ptr< PositionPartialScaling > >
            analyticalPartialSet = ObservationPartialCreator< 3, double, double >::createObservationPartials(
    ...
    for( LinkEnds::const_iterator linkEndIterator = linkEnds.begin( ); linkEndIterator != linkEnds.end( ); linkEndIterator++ )
    {
        ...
        for( typename std::map< std::pair< int, int >, std::shared_ptr< ObservationPartial< 3 > > >::const_iterator partialIterator =
                     analyticalPartialSet.first.begin( );
             partialIterator != analyticalPartialSet.first.end( );
             partialIterator++ )
        {
            partialList.push_back( partialIterator->second->calculatePartial(
                    vectorOfStates, vectorOfTimes, linkEndIterator->first, nullptr, currentObservation ) );
        }
