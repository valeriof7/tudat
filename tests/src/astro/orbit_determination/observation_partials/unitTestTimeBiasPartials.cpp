...
// int main( )
{
    ...
    //    for( int stepIndex = -2; stepIndex < 2; stepIndex++ )
    {
        ...
        bodies.at( "Earth" )
                ...
                ->setTransmittingFrequencyCalculator( std::make_shared< ConstantFrequencyInterpolator >( 7E9 ) );
        bodies.at( "Earth" )
        ...
        std::shared_ptr< IntegratorSettings< double > > integratorSettings =
                rungeKuttaFixedStepSettings( 120.0, CoefficientSets::rungeKuttaFehlberg78 );
        ...
        for( unsigned int observableCase = 0; observableCase < 11; observableCase++ )
        {
            ...
            for( unsigned int testCase = 0; testCase < 2; testCase++ )
            {
                ...
                parameterNames.push_back( std::make_shared< InitialTranslationalStateEstimatableParameterSettings< double > >(
                        "Vehicle", initialState1, "Earth" ) );
                parameterNames.push_back( std::make_shared< InitialTranslationalStateEstimatableParameterSettings< double > >(
                        "Vehicle2", initialState2, "Earth" ) );
                ...
                for( int parameterIndex = 12; parameterIndex < parametersToEstimate->getParameterSetSize( ); parameterIndex++ )
                {
                    ...
                    //                    std::cout<<"Relative error "<<( numericalPartials - analyticalPartials ).cwiseQuotient(
                    //                    analyticalPartials ).transpose( )<<std::endl;
                }
            }
        }
        std::cout << std::endl;
