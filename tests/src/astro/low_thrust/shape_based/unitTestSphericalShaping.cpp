...
BOOST_AUTO_TEST_CASE( test_spherical_shaping_earth_mars_transfer )
{
    ...
    Eigen::Vector6d finalState = pointerToArrivalBodyEphemeris->getCartesianState( julianDate + timeOfFlight );
    ...
    for( unsigned int creationType = 0; creationType < 2; creationType++ )
    {
        if( creationType == 0 )
        {
            sphericalShapingLegPointer = new SphericalShapingLeg( pointerToDepartureBodyEphemeris,
...
BOOST_AUTO_TEST_CASE( test_spherical_shaping_earth_1989ML_transfer )
{
    ...
    Eigen::Vector6d finalState = ( Eigen::Vector6d( ) << 1.197701029846094E+00 * physical_constants::ASTRONOMICAL_UNIT,
                                   ...
                                   -2.980245580772588E-04 * physical_constants::ASTRONOMICAL_UNIT / physical_constants::JULIAN_DAY )
                                         .finished( );
...
BOOST_AUTO_TEST_CASE( test_spherical_shaping_earth_mars_transfer_multi_revolutions )
{
    ...
    for( unsigned int currentTestCase = 0; currentTestCase < numberOfRevolutionsVector.size( ); currentTestCase++ )
    {
        ...
        Eigen::Vector6d finalState =
                pointerToArrivalBodyEphemeris->getCartesianState( julianDate + timeOfFlightVector.at( currentTestCase ) );

        // Compute shaped trajectory.
        SphericalShapingLeg sphericalShapingLeg = SphericalShapingLeg( pointerToDepartureBodyEphemeris,
                                                                       pointerToArrivalBodyEphemeris,
                                                                       spice_interface::getBodyGravitationalParameter( "Sun" ),
                                                                       rootFinderSettings,
                                                                       freeParameterLowerBoundVector.at( currentTestCase ),
                                                                       freeParameterUpperBoundVector.at( currentTestCase ) );
        sphericalShapingLeg.updateLegParameters( ( Eigen::Vector3d( ) << julianDate,
        ...
        BOOST_CHECK_CLOSE_FRACTION(
                statesAlongTrajectory.rbegin( )->first, julianDate + timeOfFlightVector.at( currentTestCase ), 1.0E-14 );
...
//
//     // Define root finder settings (used to update the updated value of the free coefficient, so that it matches the required time of
//     flight). std::shared_ptr< RootFinderSettings > rootFinderSettings =
//             tudat::root_finders::bisectionRootFinderSettings( 1.0E-6, TUDAT_NAN, TUDAT_NAN, 30 );
//
//     // Compute shaped trajectory.
...
//    for ( int i = 0 ; i < 6 ; i++ )
//    {
////        BOOST_CHECK_SMALL( std::fabs( shapingMethodResults[ itr->first ][ i ] - itr->second[ i ] ) / shapingMethodResults[ itr->first ][
/// i ] , 1.0e-6 );
//    }
...
//    for ( int i = 0 ; i < 6 ; i++ )
//    {
//        BOOST_CHECK_SMALL( std::fabs( shapingMethodResults[ itr->first ][ i ] - itr->second[ i ] ) / shapingMethodResults[ itr->first ][ i
//        ] , 1.0e-6 );
//    }
...
//        Eigen::Vector3d thrustAccelerationVector = sphericalShaping.computeCurrentThrustAccelerationFromAzimuth( itr->first,
//        specificImpulseFunction, integratorSettings ); Eigen::Vector3d thrustVector = sphericalShaping.computeCurrentThrustForce(
//        itr->first, specificImpulseFunction, integratorSettings ); double mass = sphericalShaping.computeCurrentMass( itr->first,
