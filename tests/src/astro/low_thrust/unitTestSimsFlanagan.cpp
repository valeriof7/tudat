...
BOOST_AUTO_TEST_CASE( test_Sims_Flanagan_limit_case )
{
    ...
    SimsFlanaganModel simsFlanaganModel = SimsFlanaganModel( stateAtDeparture,
                                                             stateAtArrival,
    ...
    for( int i = 0; i < 3; i++ )
    {
        BOOST_CHECK_SMALL( std::fabs( simsFlanaganForwardPropagation[ i ] - keplerianForwardPropagation[ i ] ) /
                                   keplerianForwardPropagation.segment( 0, 3 ).norm( ),
                           1.0e-14 );
        BOOST_CHECK_SMALL( std::fabs( simsFlanaganForwardPropagation[ i + 3 ] - keplerianForwardPropagation[ i + 3 ] ) /
                                   keplerianForwardPropagation.segment( 3, 3 ).norm( ),
                           1.0e-14 );
        BOOST_CHECK_SMALL( std::fabs( simsFlanaganBackwardPropagation[ i ] - keplerianBackwardPropagation[ i ] ) /
                                   keplerianBackwardPropagation.segment( 0, 3 ).norm( ),
                           1.0e-14 );
        BOOST_CHECK_SMALL( std::fabs( simsFlanaganBackwardPropagation[ i + 3 ] - keplerianBackwardPropagation[ i + 3 ] ) /
                                   keplerianBackwardPropagation.segment( 3, 3 ).norm( ),
                           1.0e-14 );
    }
}
...
BOOST_AUTO_TEST_CASE( test_Sims_Flanagan_implementation )
{
    ...
    for( int i = 0; i < numberSegmentsForwardPropagation; i++ )
    {
        currentState = simsFlanaganModel.propagateInsideForwardSegment(
                timesAtNodes[ i ], timesAtNodes[ i + 1 ], segmentDurationForwardPropagation, currentState );
    }
    ...
    {
        currentState = simsFlanaganModel.propagateInsideBackwardSegment(
                timesAtNodes[ i ], timesAtNodes[ i - 1 ], segmentDurationBackwardPropagation, currentState );
    }
...
BOOST_AUTO_TEST_CASE( test_Sims_Flanagan_pykep )
{
    ...
    SimsFlanaganModel simsFlanaganModel = SimsFlanaganModel( stateAtDeparture,
                                                             stateAtArrival,
                                                             centralBodyGravitionalParameter,
                                                             vehicleInitialMass,
                                                             maximumThrust,
                                                             specificImpulseFunction,
                                                             timeOfFlight,
    ...
    std::vector< double > expectedMassesAtNodes = { 2776.2969549424233, 2745.1123152911364, 2678.420656242383, 2557.4910155457815,
                                                    2379.739411828715,  2156.689164020016,  1909.379054895334, 1661.2975125972175,
                                                    1431.9008589844336, 1233.184537707585,  1069.592792583044, 939.9855510907765,
                                                    840.1487300900216,  764.8194149185431,  708.840224065782,  667.6603951232578,
                                                    637.615332355369,   615.9793835860186,  600.4192020709157 };
    ...
    for( int i = 1; i < numberSegmentsBackwardPropagation; i++ )
    {
        Eigen::Vector3d calculatedPosition =
                simsFlanaganTrajectory[ timesAtNodes[ i - 1 + numberSegmentsForwardPropagation ] ].segment( 0, 3 );
        Eigen::Vector3d calculatedVelocity =
                simsFlanaganTrajectory[ timesAtNodes[ i - 1 + numberSegmentsForwardPropagation ] ].segment( 3, 3 );
        ...
        BOOST_CHECK_SMALL( std::fabs( simsFlanaganModel.getMassAtSegment( i + numberSegmentsForwardPropagation ) -
                                      expectedMassesAtNodes[ i - 1 + numberSegmentsForwardPropagation ] ),
                           1.0e-15 );
    }
}
...
BOOST_AUTO_TEST_CASE( test_Sims_Flanagan_impulsive_shots )
{
    ...
    Eigen::Vector6d stateAtHalfTimeOfFlightBackwardPropagation;
    {
        ...
        // Create sims-flanagan object
        double centralBodyGravitionalParameter = spice_interface::getBodyGravitationalParameter( "Sun" );
        SimsFlanagan simsFlanagan = SimsFlanagan( stateAtDeparture,
                                                  stateAtArrival,
                                                  centralBodyGravitionalParameter,
                                                  vehicleInitialMass,
                                                  maximumThrust,
        ...
        for( int i = 0; i < numberSegments; i++ )
        {
            bestThrottles.push_back(
                    ( Eigen::Vector3d( ) << bestIndividual[ i * 3 ], bestIndividual[ i * 3 + 1 ], bestIndividual[ i * 3 + 2 ] )
                            .finished( ) );
        }
    ...
    for( int i = 0; i < numberSegmentsBackwardPropagation; i++ )
    {
        Eigen::Vector3d currentDeltaVvector =
                maximumThrust * bestThrottles[ i + numberSegmentsForwardPropagation ] * segmentDurationBackwardPropagation / currentMass;
        deltaVs.push_back( currentDeltaVvector );
        currentMass *= std::exp( -currentDeltaVvector.norm( ) /
                                 ( specificImpulseFunction( 0.0 ) * physical_constants::SEA_LEVEL_GRAVITATIONAL_ACCELERATION ) );
    }
    ...
    bodyToPropagateAccelerations[ bodyToPropagate ].push_back( std::make_shared< MomentumWheelDesaturationAccelerationSettings >(
    ...
    // Propagate orbit forward and backward
    {
        ...
        std::shared_ptr< TranslationalStatePropagatorSettings< double > > propagatorSettingsImpulsiveDeltaV =
                std::make_shared< TranslationalStatePropagatorSettings< double > >( std::vector< std::string >{ centralBody },
        ...
        for( int i = 0; i < numberSegmentsForwardPropagation; i++ )
        {
            ...
            terminationSettings =
                    std::make_shared< PropagationTimeTerminationSettings >( ( i + 1 ) * segmentDurationForwardPropagation, true );
            ...
            for( int i = 0; i < 3; i++ )
            {
                BOOST_CHECK_SMALL( std::fabs( currentStateSimsFlanagan[ i ] - currentStateImpulsiveShots[ i ] ) /
                                           currentStateImpulsiveShots.segment( 0, 3 ).norm( ),
                                   1.0e-6 );
                BOOST_CHECK_SMALL( std::fabs( currentStateSimsFlanagan[ i + 3 ] - currentStateImpulsiveShots[ i + 3 ] ) /
                                           currentStateImpulsiveShots.segment( 3, 3 ).norm( ),
                                   1.0e-6 );
            }
        }
        ...
        propagatorSettingsImpulsiveDeltaV =
                std::make_shared< TranslationalStatePropagatorSettings< double > >( std::vector< std::string >{ centralBody },
                                                                                    accelerationModelMap,
                                                                                    std::vector< std::string >{ bodyToPropagate },
                                                                                    stateAtArrival,
        ...
        for( int i = 0; i < numberSegmentsBackwardPropagation; i++ )
        {
            ...
            terminationSettings = std::make_shared< PropagationTimeTerminationSettings >(
                    timeOfFlight - ( i + 1 ) * segmentDurationBackwardPropagation, true );
            propagatorSettingsImpulsiveDeltaV->resetTerminationSettings( terminationSettings );
            propagatorSettingsImpulsiveDeltaV->resetInitialStates( currentState );
...
BOOST_AUTO_TEST_CASE( test_Sims_Flanagan_full_propagation )
{
    ...
    // Define (constant) specific impulse function.
    std::function< double( const double ) > specificImpulseFunction = [ = ]( const double currentTime ) { return specificImpulse; };
    ...
    {
        bestThrottles.push_back(
                ( Eigen::Vector3d( ) << bestIndividual[ i * 3 ], bestIndividual[ i * 3 + 1 ], bestIndividual[ i * 3 + 2 ] ).finished( ) );
    }
    ...
    std::pair< std::shared_ptr< PropagatorSettings< double > >, std::shared_ptr< PropagatorSettings< double > > > propagatorSettings =
            simsFlanagan.createLowThrustPropagatorSettings( bodies,
                                                            bodyToPropagate,
                                                            centralBody,
                                                            specificImpulseFunction,
                                                            perturbingAccelerationsMap,
                                                            integratorSettings,
    ...
    simsFlanagan.computeSemiAnalyticalAndFullPropagation(
            bodies, integratorSettings, propagatorSettings, fullPropagationResults, simsFlanaganResults, dependentVariablesHistory );
    ...
    for( int i = 0; i < 3; i++ )
    {
        BOOST_CHECK_SMALL( ( std::fabs( stateAtDepartureSimsFlanagan[ i ] - stateAtDepartureFullPropagation[ i ] ) /
                             stateAtDeparture.segment( 0, 3 ).norm( ) ),
                           1.0e-3 );
        BOOST_CHECK_SMALL( ( std::fabs( stateAtDepartureSimsFlanagan[ i + 3 ] - stateAtDepartureFullPropagation[ i + 3 ] ) /
                             stateAtDeparture.segment( 3, 3 ).norm( ) ),
                           1.0e-2 );
        BOOST_CHECK_SMALL( ( std::fabs( stateAtArrivalSimsFlanagan[ i ] - stateAtArrivalFullPropagation[ i ] ) /
                             stateAtArrival.segment( 0, 3 ).norm( ) ),
                           1.0e-3 );
        BOOST_CHECK_SMALL( ( std::fabs( stateAtArrivalSimsFlanagan[ i + 3 ] - stateAtArrivalFullPropagation[ i + 3 ] ) /
                             stateAtArrival.segment( 3, 3 ).norm( ) ),
                           1.0e-2 );
    }
    ...
    basic_astrodynamics::AccelerationMap lowThrustTrajectoryAccelerations = simsFlanagan.retrieveLowThrustAccelerationMap(
    ...
    massRateModels[ bodyToPropagate ] = createMassRateModel(
            bodyToPropagate, std::make_shared< FromThrustMassModelSettings >( 1 ), bodies, lowThrustTrajectoryAccelerations );

    // Define list of dependent variables to save.
    dependentVariablesList.push_back(
            std::make_shared< SingleDependentVariableSaveSettings >( total_mass_rate_dependent_variables, bodyToPropagate ) );
    ...
    for( std::map< double, Eigen::Vector6d >::iterator itr = trajectory.begin( ); itr != trajectory.end( ); itr++ )
    {
        ...
        std::shared_ptr< TranslationalStatePropagatorSettings< double > > translationalStatePropagatorSettings =
                std::make_shared< TranslationalStatePropagatorSettings< double > >( std::vector< std::string >{ centralBody },
                                                                                    lowThrustTrajectoryAccelerations,
                                                                                    std::vector< std::string >{ bodyToPropagate },
                                                                                    stateAtDeparture,
                                                                                    terminationSettings,
        ...
        std::shared_ptr< MassPropagatorSettings< double > > massPropagatorSettings = std::make_shared< MassPropagatorSettings< double > >(
                std::vector< std::string >{ bodyToPropagate },
                massRateModels,
                ( Eigen::Matrix< double, 1, 1 >( ) << vehicleInitialMass ).finished( ),
                terminationSettings );
        ...
        Eigen::Vector3d calculatedThrust =
                simsFlanagan.computeCurrentThrustForce( currentEpoch, specificImpulseFunction, integratorSettings ).transpose( );
        Eigen::Vector3d calculatedThrustAcceleration =
                simsFlanagan.computeCurrentThrustAcceleration( currentEpoch, specificImpulseFunction, integratorSettings );

        for( int i = 0; i < 3; i++ )
        {
            // Check consistency between output of getTrajectory function and of computeCurrentStateVector.
            BOOST_CHECK_SMALL(
                    ( std::fabs( trajectory[ itr->first ][ i ] - calculatedState[ i ] ) / calculatedState.segment( 0, 3 ).norm( ) ),
                    1.0e-6 );
            BOOST_CHECK_SMALL(
                    ( std::fabs( trajectory[ itr->first ][ i + 3 ] - calculatedState[ i + 3 ] ) / calculatedState.segment( 3, 3 ).norm( ) ),
                    1.0e-6 );
            ...
            BOOST_CHECK_SMALL( ( std::fabs( thrustAccelerationProfile[ itr->first ][ i ] - currentExpectedThrustAcceleration[ i ] ) ),
                               1.0e-7 );
...
BOOST_AUTO_TEST_CASE( test_Sims_Flanagan_Shape_Based )
{
    ...
    std::shared_ptr< HodographicShaping > hodographicShaping =
            std::make_shared< HodographicShaping >( cartesianStateDepartureBody,
    ...
    std::function< Eigen::Vector3d( const double ) > initialGuessThrustFromShaping = getInitialGuessFunctionFromShaping(
            hodographicShaping, numberSegments, timeOfFlight, specificImpulseFunction, integratorSettings );

    std::vector< double > initialGuessVector = convertToSimsFlanaganThrustModel( initialGuessThrustFromShaping,
                                                                                 maximumThrust,
                                                                                 timeOfFlight,
                                                                                 numberSegmentsForwardPropagation,
                                                                                 numberSegmentsBackwardPropagation );
    ...
    std::shared_ptr< OptimisationSettings > optimisationSettings =
            std::make_shared< OptimisationSettings >( optimisationAlgorithm, 1, 10, 1.0e-6, std::make_pair( initialGuessVector, 0.002 ) );

    // Create Sims-Flanagan trajectory.
    SimsFlanagan simsFlanagan = SimsFlanagan( cartesianStateDepartureBody,
                                              cartesianStateArrivalBody,
                                              centralBodyGravitationalParameter,
                                              vehicleInitialMass,
                                              maximumThrust,
    ...
    for( std::map< double, Eigen::Vector6d >::iterator itr = simsFlanaganTrajectory.begin( ); itr != simsFlanaganTrajectory.end( ); itr++ )
    {
        ...
        for( int i = 0; i < 3; i++ )
        {
            BOOST_CHECK_SMALL( ( std::fabs( simsFlanaganTrajectory[ itr->first ][ i ] - hodographicShapingTrajectory[ itr->first ][ i ] ) /
                                 hodographicShapingTrajectory[ itr->first ].segment( 0, 3 ).norm( ) ),
                               5.0e-2 );
            BOOST_CHECK_SMALL(
                    ( std::fabs( simsFlanaganTrajectory[ itr->first ][ i + 3 ] - hodographicShapingTrajectory[ itr->first ][ i + 3 ] ) /
                      hodographicShapingTrajectory[ itr->first ].segment( 3, 3 ).norm( ) ),
                    1.0e-1 );
        }
    }
    ...
    for( int i = 0; i < numberSegmentsBackwardPropagation; i++ )
    {
        ...
        Eigen::Vector3d initialGuessThrust =
                initialGuessThrustFromShaping( timeOfFlight / 2.0 + ( i + 0.5 ) * segmentDurationBackwardPropagation );
        for( int j = 0; j < 3; j++ )
        {
            BOOST_CHECK_SMALL( std::fabs( ( simsFlanaganThrust[ j ] - initialGuessThrust[ j ] ) / initialGuessThrust[ j ] ), 5.0e-3 );
        }
