...
BOOST_AUTO_TEST_CASE( test_hybrid_method_implementation )
{
    ...
    Eigen::Vector6d initialKeplerianElements =
            ( Eigen::Vector6d( ) << 24505.9e3, 0.725, 7.0 * mathematical_constants::PI / 180.0, 0.0, 0.0, 0.0 ).finished( );
    Eigen::Vector6d finalKeplerianElements =
            ( Eigen::Vector6d( ) << 42164.65e3, 5.53e-4, 7.41e-5 * mathematical_constants::PI / 180.0, 0.0, 0.0, 0.0 ).finished( );
    ...
    HybridMethod hybridMethod = HybridMethod( stateAtDeparture,
                                              stateAtArrival,
                                              maximumThrust,
                                              specificImpulse,
                                              timeOfFlight,
    ...
    Eigen::VectorXd bestInitialMEEcostates;
    bestInitialMEEcostates.resize( 5 );
    Eigen::VectorXd bestFinalMEEcostates;
    bestFinalMEEcostates.resize( 5 );
    for( int i = 0; i < 5; i++ )
    {
    ...
    for( std::map< double, Eigen::Vector6d >::iterator itr = trajectory.begin( ); itr != trajectory.end( ); itr++ )
    {
        ...
        std::shared_ptr< propagators::MassPropagatorSettings< double > > massPropagatorSettings =
                std::make_shared< propagators::MassPropagatorSettings< double > >(
                        std::vector< std::string >{ bodyToPropagate },
                        massRateModels,
                        ( Eigen::Matrix< double, 1, 1 >( ) << currentMass ).finished( ),
                        terminationSettings );
        ...
        for( int i = 0; i < 3; i++ )
        {
            BOOST_CHECK_SMALL( ( std::fabs( trajectory[ itr->first ][ i ] - currentExpectedState[ i ] ) /
                                 currentExpectedState.segment( 0, 3 ).norm( ) ),
                               1.0e-12 );
            BOOST_CHECK_SMALL( ( std::fabs( trajectory[ itr->first ][ i + 3 ] - currentExpectedState[ i + 3 ] ) /
                                 currentExpectedState.segment( 3, 3 ).norm( ) ),
                               1.0e-12 );
    ...
    std::pair< std::shared_ptr< propagators::PropagatorSettings< double > >, std::shared_ptr< propagators::PropagatorSettings< double > > >
            propagatorSettings = hybridMethod.createLowThrustPropagatorSettings(
                    specificImpulseFunction, perturbingAccelerationsMap, integratorSettings, dependentVariablesToSave );

    // Compute full propagation.
    hybridMethod.computeSemiAnalyticalAndFullPropagation(
            integratorSettings, propagatorSettings, fullPropagationResults, hybridMethodResults, dependentVariablesHistory );

    for( int i = 0; i < 3; i++ )
    {
        BOOST_CHECK_SMALL( ( std::fabs( hybridMethodResults.begin( )->second[ i ] - fullPropagationResults.begin( )->second[ i ] ) /
                             stateAtDeparture.segment( 0, 3 ).norm( ) ),
                           2.0e-6 );
        BOOST_CHECK_SMALL( ( std::fabs( hybridMethodResults.begin( )->second[ i + 3 ] - fullPropagationResults.begin( )->second[ i + 3 ] ) /
                             stateAtDeparture.segment( 3, 3 ).norm( ) ),
                           2.0e-6 );

        BOOST_CHECK_SMALL( ( std::fabs( hybridMethodResults.rbegin( )->second[ i ] - fullPropagationResults.rbegin( )->second[ i ] ) /
                             stateAtArrival.segment( 0, 3 ).norm( ) ),
                           2.0e-6 );
        BOOST_CHECK_SMALL(
                ( std::fabs( hybridMethodResults.rbegin( )->second[ i + 3 ] - fullPropagationResults.rbegin( )->second[ i + 3 ] ) /
