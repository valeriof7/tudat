...
BOOST_AUTO_TEST_SUITE( test_unified_state_model_propagator )

// Test USM propagator for point mass central body.
BOOST_AUTO_TEST_CASE( testUnifiedStateModelPopagatorForPointMassCentralBodies )
{
    for( int propagatorType = 0; propagatorType < 3; propagatorType++ )
    {
        ...
        BodyListSettings bodySettings =
                getDefaultBodySettings( bodyNames, initialEphemerisTime - buffer, finalEphemerisTime + buffer, "SSB", "J2000" );
        ...
        Eigen::Vector6d earthInitialKeplerElements =
                convertCartesianToKeplerianElements( Eigen::Vector6d( systemInitialState.segment( 0, 6 ) ),
        ...
        systemInitialState.segment( 0, 6 ) =
                convertKeplerianToCartesianElements( earthInitialKeplerElements,
                                                     bodies.at( "Earth" )->getGravityFieldModel( )->getGravitationalParameter( ) +
                                                             bodies.at( "Sun" )->getGravityFieldModel( )->getGravitationalParameter( ) );
        ...
        std::map< double, Eigen::Matrix< double, 18, 1 > > usmIntegrationResults;
        while( currentTestTime < finalTestTime )
        {
            usmIntegrationResults[ currentTestTime ].segment( 0, 6 ) =
                    bodies.at( "Earth" )->getStateInBaseFrameFromEphemeris( currentTestTime );
            usmIntegrationResults[ currentTestTime ].segment( 6, 6 ) =
                    bodies.at( "Mars" )->getStateInBaseFrameFromEphemeris( currentTestTime );
            usmIntegrationResults[ currentTestTime ].segment( 12, 6 ) =
                    bodies.at( "Venus" )->getStateInBaseFrameFromEphemeris( currentTestTime );
            currentTestTime += testTimeStep;
        }

        // Compare results of Cowell and USM propagations
        std::map< double, Eigen::Matrix< double, 18, 1 > >::iterator usmIterator = usmIntegrationResults.begin( );
        ...
        for( unsigned int i = 0; i < usmIntegrationResults.size( ); i++ )
        {
            ...
            for( int j = 12; j < 15; j++ )
            {
                BOOST_CHECK_SMALL( ( usmIterator->second - cowellIterator->second ).segment( j, 1 )( 0 ), 0.075 );
            }

            for( int j = 3; j < 6; j++ )
            {
                BOOST_CHECK_SMALL( ( usmIterator->second - cowellIterator->second ).segment( j, 1 )( 0 ), 1.0E-8 );
            }
            ...
            for( int j = 15; j < 18; j++ )
            {
                BOOST_CHECK_SMALL( ( usmIterator->second - cowellIterator->second ).segment( j, 1 )( 0 ), 1.0E-6 );
            }
            usmIterator++;
            cowellIterator++;
        }
...
// Test USM propagator for point mass, and spherical harmonics central body.
BOOST_AUTO_TEST_CASE( testUnifiedStateModelPopagatorForSphericalHarmonicCentralBodies )
{
    ...
    for( int propagatorType = 0; propagatorType < 3; propagatorType++ )
    {
        ...
        for( unsigned int simulationCase = 0; simulationCase < 6; simulationCase++ )
        {
            ...
            if( simulationCase >= 4 )
            {
                ...
                verticesDefiningEachFacet << 2, 1, 0, 1, 2, 3, 4, 2, 0, 2, 4, 6, 1, 4, 0, 4, 1, 5, 6, 5, 7, 5, 6, 4, 3, 6, 7, 6, 3, 2, 5, 3,
                        7, 3, 5, 1;
                ...
                bodySettings.at( "Earth" )->gravityFieldSettings =
                        polyhedronGravitySettingsFromMu( celestial_body_constants::EARTH_GRAVITATIONAL_PARAMETER,
                                                         verticesCoordinates,
            ...
            currentTestTime = initialTestTime;
            std::map< double, Eigen::Matrix< double, 6, 1 > > usmIntegrationResults;
            while( currentTestTime < finalTestTime )
            {
                usmIntegrationResults[ currentTestTime ].segment( 0, 6 ) =
                        bodies.at( "Vehicle" )->getEphemeris( )->getCartesianState( currentTestTime );
            ...
            // Compare results of Cowell and USM propagations
            std::map< double, Eigen::Matrix< double, 6, 1 > >::iterator usmIterator = usmIntegrationResults.begin( );
            ...
            for( unsigned int i = 0; i < usmIntegrationResults.size( ); i++ )
            {
                ...
                for( int j = 3; j < 6; j++ )
                {
                    BOOST_CHECK_SMALL( ( usmIterator->second - cowellIterator->second )( j ), 1.0E-5 );
                }
                usmIterator++;
                cowellIterator++;
            }
