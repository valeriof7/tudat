...
BOOST_AUTO_TEST_CASE( testFullPropagationMGA )
{
    ...
    gravitationalParametersTransferBodies.push_back(
            simulation_setup::createGravityFieldModel( simulation_setup::getDefaultGravityFieldSettings( "Earth", TUDAT_NAN, TUDAT_NAN ),
                                                       "Earth" )
                    ->getGravitationalParameter( ) );
    gravitationalParametersTransferBodies.push_back(
            simulation_setup::createGravityFieldModel( simulation_setup::getDefaultGravityFieldSettings( "Venus", TUDAT_NAN, TUDAT_NAN ),
                                                       "Venus" )
                    ->getGravitationalParameter( ) );
    gravitationalParametersTransferBodies.push_back(
            simulation_setup::createGravityFieldModel( simulation_setup::getDefaultGravityFieldSettings( "Venus", TUDAT_NAN, TUDAT_NAN ),
    ...
    gravitationalParametersTransferBodies.push_back(
            simulation_setup::createGravityFieldModel( simulation_setup::getDefaultGravityFieldSettings( "Earth", TUDAT_NAN, TUDAT_NAN ),
                    ...
                    ->getGravitationalParameter( ) );
    gravitationalParametersTransferBodies.push_back(
            simulation_setup::createGravityFieldModel( simulation_setup::getDefaultGravityFieldSettings( "Jupiter", TUDAT_NAN, TUDAT_NAN ),
                                                       "Jupiter" )
                    ->getGravitationalParameter( ) );
    gravitationalParametersTransferBodies.push_back(
            simulation_setup::createGravityFieldModel( simulation_setup::getDefaultGravityFieldSettings( "Saturn", TUDAT_NAN, TUDAT_NAN ),
                                                       "Saturn" )
                    ->getGravitationalParameter( ) );

    // Create variable vector.
    std::vector< double > variableVector;
    variableVector.push_back( -789.8117 * physical_constants::JULIAN_DAY );
    variableVector.push_back( 158.302027105278 * physical_constants::JULIAN_DAY );
    variableVector.push_back( 449.385873819743 * physical_constants::JULIAN_DAY );
    variableVector.push_back( 54.7489684339665 * physical_constants::JULIAN_DAY );
    variableVector.push_back( 1024.36205846918 * physical_constants::JULIAN_DAY );
    variableVector.push_back( 4552.30796805542 * physical_constants::JULIAN_DAY );
    ...
    simulation_setup::SystemOfBodies bodies =
            propagators::setupBodyMapFromUserDefinedEphemeridesForPatchedConicsTrajectory( centralBody[ 0 ],
                                                                                           bodyToPropagate,
                                                                                           nameBodiesTrajectory,
                                                                                           ephemerisVectorTransferBodies,
                                                                                           gravitationalParametersTransferBodies,
                                                                                           "ECLIPJ2000" );
    ...
    semiMajorAxes.push_back( std::numeric_limits< double >::infinity( ) );
    semiMajorAxes.push_back( 1.0895e8 / 0.02 );
    std::vector< double > eccentricities;
    eccentricities.push_back( 0.0 );
    eccentricities.push_back( 0.98 );

    // Create minimum pericenter radii vector
    std::vector< double > minimumPericenterRadii;
    minimumPericenterRadii.push_back( 6778000.0 );
    minimumPericenterRadii.push_back( 6351800.0 );
    minimumPericenterRadii.push_back( 6351800.0 );
    minimumPericenterRadii.push_back( 6778000.0 );
    minimumPericenterRadii.push_back( 600000000.0 );
    minimumPericenterRadii.push_back( 600000000.0 );
    ...
    for( int terminationType = 0; terminationType < 2; terminationType++ )
    {
        ...
        propagators::fullPropagationPatchedConicsTrajectory( bodies,
                                                             accelerationMap,
                                                             nameBodiesTrajectory,
                                                             centralBody[ 0 ],
                                                             bodyToPropagate,
        ...
        {
            double legTimeDifference = nextLegIterator->second.begin( )->first - legIterator->second.rbegin( )->first;
            Eigen::Vector6d legStateDifference = nextLegIterator->second.begin( )->second - legIterator->second.rbegin( )->second;
            ...
            //            std::cout<<std::setprecision( 16 )<<"State. "<<( legIterator->second.rbegin( )->second ).transpose( )<<std::endl;
            //            //Wrong std::cout<<std::setprecision( 16 )<<"State. "<<( nextLegIterator->second.begin( )->second ).transpose(
            //            )<<std::endl; //Correct std::cout<<std::setprecision( 16 )<<"Venus state. "<<ephemerisVenus->getCartesianState(
            //            nextLegIterator->second.begin( )->first ).transpose( )<<std::endl<<std::endl;
...
BOOST_AUTO_TEST_CASE( testFullPropagationMGAwithDSM )
{
    ...
    // Create system of bodies.
    simulation_setup::SystemOfBodies bodies =
            propagators::setupBodyMapFromEphemeridesForPatchedConicsTrajectory( centralBody[ 0 ], bodyToPropagate, transferBodyTrajectory );
    ...
    // Create minimum pericenter radii vector
    std::vector< double > minimumPericenterRadii;
    minimumPericenterRadii.push_back( TUDAT_NAN );
    minimumPericenterRadii.push_back( TUDAT_NAN );
    minimumPericenterRadii.push_back( TUDAT_NAN );
    minimumPericenterRadii.push_back( TUDAT_NAN );
    minimumPericenterRadii.push_back( TUDAT_NAN );
    ...
    for( int terminationType = 0; terminationType < 2; terminationType++ )
    {
        ...
        tudat::propagators::fullPropagationPatchedConicsTrajectory( bodies,
                                                                    accelerationMap,
