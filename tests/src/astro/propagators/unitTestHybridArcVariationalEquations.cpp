        ...
        const double arcOverlap = 5.0E3 )
{
    ...
    AccelerationMap singleArcAccelerationModelMap =
            createAccelerationModelsMap( bodies, singleArcAccelerationMap, singleArcBodiesToIntegrate, singleArcCentralBodies );
    ...
    AccelerationMap multiArcAccelerationModelMap =
            createAccelerationModelsMap( bodies, multiArcAccelerationMap, multiArcBodiesToIntegrate, multiArcCentralBodies );
    ...
    if( forcedMultiArcInitialStates.size( ) == 0 )
    {
        for( unsigned int j = 0; j < numberOfIntegrationArcs; j++ )
        {
            ...
            orbiterInitialStateInKeplerianElements( argumentOfPeriapsisIndex ) = convertDegreesToRadians( 235.7 - j );
            orbiterInitialStateInKeplerianElements( longitudeOfAscendingNodeIndex ) = convertDegreesToRadians( 23.4 + j );
            ...
            multiArcSystemInitialStates[ j ] =
                    convertKeplerianToCartesianElements( orbiterInitialStateInKeplerianElements, marsGravitationalParameter ) +
                    initialStateDifference.segment( singleArcInitialStates.rows( ), 6 );
        }
    }
    else
    {
        ...
        {
            multiArcSystemInitialStates[ j ] =
                    forcedMultiArcInitialStates.at( j ) + initialStateDifference.segment( singleArcInitialStates.rows( ), 6 );
        }
    }
    ...
    {
        parameterNames = getInitialStateParameterSettings< double >( hybridArcPropagatorSettings, bodies, integrationArcStarts );
            ...
            results;
    {
        ...
        HybridArcVariationalEquationsSolver< StateScalarType, TimeType > variationalEquations =
                HybridArcVariationalEquationsSolver< StateScalarType, TimeType >(
                        bodies, hybridArcPropagatorSettings, parametersToEstimate );
         ...
         testStates.block( 0, 0, 6, 1 );*/
            ...
            if( propagateVariationalEquations )
            {
                results.first.push_back(
                        variationalEquations.getStateTransitionMatrixInterface( )->getCombinedStateTransitionAndSensitivityMatrix(
                                testEpoch ) );
                ...
                Eigen::MatrixXd testMatrixFull =
                        variationalEquations.getStateTransitionMatrixInterface( )->getFullCombinedStateTransitionAndSensitivityMatrix(
                                testEpoch );

                TUDAT_CHECK_MATRIX_CLOSE_FRACTION( testMatrixDirect.block( 0, 0, 12, 6 ),
                                                   testMatrixFull.block( 0, 0, 12, 6 ),
                                                   std::numeric_limits< double >::epsilon( ) );

                TUDAT_CHECK_MATRIX_CLOSE_FRACTION( testMatrixDirect.block( 0, 6, 12, 6 ),
                                                   testMatrixFull.block( 0, 6 * ( arc + 1 ), 12, 6 ),
                                                   std::numeric_limits< double >::epsilon( ) );

                TUDAT_CHECK_MATRIX_CLOSE_FRACTION( testMatrixDirect.block( 0, 12, 12, 2 ),
...
BOOST_AUTO_TEST_CASE( testMarsAndOrbiterHybridArcVariationalEquationCalculation )
{
    ...
    statePerturbation =
            ( Eigen::Matrix< double, 12, 1 >( ) << 1.0E10, 1.0E10, 1.0E10, 5.0E4, 5.0E4, 10.0E4, 10.0, 10.0, 10.0, 0.1, 0.1, 0.1 )
                    .finished( );
        ...
        for( unsigned int k = 0; k < 1; k++ )
        {
            ...
            for( unsigned int j = 0; j < 12; j++ )
            {
                ...
                upPerturbedState = executeHybridArcMarsAndOrbiterSensitivitySimulation< double, double >(
                                           perturbedState, Eigen::VectorXd::Zero( 2 ), false, false, nominalArcStartStates )
                                           .second;
                ...
                {
                    manualPartial[ arc ].block( 0, j, 12, 1 ) = ( -upPerturbedState[ arc ] + 8.0 * upPerturbedState2[ arc ] -
                                                                  8.0 * downPerturbedState2[ arc ] + downPerturbedState[ arc ] ) /
            ...
            for( unsigned int j = 0; j < 2; j++ )
            {
                ...
                upPerturbedState = executeHybridArcMarsAndOrbiterSensitivitySimulation< double, double >(
                                           perturbedState, perturbedParameter, false, false, nominalArcStartStates )
                                           .second;
                ...
                for( unsigned int arc = 0; arc < upPerturbedState.size( ); arc++ )
                {
                    manualPartial[ arc ].block( 0, j + 12, 12, 1 ) = ( -upPerturbedState[ arc ] + 8.0 * upPerturbedState2[ arc ] -
                                                                       8.0 * downPerturbedState2[ arc ] + downPerturbedState[ arc ] ) /
            ...
            for( unsigned int arc = 0; arc < manualPartial.size( ); arc++ )
            {
                TUDAT_CHECK_MATRIX_CLOSE_FRACTION( stateTransitionAndSensitivityMatrixAtEpoch.at( arc ).block( 0, 0, 6, 6 ),
                                                   manualPartial.at( arc ).block( 0, 0, 6, 6 ),
                                                   5.0E-5 );
                TUDAT_CHECK_MATRIX_CLOSE_FRACTION( stateTransitionAndSensitivityMatrixAtEpoch.at( arc ).block( 6, 6, 6, 6 ),
                                                   manualPartial.at( arc ).block( 6, 6, 6, 6 ),
                                                   5.0E-5 );
                ...
                TUDAT_CHECK_MATRIX_CLOSE_FRACTION( stateTransitionAndSensitivityMatrixAtEpoch.at( arc ).block( 6, 0, 6, 6 ),
                                                   manualPartial.at( arc ).block( 6, 0, 6, 6 ),
                                                   couplingTolerance );
                TUDAT_CHECK_MATRIX_CLOSE_FRACTION( stateTransitionAndSensitivityMatrixAtEpoch.at( arc ).block( 0, 12, 6, 1 ),
                                                   manualPartial.at( arc ).block( 0, 12, 6, 1 ),
                                                   5.0E-5 );
                TUDAT_CHECK_MATRIX_CLOSE_FRACTION( stateTransitionAndSensitivityMatrixAtEpoch.at( arc ).block( 6, 12, 6, 1 ),
                                                   manualPartial.at( arc ).block( 6, 12, 6, 1 ),
                                                   5.0E-3 );
                TUDAT_CHECK_MATRIX_CLOSE_FRACTION( stateTransitionAndSensitivityMatrixAtEpoch.at( arc ).block( 6, 13, 6, 1 ),
                                                   manualPartial.at( arc ).block( 6, 13, 6, 1 ),
                                                   5.0E-5 );

                //                    std::cout<<"Arc: "<<arc<<std::endl<<stateTransitionAndSensitivityMatrixAtEpoch.at( arc ).block( 6, 0,
                //                    6, 6 )<<std::endl<<std::endl<<
                //                               manualPartial.at( arc ).block( 6, 0, 6, 6 )<<std::endl<<std::endl<<
                //                               ( stateTransitionAndSensitivityMatrixAtEpoch.at( arc ) - manualPartial.at( arc ) ).block(
                //                               6, 0, 6, 6 ).cwiseQuotient(
                //                                manualPartial.at( arc ).block( 6, 0, 6, 6 ) )<<std::endl<<std::endl;

                //                std::cout<<"Arc: "<<arc<<std::endl<<stateTransitionAndSensitivityMatrixAtEpoch.at( arc ).block( 0, 12, 12,
                //                2 )<<std::endl<<std::endl<<
                //                           manualPartial.at( arc ).block( 0, 12, 12, 2 )<<std::endl<<std::endl<<
                //                           ( stateTransitionAndSensitivityMatrixAtEpoch.at( arc ) - manualPartial.at( arc ) ).block( 0,
                //                           12, 12, 2 ).cwiseQuotient(
                //                               manualPartial.at( arc ).block( 0, 12, 12, 2 ) )<<std::endl<<std::endl;
            }
        }
    }
...
BOOST_AUTO_TEST_CASE( testVaryingCentralBodyHybridArcVariationalEquations )
{
    ...
    // Get body settings.
    BodyListSettings bodySettings = getDefaultBodySettings( bodyNames, initialTime - 3600.0, finalTime + 3600.0, "Jupiter", "ECLIPJ2000" );
    ...
    bodies.at( "Spacecraft" )
            ->setEphemeris(
                    std::make_shared< MultiArcEphemeris >( std::map< double, std::shared_ptr< Ephemeris > >( ), "Jupiter", "ECLIPJ2000" ) );
    bodies.processBodyFrameDefinitions( );
    ...
    basic_astrodynamics::AccelerationMap singleArcAccelerationModelMap =
            createAccelerationModelsMap( bodies, singleArcAccelerationMap, singleArcBodiesToPropagate, singleArcCentralBodies );
    ...
    std::vector< std::string > multiArcCentralBodies = { "Io", "Ganymede", "Europa", "Ganymede", "Io", "Europa" };

    std::vector< double > arcStartTimes = { 3600.0, 3.0 * 3600, 5.0 * 3600.0, 7.0 * 3600.0, 9.0 * 3600.0, 11.0 * 3600.0 };
    ...
    for( unsigned int i = 0; i < arcStartTimes.size( ); i++ )
    {
        ...
        spacecraftInitialStateInKeplerianElements( inclinationIndex ) =
                unit_conversions::convertDegreesToRadians( static_cast< double >( i * 30 ) );
        spacecraftInitialStateInKeplerianElements( argumentOfPeriapsisIndex ) =
                unit_conversions::convertDegreesToRadians( static_cast< double >( i * 30 ) );
        spacecraftInitialStateInKeplerianElements( longitudeOfAscendingNodeIndex ) =
                unit_conversions::convertDegreesToRadians( static_cast< double >( i * 30 ) );
        spacecraftInitialStateInKeplerianElements( trueAnomalyIndex ) =
                unit_conversions::convertDegreesToRadians( static_cast< double >( i * 30 ) );
        double centralBodyGravitationalParameter =
                bodies.at( multiArcCentralBodies.at( i ) )->getGravityFieldModel( )->getGravitationalParameter( );
        multiArcSystemInitialStates.push_back(
                convertKeplerianToCartesianElements( spacecraftInitialStateInKeplerianElements, centralBodyGravitationalParameter ) );
        ...
        multiArcPropagationSettingsList.push_back( std::make_shared< TranslationalStatePropagatorSettings< double > >(
    ...
    for( unsigned int i = 0; i < singleArcBodiesToPropagate.size( ); i++ )
    {
        parameterNames.push_back(
                std::make_shared< EstimatableParameterSettings >( singleArcBodiesToPropagate.at( i ), gravitational_parameter ) );
    }
    ...
    for( unsigned int i = 0; i < singleArcBodiesToPropagate.size( ); i++ )
    {
        std::shared_ptr< MultiArcPropagatorSettings<> > multiArcPerBodyPropagationSettings =
                std::make_shared< MultiArcPropagatorSettings<> >(
                        multiArcPropagationSettingsListPerCentralBody.at( singleArcBodiesToPropagate.at( i ) ) );
        std::shared_ptr< HybridArcPropagatorSettings<> > hybridArcPerBodyPropagatorSettings =
                std::make_shared< HybridArcPropagatorSettings<> >( singleArcPropagatorSettings, multiArcPerBodyPropagationSettings );
        ...
        for( unsigned int j = 0; j < singleArcBodiesToPropagate.size( ); j++ )
        {
            parameterNamesPerBody.push_back(
                    std::make_shared< EstimatableParameterSettings >( singleArcBodiesToPropagate.at( j ), gravitational_parameter ) );
        }
        ...
        auto perBodyMultiArcVariationalResults =
        ...
        for( unsigned int j = 0; j < perBodyIndicesInFullPropagation.at( singleArcBodiesToPropagate.at( i ) ).size( ); j++ )
        {
            for( unsigned int k = 0; k < 2; k++ )
            {
                ...
                std::map< double, Eigen::MatrixXd > fullMultiArcMatrixHistory = k == 0
                        ? fullMultiArcVariationalResults->getSingleArcResults( ).at( currentArc )->getStateTransitionSolution( )
                ...
                std::map< double, Eigen::MatrixXd > perBodyMultiMatrixHistory = k == 0
                        ? perBodyMultiArcVariationalResults->getSingleArcResults( ).at( j )->getStateTransitionSolution( )
                ...
                for( unsigned int i = 0; i < fullMultiArcMatrixHistory.size( ); i++ )
                {
                    ...
                    TUDAT_CHECK_MATRIX_CLOSE_FRACTION(
                            fullIterator->second, perBodyIterator->second, std::numeric_limits< double >::epsilon( ) );
