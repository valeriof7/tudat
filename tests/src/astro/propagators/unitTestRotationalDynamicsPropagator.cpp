...
RotationalPropagatorType getRotationalPropagator( const int index )
{
    RotationalPropagatorType propagator;
    switch( index )
    {
        case 0:
            propagator = propagators::quaternions;
            break;
        case 1:
            propagator = propagators::modified_rodrigues_parameters;
            break;
        case 2:
            propagator = propagators::exponential_map;
            break;
        default:
            propagator = propagators::undefined_rotational_propagator;
            break;
    }
    return propagator;
}
...
SystemOfBodies getTestBodyMap( const double phobosSemiMajorAxis,
                               ...
                               const bool useMarsTides = 0,
                               const bool offsetElements = 0 )
{
    ...
    bodies.at( "Mars" )->setGravityFieldModel(
            std::make_shared< gravitation::GravityFieldModel >( spice_interface::getBodyGravitationalParameter( "Mars" ) ) );
    ...
    if( useMarsTides )
    {
        gravityFieldVariationSettings.push_back( fixedSingleDegreeLoveNumberGravityFieldVariationSettings( "Mars", 1.0E3, 2 ) );
    }
    addGravityFieldModel( bodies,
                          "Phobos",
                          std::make_shared< SphericalHarmonicsGravityFieldSettings >( phobosGravitationalParameter,
                                                                                      phobosReferenceRadius,
                                                                                      phobosCosineGravityFieldCoefficients,
                                                                                      phobosSineGravityFieldCoefficients,
                                                                                      "Phobos_Fixed",
    ...
    if( offsetElements )
    {
        phobosKeplerElements( 1 ) = 0.01;
        phobosKeplerElements( 2 ) = 0.01;
    }

    bodies.at( "Phobos" )
            ->setEphemeris( std::make_shared< ephemerides::KeplerEphemeris >(
                    phobosKeplerElements, 0.0, spice_interface::getBodyGravitationalParameter( "Mars" ), "Mars", "ECLIPJ2000" ) );
    ...
    for( int propagatorType = 0; propagatorType < 3; propagatorType++ )
    {
        ...
        for( unsigned axisCase = 0; axisCase < 3; axisCase++ )
        {
            ...
            Eigen::Quaterniond initialRotation =
                    reference_frames::getRotatingPlanetocentricToLocalVerticalFrameTransformationQuaternion( 0.2, 0.7 );
            ...
            systemInitialState.segment( 0, 4 ) = linear_algebra::convertQuaternionToVectorFormat( initialRotation );
            ...
            while( currentTime < endTime )
            {
                ...
                Eigen::Matrix3d baseRotationToTargetFrame = Eigen::AngleAxisd( -1.0 * currentAngle, rotationAxis ).toRotationMatrix( );

                currentRotationMatrixToTargetFrame = phobosRotationalEphemeris->getRotationToTargetFrame( currentTime );
                expectedRotationToTargetFrameFromInitialRotation = baseRotationToTargetFrame * initialRotationMatrixToTargetFrame;
                for( unsigned int i = 0; i < 3; i++ )
                {
                    for( unsigned int j = 0; j < 3; j++ )
                    {
                        BOOST_CHECK_SMALL( std::fabs( currentRotationMatrixToTargetFrame( i, j ) -
                                                      expectedRotationToTargetFrameFromInitialRotation( i, j ) ),
                                           1.0E-10 );
                    }
                }

                currentRotationMatrixToBaseFrame = phobosRotationalEphemeris->getRotationToBaseFrame( currentTime );
                expectedRotationToBaseFrameFromInitialRotation = initialRotationMatrixToBaseFrame * baseRotationToTargetFrame.transpose( );
                for( unsigned int i = 0; i < 3; i++ )
                {
                    for( unsigned int j = 0; j < 3; j++ )
                    {
                        BOOST_CHECK_SMALL( std::fabs( currentRotationMatrixToBaseFrame( i, j ) -
                                                      expectedRotationToBaseFrameFromInitialRotation( i, j ) ),
                                           1.0E-10 );
                    }
                }
                ...
                expectedRotationMatrixDerivativeToTargetFrame =
                        meanMotion * premultiplierMatrix * baseRotationToTargetFrame * initialRotationMatrixToTargetFrame;
                ...
                BOOST_CHECK_SMALL(
                        std::fabs( currentRotationalVelocityInTargetFrame( 0 ) - expectedRotationalVelocityVectorInTargetFrame( 0 ) ),
                        meanMotion * 1.0E-15 );
                BOOST_CHECK_SMALL(
                        std::fabs( currentRotationalVelocityInTargetFrame( 1 ) - expectedRotationalVelocityVectorInTargetFrame( 1 ) ),
                        meanMotion * 1.0E-15 );
                BOOST_CHECK_SMALL(
                        std::fabs( currentRotationalVelocityInTargetFrame( 2 ) - expectedRotationalVelocityVectorInTargetFrame( 2 ) ),
                        meanMotion * 1.0E-15 );
                ...
                BOOST_CHECK_SMALL(
                        std::fabs( currentRotationalVelocityInBaseFrame( 0 ) - expectedRotationalVelocityVectorInBaseFrame( 0 ) ),
                        meanMotion * 1.0E-15 );
                BOOST_CHECK_SMALL(
                        std::fabs( currentRotationalVelocityInBaseFrame( 1 ) - expectedRotationalVelocityVectorInBaseFrame( 1 ) ),
                        meanMotion * 1.0E-15 );
                BOOST_CHECK_SMALL(
                        std::fabs( currentRotationalVelocityInBaseFrame( 2 ) - expectedRotationalVelocityVectorInBaseFrame( 2 ) ),
                        meanMotion * 1.0E-15 );
                ...
                for( unsigned int i = 0; i < 3; i++ )
                {
                    for( unsigned int j = 0; j < 3; j++ )
                    {
                        BOOST_CHECK_SMALL( std::fabs( numericalRotationMatrixDerivativeToBaseFrame( i, j ) -
                                                      currentRotationMatrixDerivativeToBaseFrame( i, j ) ),
                                           1.0E-12 );
                    }
                }
                ...
                for( unsigned int i = 0; i < 3; i++ )
                {
                    for( unsigned int j = 0; j < 3; j++ )
                    {
                        BOOST_CHECK_SMALL( std::fabs( numericalRotationMatrixDerivativeToTargetFrame( i, j ) -
                                                      currentRotationMatrixDerivativeToTargetFrame( i, j ) ),
                                           1.0E-12 );
                    }
                }
...
BOOST_AUTO_TEST_CASE( testSimpleRotationalDynamicsPropagationWithObliquity )
{
    ...
    for( int propagatorType = 0; propagatorType < 3; propagatorType++ )
    {
        ...
        while( currentTime < endTime )
        {
            ...
            indirectRotationalVelocityInBaseFrame = getRotationalVelocityVectorInBaseFrameFromMatrices(
            ...
            for( unsigned int j = 0; j < 3; j++ )
            {
                BOOST_CHECK_SMALL( std::fabs( indirectRotationalVelocityInBaseFrame( j ) - currentRotationalVelocityInBaseFrame( j ) ),
                                   1.0E-15 );
            }
...
BOOST_AUTO_TEST_CASE( testRotationalAndTranslationalDynamicsPropagation )
{
    ...
    BodyListSettings bodySettings = getDefaultBodySettings(
            { "Earth" }, simulationStartEpoch - 10.0 * fixedStepSize, simulationEndEpoch + 10.0 * fixedStepSize, "SSB", "J2000" );
    ...
    std::shared_ptr< interpolators::OneDimensionalInterpolator< double, Eigen::Matrix< double, 7, 1 > > > dummyRotationInterpolator =
            std::make_shared< interpolators::LinearInterpolator< double, Eigen::Matrix< double, 7, 1 > > >( dummyRotationMap );
    bodies.at( "Apollo" )
    ...
    dummyStateMap[ 1.0E100 ] = Eigen::Matrix< double, 6, 1 >::Zero( );
    std::shared_ptr< interpolators::OneDimensionalInterpolator< double, Eigen::Matrix< double, 6, 1 > > > dummyStateInterpolator =
            std::make_shared< interpolators::LinearInterpolator< double, Eigen::Matrix< double, 6, 1 > > >( dummyStateMap );
    bodies.at( "Apollo" )
            ->setEphemeris( std::make_shared< TabulatedCartesianEphemeris< double, double > >( dummyStateInterpolator, "SSB", "J2000" ) );
    ...
    for( int propagatorType = 0; propagatorType < 3; propagatorType++ )
    {
        ...
        for( int simulationCase = 0; simulationCase < 2; simulationCase++ )
        {
            ...
            apolloSphericalEntryState( SphericalOrbitalStateElementIndices::flightPathIndex ) = -1.2 * mathematical_constants::PI / 180.0;
            ...
            std::shared_ptr< ephemerides::RotationalEphemeris > earthRotationalEphemeris = bodies.at( "Earth" )->getRotationalEphemeris( );
            ...
            systemInitialRotationalState.segment( 0, 4 ) = linear_algebra::convertQuaternionToVectorFormat( initialRotation );
            ...
            basic_astrodynamics::TorqueModelMap torqueModelMap = createTorqueModelsMap( bodies, selectedTorqueModelMap, bodiesToPropagate );
            ...
            dependentVariablesList.push_back(
                    std::make_shared< BodyAerodynamicAngleVariableSaveSettings >( "Apollo", reference_frames::heading_angle ) );
            dependentVariablesList.push_back(
                    std::make_shared< BodyAerodynamicAngleVariableSaveSettings >( "Apollo", reference_frames::flight_path_angle ) );
            dependentVariablesList.push_back(
            ...
            if( simulationCase == 1 )
            {
                ...
                dependentVariablesList.push_back(
                        std::make_shared< SingleTorqueDependentVariableSaveSettings >( aerodynamic_torque, "Apollo", "Earth" ) );
                dependentVariablesList.push_back(
                        std::make_shared< SingleDependentVariableSaveSettings >( total_torque_dependent_variable, "Apollo" ) );
            }
            ...
            double currentLatitude, currentLongitude, currentFlightPathAngle, currentHeadingAngle, currentAngleOfAttack,
                    currentSideslipAngle, currentBankAngle, currentRotationAngle;
            Eigen::Matrix3d currentInertialToBodyFixedFrameRotation, currentEarthFixedToTnwFrameRotation,
                    currentTnwToTrajectoryFrameRotation, currentTrajectoryToAerodynamicFrameRotation,
                    currentAerodynamicToBodyFixesFrameRotation, currentInertialToBodyFixedFrame, expectedInertialToBodyFixedFrame;
            ...
            for( std::map< double, Eigen::VectorXd >::const_iterator variableIterator = dependentVariableHistory.begin( );
                 variableIterator != dependentVariableHistory.end( );
                 variableIterator++ )
            {
                if( simulationCase == 0 )
                {
                    ...
                    currentEarthFixedToTnwFrameRotation =
                            getRotatingPlanetocentricToLocalVerticalFrameTransformationQuaternion( currentLongitude, currentLatitude );
                    currentTnwToTrajectoryFrameRotation =
                            getLocalVerticalFrameToTrajectoryTransformationQuaternion( currentFlightPathAngle, currentHeadingAngle );
                    currentTrajectoryToAerodynamicFrameRotation =
                            getTrajectoryToAerodynamicFrameTransformationQuaternion( currentBankAngle );
                    currentAerodynamicToBodyFixesFrameRotation =
                            getAirspeedBasedAerodynamicToBodyFrameTransformationQuaternion( currentAngleOfAttack, currentSideslipAngle );
                    currentInertialToBodyFixedFrame = currentAerodynamicToBodyFixesFrameRotation *
                            currentTrajectoryToAerodynamicFrameRotation * currentTnwToTrajectoryFrameRotation *
                            currentEarthFixedToTnwFrameRotation * currentInertialToBodyFixedFrameRotation;
                    ...
                    for( unsigned int i = 0; i < 3; i++ )
                    {
                        ...
                        {
                            BOOST_CHECK_SMALL(
                                    std::fabs( expectedInertialToBodyFixedFrame( i, j ) - currentInertialToBodyFixedFrame( i, j ) ),
                                    1.0E-13 );
                        }
                    }
                }
                ...
                {
                    Eigen::Matrix3d currentRotationFromApolloFixedToInertialFrame =
                            apolloRotationModel->getRotationToBaseFrame( variableIterator->first ).toRotationMatrix( );
            ...
            if( simulationCase == 1 )
            {
                ...
                for( std::map< double, Eigen::Vector3d >::const_iterator variableIterator = ( inertialTorqueMap.begin( )++ );
                     variableIterator != inertialTorqueMap.end( );
                     variableIterator++ )
                {
                        ...
                        variableIterator->first > inertialTorqueMap.begin( )->first + 10.0 )
                    {
                        ...
                        {
                            BOOST_CHECK_SMALL(
                                    std::fabs( inertialTorqueMap.at( variableIterator->first )( i ) - angularMomentumDerivative( i ) ),
                                    0.25 );
                        }
                    }
                }
...
BOOST_AUTO_TEST_CASE( testSimpleRotationalDynamicsPropagationWithLibration )
{
    ...
    double meanMotion = std::sqrt( marsGravitationalParameter / std::pow( phobosSemiMajorAxis, 3.0 ) );
    ...
    double frequencySquared = 3.0 * marsGravitationalParameter / std::pow( phobosSemiMajorAxis, 3.0 ) *
            ( phobosInertiaTensor( 1, 1 ) - phobosInertiaTensor( 0, 0 ) ) / phobosInertiaTensor( 2, 2 );

    double beta = std::atan2( -meanMotion * initialRotationRatePerturbation, -std::sqrt( frequencySquared ) * initialAnglePerturbation );
    ...
    for( int propagatorType = 0; propagatorType < 3; propagatorType++ )
    {
        ...
        for( int torqueType = 0; torqueType < 2; torqueType++ )
        {
            ...
            {
                torqueMap[ "Phobos" ][ "Mars" ].push_back( std::make_shared< SphericalHarmonicTorqueSettings >( 2, 2 ) );
            }
            ...
            while( currentTime < endTime )
            {
                ...
                double expectedRotationRateDeviation =
                        -alpha * std::sqrt( frequencySquared ) * std::sin( std::sqrt( frequencySquared ) * currentTime + beta );
...
BOOST_AUTO_TEST_CASE( testSimpleRotationalDynamicsPropagationWithVaryinInertiaTensor )
{
    ...
    // Retrieve list of body objects.
    SystemOfBodies bodies = getTestBodyMap( 9376.0E3, 0, 1, 1 );

    // Define time range of test.
    double initialEphemerisTime = 0.0;
    double finalEphemerisTime = initialEphemerisTime + 3600.0;
    ...
    dependentVariablesList.push_back(
            std::make_shared< SingleTorqueDependentVariableSaveSettings >( second_order_gravitational_torque, "Phobos", "Mars" ) );
    dependentVariablesList.push_back(
            std::make_shared< SingleDependentVariableSaveSettings >( total_torque_dependent_variable, "Phobos" ) );
    dependentVariablesList.push_back( std::make_shared< TotalGravityFieldVariationSettings >( "Phobos", 2, 2, 0, 2, true ) );
    ...
    std::shared_ptr< gravitation::SphericalHarmonicsGravityField > phobosGravityField =
            std::dynamic_pointer_cast< gravitation::SphericalHarmonicsGravityField >( bodies.at( "Phobos" )->getGravityFieldModel( ) );

    std::shared_ptr< gravitation::GravityFieldModel > marsGravityField = bodies.at( "Mars" )->getGravityFieldModel( );

    double normalization20 = calculateLegendreGeodesyNormalizationFactor( 2, 0 );
    ...
    for( auto it: dependentVariableHistory )
    {
        ...
        {
            nominalInertiaTensor = inertiaTensor;
            nominalCosineCorrection = cosineCorrection;
            nominalSineCorrection = sineCorrection;
        }
        ...
        Eigen::Matrix3d computedInertiaCorrectionWrtStart = gravitation::getInertiaTensor(
                cosineCorrectionWrtStart( 0 ) * normalization20,
                cosineCorrectionWrtStart( 1 ) * normalization21,
                cosineCorrectionWrtStart( 2 ) * normalization22,
                sineCorrectionWrtStart( 0 ) * normalization21,
                sineCorrectionWrtStart( 1 ) * normalization22,
                0.0,
