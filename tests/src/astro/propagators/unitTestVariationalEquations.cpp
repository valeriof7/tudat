        ...
        const bool propagateVariationalEquations = 1 )
{
    ...
    initialTranslationalState =
            getInitialStatesOfBodies< TimeType, StateScalarType >( bodiesToIntegrate, centralBodies, bodies, initialIntegrationTime );
...
BOOST_AUTO_TEST_CASE( testEarthMoonVariationalEquationCalculation )
{
    ...
    for( unsigned int i = 0; i < centralBodiesSet.size( ); i++ )
    {
        ...
        if( i == 0 )
        {
            statePerturbation = ( Eigen::Matrix< double, 12, 1 >( ) << 100000.0,
                                  100000.0,
                                  100000.0,
                                  0.1,
                                  0.1,
        ...
        for( unsigned int k = 0; k < maximumPropagatorType; k++ )
        {
            ...
            currentOutput =
                    executeEarthMoonSimulation< double, double >( centralBodiesSet[ i ], Eigen::Matrix< double, 12, 1 >::Zero( ), k );
            ...
            for( unsigned int j = 0; j < 12; j++ )
            {
                ...
                upPerturbedState = executeEarthMoonSimulation< double, double >(
                                           centralBodiesSet[ i ], perturbedState, k, Eigen::Vector3d::Zero( ), 0 )
                                           .second.at( 0 );
            ...
            for( unsigned int j = 0; j < 3; j++ )
            {
                ...
                upPerturbedState =
                        executeEarthMoonSimulation< double, double >( centralBodiesSet[ i ], perturbedState, k, perturbedParameter, 0 )
                                .second.at( 0 );
            ...
            // Check results
            TUDAT_CHECK_MATRIX_CLOSE_FRACTION( stateTransitionAndSensitivityMatrixAtEpoch.block( 0, 0, 12, 15 ), manualPartial, 2.0E-4 );
        }
    }
}
        ...
        const bool propagateVariationalEquations = 1 )
{
    ...
    // Create integrator settings
    std::shared_ptr< IntegratorSettings< TimeType > > integratorSettings =
            std::make_shared< IntegratorSettings< TimeType > >( rungeKutta4, TimeType( initialEphemerisTime ), 5.0 );
    ...
    // Set (perturbed) initial state.
    Eigen::Matrix< StateScalarType, 6, 1 > initialTranslationalState =
...
BOOST_AUTO_TEST_CASE( testEarthOrbiterVariationalEquationCalculation )
{
    ...
    Eigen::Matrix< double, 10, 1 > parameterPerturbation = ( Eigen::Matrix< double, 10, 1 >( ) << 10.0,
                                                             1.0,
                                                             1.0E12,
    ...
    // Define state perturbation
    statePerturbation = ( Eigen::Matrix< double, 6, 1 >( ) << 10.0, 10.0, 10.0, 0.01, 0.01, 0.01 ).finished( );

    // Compute state transition and sensitivity matrices
    currentOutput = executeOrbiterSimulation< double, double >( Eigen::Matrix< double, 6, 1 >::Zero( ) );
    Eigen::MatrixXd stateTransitionAndSensitivityMatrixAtEpoch = currentOutput.first.at( 0 );
    ...
    for( unsigned int j = 0; j < 6; j++ )
    {
        ...
        perturbedState( j ) += statePerturbation( j );
        upPerturbedState =
                executeOrbiterSimulation< double, double >( perturbedState, Eigen::Matrix< double, 10, 1 >::Zero( ), 0 ).second.at( 0 );
    ...
    for( int j = 0; j < numberOfParametersToEstimate; j++ )
    {
        ...
        upPerturbedState = executeOrbiterSimulation< double, double >( perturbedState, perturbedParameter ).second.at( 0 );

        perturbedParameter.setZero( );
        perturbedParameter( j ) -= parameterPerturbation( j );
        downPerturbedState = executeOrbiterSimulation< double, double >( perturbedState, perturbedParameter ).second.at( 0 );
    ...
    // Check results
    TUDAT_CHECK_MATRIX_CLOSE_FRACTION( stateTransitionAndSensitivityMatrixAtEpoch, manualPartial, 5.0E-5 );
}

template< typename TimeType = double, typename StateScalarType = double >
std::pair< std::vector< Eigen::Matrix< StateScalarType, Eigen::Dynamic, Eigen::Dynamic > >,
           std::vector< Eigen::Matrix< StateScalarType, Eigen::Dynamic, 1 > > >
executePhobosRotationSimulation( const Eigen::Matrix< StateScalarType, 13, 1 > initialStateDifference,
                                 ...
                                 const bool propagateVariationalEquations = 1 )
{
    ...
    bodies.at( "Mars" )->setRotationalEphemeris( simulation_setup::createRotationModel(
            simulation_setup::getDefaultRotationModelSettings( "Mars", initialEphemerisTime, finalEphemerisTime ), "Mars" ) );
    bodies.at( "Mars" )->setGravityFieldModel( simulation_setup::createGravityFieldModel(
            simulation_setup::getDefaultGravityFieldSettings( "Mars", initialEphemerisTime, finalEphemerisTime ), "Mars", bodies ) );
...

    {
        ...
        Eigen::Matrix< StateScalarType, Eigen::Dynamic, 1 > testStates = Eigen::Matrix< StateScalarType, Eigen::Dynamic, 1 >::Zero( 13 );
...
BOOST_AUTO_TEST_CASE( testPhobosRotationVariationalEquationCalculation )
{
    ...
    currentOutput = executePhobosRotationSimulation< double, double >( Eigen::Matrix< double, 13, 1 >::Zero( ), appliedStateDifference );
    Eigen::MatrixXd stateTransitionAndSensitivityMatrixAtEpoch = currentOutput.first.at( 0 );
    ...
    // Define state perturbation
    statePerturbation = ( Eigen::Matrix< double, 13, 1 >( ) << 10.0,
                          10.0,
                          10.0,
                          0.1,
                          0.01,
