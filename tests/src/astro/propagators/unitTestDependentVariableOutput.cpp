        ...
        for( unsigned int propagatorType = 0; propagatorType < 2; propagatorType++ )
        {
            ...
            bodies.at( "Apollo" )
                    ->setRotationalEphemeris( createRotationModel(
                            std::make_shared< PitchTrimRotationSettings >( "Earth", "ECLIPJ2000", "VehicleFixed" ), "Apollo", bodies ) );
            ...
            dependentVariables.push_back(
                    std::make_shared< SingleDependentVariableSaveSettings >( relative_distance_dependent_variable, "Apollo", "Earth" ) );
            dependentVariables.push_back(
                    std::make_shared< SingleDependentVariableSaveSettings >( relative_speed_dependent_variable, "Apollo", "Earth" ) );
            dependentVariables.push_back(
                    std::make_shared< SingleAccelerationDependentVariableSaveSettings >( point_mass_gravity, "Apollo", "Earth", 1 ) );

            dependentVariables.push_back(
                    std::make_shared< SingleDependentVariableSaveSettings >( total_aerodynamic_g_load_variable, "Apollo", "Earth" ) );
            dependentVariables.push_back( std::make_shared< SingleDependentVariableSaveSettings >(
                    stagnation_point_heat_flux_dependent_variable, "Apollo", "Earth" ) );

            dependentVariables.push_back(
                    std::make_shared< SingleDependentVariableSaveSettings >( local_temperature_dependent_variable, "Apollo", "Earth" ) );
            dependentVariables.push_back(
            ...
            dependentVariables.push_back(
                    std::make_shared< BodyAerodynamicAngleVariableSaveSettings >( "Apollo", reference_frames::latitude_angle ) );
            dependentVariables.push_back(
                    std::make_shared< BodyAerodynamicAngleVariableSaveSettings >( "Apollo", reference_frames::longitude_angle ) );
            ...
            dependentVariables.push_back(
                    std::make_shared< SingleDependentVariableSaveSettings >( relative_position_dependent_variable, "Apollo", "Earth" ) );
            dependentVariables.push_back(
                    std::make_shared< SingleDependentVariableSaveSettings >( relative_velocity_dependent_variable, "Apollo", "Earth" ) );
            dependentVariables.push_back(
                    std::make_shared< SingleAccelerationDependentVariableSaveSettings >( point_mass_gravity, "Apollo", "Earth", 0 ) );
            dependentVariables.push_back(
                    std::make_shared< SingleDependentVariableSaveSettings >( total_acceleration_dependent_variable, "Apollo" ) );
            dependentVariables.push_back( std::make_shared< SingleDependentVariableSaveSettings >(
            ...
            dependentVariables.push_back(
                    std::make_shared< SingleAccelerationDependentVariableSaveSettings >( point_mass_gravity, "Apollo", "Moon", 0 ) );
            ...
            dependentVariables.push_back(
                    std::make_shared< SingleDependentVariableSaveSettings >( keplerian_state_dependent_variable, "Apollo", "Earth" ) );
            dependentVariables.push_back( std::make_shared< SingleDependentVariableSaveSettings >(
                    modified_equinocial_state_dependent_variable, "Apollo", "Earth" ) );
            dependentVariables.push_back(
                    std::make_shared< SingleDependentVariableSaveSettings >( body_fixed_relative_cartesian_position, "Apollo", "Earth" ) );
            ...
            dependentVariablesToAdd.push_back(
                    std::make_shared< SingleDependentVariableSaveSettings >( body_fixed_relative_cartesian_position, "Apollo", "Moon" ) );
            ...
            std::shared_ptr< ephemerides::RotationalEphemeris > earthRotationModel = bodies.at( "Earth" )->getRotationalEphemeris( );
            std::shared_ptr< aerodynamics::AtmosphereModel > earthAtmosphereModel = bodies.at( "Earth" )->getAtmosphereModel( );
            std::shared_ptr< gravitation::GravityFieldModel > earthGravityModel = bodies.at( "Earth" )->getGravityFieldModel( );
            std::shared_ptr< gravitation::GravityFieldModel > moonGravityModel = bodies.at( "Moon" )->getGravityFieldModel( );

            std::shared_ptr< aerodynamics::AtmosphericFlightConditions > apolloFlightConditions =
                    std::dynamic_pointer_cast< aerodynamics::AtmosphericFlightConditions >( bodies.at( "Apollo" )->getFlightConditions( ) );
                 ...
                 variableIterator++ )
            {
                ...
                manualCentralGravity = -bodies.at( "Earth" )->getGravityFieldModel( )->getGravitationalParameter( ) * relativePosition /
                ...
                {
                    BOOST_CHECK_SMALL( std::fabs( numericalSolution.at( variableIterator->first )( i ) - relativePosition( i ) ), 2.0E-5 );
                    BOOST_CHECK_SMALL( std::fabs( numericalSolution.at( variableIterator->first )( 3 + i ) - relativeVelocity( i ) ),
                                       5.0E-11 );
                }

                // Check central gravity acceleration
                TUDAT_CHECK_MATRIX_CLOSE_FRACTION( manualCentralGravity.segment( 0, 3 ),
                ...
                if( propagatorType == 0 )
                {
                    ...
                    for( unsigned int i = 0; i < 3; i++ )
                    {
                        BOOST_CHECK_SMALL( std::fabs( currentStateDerivative( 3 + i ) - totalAcceleration( i ) ), 1.0E-13 );
                    }
                ...
                BOOST_CHECK_SMALL(
                        std::fabs( ( numericalSolution.at( variableIterator->first ).segment( 0, 3 ) ).norm( ) - relativeDistance ),
                        2.0E-5 );
                BOOST_CHECK_SMALL( std::fabs( ( numericalSolution.at( variableIterator->first ).segment( 3, 3 ) ).norm( ) - relativeSpeed ),
                                   2.0E-11 );

                // Check central gravity acceleration norm
                BOOST_CHECK_CLOSE_FRACTION(
                        manualCentralGravity.norm( ), gravitationalAccelerationNorm, 6.0 * std::numeric_limits< double >::epsilon( ) );

                // Check Mach number
                BOOST_CHECK_CLOSE_FRACTION(
                        apolloFlightConditions->getCurrentAirspeed( ) / apolloFlightConditions->getCurrentSpeedOfSound( ),
                        machNumber,
                        std::numeric_limits< double >::epsilon( ) );

                // Check altitude.
                BOOST_CHECK_CLOSE_FRACTION(
                        apolloFlightConditions->getCurrentAltitude( ), altitude, std::numeric_limits< double >::epsilon( ) );

                // Check latitude and longitude
                BOOST_CHECK_SMALL( std::fabs( mathematical_constants::PI / 2.0 - computedSphericalBodyFixedPosition( 1 ) - latitude ),
                                   6.0 * std::numeric_limits< double >::epsilon( ) );
                BOOST_CHECK_SMALL( std::fabs( computedSphericalBodyFixedPosition( 2 ) - longitude ),
                                   8.0 * std::numeric_limits< double >::epsilon( ) );

                // Check geodetic latitude.
                if( !isOblateSpheroidUsed )
                {
                    BOOST_CHECK_SMALL( std::fabs( latitude - geodeticLatitude ), 6.0 * std::numeric_limits< double >::epsilon( ) );
                }
                else
                {
                    ...
                    BOOST_CHECK_SMALL( std::fabs( computedGeodeticLatitude - geodeticLatitude ),
                                       6.0 * std::numeric_limits< double >::epsilon( ) );
                }
                BOOST_CHECK_SMALL( std::fabs( geodeticLatitude - apolloFlightConditions->getCurrentGeodeticLatitude( ) ),
                                   6.0 * std::numeric_limits< double >::epsilon( ) );

                // Check sideslip anf bank angle
                BOOST_CHECK_SMALL( std::fabs( sideslipAngle ), 6.0 * std::numeric_limits< double >::epsilon( ) );
                BOOST_CHECK_SMALL( std::fabs( bankAngle ), 6.0 * std::numeric_limits< double >::epsilon( ) );

                // Check g-load
                BOOST_CHECK_CLOSE_FRACTION( gLoad,
                                            aerodynamicAcceleration.norm( ) / physical_constants::SEA_LEVEL_GRAVITATIONAL_ACCELERATION,
                                            6.0 * std::numeric_limits< double >::epsilon( ) );
                ...
                for( unsigned int i = 0; i < 3; i++ )
                {
                    BOOST_CHECK_SMALL( std::fabs( computedForceCoefficients( i ) - forceCoefficients( i ) ),
                                       6.0 * std::numeric_limits< double >::epsilon( ) );
                }

                // Check heat flux
                double expectedHeatFlux = aerodynamics::computeEquilibriumFayRiddellHeatFlux( freestreamDensity,
                                                                                              apolloFlightConditions->getCurrentAirspeed( ),
                                                                                              freestreamTemperature,
                                                                                              machNumber,
                                                                                              noseRadius,
                                                                                              wallEmissivity );
                BOOST_CHECK_CLOSE_FRACTION( expectedHeatFlux, stagnationPointHeatFlux, 6.0 * std::numeric_limits< double >::epsilon( ) );

                // Check trimmed condition (y-term)/symmetric vehicle shape (x- and z-term).
                BOOST_CHECK_SMALL( std::fabs( momentCoefficients( 0 ) ), 1.0E-14 );
                BOOST_CHECK_SMALL( std::fabs( momentCoefficients( 1 ) ), 1.0E-10 );
                BOOST_CHECK_SMALL( std::fabs( momentCoefficients( 2 ) ), 1.0E-14 );

                // Check if third-body gravity saving is done correctly
                TUDAT_CHECK_MATRIX_CLOSE_FRACTION(
                        moonAcceleration1, moonAcceleration2, ( 2.0 * std::numeric_limits< double >::epsilon( ) ) );

                Eigen::Vector6d expectedKeplerElements = tudat::orbital_element_conversions::convertCartesianToKeplerianElements(
                        Eigen::Vector6d( numericalSolution.at( variableIterator->first ) ), earthGravitationalParameter );

                TUDAT_CHECK_MATRIX_CLOSE_FRACTION(
                        expectedKeplerElements, keplerElements, ( 10.0 * std::numeric_limits< double >::epsilon( ) ) );
                ...
                for( unsigned int i = 0; i < 3; i++ )
                {
                    BOOST_CHECK_SMALL( std::fabs( computedBodyFixedPosition( i ) - bodyFixedCartesianPosition( i ) ), 1.0E-8 );
                }
                BOOST_CHECK_SMALL( std::fabs( computedSphericalBodyFixedPosition( 0 ) - bodyFixedSphericalPosition( 0 ) ), 1.0E-8 );
                BOOST_CHECK_SMALL( std::fabs( tudat::mathematical_constants::PI / 2.0 - computedSphericalBodyFixedPosition( 1 ) -
                                              bodyFixedSphericalPosition( 1 ) ),
                                   10.0 * std::numeric_limits< double >::epsilon( ) );
                BOOST_CHECK_SMALL( std::fabs( computedSphericalBodyFixedPosition( 2 ) - bodyFixedSphericalPosition( 2 ) ),
                                   10.0 * std::numeric_limits< double >::epsilon( ) );
                Eigen::Matrix3d computedRswRotationMatrix = tudat::reference_frames::getRswSatelliteCenteredToInertialFrameRotationMatrix(
                        numericalSolution.at( variableIterator->first ) );

                TUDAT_CHECK_MATRIX_CLOSE_FRACTION(
                        rswToInertialRotationMatrix, computedRswRotationMatrix, ( 10.0 * std::numeric_limits< double >::epsilon( ) ) );
                TUDAT_CHECK_MATRIX_CLOSE_FRACTION(
                        customVariable1, ( relativePosition / 2.0 ), ( 10.0 * std::numeric_limits< double >::epsilon( ) ) );
                BOOST_CHECK_CLOSE_FRACTION(
                        customVariable2( 0 ), ( relativePosition.norm( ) / 2.0 ), ( 10.0 * std::numeric_limits< double >::epsilon( ) ) );

                // Check central body gravitational potential
                BOOST_CHECK_CLOSE_FRACTION( earthGravitationalPotential,
...
BOOST_AUTO_TEST_CASE( testSphericalHarmonicDependentVariableOutput )
{
    ...
    accelerationsOfAsterix[ "Earth" ].push_back( std::make_shared< SphericalHarmonicAccelerationSettings >( 6, 6 ) );
    ...
    dependentVariables.push_back(
            std::make_shared< SingleAccelerationDependentVariableSaveSettings >( spherical_harmonic_gravity, "Asterix", "Earth" ) );
    ...
    dependentVariables.push_back( std::make_shared< SphericalHarmonicAccelerationTermsDependentVariableSaveSettings >(
            "Asterix", "Earth", separateTermsToSave ) );

    dependentVariables.push_back(
            std::make_shared< SphericalHarmonicAccelerationTermsDependentVariableSaveSettings >( "Asterix", "Earth", 6, 6 ) );
    ...
    for( std::map< double, Eigen::VectorXd >::iterator variableIterator = depdendentVariableResult.begin( );
         variableIterator != depdendentVariableResult.end( );
         variableIterator++ )
    {
        Eigen::VectorXd currentDependentVariables = variableIterator->second;
        ...
        BOOST_CHECK_SMALL( std::fabs( ( currentDependentVariables.segment( 96, 3 ) ).norm( ) - currentDependentVariables( 99 ) ),
                           10.0 * ( currentDependentVariables.segment( 96, 3 ) ).norm( ) * std::numeric_limits< double >::epsilon( ) );
        for( unsigned int i = 0; i < 3; i++ )
        {
            BOOST_CHECK_SMALL( std::fabs( manualAccelerationSum( i ) - currentDependentVariables( i ) ),
                               10.0 * manualAccelerationSum.norm( ) * std::numeric_limits< double >::epsilon( ) );
            BOOST_CHECK_SMALL( std::fabs( currentDependentVariables( 3 + i ) - currentDependentVariables( 15 + 3 * 1 + i ) ),
                               10.0 * ( currentDependentVariables.segment( 3, 3 ) ).norm( ) * std::numeric_limits< double >::epsilon( ) );
            BOOST_CHECK_SMALL( std::fabs( currentDependentVariables( 6 + i ) - currentDependentVariables( 15 + 3 * 6 + i ) ),
                               10.0 * ( currentDependentVariables.segment( 6, 3 ) ).norm( ) * std::numeric_limits< double >::epsilon( ) );
            BOOST_CHECK_SMALL( std::fabs( currentDependentVariables( 9 + i ) - currentDependentVariables( 15 + 3 * 8 + i ) ),
                               10.0 * ( currentDependentVariables.segment( 9, 3 ) ).norm( ) * std::numeric_limits< double >::epsilon( ) );
            BOOST_CHECK_SMALL( std::fabs( currentDependentVariables( 12 + i ) - currentDependentVariables( 15 + 3 * 7 + i ) ),
                               10.0 * ( currentDependentVariables.segment( 12, 3 ) ).norm( ) * std::numeric_limits< double >::epsilon( ) );
        }
    }
}

Eigen::VectorXd getCustomDependentVariable( const SystemOfBodies& bodies )
{
...
BOOST_AUTO_TEST_CASE( testDependentVariableEnvironmentUpdate )
{
    ...
    bodySettings.at( "Sun" )->radiationSourceModelSettings = isotropicPointRadiationSourceModelSettings(
    ...
    bodySettings.at( "Earth" )->radiationPressureTargetModelSettings =
            cannonballRadiationPressureTargetModelSettings( 4 * mathematical_constants::PI * earthRadius * earthRadius, 1.5 );
    SystemOfBodies bodies = createSystemOfBodies( bodySettings );

    SelectedAccelerationMap accelerationMap;
    accelerationMap[ "Earth" ][ "Moon" ].push_back( std::make_shared< AccelerationSettings >( point_mass_gravity ) );
    accelerationMap[ "Earth" ][ "Sun" ].push_back( std::make_shared< AccelerationSettings >( point_mass_gravity ) );
    accelerationMap[ "Earth" ][ "Sun" ].push_back( std::make_shared< AccelerationSettings >( radiation_pressure ) );
    ...
    dependentVariables.push_back(
            std::make_shared< SingleDependentVariableSaveSettings >( relative_position_dependent_variable, "Sun", "Venus" ) );
    dependentVariables.push_back(
            std::make_shared< BodyAerodynamicAngleVariableSaveSettings >( "Moon", reference_frames::latitude_angle, "Earth" ) );
    dependentVariables.push_back(
            std::make_shared< BodyAerodynamicAngleVariableSaveSettings >( "Moon", reference_frames::longitude_angle, "Earth" ) );
    dependentVariables.push_back(
            std::make_shared< BodyAerodynamicAngleVariableSaveSettings >( "Moon", reference_frames::heading_angle, "Earth" ) );
    dependentVariables.push_back(
            std::make_shared< BodyAerodynamicAngleVariableSaveSettings >( "Moon", reference_frames::flight_path_angle, "Earth" ) );
    // Earth-Sun distance is not tested, only used to calculate expected received irradiance
    dependentVariables.push_back(
            std::make_shared< SingleDependentVariableSaveSettings >( relative_distance_dependent_variable, "Earth", "Sun" ) );
    dependentVariables.push_back( std::make_shared< SingleDependentVariableSaveSettings >( received_irradiance, "Earth", "Sun" ) );
    dependentVariables.push_back( std::make_shared< SingleDependentVariableSaveSettings >( received_fraction, "Earth", "Sun" ) );
    dependentVariables.push_back(
         ...
         variableIterator++ )
    {
        ...
        Eigen::Vector3d expectedRelativePosition =
                tudat::spice_interface::getBodyCartesianPositionAtEpoch( "Sun", "Venus", "ECLIPJ2000", "None", variableIterator->first );
        Eigen::Vector3d computedRelativePosition = currentDependentVariables.segment( 0, 3 );

        for( unsigned int i = 0; i < 3; i++ )
        {
            BOOST_CHECK_SMALL( std::fabs( expectedRelativePosition( i ) - computedRelativePosition( i ) ), 1.0E-4 );
        }

        Eigen::Vector6d moonRelativeCartesianState =
                tudat::spice_interface::getBodyCartesianStateAtEpoch( "Moon", "SSB", "ECLIPJ2000", "None", variableIterator->first ) -
        ...
        BOOST_CHECK_SMALL(
                std::fabs( currentDependentVariables( 3 ) - ( tudat::mathematical_constants::PI / 2.0 - moonSphericalPosition( 1 ) ) ),
                1.0E-14 );
        BOOST_CHECK_SMALL( std::fabs( currentDependentVariables( 4 ) - moonSphericalPosition( 2 ) ), 1.0E-14 );

        Eigen::Vector6d moonRelativeSphericalState =
                tudat::orbital_element_conversions::convertCartesianToSphericalOrbitalState( moonRelativeEarthFixedCartesianState );

        BOOST_CHECK_SMALL(
                std::fabs( currentDependentVariables( 3 ) - moonRelativeSphericalState( orbital_element_conversions::latitudeIndex ) ),
                1.0E-14 );
        BOOST_CHECK_SMALL(
                std::fabs( currentDependentVariables( 4 ) - moonRelativeSphericalState( orbital_element_conversions::longitudeIndex ) ),
                1.0E-14 );
        BOOST_CHECK_SMALL(
                std::fabs( currentDependentVariables( 5 ) - moonRelativeSphericalState( orbital_element_conversions::headingAngleIndex ) ),
                1.0E-14 );
        BOOST_CHECK_SMALL(
                std::fabs( currentDependentVariables( 6 ) - moonRelativeSphericalState( orbital_element_conversions::flightPathIndex ) ),
                1.0E-14 );

        double earthSunDistance = currentDependentVariables( 7 );
        double expectedReceivedIrradiance = 1371 * physical_constants::ASTRONOMICAL_UNIT * physical_constants::ASTRONOMICAL_UNIT /
...
BOOST_AUTO_TEST_CASE( test_GravityFieldVariationAccelerationSaving )
{
    ...
    dependentVariables.push_back(
            std::make_shared< SingleDependentVariableSaveSettings >( total_gravity_field_variation_acceleration, "Vehicle", "Earth" ) );
    dependentVariables.push_back( std::make_shared< SingleVariationSphericalHarmonicAccelerationSaveSettings >(
            "Vehicle", "Earth", gravitation::basic_solid_body, "Sun" ) );
    dependentVariables.push_back( std::make_shared< SingleVariationSphericalHarmonicAccelerationSaveSettings >(
            "Vehicle", "Earth", gravitation::basic_solid_body, "Moon" ) );
    dependentVariables.push_back( std::make_shared< SingleVariationSingleTermSphericalHarmonicAccelerationSaveSettings >(
    ...
    std::shared_ptr< gravitation::SphericalHarmonicsGravitationalAccelerationModel > sphericalHarmonicAcceleration =
            std::dynamic_pointer_cast< gravitation::SphericalHarmonicsGravitationalAccelerationModel >(
                    getAccelerationBetweenBodies( "Vehicle",
                                                  "Earth",
                                                  dynamicsSimulator.getDynamicsStateDerivative( )->getStateDerivativeModels( ),
         ...
         variableIterator++ )
    {
        ...
        Eigen::VectorXd sphericalHarmonicCosineCoefficientCorrection =
                dependentVariableSolution.at( variableIterator->first ).segment( 54, 9 );
        Eigen::VectorXd sphericalHarmonicSineCoefficientCorrection =
                dependentVariableSolution.at( variableIterator->first ).segment( 63, 6 );
        ...
        Eigen::Vector3d computedTotalCoefficientAcceleration = sphericalHarmonicAcceleration->getAccelerationWithAlternativeCoefficients(
                earthGravityField->getCosineCoefficients( ).block( 0, 0, 4, 4 ),
                earthGravityField->getSineCoefficients( ).block( 0, 0, 4, 4 ) );
        Eigen::Vector3d computedNominalCoefficientAcceleration = sphericalHarmonicAcceleration->getAccelerationWithAlternativeCoefficients(
                earthGravityField->getNominalCosineCoefficients( ).block( 0, 0, 4, 4 ),
                earthGravityField->getNominalSineCoefficients( ).block( 0, 0, 4, 4 ) );
        Eigen::Vector3d computedTotalTidalCorrection = computedTotalCoefficientAcceleration - computedNominalCoefficientAcceleration;
        ...
        for( int i = 0; i < 3; i++ )
        {
            BOOST_CHECK_SMALL( std::fabs( currentTotalSunTidalCorrection( i ) + currentTotalMoonTidalCorrection( i ) -
                                          currentTotalTidalCorrection( i ) ),
                               1.0E-14 );

            BOOST_CHECK_SMALL( std::fabs( currentTotalAcceleration( i ) - currentTotalTidalCorrection( i ) -
                                          computedNominalCoefficientAcceleration( i ) ),
                               1.0E-14 );
            BOOST_CHECK_SMALL( std::fabs( currentTotalTidalCorrection( i ) - computedTotalTidalCorrection( i ) ), 1.0E-14 );
            BOOST_CHECK_SMALL( std::fabs( reconstructedTotalMoonTidalCorrection( i ) - currentTotalMoonTidalCorrection( i ) ), 1.0E-14 );
            BOOST_CHECK_SMALL( std::fabs( perTermMoonTidalCorrections( i + 3 ) - perTermMoonTidalSelectedCorrections( i ) ), 1.0E-14 );
            BOOST_CHECK_SMALL( std::fabs( perTermMoonTidalCorrections( i + 9 ) - perTermMoonTidalSelectedCorrections( i + 3 ) ), 1.0E-14 );
            BOOST_CHECK_SMALL( std::fabs( perTermMoonTidalCorrections( i + 15 ) - perTermMoonTidalSelectedCorrections( i + 6 ) ), 1.0E-14 );
            BOOST_CHECK_SMALL( std::fabs( perTermMoonTidalCorrections( i + 21 ) - perTermMoonTidalSelectedCorrections( i + 9 ) ), 1.0E-14 );
        }
...
BOOST_AUTO_TEST_CASE( test_AccelerationPartialSaving )
{
    ...
    for( int test = 0; test < 5; test++ )
    {
        ...
        if( test == 0 || test == 3 || test == 4 )
        {
            dependentVariables.push_back( std::make_shared< AccelerationPartialWrtStateSaveSettings >(
        ...
        if( test == 0 )
        {
            ...
            dependentVariables.push_back(
                    std::make_shared< AccelerationPartialWrtStateSaveSettings >( "Vehicle", "Earth", spherical_harmonic_gravity, "Moon" ) );
        }

        if( test == 1 || test == 3 )
        {
            dependentVariables.push_back(
                    std::make_shared< AccelerationPartialWrtStateSaveSettings >( "Vehicle", "Moon", point_mass_gravity, "Vehicle" ) );
            dependentVariables.push_back(
                    std::make_shared< AccelerationPartialWrtStateSaveSettings >( "Vehicle", "Moon", point_mass_gravity, "Moon" ) );
        }
        ...
        {
            dependentVariables.push_back(
                    std::make_shared< AccelerationPartialWrtStateSaveSettings >( "Vehicle", "Sun", point_mass_gravity, "Vehicle" ) );
        }
        else if( test == 4 )
        {
            dependentVariables.push_back( std::make_shared< AccelerationPartialWrtStateSaveSettings >(
        ...
        for( unsigned int i = 0; i < dependentVariableSolution.size( ) - 2; i++ )
        {
            ...
            if( test == 0 )
            {
                TUDAT_CHECK_MATRIX_CLOSE_FRACTION( variableIteratorMid->second.segment( 0, 18 ),
                                                   ( -variableIteratorMid->second.segment( 18, 18 ) ),
                                                   std::numeric_limits< double >::epsilon( ) );
                TUDAT_CHECK_MATRIX_CLOSE_FRACTION( variableIteratorMid->second.segment( 36, 18 ),
                ...
                TUDAT_CHECK_MATRIX_CLOSE_FRACTION( computedTotalAccelerationPartials,
                                                   variableIteratorBack->second.segment( 54, 18 ),
                                                   std::numeric_limits< double >::epsilon( ) );
            }
            else if( test == 1 || test == 2 )
            {
                ...
                Eigen::VectorXd computedTotalAccelerationPartials = variableIteratorBack->second.segment( 0, 18 );
                TUDAT_CHECK_MATRIX_CLOSE_FRACTION( computedTotalAccelerationPartials,
                                                   variableIteratorBack->second.segment( 36, 18 ),
                                                   std::numeric_limits< double >::epsilon( ) );

                // Check consistency of partial of total acceleration of Vehicle w.r.t. Moon's translational state.
                TUDAT_CHECK_MATRIX_CLOSE_FRACTION( variableIteratorBack->second.segment( 18, 18 ),
                                                   variableIteratorBack->second.segment( 54, 18 ),
                                                   std::numeric_limits< double >::epsilon( ) );
            }
            else if( test == 3 || test == 4 )
            {
                ...
                TUDAT_CHECK_MATRIX_CLOSE_FRACTION( computedTotalAccelerationPartials,
                                                   variableIteratorBack->second.segment( 72, 18 ),
                                                   std::numeric_limits< double >::epsilon( ) );

                // Check consistency of partial of total acceleration of Vehicle w.r.t. Moon's translational state.
                TUDAT_CHECK_MATRIX_CLOSE_FRACTION( variableIteratorBack->second.segment( 36, 18 ),
                                                   variableIteratorBack->second.segment( 90, 18 ),
                                                   std::numeric_limits< double >::epsilon( ) );
            }
...
BOOST_AUTO_TEST_CASE( test_GravitationalPotentialAndLaplacianSaving )
{
    ...
    for( unsigned int gravityModelsId: { 0, 1 } )
    {
        ...
        if( gravityModelsId == 1 )
        {
            ...
            verticesDefiningEachFacet << 2, 1, 0, 1, 2, 3, 4, 2, 0, 2, 4, 6, 1, 4, 0, 4, 1, 5, 6, 5, 7, 5, 6, 4, 3, 6, 7, 6, 3, 2, 5, 3, 7,
                    3, 5, 1;

            bodySettings.at( "Earth" )->gravityFieldSettings = polyhedronGravitySettingsFromMu(
                    celestial_body_constants::EARTH_GRAVITATIONAL_PARAMETER, verticesCoordinates, verticesDefiningEachFacet, "IAU_Earth" );
            bodySettings.at( "Moon" )->gravityFieldSettings = polyhedronGravitySettingsFromMu(
                    celestial_body_constants::MOON_GRAVITATIONAL_PARAMETER, verticesCoordinates, verticesDefiningEachFacet, "IAU_Moon" );
        }
        ...
        dependentVariables.push_back(
                std::make_shared< SingleDependentVariableSaveSettings >( gravity_field_potential_dependent_variable, "Asterix", "Earth" ) );
        dependentVariables.push_back(
                std::make_shared< SingleDependentVariableSaveSettings >( gravity_field_potential_dependent_variable, "Asterix", "Moon" ) );
        dependentVariables.push_back(
                std::make_shared< SingleDependentVariableSaveSettings >( body_fixed_relative_cartesian_position, "Asterix", "Earth" ) );
        dependentVariables.push_back(
                std::make_shared< SingleDependentVariableSaveSettings >( body_fixed_relative_cartesian_position, "Asterix", "Moon" ) );
        //        dependentVariables.push_back(
             ...
             variableIterator++ )
        {
            ...
            if( gravityModelsId == 0 )
            {
                ...
                /*BOOST_CHECK_CLOSE_FRACTION(
                        ...
                        1e-7 );*/
                BOOST_CHECK( std::fabs( earthGravitationalPotential -
                                        earthGravityModel->getGravitationalPotential( earthBodyFixedCartesianPosition ) ) <
                             std::fabs( std::min( earthGravitationalPotential,
                                                  earthGravityModel->getGravitationalPotential( earthBodyFixedCartesianPosition ) ) *
                                        1e-7 ) );
                ...
                /*BOOST_CHECK_CLOSE_FRACTION(
                        ...
                        1e-11 );*/
                BOOST_CHECK( std::fabs( moonGravitationalPotential -
                                        moonGravityModel->getGravitationalPotential( moonBodyFixedCartesianPosition ) ) <
                             std::fabs( std::min( moonGravitationalPotential,
                                                  moonGravityModel->getGravitationalPotential( moonBodyFixedCartesianPosition ) ) *
                                        1e-11 ) );
            }
            // Polyhedron
...
std::pair< int, double > getClosestSatelliteDistance( const SystemOfBodies& bodies,
...
std::tuple< int, double, double > getClosestStationSatelliteDistance( const SystemOfBodies& bodies,
                                                                      ...
                                                                      const double time )
{
    ...
    Eigen::Vector6d stationState =
            getLinkEndCompleteEphemerisFunction( observation_models::LinkEndId( "Earth", groundStation->getStationId( ) ), bodies )( time );

    for( unsigned int i = 0; i < bodyList.size( ); i++ )
    {
        Eigen::Vector3d relativePosition =
                bodies.at( bodyList.at( i ) )->getEphemeris( )->getCartesianPosition( time ) - stationState.segment( 0, 3 );
        double elevationAngle =
                groundStation->getPointingAnglesCalculator( )->calculateElevationAngleFromInertialVector( relativePosition, time );
    ...
    if( elevationAngles.size( ) > 0 )
    {
        ...
        return std::make_tuple(
                indices.at( minimumDistanceIndex ), distances.at( minimumDistanceIndex ), elevationAngles.at( minimumDistanceIndex ) );
    }
    else
    {
...
BOOST_AUTO_TEST_CASE( test_ConstellationVariables )
{
    ...
    accelerationsOfSatellite1[ "Earth" ].push_back( std::make_shared< SphericalHarmonicAccelerationSettings >( 2, 2 ) );
    //    accelerationsOfSatellite1[ "Earth" ].push_back( std::make_shared< AccelerationSettings >(
    //                                                     aerodynamic ) );
    accelerationsOfSatellite2[ "Earth" ].push_back( std::make_shared< SphericalHarmonicAccelerationSettings >( 2, 2 ) );
    ...
    bodies.at( "Satellite3" )
            ->setEphemeris( createBodyEphemeris( std::make_shared< KeplerEphemerisSettings >(
    ...
    createGroundStation( bodies.at( "Earth" ),
                         "Station",
                         ( Eigen::Vector3d( ) << 0.0, 1.5, 2.1 ).finished( ),
    ...
    dependentVariables.push_back( std::make_shared< MinimumConstellationDistanceDependentVariableSaveSettings >(
    ...
    dependentVariables.push_back( std::make_shared< MinimumConstellationStationDistanceDependentVariableSaveSettings >(
            "Earth",
            "Station",
    ...
    for( auto it: dependentVariableResults )
    {
        ...
        testTuple = getClosestStationSatelliteDistance(
                bodies,
                bodies.at( "Earth" )->getGroundStation( "Station" ),
                std::vector< std::string >( { "Satellite1", "Satellite2", "Satellite3", "Satellite4", "Satellite5" } ),
                it.first );
