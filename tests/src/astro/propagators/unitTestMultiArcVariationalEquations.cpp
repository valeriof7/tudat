        ...
        const double arcOverlap = 5.0E3 )
{
    ...
    if( patchArcsTogether && forcedArcInitialStates.size( ) == 0 )
    {
        multiArcPropagatorSettings =
                std::make_shared< MultiArcPropagatorSettings< StateScalarType, TimeType > >( propagatorSettingsList, patchArcsTogether );
    }
    else
    {
        multiArcPropagatorSettings = std::make_shared< MultiArcPropagatorSettings< StateScalarType, TimeType > >( propagatorSettingsList );
    }

    // Define parameters.
    std::vector< std::shared_ptr< EstimatableParameterSettings > > parameterNames;
    {
        parameterNames = getInitialMultiArcParameterSettings<>( multiArcPropagatorSettings, bodies, arcStartTimes );
            ...
            results;
    {
        ...
        for( unsigned int arc = 0; arc < numberOfArcs; arc++ )
        {
            ...
            if( propagateVariationalEquations )
            {
                ...
                {
                    TUDAT_CHECK_MATRIX_CLOSE_FRACTION( testMatrixDirect.block( 0, j * 6, 12, 6 ),
                                                       testMatrixFull.block( 0, j * numberOfArcs * 6 + 6 * arc, 12, 6 ),
                                                       std::numeric_limits< double >::epsilon( ) );

                    TUDAT_CHECK_MATRIX_CLOSE_FRACTION( testMatrixDirect.block( 0, 12, 12, 3 ),
        ...
        for( unsigned int patchArcs = 0; patchArcs < 2; patchArcs++ )
        {
            ...
            for( unsigned int k = 0; k < maximumPropagatorType; k++ )
            {
                ...
                for( unsigned int j = 0; j < 12; j++ )
                {
                    ...
                    upPerturbedState = executeMultiArcEarthMoonSimulation< double, double >( centralBodiesSet[ i ],
                                                                                             perturbedState,
                                                                                             k,
                                                                                             patchArcs,
                                                                                             Eigen::Vector3d::Zero( ),
                                                                                             0,
                                                                                             nominalArcStartStates )
                                               .second;

                    perturbedState.setZero( );
                    perturbedState( j ) -= statePerturbation( j );
                    downPerturbedState = executeMultiArcEarthMoonSimulation< double, double >( centralBodiesSet[ i ],
                                                                                               perturbedState,
                                                                                               k,
                                                                                               patchArcs,
                                                                                               Eigen::Vector3d::Zero( ),
                ...
                for( unsigned int j = 0; j < 3; j++ )
                {
                    ...
                    upPerturbedState =
                            executeMultiArcEarthMoonSimulation< double, double >(
                                    centralBodiesSet[ i ], perturbedState, k, patchArcs, perturbedParameter, 0, nominalArcStartStates )
                                    .second;
                ...
                for( unsigned int arc = 0; arc < manualPartial.size( ); arc++ )
                {
                    TUDAT_CHECK_MATRIX_CLOSE_FRACTION( stateTransitionAndSensitivityMatrixAtEpoch.at( arc ).block( 0, 0, 12, 15 ),
                                                       manualPartial.at( arc ).block( 0, 0, 12, 15 ),
                                                       5.0E-4 );
                }
            }
        }
