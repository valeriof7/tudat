...
BOOST_AUTO_TEST_CASE( testNonSequentialSingleArcDynamics )
{
    ...
    BOOST_CHECK_EQUAL( nonsequentialDependentVariablesHistory.size( ),
                       forwardDependentVariablesHistory.size( ) + backwardDependentVariablesHistory.size( ) - 1 );

    // Check propagated state and dependent variables at initial time.
    TUDAT_CHECK_MATRIX_CLOSE_FRACTION(
            nonsequentialStateHistory.begin( )->second, backwardStateHistory.begin( )->second, std::numeric_limits< double >::epsilon( ) );
    TUDAT_CHECK_MATRIX_CLOSE_FRACTION( nonsequentialDependentVariablesHistory.begin( )->second,
                                       backwardDependentVariablesHistory.begin( )->second,
                                       std::numeric_limits< double >::epsilon( ) );

    // Check propagated state and dependent variables at final time.
    TUDAT_CHECK_MATRIX_CLOSE_FRACTION(
            nonsequentialStateHistory.rbegin( )->second, forwardStateHistory.rbegin( )->second, std::numeric_limits< double >::epsilon( ) );
    TUDAT_CHECK_MATRIX_CLOSE_FRACTION( nonsequentialDependentVariablesHistory.rbegin( )->second,
                                       forwardDependentVariablesHistory.rbegin( )->second,
                                       std::numeric_limits< double >::epsilon( ) );
}
...
BOOST_AUTO_TEST_CASE( testNonSequentialMultiArcDynamics )
{
    ...
    MultiArcDynamicsSimulator<> nonSequentialDynamicsSimulator =
            MultiArcDynamicsSimulator<>( bodies, nonsequentialPropagatorSettings, true );
    ...
    {
        BOOST_CHECK_EQUAL( nonsequentialStateHistory.at( i ).size( ),
                           forwardStateHistory.at( i ).size( ) + backwardStateHistory.at( i ).size( ) - 1 );

        TUDAT_CHECK_MATRIX_CLOSE_FRACTION( nonsequentialStateHistory.at( i ).begin( )->second,
                                           backwardStateHistory.at( i ).begin( )->second,
                                           std::numeric_limits< double >::epsilon( ) );
        TUDAT_CHECK_MATRIX_CLOSE_FRACTION( nonsequentialStateHistory.at( i ).rbegin( )->second,
                                           forwardStateHistory.at( i ).rbegin( )->second,
                                           std::numeric_limits< double >::epsilon( ) );
    }
}
...
BOOST_AUTO_TEST_CASE( testNonSequentialHybridArcDynamics )
{
    ...
    basic_astrodynamics::AccelerationMap accelerationsMap =
            createAccelerationModelsMap( bodies, accelerationSettings, multiArcBodiesToPropagate, multiArcCentralBodies );
    ...
    HybridArcDynamicsSimulator<> nonSequentialDynamicsSimulator =
            HybridArcDynamicsSimulator<>( bodies, nonSequentialPropagatorSettings, true );
    ...
    TUDAT_CHECK_MATRIX_CLOSE_FRACTION(
            nonsequentialSingleArcStateHistory.begin( )->second, backwardSingleArcStateHistory.begin( )->second, 1.0e-10 );
    TUDAT_CHECK_MATRIX_CLOSE_FRACTION(
            nonsequentialSingleArcStateHistory.rbegin( )->second, forwardSingleArcStateHistory.rbegin( )->second, 1.0e-10 );

    for( unsigned int i = 0; i < arcStartTimes.size( ); i++ )
    {
        BOOST_CHECK_EQUAL( nonsequentialMultiArcStateHistory.at( i ).size( ),
                           forwardMultiArcStateHistory.at( i ).size( ) + backwardMultiArcStateHistory.at( i ).size( ) - 1 );
        BOOST_CHECK_EQUAL(
                nonsequentialMultiArcDependentVariablesHistory.at( i ).size( ),
                forwardMultiArcDependentVariablesHistory.at( i ).size( ) + backwardMultiArcDependentVariablesHistory.at( i ).size( ) - 1 );

        TUDAT_CHECK_MATRIX_CLOSE_FRACTION( nonsequentialMultiArcStateHistory.at( i ).begin( )->second,
                                           backwardMultiArcStateHistory.at( i ).begin( )->second,
                                           1.0e-10 );
        TUDAT_CHECK_MATRIX_CLOSE_FRACTION( nonsequentialMultiArcStateHistory.at( i ).rbegin( )->second,
                                           forwardMultiArcStateHistory.at( i ).rbegin( )->second,
                                           1.0e-10 );

        TUDAT_CHECK_MATRIX_CLOSE_FRACTION( nonsequentialMultiArcDependentVariablesHistory.at( i ).begin( )->second,
                                           backwardMultiArcDependentVariablesHistory.at( i ).begin( )->second,
                                           1.0e-10 );
        TUDAT_CHECK_MATRIX_CLOSE_FRACTION( nonsequentialMultiArcDependentVariablesHistory.at( i ).rbegin( )->second,
                                           forwardMultiArcDependentVariablesHistory.at( i ).rbegin( )->second,
                                           1.0e-10 );
    }
}
