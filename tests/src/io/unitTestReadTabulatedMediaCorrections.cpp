...
BOOST_AUTO_TEST_CASE( testCspCommandsExtraction )
{
    ...
    {
        std::shared_ptr< CspRawFile > cspFile =
                std::make_shared< CspRawFile >( tudat::paths::getTudatTestDataPath( ) + "mromagr2017_091_2017_121.tro.txt" );
        ...
        for( unsigned int cspCommandId: cspCommandsIds )
        {
            ...
            if( cspCommandId == 0 )
            {
                std::vector< double > expectedCoefficients = { -.0048, .0077, .0238, -.0872, -.0445, .1499, .0303, -.0995, -.0066, .0228 };
                ...
                {
                    BOOST_CHECK_EQUAL( atmosphericCommand->computationCoefficients_.at( i ), expectedCoefficients.at( i ) );
                }
            ...
            {
                std::vector< double > expectedCoefficients = { .0031, .0043, -.0006, -.0054, -.0015, .0069, .0008, -.0024 };
                BOOST_CHECK_EQUAL( atmosphericCommand->modelIdentifier_, "DRY NUPART" );
                BOOST_CHECK_EQUAL( atmosphericCommand->dataTypesIdentifier_, "ALL" );
                BOOST_CHECK_EQUAL( atmosphericCommand->computationSpecifier_, "BY NRMPOW" );
                for( unsigned int i = 0; i < expectedCoefficients.size( ); ++i )
    ...
    {
        std::shared_ptr< CspRawFile > cspFile =
                std::make_shared< CspRawFile >( tudat::paths::getTudatTestDataPath( ) + "mess_rs_2007335_001_dp_ion.txt" );
        std::vector< std::shared_ptr< CspCommand > > extractedCspCommands = cspFile->getCspCommands( );

        std::shared_ptr< AtmosphericCorrectionCspCommand > atmosphericCommand =
                std::dynamic_pointer_cast< AtmosphericCorrectionCspCommand >( extractedCspCommands.at( 0 ) );

        std::vector< double > expectedCoefficients = {
            1.4304, -0.2192, 3.2505, 3.7560, -11.1495, -8.3949, 17.8637, 7.6619, -8.8524, -2.3772
        };
