...
BOOST_AUTO_TEST_CASE( testSingleOdfFileReader )
{
    ...
    std::shared_ptr< input_output::OdfRawFileContents > rawOdfContents = std::make_shared< input_output::OdfRawFileContents >( file );
    ...
    BOOST_CHECK_EQUAL( static_cast< unsigned long long >( std::round( 1000.0 * rangeDataBlock->getReferenceFrequency( ) ) ),
                       7177004669452LL );
    ...
    std::shared_ptr< input_output::OdfRampBlock > rampBlock =
            std::dynamic_pointer_cast< input_output::OdfRampBlock >( rawOdfContents->getRampBlocks( ).at( 63 ).at( 5 ) );
...
BOOST_AUTO_TEST_CASE( testProcessSingleOdfFile )
{
    ...
    for( unsigned int i = 0; i < 2; ++i )
    {
        ...
        double timeUtc =
                timeScaleConverter.getCurrentTime< double >( basic_astrodynamics::tdb_scale, basic_astrodynamics::utc_scale, time );

        // Get UTC calendar date
        int year, month, day;
        double fractionOfDay;
        iauJd2cal(
                basic_astrodynamics::JULIAN_DAY_ON_J2000, timeUtc / physical_constants::JULIAN_DAY, &year, &month, &day, &fractionOfDay );
