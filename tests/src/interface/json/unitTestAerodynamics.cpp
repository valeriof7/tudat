...
{
    BOOST_CHECK_EQUAL_ENUM( INPUT( "variables" ), aerodynamics::aerodynamicVariables, aerodynamics::unsupportedAerodynamicVariables );
}
...
BOOST_AUTO_TEST_CASE( test_json_aerodynamics_dragCoefficient )
{
    ...
    const std::shared_ptr< AerodynamicCoefficientSettings > manualSettings =
            std::make_shared< ConstantAerodynamicCoefficientSettings >( referenceArea, forceCoefficients );
...
BOOST_AUTO_TEST_CASE( test_json_aerodynamics_tabulated1 )
{
    ...
    const std::vector< Eigen::Vector3d > forceCoefficients = { ( Eigen::Vector3d( ) << 0.7, 0.8, 0.9 ).finished( ),
                                                               ( Eigen::Vector3d( ) << 1.7, 1.8, 1.9 ).finished( ),
                                                               ( Eigen::Vector3d( ) << 2.7, 2.8, 2.9 ).finished( ),
                                                               ( Eigen::Vector3d( ) << 3.7, 3.8, 3.9 ).finished( ) };
    const std::vector< Eigen::Vector3d > momentCoefficients = { ( Eigen::Vector3d( ) << 1.0, 2.0, 3.0 ).finished( ),
...
BOOST_AUTO_TEST_CASE( test_json_aerodynamics_tabulatedN )
{
    ...
    const double lateralReferenceLength = 4.0;
    const Eigen::Vector3d momentReferencePoint = ( Eigen::Vector3d( ) << 0.7, 0.8, 0.9 ).finished( );
    const std::vector< AerodynamicCoefficientsIndependentVariables > independentVariableNames = { mach_number_dependent,
                                                                                                  angle_of_attack_dependent };
    const bool areCoefficientsInAerodynamicFrame = true;
    const bool areCoefficientsInNegativeAxisDirection = true;
    const std::shared_ptr< AerodynamicCoefficientSettings > manualSettings =
            readTabulatedAerodynamicCoefficientsFromFiles( forceCoefficientsFiles,
