...
void testTimeConversions( )
{
    ...
    // Get UTC time.
    ScalarType utcSecondsSinceJ2000 = convertCalendarDateToJulianDaysSinceEpoch< ScalarType >(
                                              year, months, days, hours, minutes, seconds, getJulianDayOnJ2000< ScalarType >( ) ) *
    ...
    BOOST_CHECK_CLOSE_FRACTION( taiSecondsSinceJ2000 - utcSecondsSinceJ2000,
                                numberOfLeapSeconds,
    ...
    BOOST_CHECK_SMALL( recoveredUtcSecondsSinceJ2000 - utcSecondsSinceJ2000, std::numeric_limits< ScalarType >::epsilon( ) );
...
BOOST_AUTO_TEST_CASE( testSofaTimeConversions )
{
    ...
    // Test TDB - TT calculation, using code from SOFA Time Conversions cookbook.
    {
        ...
        int minutes = 24;
        double seconds = 37.5;
        double utcSecondsSinceJ2000 = convertCalendarDateToJulianDaysSinceEpoch< double >(
        ...
        double slatn, slonw, hm, elon, phi, xyz[ 3 ], u, v, sec, utc1, utc2, ut11, ut12, ut, tai1, tai2, tt1, tt2, tcg1, tcg2;
...
BOOST_AUTO_TEST_CASE( testLeapSecondIdentification )
{
    ...
    for( unsigned int i = 0; i < leapSecondDays.rows( ); i++ )
    {
        ...
        BOOST_CHECK_EQUAL( sofa_interface::getDeltaAtFromUtc( utcTimeOfLeapSeconds - 1.0E-6 ) -
                                   sofa_interface::getDeltaAtFromUtc( utcTimeOfLeapSeconds + 1.0E-6 ),
                           -1.0 );
        BOOST_CHECK_EQUAL( sofa_interface::getDeltaAtFromUtc( utcTimeOfLeapSeconds + 1.0E-6 ) -
                                   sofa_interface::getDeltaAtFromUtc( utcTimeOfLeapSeconds + 2.0E-6 ),
                           0.0 );

        double taiPreLeap =
                tudat::sofa_interface::convertUTCtoTAI< double >( utcTimeOfLeapSeconds * physical_constants::JULIAN_DAY - 1.0E-6 );
        double taiPostLeap =
                tudat::sofa_interface::convertUTCtoTAI< double >( utcTimeOfLeapSeconds * physical_constants::JULIAN_DAY + 1.0E-6 );
    ...
    for( unsigned int i = 0; i < leapSecondDays.rows( ); i++ )
    {
        ...
        Time taiPreLeap = tudat::sofa_interface::convertUTCtoTAI< Time >( utcTimeOfLeapSeconds * physical_constants::JULIAN_DAY - 1.0E-6 );
        Time taiPostLeap = tudat::sofa_interface::convertUTCtoTAI< Time >( utcTimeOfLeapSeconds * physical_constants::JULIAN_DAY + 1.0E-6 );
