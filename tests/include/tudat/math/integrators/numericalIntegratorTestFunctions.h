 ...
 */
static inline Eigen::VectorXd computeZeroStateDerivative( const double time, const Eigen::VectorXd& state )
{
    return Eigen::VectorXd::Zero( state.rows( ) );
}

static inline Eigen::VectorXd computeConstantStateDerivative( const double time, const Eigen::VectorXd& state )
{
 ...
 */
static inline Eigen::VectorXd computeAnalyticalStateForZeroStateDerivative( const double time, const Eigen::VectorXd& initialState )
{
    return initialState;
}
 ...
 */
static inline Eigen::VectorXd computeVanDerPolStateDerivative( const double time, const Eigen::VectorXd& state )
{
    ...
    vanDerPolStateDerivative( 1 ) = ( 1.0 - std::pow( state( 0 ), 2.0 ) ) * state( 1 ) + state( 0 );
 ...
 * \return Computed state derivative.
 */
static inline Eigen::VectorXd computeFehlbergLogirithmicTestODEStateDerivative( const double time, const Eigen::VectorXd& state )
{
...
static inline Eigen::VectorXd computeAnalyticalStateFehlbergODE( const double time, const Eigen::VectorXd& initialState )
{
    // Declare state derivative vector
 ...
 */
static inline Eigen::VectorXd computeNonAutonomousModelStateDerivative( const double time, const Eigen::VectorXd& state )
{
 ...
 * \return State derivative, length equal to state values according to above expression.
 */
static inline Eigen::VectorXd computeSecondNonAutonomousModelStateDerivative( const double time, const Eigen::VectorXd& state )
{
