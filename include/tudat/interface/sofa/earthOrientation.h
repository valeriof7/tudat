...
enum IAUConventions { iau_2000_a, iau_2000_b, iau_2006 };
 ...
 */
double calculateGreenwichMeanSiderealTime( const double terrestrialTime,
 ...
 */
double calculateEquationOfEquinoxes( const double terrestrialTime,
                                     const double referenceJulianDay = basic_astrodynamics::JULIAN_DAY_ON_J2000,
                                     const basic_astrodynamics::IAUConventions iauConvention = basic_astrodynamics::iau_2000_b );
 ...
 * IAU models. It is a function of the epoch that is the argument of this function. Since Tudat usually works with times measured from
 * J2000, it is normally sufficient to provide the first argument only, containing the number of seconds terrestrial time since the J2000
 * epoch. If a different reference epoch is desired, it is mandatory to provide this epoch as a Julian day as the second argument.
...
/*!
 * Function to retrieve the IAU-1976 precession model angles zeta, z, and theta. These angles embody the transformation from the
 * Mean-Of-Date frame to the GCRF frame. The definition of these angles can be found on pages 226 to 228 in Vallado (2013).
 ...
 */
double calculateEarthRotationAngle( const double ut1, const double referenceJulianDay );
 ...
 * \return
 */
Eigen::Matrix3d getFrameBias( const double julianDaysSinceReference,
