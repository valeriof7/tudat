...
#include "tudat/interface/json/propagation/massRateModel.h"
#include "tudat/interface/json/propagation/state.h"
#include "tudat/interface/json/propagation/termination.h"
#include "tudat/interface/json/propagation/torque.h"
...
void determineInitialStates( nlohmann::json& jsonObject,
                             ...
                             const std::shared_ptr< numerical_integrators::IntegratorSettings< TimeType > >& integratorSettings )
{
    ...
    for( nlohmann::json& jsonPropagator: jsonPropagators )
    {
        if( !isDefined( jsonPropagator, K::initialStates ) )
        {
            ...
            const IntegratedStateType integratedStateType = getValue( jsonPropagator, K::integratedStateType, translational_state );
            ...
            Eigen::Matrix< StateScalarType, Eigen::Dynamic, 1 > initialStates =
                    Eigen::Matrix< StateScalarType, Eigen::Dynamic, 1 >::Zero( stateSize * bodiesToPropagate.size( ), 1 );
                ...
                try
                {
                    if( integratedStateType == translational_state )
                    {
                        ...
                        try
                        {
                            ...
                            initialStateOrigin = getValue< std::string >( jsonState, Keys::Body::initialStateOrigin );

                            if( initialStateOrigin != centralBodyName )
                            {
                                stateToAdd = getInitialStateOfBody< TimeType, StateScalarType >(
                                        initialStateOrigin, centralBodyName, bodies, integratorSettings->initialTime_ );
                            }
                        }
                        catch( ... )
                        {
                        ...
                        bodyState = getCartesianState< StateScalarType >(
                                            jsonObject, stateKeyPath, stateOriginBody, integratorSettings->initialTime_ ) +
                                stateToAdd;
                    }
                    else
                    {
                        bodyState = getValue< Eigen::Matrix< StateScalarType, Eigen::Dynamic, 1 > >( jsonObject, stateKeyPath );
                    }
                }
                catch( ... )
                {
                    if( integratedStateType == translational_state )
                    {
                        bodyState = getInitialStateOfBody< TimeType, StateScalarType >(
...
void resetDependentVariableSaveSettings(
        std::shared_ptr< propagators::MultiTypePropagatorSettings< StateScalarType, TimeType > >& propagatorSettings,
...
TimeType getTerminationEpoch( const std::shared_ptr< propagators::PropagationTerminationSettings >& terminationSettings )
{
    using namespace propagators;
    ...
    if( hybridTerminationSettings )
    {
        ...
        {
            const TimeType endEpoch = getTerminationEpoch< TimeType >( hybridTerminationSettings->terminationSettings_.at( i ) );
            if( !isNaN( endEpoch ) )
            {
                return endEpoch;
            }
...
static std::map< IntegratedStateType, std::string > integratedStateTypes = { { hybrid, "hybrid" },
...
static std::map< RotationalPropagatorType, std::string > rotationalPropagatorTypes = { { quaternions, "quaternions" },
                                                                                       { modified_rodrigues_parameters,
                                                                                         "modifiedRodriguesParameters" },
...
void to_json( nlohmann::json& jsonObject,
              const std::shared_ptr< MultiTypePropagatorSettings< StateScalarType, TimeType > >& multiTypePropagatorSettings )
{
    ...
    jsonObject[ Keys::propagators ] = getFlattenedMapValues< std::map,
                                                             IntegratedStateType,
                                                             std::shared_ptr< SingleArcPropagatorSettings< StateScalarType, TimeType > > >(
                ...
                std::shared_ptr< MultiTypePropagatorSettings< StateScalarType, TimeType > >& multiTypePropagatorSettings )
{
    ...
    if( !isDefined( jsonObject, Keys::termination ) || ( isDefined( jsonObject, Keys::finalEpoch ) && timeConditionMissing ) )
    {
        terminationConditions.push_back(
                std::make_shared< PropagationTimeTerminationSettings >( getValue< double >( jsonObject, Keys::finalEpoch ) ) );
    }
    ...
    multiTypePropagatorSettings = std::make_shared< MultiTypePropagatorSettings< StateScalarType, TimeType > >(
            getValue< std::vector< std::shared_ptr< SingleArcPropagatorSettings< StateScalarType, TimeType > > > >( jsonObject,
                                                                                                                    Keys::propagators ),
            terminationSettings,
    ...
    else
    {
        ...
        switch( integratedStateType )
        {
            case translational_state: {
                std::shared_ptr< TranslationalStatePropagatorSettings< StateScalarType, TimeType > > translationalStatePropagatorSettings =
            ...
            case body_mass_state: {
                std::shared_ptr< MassPropagatorSettings< StateScalarType, TimeType > > massPropagatorSettings =
                        std::dynamic_pointer_cast< MassPropagatorSettings< StateScalarType, TimeType > >( singleArcPropagatorSettings );
            ...
            case rotational_state: {
                std::shared_ptr< RotationalStatePropagatorSettings< StateScalarType, TimeType > > rotationalStatePropagatorSettings =
                ...
                std::shared_ptr< SingleArcPropagatorSettings< StateScalarType, TimeType > >& singleArcPropagatorSettings )
{
    ...
    // Integrated state type
    const IntegratedStateType integratedStateType = getValue( jsonObject, K::integratedStateType, translational_state );

    // Named of bodies to propagate
    const std::vector< std::string > bodiesToPropagate = getValue< std::vector< std::string > >( jsonObject, K::bodiesToPropagate );
