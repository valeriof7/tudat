...
enum StateType { cartesianElements, keplerianElements, sphericalElements };

//! Map of `StateType` string representations.
static std::map< StateType, std::string > stateTypes = { { cartesianElements, "cartesian" },
...
Eigen::Matrix< StateScalarType, 6, 1 > getCartesianState( const nlohmann::json& jsonObject,
        ...
        switch( stateType )
        {
            ...
            case keplerianElements: {
                ...
                else  // generic
                {
                        ...
                        isDefined( jsonState, K::periapsisDistance ) || isDefined( jsonState, K::periapsisAltitude ) )
                    {
                        ...
                        {
                            apoapsisDistance = centralBodyAverageRadius + getValue< StateScalarType >( jsonState, K::apoapsisAltitude );
                            usedCentralBodyAverageRadius = true;
                        }
                        ...
                        {
                            periapsisDistance = centralBodyAverageRadius + getValue< StateScalarType >( jsonState, K::periapsisAltitude );
                            usedCentralBodyAverageRadius = true;
                        }
                    ...
                    else
                    {
                        ...
                        {
                            semiMajorAxis =
                                    getValue< StateScalarType >( jsonState, K::semiLatusRectum ) / ( 1.0 - std::pow( eccentricity, 2.0 ) );
                        }
                        else if( isDefined( jsonState, K::meanMotion ) || isDefined( jsonState, K::period ) )
                        {
                            ...
                            {
                                meanMotion = 2.0 * mathematical_constants::PI / getValue< StateScalarType >( jsonState, K::period );
                            }
                            semiMajorAxis = std::pow( ( centralBodyGravitationalParameter / std::pow( meanMotion, 2.0 ) ), 1.0 / 3.0 );
                        }
                        else
                        {
                            semiMajorAxis = getValue< StateScalarType >( jsonState, K::semiMajorAxis );
                ...
                StateScalarType longitudeOfAscendingNode = getValue< StateScalarType >( jsonState, K::longitudeOfAscendingNode, 0.0 );
                ...
                if( isDefined( jsonState, K::meanAnomaly ) || isDefined( jsonState, K::eccentricAnomaly ) )
                {
                    ...
                    {
                        eccentricAnomaly = convertMeanAnomalyToEccentricAnomaly( eccentricity,
            ...
            case sphericalElements: {
                ...
                bodyState = tudat::ephemerides::transformStateToInertialOrientation< StateScalarType >(
                        convertSphericalOrbitalToCartesianState( sphericalElements ), epoch, centralBody->getRotationalEphemeris( ) );
            ...
            else if( centralBody )
            {
                if( !std::dynamic_pointer_cast< SphericalBodyShapeModel >( centralBody->getShapeModel( ) ) )
                {
                    std::cerr << "Using average radius of a non-spherical body "
                                 "to determine the initial Cartesian state of body."
                              << std::endl;
                }
            }
        }
