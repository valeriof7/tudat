        ...
        const bool variationalEquationsAreSaved = false )
{
    ...
    for( std::shared_ptr< ExportSettings > exportSettings: exportSettingsVector )
    {
            ...
            switch( variable->variableType_ )
            {
                ...
                case dependentVariable: {
                    ...
                    try
                    {
                        ...
                        try
                        {
                            variableIndex = getKeyWithValue( singleArcDynamicsSimulator->getDependentVariableIds( ), variableID );
                            try
                            {
                        ...
                        {
                            std::cerr << "Could not export the results for variable \"" << variableID << "\" "
                                      << "because the main propagator was not configured to compute this variable." << std::endl;
                        }
                    }
                    catch( ... )
                    {
                        std::cerr << "Could not export results for dependent variable of type " << dependentVar->dependentVariableType_
                ...
                case stateTransitionMatrix: {
                    if( !variationalEquationsAreSaved )
                    {
                        throw std::runtime_error(
                                "Error, requested save of state transition matrix, but only equations of motion available" );
                    }
                    break;
                }
                ...
                default: {
                    std::cerr << "Could not export results for variable of unsupported type " << variable->variableType_ << "."
                              << std::endl;
                    break;
                }
            }
        ...
        {
            if( ( it == statesHistory.begin( ) && !exportSettings->onlyInitialStep_ && exportSettings->onlyFinalStep_ ) ||
                ( it == --statesHistory.end( ) && !exportSettings->onlyFinalStep_ && exportSettings->onlyInitialStep_ ) ||
            ...
            for( unsigned int i = 0; i < variables.size( ); ++i )
            {
                ...
                switch( variable->variableType_ )
                {
                    ...
                    case cpuTimeVariable: {
                        result.segment( currentIndex, variableSize ) = ( Eigen::VectorXd( 1 ) << cpuTimes.at( epoch ) ).finished( );
                        break;
                    }
                    case stateVariable: {
