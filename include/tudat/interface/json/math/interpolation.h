...
{
    oneDimensionalInterpolatorType = json_interface::enumFromString( jsonObject, oneDimensionalInterpolatorTypes );
}

// AvailableLookupScheme

//! Map of `AvailableLookupScheme`s string representations.
static std::map< AvailableLookupScheme, std::string > lookupSchemeTypes = { { huntingAlgorithm, "huntingAlgorithm" },
...
static std::vector< BoundaryInterpolationType > unsupportedBoundaryInterpolationTypes = { use_default_value,
...
{
    jsonObject = json_interface::stringFromEnum( lagrangeInterpolatorBoundaryHandling, lagrangeInterpolatorBoundaryHandlings );
}

//! Convert `json` to `LagrangeInterpolatorBoundaryHandling`.
inline void from_json( const nlohmann::json& jsonObject, LagrangeInterpolatorBoundaryHandling& lagrangeInterpolatorBoundaryHandling )
{
    lagrangeInterpolatorBoundaryHandling = json_interface::enumFromString( jsonObject, lagrangeInterpolatorBoundaryHandlings );
}
...
void to_json( nlohmann::json& jsonObject, const std::shared_ptr< DataMapSettings< I, D > >& dataMapSettings )
{
    ...
    if( hermiteDataSettings )
    {
        ...
        jsonObject[ K::dependentVariableFirstDerivativeValues ] = hermiteDataSettings->firstDerivativeOfDependentVariables_;
        return;
    }
...
void from_json( const nlohmann::json& jsonObject, std::shared_ptr< DataMapSettings< I, D > >& dataMapSettings )
{
    ...
    {
        dataMapSettings =
                std::make_shared< FromFileDataMapSettings< D > >( getValue< boost::filesystem::path >( jsonObject, K::file ).string( ) );
        return;
    }
    ...
    dataMapSettings = std::make_shared< DataMapSettings< I, D > >( getValue< std::map< I, D > >( jsonObject, K::map ) );
    return;
}
...
template< typename I, typename D >
void to_json( nlohmann::json& jsonObject, const std::shared_ptr< DataInterpolationSettings< I, D > >& dataInterpolationSettings )
{
...
template< typename I, typename D >
void from_json( const nlohmann::json& jsonObject, std::shared_ptr< DataInterpolationSettings< I, D > >& dataInterpolationSettings )
{
    using namespace json_interface;
    using K = Keys::Interpolation::DataInterpolation;
