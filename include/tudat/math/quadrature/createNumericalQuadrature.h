...
enum AvailableQuadratures { trapezoidal, gaussian };
...
public:
     ...
     *  \param numberOfNodes Number of nodes at which the function to be integrated will be evaluated. Must be an integer value between 2
     * and 64.
     */
    GaussianQuadratureSettings( const IndependentVariableType initialIndependentVariable, const unsigned int numberOfNodes ):
        QuadratureSettings< double >( gaussian ), initialIndependentVariable_( initialIndependentVariable ), numberOfNodes_( numberOfNodes )
    {
    ...
    switch( quadratureSettings->quadratureType_ )
    {
        case gaussian: {
            ...
            quadrature = std::make_shared< GaussianQuadrature< IndependentVariableType, DependentVariableType > >(
                    derivativeFunction,
                    gaussianQuadratureSettings->initialIndependentVariable_,
        ...
        case trapezoidal: {
            ...
            for( unsigned int currentIndependentValue = 0; currentIndependentValue < independentVariables.size( );
                 currentIndependentValue++ )
            {
