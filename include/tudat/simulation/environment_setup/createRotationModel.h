...
public:
    ...
    RotationModelType getRotationType( )
    {
...
public:
     ...
     */
    Eigen::Quaterniond getInitialOrientation( )
    {
        return initialOrientation_;
    }

    Eigen::Matrix3d getInitialOrientationMatrix( )
    {
        return initialOrientation_.toRotationMatrix( );
    }
...
struct EopCorrectionSettings {
     ...
     */
    EopCorrectionSettings( const double conversionFactor,
...
public:
    ...
    GcrsToItrsRotationModelSettings(
            ...
            const std::shared_ptr< EopCorrectionSettings > ut1CorrectionSettings = std::make_shared< EopCorrectionSettings >(
                    1.0E-6,
                    0.0,
                    std::vector< std::string >{ paths::getEarthOrientationDataFilesPath( ) + "/utcLibrationAmplitudes.txt",
                                                paths::getEarthOrientationDataFilesPath( ) + "/utcOceanTidesAmplitudes.txt" },
                    std::vector< std::string >{
                            paths::getEarthOrientationDataFilesPath( ) + "/utcLibrationFundamentalArgumentMultipliers.txt",
                            paths::getEarthOrientationDataFilesPath( ) + "/utcOceanTidesFundamentalArgumentMultipliers.txt" } ),
            const std::shared_ptr< EopCorrectionSettings > polarMotionCorrectionSettings = std::make_shared< EopCorrectionSettings >(
                    unit_conversions::convertArcSecondsToRadians< double >( 1.0E-6 ),
                    0.0,
                    std::vector< std::string >{
                            paths::getEarthOrientationDataFilesPath( ) + "/polarMotionLibrationAmplitudesQuasiDiurnalOnly.txt",
                            paths::getEarthOrientationDataFilesPath( ) + "/polarMotionOceanTidesAmplitudes.txt",
                    },
                    std::vector< std::string >{ paths::getEarthOrientationDataFilesPath( ) +
...
public:
     ...
     */
    SynchronousRotationModelSettings( const std::string& centralBodyName,
...
{
public:
    TabulatedRotationSettings( const std::map< double, Eigen::Vector7d >& rotationalStateHistory,
                               const std::string& baseFrameOrientation,
                               const std::string& targetFrameOrientation,
...
{
public:
    AerodynamicAngleRotationSettings( const std::string& centralBody,
                                      const std::string& baseFrameOrientation,
                                      const std::string& targetFrameOrientation,
...
{
public:
    PitchTrimRotationSettings( const std::string& centralBody,
                               const std::string& baseFrameOrientation,
                               const std::string& targetFrameOrientation,
...
enum InertialDirectionTypes { custom_inertial_direction, state_based_inertial_direction, bilinear_tangent_inertial_direction };
...
public:
    InertialDirectionSettings( const InertialDirectionTypes inertialDirectionType,
        ...
        inertialDirectionType_( inertialDirectionType ), directionFrame_( directionFrame )
    {
            ...
            directionFrame.first != ephemerides::inertial_satellite_based_frame )
        {
            throw std::runtime_error(
                    "Error when making inertial direction settings for direction-based rotation model, state-based direction can only be "
                    "provided in inertial coordinates" );
        }
    }
...
{
public:
    CustomInertialDirectionSettings( const std::function< Eigen::Vector3d( const double ) > inertialBodyAxisDirectionFunction,
                                     std::pair< ephemerides::SatelliteBasedFrames, std::string > directionFrame =
                                             std::make_pair( ephemerides::inertial_satellite_based_frame, "" ) ):
...
{
public:
    StateBasedInertialDirectionSettings( const std::string& centralBody,
...
{
public:
    BilinearTangentInertialDirectionSettings( const std::vector< double >& arcStartTimes,
...
{
public:
    BodyFixedDirectionBasedRotationSettings( const std::function< Eigen::Vector3d( const double ) > inertialBodyAxisDirectionFunction,
                                             const std::string& baseFrameOrientation,
                                             const std::string& targetFrameOrientation,
...
{
public:
    OrbitalStateBasedRotationSettings( const std::string& centralBody,
                                       const bool isColinearWithVelocity,
                                       const bool directionIsOppositeToVector,
                                       const std::string& baseFrameOrientation,
                                       const std::string& targetFrameOrientation,
...
{
public:
    CustomRotationModelSettings( const std::string& baseFrameOrientation,
                                 const std::string& targetFrameOrientation,
...
public:
    PlanetaryRotationModelSettings(
        ...
        RotationModelSettings( planetary_rotation_model, originalFrame, targetFrame ), angleN_( angleN ), angleJ_( angleJ ),
        ...
        coreFactor_( coreFactor ), freeCoreNutationRate_( freeCoreNutationRate ),
 ...
 */
std::shared_ptr< aerodynamics::TrimOrientationCalculator > setTrimmedConditions( const std::shared_ptr< Body > bodyWithFlightConditions );
...
inline std::shared_ptr< RotationModelSettings > simpleRotationModelSettings( const std::string& originalFrame,
                                                                             ...
                                                                             const double rotationRate )
{
    return std::make_shared< SimpleRotationModelSettings >( originalFrame, targetFrame, initialOrientation, initialTime, rotationRate );
}
...
inline std::shared_ptr< RotationModelSettings > constantRotationModelSettings( const std::string& originalFrame,
                                                                               ...
                                                                               const Eigen::Quaterniond& initialOrientation )
{
    return std::make_shared< SimpleRotationModelSettings >( originalFrame, targetFrame, initialOrientation, 0.0, 0.0 );
}
...
inline std::shared_ptr< RotationModelSettings > spiceRotationModelSettings( const std::string& originalFrame,
                                                                            const std::string& targetFrame,
                                                                            const std::string& spiceFrameName )
{
    return std::make_shared< SpiceRotationModelSettings >( originalFrame, targetFrame, spiceFrameName );
}
        ...
        const std::shared_ptr< interpolators::InterpolatorGenerationSettings< double > > shortTermInterpolatorSettings = nullptr )
{
    std::shared_ptr< GcrsToItrsRotationModelSettings > rotationSettings =
            std::make_shared< GcrsToItrsRotationModelSettings >( nutationTheory, baseFrameName );
...
inline std::shared_ptr< RotationModelSettings > synchronousRotationModelSettings( const std::string& centralBodyName,
                                                                                  const std::string& baseFrameOrientation,
                                                                                  const std::string& targetFrameOrientation )
{
    return std::make_shared< SynchronousRotationModelSettings >( centralBodyName, baseFrameOrientation, targetFrameOrientation );
}
...
inline std::shared_ptr< RotationModelSettings > orbitalStateBasedRotationSettings(
        ...
        const std::function< double( const double ) > freeRotationAngleFunction = nullptr )
{
    return std::make_shared< OrbitalStateBasedRotationSettings >( centralBody,
                                                                  isColinearWithVelocity,
