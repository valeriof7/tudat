...
public:
    ...
    DirectFirstOrderDopplerProperTimeRateSettings( const std::string centralBodyName ):
        DopplerProperTimeRateSettings( direct_first_order_doppler_proper_time_rate ), centralBodyName_( centralBodyName )
    { }
...
public:
    ...
    TwoWayDopplerObservationSettings( const std::shared_ptr< OneWayDopplerObservationSettings > uplinkOneWayDopplerSettings,
                                      ...
                                      const std::shared_ptr< ObservationBiasSettings > biasSettings = nullptr ):
        ObservationModelSettings( two_way_doppler,
...
inline std::shared_ptr< ObservationModelSettings > twoWayOpenLoopDoppler(
        ...
        const std::shared_ptr< ObservationBiasSettings > biasSettings = nullptr )
{
    return std::make_shared< TwoWayDopplerObservationSettings >( uplinkOneWayDopplerSettings, downlinkOneWayDopplerSettings, biasSettings );
}
        ...
        const LinkEndType linkEndForCalculator )
{
    ...
    switch( properTimeRateSettings->dopplerProperTimeRateType_ )
    {
        case direct_first_order_doppler_proper_time_rate: {
            ...
            {
                throw std::runtime_error(
                        "Error when making DopplerProperTimeRateInterface, input type "
                        "(direct_first_order_doppler_proper_time_rate) is inconsistent" );
            }
            ...
            else
            {
                ...
                {
                    throw std::runtime_error(
                            "Error when making DirectFirstOrderDopplerProperTimeRateInterface, no "
                            "gravity field found for " +
                            directFirstOrderDopplerProperTimeRateSettings->centralBodyName_ );
                }
                else
                {
                    // Retrieve gravitational parameter
                    std::function< double( ) > gravitationalParameterFunction = std::bind(
                            &gravitation::GravityFieldModel::getGravitationalParameter,
                            bodies.at( directFirstOrderDopplerProperTimeRateSettings->centralBodyName_ )->getGravityFieldModel( ) );

                    // Create calculation object.
                    LinkEndId referencePointId = LinkEndId( directFirstOrderDopplerProperTimeRateSettings->centralBodyName_, "" );
                    ...
                    else
                    {
                        throw std::runtime_error(
                                ...
                                "implemented for DopplerProperTimeRateInterface creation" );
                    }
                }
            }
            break;
            ...
            ObservableType topLevelObservableType = undefined_observation_model )
    {
        ...
        switch( observationSettings->observableType_ )
        {
            ...
            case two_way_doppler: {
                ...
                {
                    observationModel = std::make_shared< TwoWayDopplerObservationModel< ObservationScalarType, TimeType > >(
                            linkEnds,
                ...
                else
                {
                    observationModel = std::make_shared< TwoWayDopplerObservationModel< ObservationScalarType, TimeType > >(
                            ...
                            observationBias,
                            twoWayDopplerSettings->normalizeWithSpeedOfLight_ );
                }
            ...
            case one_way_differenced_range: {
                //            std::shared_ptr< OneWayDifferencedRangeRateObservationSettings >
                //            rangeRateObservationSettings =
                //                    std::dynamic_pointer_cast<
                //                    OneWayDifferencedRangeRateObservationSettings >(
                //                    observationSettings );
                //            if( rangeRateObservationSettings == nullptr )
                //            {
                //                throw std::runtime_error( "Error when making differenced one-way
                //                range rate, input type is inconsistent" );
                //            }
                // Check consistency input.
                if( linkEnds.size( ) != 2 )
                {
            ...
            case doppler_measured_frequency: {
                ...
                try
                {
                    ...
                    auto twoWaySettings =
                            std::make_shared< TwoWayDopplerObservationSettings >( uplinkOneWaySettings, downlinkOneWaySettings );
                    ...
                    {
                        throw std::runtime_error(
                                "Error when getting two-way Doppler model for measured frequency "
                                "observable, model in null." );
                    }
                }
                catch( const std::exception& caughtException )
                {
