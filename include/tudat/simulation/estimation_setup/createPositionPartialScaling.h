            ...
            case observation_models::one_way_doppler: {
                ...
                if( dopplerObservationModel == nullptr )
                {
                    throw std::runtime_error(
                            "Error when creating one-way Doppler partial scaling object, input observation model is incompatible" );
                }
                ...
                std::shared_ptr< OneWayDopplerProperTimeComponentScaling > transmitterProperTimePartials = createDopplerProperTimePartials(
                        dopplerObservationModel->getTransmitterProperTimeRateCalculator( ), linkEnds, observation_models::transmitter );
...

    {
        ...
        switch( differencedObservableType )
        {
            case observation_models::one_way_differenced_range: {
                ...
                {
                    throw std::runtime_error(
                            "Error when creating one-way differenced range partial scaling object, first range partial is of incompatible "
                            "type" );
                }
                if( std::dynamic_pointer_cast< OneWayRangeScaling >( secondPositionPartialScaling ) == nullptr )
                {
                    throw std::runtime_error(
                            "Error when creating one-way differenced range partial scaling object, second range partial is of incompatible "
                            "type" );
                }
            ...
            default:
                throw std::runtime_error( "Error when creating differenced observable partial scaler for " +
                                          observation_models::getObservableName( differencedObservableType ) + ", type not recognized. " );
        }
        return positionPartialScaler;
    }
};
...

    {
        ...
        switch( differencedObservableType )
        {
            case observation_models::relative_angular_position: {
                ...
                {
                    throw std::runtime_error(
                            "Error when creating relative angular position partial scaling object, first range partial is of incompatible "
                            "type" );
                }
                if( std::dynamic_pointer_cast< AngularPositionScaling >( secondPositionPartialScaling ) == nullptr )
                {
                    throw std::runtime_error(
                            "Error when creating relative angular position partial scaling object, second range partial is of incompatible "
                            "type" );
                }
                std::function< void( const observation_models::LinkEndType ) > customCheckFunction =
                        []( const observation_models::LinkEndType fixedLinkEnd ) {
                            ...
                            {
                                throw std::runtime_error(
                                        "Error when updating a relative angular position scaling object, fixed link end time different "
                                        "from receiver." );
                            }
                        };
                positionPartialScaler = std::make_shared< DifferencedObservablePartialScaling >(
