...
{
public:
    typedef std::map< ObservableType,
                      std::map< LinkEnds, std::vector< std::shared_ptr< SingleObservationSet< ObservationScalarType, TimeType > > > > >
            ...
            std::shared_ptr< ObservationCollectionParser > observationParser = std::make_shared< ObservationCollectionParser >( ) ) const
    {
        ...
        for( auto observableIt: getSingleObservationSetsIndices( observationParser ) )
        {
            for( auto linkEndsIt: observableIt.second )
            {
                ...
                {
                    timeBounds.push_back(
                            observationSetList_.at( observableIt.first ).at( linkEndsIt.first ).at( index )->getTimeBounds( ) );
                }
            }
        }
        return timeBounds;
    ...
    std::map< ObservableType, std::vector< LinkEnds > > getLinkEndsPerObservableType( )
    {
        ...
        for( auto observableTypeIt = observationSetList_.begin( ); observableTypeIt != observationSetList_.end( ); ++observableTypeIt )
        {
            ...
            for( auto linkEndsIt = observableTypeIt->second.begin( ); linkEndsIt != observableTypeIt->second.end( ); ++linkEndsIt )
            {
            ...
            for( auto linkEndsIt: observableIt.second )
            {
                for( auto index: linkEndsIt.second )
                {
                    observations.push_back(
                            observationSetList_.at( observableIt.first ).at( linkEndsIt.first ).at( index )->getObservationsVector( ) );
                }
            }
        }
        return observations;
            ...
            std::shared_ptr< ObservationCollectionParser > observationParser = std::make_shared< ObservationCollectionParser >( ) ) const
    {
        ...
        for( auto observableIt: observationSetsIndices )
        {
            for( auto linkEndsIt: observableIt.second )
            {
                for( auto index: linkEndsIt.second )
                {
                    observationsTimes.push_back(
                            observationSetList_.at( observableIt.first ).at( linkEndsIt.first ).at( index )->getObservationTimes( ) );
                }
            }
        }
    ...
    std::pair< std::vector< Eigen::Matrix< ObservationScalarType, Eigen::Dynamic, 1 > >, std::vector< std::vector< TimeType > > >
    getObservationsAndTimes(
            std::shared_ptr< ObservationCollectionParser > observationParser = std::make_shared< ObservationCollectionParser >( ) )
    {
            ...
            std::shared_ptr< ObservationCollectionParser > observationParser = std::make_shared< ObservationCollectionParser >( ) ) const
    {
        ...
        for( auto observableIt: observationSetsIndices )
        {
            for( auto linkEndsIt: observableIt.second )
            {
                ...
                {
                    residuals.push_back(
                            observationSetList_.at( observableIt.first ).at( linkEndsIt.first ).at( index )->getResidualsVector( ) );
                }
            }
        }
        return residuals;
            ...
            std::shared_ptr< ObservationCollectionParser > observationParser = std::make_shared< ObservationCollectionParser >( ) ) const
    {
        ...
        for( auto observableIt: observationSetsIndices )
        {
            for( auto linkEndsIt: observableIt.second )
            {
                for( auto index: linkEndsIt.second )
                {
                    rmsResiduals.push_back(
                            observationSetList_.at( observableIt.first ).at( linkEndsIt.first ).at( index )->getRmsResiduals( ) );
                }
            }
        }
        return rmsResiduals;
            ...
            std::shared_ptr< ObservationCollectionParser > observationParser = std::make_shared< ObservationCollectionParser >( ) ) const
    {
        ...
        for( auto observableIt: observationSetsIndices )
        {
            for( auto linkEndsIt: observableIt.second )
            {
                for( auto index: linkEndsIt.second )
                {
                    meanResiduals.push_back(
                            observationSetList_.at( observableIt.first ).at( linkEndsIt.first ).at( index )->getMeanResiduals( ) );
                }
            }
        }
        return meanResiduals;
    ...
    {
        std::vector< Eigen::Matrix< ObservationScalarType, Eigen::Dynamic, 1 > > computedObservations =
                getComputedObservations( observationParser );
            ...
            for( auto linkEndsIt: observableIt.second )
            {
                for( auto index: linkEndsIt.second )
                {
                    currentObservableTypeIndices[ linkEndsIt.first ].push_back(
                            observationSetStartAndSize_.at( observableIt.first ).at( linkEndsIt.first ).at( index ) );
                }
            }
    ...
    void filterObservations(
            std::shared_ptr< ObservationFilterBase > observationFilter,
            std::shared_ptr< ObservationCollectionParser > observationParser = std::make_shared< ObservationCollectionParser >( ),
            const bool saveFilteredObservations = true )
    {
    ...
    void splitObservationSets(
            std::shared_ptr< ObservationSetSplitterBase > observationSetSplitter,
            std::shared_ptr< ObservationCollectionParser > observationParser = std::make_shared< ObservationCollectionParser >( ) )
    {
        // Retrieve single observation sets based on observation parser
        std::map< ObservableType, std::map< LinkEnds, std::vector< unsigned int > > > singleObsSetsIndices =
                getSingleObservationSetsIndices( observationParser );
        ...
        for( auto observableIt: singleObsSetsIndices )
        {
            for( auto linkEndsIt: observableIt.second )
            {
                ...
                for( auto indexSetToSplit: linkEndsIt.second )
                {
                    ...
                    std::vector< std::shared_ptr< SingleObservationSet< ObservationScalarType, TimeType > > > newObsSets =
                            splitObservationSet(
                                    singleObsSets.at( indexSetToSplit + obsSetCounter ), observationSetSplitter, !warningPrinted );
                    warningPrinted = true;
            ...
            for( auto linkEndsIt: observableIt.second )
            {
                ...
                for( auto indexToRemove: linkEndsIt.second )
                {
                    if( indexToRemove - counterRemovedSets >
                        observationSetList_.at( observableIt.first ).at( linkEndsIt.first ).size( ) - 1 )
                    {
                        throw std::runtime_error(
    ...
    void printObservationSetsStartAndSize( ) const
    {
        ...
        for( auto observableIt: observationSetStartAndSize_ )
        {
            for( auto linkEndsIt: observableIt.second )
            {
                ...
                {
                    std::cout << "start index " << indices.first << " - size " << indices.second << " : " << "type " << observableIt.first
                              << ", link ends " << getLinkEndsString( linkEndsIt.first ) << std::endl;
                }
            }
        }
        std::cout << "\n\n";
            ...
            const std::shared_ptr< ObservationCollectionParser > observationParser = std::make_shared< ObservationCollectionParser >( ) )
    {
        ...
        std::map< std::string, std::shared_ptr< ephemerides::ConstantEphemeris > > existingReferencePoints =
                bodies.at( spacecraftName )->getVehicleSystems( )->getFixedReferencePoints( );
        ...
        for( auto refPointsIt: existingReferencePoints )
        {
            ...
            else if( ( refPointsIt.second->getCartesianState( ).segment( 0, 3 ) == antennaPosition ) &&
            ...
            const std::shared_ptr< ObservationCollectionParser > observationParser = std::make_shared< ObservationCollectionParser >( ) )
    {
        ...
        {
            std::cerr << "Warning when setting reference point (" << spacecraftName << ", " << antennaName
            ...
            const std::shared_ptr< ObservationCollectionParser > parser = std::make_shared< ObservationCollectionParser >( ) )
    {
        ...
        for( auto set: singleObsSets )
        {
            dependentVariablesValues.push_back(
                    set->getSingleDependentVariable( dependentVariableSettings, returnFirstCompatibleSettings ) );
        }
            ...
            const std::shared_ptr< ObservationCollectionParser > parser = std::make_shared< ObservationCollectionParser >( ) )
    {
        ...
        {
            dependentVariableResult.push_back(
                    set->getSingleDependentVariableHistory( dependentVariableSettings, returnFirstCompatibleSettings ) );
        }
    ...
    std::pair< std::vector< std::vector< std::shared_ptr< ObservationDependentVariableSettings > > >,
    ...
    std::pair< std::vector< std::vector< Eigen::MatrixXd > >, std::shared_ptr< ObservationCollectionParser > >
            ...
            const std::shared_ptr< ObservationCollectionParser > parser = std::make_shared< ObservationCollectionParser >( ) )
    {
        ...
        {
            concatenatedDependentVariables.block( index, 0, dependentVariables.first.at( k ).rows( ), dependentVariableSize ) =
...
private:
    void setObservationSetIndices( )
    {
        ...
        for( auto observationIterator: observationSetList_ )
        {
            ...
            observationTypeStartAndSize_[ currentObservableType ] = std::make_pair( currentTypeStartIndex, currentObservableTypeSize );
        }
    }
            ...
            const std::shared_ptr< simulation_setup::ObservationDependentVariableSettings > dependentVariableSettings )
    {
        ...
        if( !isObservationDependentVariableAncilliarySetting( dependentVariableSettings->variableType_ ) )
        {
            ...
            else if( dependentVariableSettings->linkEndType_ != unidentified_link_end )
            {
                parserList.push_back(
                        std::make_shared< ObservationCollectionLinkEndTypeParser >( dependentVariableSettings->linkEndType_ ) );
            }
            ...
            {
                parserList.push_back( std::make_shared< ObservationCollectionSingleLinkEndParser >( std::make_pair(
                        dependentVariableSettings->originatingLinkEndType_, dependentVariableSettings->originatingLinkEndId_ ) ) );
            }
            // if only originating link end id is specified
            else if( dependentVariableSettings->originatingLinkEndId_ != LinkEndId( "", "" ) )
            {
                parserList.push_back(
                        std::make_shared< ObservationCollectionLinkEndIdParser >( dependentVariableSettings->originatingLinkEndId_ ) );
            }
            ...
            {
                parserList.push_back(
                        std::make_shared< ObservationCollectionLinkEndTypeParser >( dependentVariableSettings->originatingLinkEndType_ ) );
            }
        ...
        else
        {
            ...
            if( std::dynamic_pointer_cast< simulation_setup::AncillaryObservationDependentVariableSettings >( dependentVariableSettings ) !=
                nullptr )
            {
                std::shared_ptr< simulation_setup::AncillaryObservationDependentVariableSettings > ancillaryDependentVariables =
                        std::dynamic_pointer_cast< simulation_setup::AncillaryObservationDependentVariableSettings >(
                                dependentVariableSettings );
                if( ancillaryDependentVariables->observableType_ != undefined_observation_model )
                {
                    parserList.push_back(
                            std::make_shared< ObservationCollectionObservableTypeParser >( ancillaryDependentVariables->observableType_ ) );
                }
                else
                {
        ...
        const std::map< std::shared_ptr< ObservationCollectionParser >, std::shared_ptr< ObservationFilterBase > >& observationFilters )
{
    ...
    std::shared_ptr< ObservationCollection< ObservationScalarType, TimeType > > newObservationCollection =
            createNewObservationCollection( observationCollection );
