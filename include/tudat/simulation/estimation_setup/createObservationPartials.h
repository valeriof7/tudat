...
//! containers
/*!
 *  Function to split observation partials and scaling object (produced by
 * observationPartialsAndScaler function) into separate containers \param
 * observationPartialsAndScalers Combined list of observation partials and scaling objects \param
 * observationPartials List of observation partials, per link ends, and per parameter indices
 * (returned by reference) \param observationPartialScalers List of position partial scaling
 * objects, per link ends (returned by reference)
 */
                ...
                observationPartialScalers )
{
    ...
    for( typename std::map< observation_models::LinkEnds,
                            std::pair< std::map< std::pair< int, int >, std::shared_ptr< ObservationPartial< ObservationSize > > >,
                                       std::shared_ptr< PositionPartialScaling > > >::const_iterator rangePartialPairIterator =
                 observationPartialsAndScalers.begin( );
         rangePartialPairIterator != observationPartialsAndScalers.end( );
 ...
 * templated free function to allow ObservationPartial derived classed with different
 * ObservationSize template arguments to be created using the same interface. This class has
 * template specializations for each value of ObservationSize, and contains a single
...
public:
     ...
     * \param observationModelList List of observation models, with the link ends of map key, for
     * which partials are to be created \param bodies Map of body objects that comprises the
     * environment \param parametersToEstimate Parameters for which partial derivatives are to be
     * computed \return Map with list of observation partials. Key is associated link ends. Value is
     * a list of observation partial objects, one for each parameter w.r.t. which the observation
     * partial is non-zero (in general). The format is a pair with: first: map with pair (parameter
     * start entry in parameter vector and parameter size) and associated observation partial object
     * as value of map second: PositionPartialScaling object associated with all partials of single
     * LinkEnds.
     */
    static std::pair< std::map< std::pair< int, int >, std::shared_ptr< ObservationPartial< ObservationSize > > >,
                      std::shared_ptr< PositionPartialScaling > >
    createObservationPartials(
...
public:
     ...
     * LinkEnds.
     */
    static std::pair< std::map< std::pair< int, int >, std::shared_ptr< ObservationPartial< 1 > > >,
                      std::shared_ptr< PositionPartialScaling > >
    createObservationPartials(
                    ...
                    std::shared_ptr< propagators::DependentVariablesInterface< TimeType > >( ) )
    {
        ...
        switch( observationModel->getObservableType( ) )
        {
            ...
            case observation_models::two_way_doppler:
                if( isPartialForConcatenatedObservable )
                {
                    throw std::runtime_error(
                            "Error when requesting partial creation for 2-way Doppler; concatenated partial not supported" );
                }
            ...
            case observation_models::doppler_measured_frequency:
                ...
                {
                    throw std::runtime_error(
                            "Error when requesting partial creation for 2-way frequency Doppler; concatenated partial not supported" );
                }
                observationPartials = createTwoWayDopplerPartials< ObservationScalarType, TimeType >(
            ...
            case observation_models::n_way_range:
                ...
                {
                    throw std::runtime_error(
                            "Error when requesting partial creation for n-way range; concatenated partial not supported" );
                }
            ...
            case observation_models::dsn_n_way_averaged_doppler:
                ...
                {
                    throw std::runtime_error(
                            "Error when requesting partial creation for n-way averaged Doppler; differenced partial not supported" );
                }
                if( isPartialForConcatenatedObservable )
                {
                    throw std::runtime_error(
                            "Error when requesting partial creation for n-way averaged Doppler; concatenated partial not supported" );
                }
                observationPartials = createDifferencedObservablePartials< ObservationScalarType, TimeType, 1 >(
...
public:
     ...
     * LinkEnds.
     */
    static std::pair< std::map< std::pair< int, int >, std::shared_ptr< ObservationPartial< 2 > > >,
                      std::shared_ptr< PositionPartialScaling > >
    createObservationPartials(
...
public:
     ...
     * LinkEnds.
     */
    static std::pair< std::map< std::pair< int, int >, std::shared_ptr< ObservationPartial< 3 > > >,
                      std::shared_ptr< PositionPartialScaling > >
    createObservationPartials(
                    ...
                    std::shared_ptr< propagators::DependentVariablesInterface< TimeType > >( ) )
    {
        ...
        switch( observationModel->getObservableType( ) )
        {
            case observation_models::position_observable:
                observationPartials =
                        createSingleLinkObservationPartials< ObservationScalarType, 3, TimeType >( observationModel,
                                                                                                   bodies,
                                                                                                   parametersToEstimate,
                                                                                                   isPartialForDifferencedObservable,
                                                                                                   isPartialForConcatenatedObservable );
            ...
            case observation_models::euler_angle_313_observable:
                observationPartials = createEulerAngleObservablePartials< ObservationScalarType >( observationModel->getLinkEnds( ),
                                                                                                   bodies,
                                                                                                   parametersToEstimate,
                                                                                                   isPartialForDifferencedObservable,
                                                                                                   isPartialForConcatenatedObservable );
                ...
                std::shared_ptr< propagators::DependentVariablesInterface< TimeType > >( ) )
{
    ...
    for( auto it: observationModelList )
    {
        ...
        {
            throw std::runtime_error(
                    "Error when creating differenced observation partials, input models are "
                    "inconsistent" );
        }
        ...
        switch( differencedObservableType )
        {
            case one_way_differenced_range: {
                            ...
                            getDifferencedPartialParameterIdentifier( firstPartial, secondPartial ).first ) )
                {
                    if( firstPartial != nullptr )
                    {
                        ...
                        {
                            throw std::runtime_error(
                                    "Error when creating one-way differenced range partial; first "
                    ...
                    if( secondPartial != nullptr )
                    {
                        ...
                        {
                            throw std::runtime_error(
                                    "Error when creating one-way differenced range partial; second "
                                    "input object type is incompatible" );
                        }
            ...
            case n_way_differenced_range: {
                if( firstPartial != nullptr )
                {
                    ...
                    {
                        throw std::runtime_error(
                                "Error when creating n-way differenced range partial; first input "
                                "object type is incompatible" );
                    }
                }
                ...
                if( secondPartial != nullptr )
                {
                    ...
                    {
                        throw std::runtime_error(
                                "Error when creating n-way differenced range partial; second input "
                                "object type is incompatible" );
                    }
                }
            ...
            case dsn_n_way_averaged_doppler: {
                if( firstPartial != nullptr )
                {
                    ...
                    {
                        throw std::runtime_error(
                                "Error when creating DSN n-way averaged Doppler partial; first "
                                "input object type is incompatible" );
                    }
                }
                ...
                if( secondPartial != nullptr )
                {
                    ...
                    {
                        throw std::runtime_error(
                                "Error when creating DSN n-way averaged Doppler partial; second "
                                "input object type is incompatible" );
                    }
                }
            ...
            default:
                throw std::runtime_error( "Error when creating differenced observable partial (size 1); observable " +
            ...
            case relative_angular_position: {
                            ...
                            getDifferencedPartialParameterIdentifier( firstPartial, secondPartial ).first ) )
                {
                    if( firstPartial != nullptr )
                    {
                        ...
                        {
                            throw std::runtime_error(
                                    "Error when creating relative angular position partial; first "
                                    "input object type is incompatible" );
                        }
                    ...
                    if( secondPartial != nullptr )
                    {
                        ...
                        {
                            throw std::runtime_error(
                                    "Error when creating relative angular position partial; second "
                                    "input object type is incompatible" );
                        }
...
std::map< std::pair< int, int >,
          std::pair< std::shared_ptr< ObservationPartial< ObservationSize > >, std::shared_ptr< ObservationPartial< ObservationSize > > > >
...
{
    std::map< std::pair< int, int >,
              std::pair< std::shared_ptr< ObservationPartial< ObservationSize > >,
        ...
        const bool isPartialForConcatenatedObservable )
{
    ...
    ObservableType undifferencedObservableType = getUndifferencedObservableType( differencedObservableType );
    ...
    std::pair< std::map< std::pair< int, int >, std::shared_ptr< ObservationPartial< ObservationSize > > >,
            ...
            firstUndifferencedObservablePartials =
                    ObservationPartialCreator< ObservationSize, ParameterType, TimeType >::createObservationPartials(
                            undifferencedObservationModelFirst, bodies, parametersToEstimate, true, isPartialForConcatenatedObservable );

    std::pair< std::map< std::pair< int, int >, std::shared_ptr< ObservationPartial< ObservationSize > > >,
               std::shared_ptr< PositionPartialScaling > >
            secondUndifferencedObservablePartials =
                    ObservationPartialCreator< ObservationSize, ParameterType, TimeType >::createObservationPartials(
                            undifferencedObservationModelSecond, bodies, parametersToEstimate, true, isPartialForConcatenatedObservable );

    std::map< std::pair< int, int >,
              std::pair< std::shared_ptr< ObservationPartial< ObservationSize > >,
                         std::shared_ptr< ObservationPartial< ObservationSize > > > >
            mergedPartials = mergeUndifferencedPartialContribution( firstUndifferencedObservablePartials.first,
                                                                    secondUndifferencedObservablePartials.first );
         ...
         parameterIterator++ )
    {
        ...
        if( !isParameterObservationLinkTimeProperty( parameterIterator->second->getParameterName( ).first ) &&
            isParameterObservationLinkProperty( parameterIterator->second->getParameterName( ).first ) )
        {
            currentDifferencedObservationPartial =
                    createObservationPartialWrtLinkProperty< ObservationSize >( linkEnds,
        ...
        if( currentDifferencedObservationPartial != nullptr )
        {
            ...
            std::pair< double, double > currentPair =
                    std::pair< int, int >( parameterIterator->first, parameterIterator->second->getParameterSize( ) );
    ...
    differencedPartialsAndScaling = std::make_pair(
            differencedObservationPartialList,
            ObservationPartialScalingCreator< ObservationSize >::template createDifferencedPositionPartialScalingObject< ParameterType,
                                                                                                                         TimeType >(
                    differencedObservableType,
                    firstUndifferencedObservablePartials.second,
                    secondUndifferencedObservablePartials.second,
                    bodies ) );
