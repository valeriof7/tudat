 ...
 */
inline std::vector< std::pair< int, int > > getSphericalHarmonicBlockIndices( const int minimumDegree,
...
public:
    ...
    SphericalHarmonicEstimatableParameterSettings( const std::vector< std::pair< int, int > > blockIndices,
                                                   ...
                                                   const EstimatebleParametersEnum parameterType ):
        EstimatableParameterSettings( associatedBody, parameterType ), blockIndices_( blockIndices ), minimumDegree_( -1 ),
        minimumOrder_( -1 ), maximumDegree_( -1 ), maximumOrder_( -1 )
    {
        for( unsigned int i = 0; i < blockIndices.size( ); i++ )
        {
            if( blockIndices.at( i ).first == 0 )
            {
                throw std::runtime_error(
                        "Error, cannot estimate degree 0 spherical harmonic gravity field coefficient, estimation gravitational parameter "
                        "instead" );
            }
            ...
            {
                throw std::runtime_error(
                        "Error, cannot estimate negative degree spherical harmonic gravity field coefficients, but found degree " +
                        std::to_string( blockIndices.at( i ).first ) );
            }

            if( blockIndices.at( i ).second == 0 && parameterType == spherical_harmonics_sine_coefficient_block )
            {
                throw std::runtime_error(
                        "Error when making spherical harmonic sine parameter settings, found order set to 0, but degree 0 spherical "
                        "harmonic sine coefficients do not exist" );
            }

            if( blockIndices.at( i ).second > blockIndices.at( i ).first )
            {
                throw std::runtime_error(
                        "Error when making spherical harmonic sine parameter settings, found coefficient with order > degree, which does "
                        "not exist" );
            }
        }
    ...
    SphericalHarmonicEstimatableParameterSettings( const int minimumDegree,
                                                   ...
                                                   const std::string associatedBody,
                                                   const EstimatebleParametersEnum parameterType ):
        EstimatableParameterSettings( associatedBody, parameterType ), minimumDegree_( minimumDegree ), minimumOrder_( minimumOrder ),
        maximumDegree_( maximumDegree ), maximumOrder_( maximumOrder )
    {
        ...
        {
            throw std::runtime_error(
                    "Error when making spherical harmonic sine parameter settings, minimum order is set to 0, but degree 0 spherical "
                    "harmonic sine coefficients do not exist" );
        }
        ...
        {
            throw std::runtime_error(
                    "Error when making spherical harmonic parameter settings, maximum degree is smaller than minimum degree" );
        }

        if( maximumOrder < minimumOrder )
        {
            throw std::runtime_error(
                    "Error when making spherical harmonic parameter settings, maximum order is smaller than minimum order" );
        }
        ...
        {
            throw std::runtime_error(
                    "Error, cannot estimate negative degree spherical harmonic gravity field coefficients, but minimum degree is set to " +
                    std::to_string( minimumDegree ) );
        }
...
public:
     ...
     */
    ConstantObservationBiasEstimatableParameterSettings( const observation_models::LinkDefinition& linkEnds,
...
public:
     ...
     * \param isBiasAdditive True if bias is absolute, false if it is relative
     */
    ArcWiseConstantObservationBiasEstimatableParameterSettings( const observation_models::LinkDefinition& linkEnds,
                                                                const observation_models::ObservableType observableType,
...
public:
     ...
     */
    ConstantTimeDriftBiasEstimatableParameterSettings( const observation_models::LinkEnds& linkEnds,
                                                       const observation_models::ObservableType observableType,
                                                       const observation_models::LinkEndType linkEndForTime,
...
public:
     ...
     */
    ArcWiseTimeDriftBiasEstimatableParameterSettings( const observation_models::LinkEnds& linkEnds,
                                                      const observation_models::ObservableType observableType,
                                                      const std::vector< double > arcStartTimes,
                                                      const observation_models::LinkEndType linkEndForTime,
...
public:
     ...
     * \param linkEndForTime Link end index from which the 'current time' is determined
     */
    ConstantTimeBiasEstimatableParameterSettings( const observation_models::LinkEnds& linkEnds,
                                                  const observation_models::ObservableType observableType,
                                                  const observation_models::LinkEndType linkEndForTime ):
        EstimatableParameterSettings( linkEnds.begin( )->second.bodyName_,
...
public:
     ...
     * \param referenceEpochs Reference epochs (per arc) at which the time drifts are initialised
     */
    ArcWiseTimeBiasEstimatableParameterSettings( const observation_models::LinkEnds& linkEnds,
                                                 const observation_models::ObservableType observableType,
                                                 const std::vector< double > arcStartTimes,
                                                 const observation_models::LinkEndType linkEndForTime ):
        EstimatableParameterSettings( linkEnds.begin( )->second.bodyName_,
...
public:
     ...
     */
    InitialTranslationalStateEstimatableParameterSettings( const std::string& associatedBody,
...
public:
    ...
    ArcWiseInitialTranslationalStateEstimatableParameterSettings(
        ...
        arcStartTimes_( arcStartTimes ), frameOrientation_( frameOrientation ), isStateSet_( 1 )
    {
     ...
     * \param frameOrientation Orientation of the frame in which the state is defined.
     */
    ArcWiseInitialTranslationalStateEstimatableParameterSettings( const std::string& associatedBody,
                                                                  const std::vector< double >& arcStartTimes,
                                                                  const std::string& centralBody = "SSB",
                                                                  const std::string& frameOrientation = "ECLIPJ2000" ):
...
public:
     ...
     * \param frameOrientation Orientation of the frame in which the state is defined.
     */
    InitialRotationalStateEstimatableParameterSettings( const std::string& associatedBody,
...
inline std::map< std::pair< int, int >, std::vector< std::pair< int, int > > > getFullModeCoupledLoveNumbers(
        const unsigned int maximumForcingDegree,
        const unsigned int maximumResponseDegree )
{
...
public:
     ...
     * \param useComplexValue True if the complex Love number is estimated, false if only the real part is considered
     */
    ModeCoupledTidalLoveNumberEstimatableParameterSettings(
            const std::string& associatedBody,
...
public:
     ...
     */
    DirectTidalTimeLagEstimatableParameterSettings( const std::string& associatedBody, const std::string& deformingBody ):
...
/*!
 *  Class to define settings for estimating the inverse of the tidal quality factor of a direct tidal acceleration model, it links to one or
 * more objects of type DirectTidalDissipationAcceleration. The user can provide a list of bodies cause deformation, and the associated
 * DirectTidalDissipationAcceleration objects will be used. If the list of bodies causing deformation is left empty, all
 * DirectTidalDissipationAcceleration objects for the given body undergoing deformation will be used
 */
...
public:
     ...
     * \param deformingBody Body causing deformed
     */
    InverseTidalQualityFactorEstimatableParameterSettings( const std::string& associatedBody, const std::string& deformingBody ):
...
{
public:
    CustomEstimatableParameterSettings( const std::string& customId,
...
{
public:
    GlobalPolynomialClockCorrectionsParameterSettings( const std::string& associatedBody,
...
{
public:
    MultiArcPolynomialClockCorrectionsParameterSettings( const std::string& associatedBody,
                                                         const std::string& associatedStation,
                                                         const std::vector< int > correctionPowers,
                                                                                      ...
                                                                                      const int maximumOrder )
{
    return std::make_shared< SphericalHarmonicEstimatableParameterSettings >(
            minimumDegree, minimumOrder, maximumDegree, maximumOrder, bodyName, spherical_harmonics_cosine_coefficient_block );
}
...
inline std::shared_ptr< EstimatableParameterSettings > sphericalHarmonicsSineBlock( const std::string bodyName,
                                                                                    const int minimumDegree,
                                                                                    const int minimumOrder,
                                                                                    const int maximumDegree,
                                                                                    const int maximumOrder )
{
    return std::make_shared< SphericalHarmonicEstimatableParameterSettings >(
            minimumDegree, minimumOrder, maximumDegree, maximumOrder, bodyName, spherical_harmonics_sine_coefficient_block );
}
...
inline std::shared_ptr< EstimatableParameterSettings > arcwiseRadiationPressureCoefficient( std::string associatedBody,
                                                                                            const std::vector< double > arcStartTimeList )
{
...
inline std::shared_ptr< EstimatableParameterSettings > arcwiseDragCoefficient( std::string associatedBody,
                                                                               const std::vector< double > arcStartTimeList )
{
    return std::make_shared< ArcWiseDragCoefficientEstimatableParameterSettings >( associatedBody, arcStartTimeList );
}

inline std::shared_ptr< EstimatableParameterSettings > constantRotationRate( std::string bodyName )
{
...
inline std::shared_ptr< EstimatableParameterSettings > observationBias( const observation_models::LinkDefinition& linkEnds,
                                                                        const observation_models::ObservableType observableType )
{
    return std::make_shared< ConstantObservationBiasEstimatableParameterSettings >( linkEnds, observableType, true );
}

inline std::shared_ptr< EstimatableParameterSettings > relativeObservationBias( const observation_models::LinkDefinition& linkEnds,
                                                                                const observation_models::ObservableType observableType )
{
    return std::make_shared< ConstantObservationBiasEstimatableParameterSettings >( linkEnds, observableType, false );
}
...
inline std::shared_ptr< EstimatableParameterSettings > timeObservationBias(
        ...
        const observation_models::LinkEndType linkEndForTime = observation_models::receiver )
{
    return std::make_shared< ConstantTimeBiasEstimatableParameterSettings >( linkEnds, observableType, linkEndForTime );
}
...
inline std::shared_ptr< EstimatableParameterSettings > arcwiseTimeObservationBias(
        ...
        const observation_models::LinkEndType linkEndForTime = observation_models::receiver )
{
    return std::make_shared< ArcWiseTimeBiasEstimatableParameterSettings >( linkEnds, observableType, arcStartTimes, linkEndForTime );
}

inline std::shared_ptr< EstimatableParameterSettings > globalPolynomialClockCorrections( const std::string& associatedBody,
                                                                                         const std::string& associatedStation,
                                                                                         const std::vector< int > correctionPowers )
{
    return std::make_shared< GlobalPolynomialClockCorrectionsParameterSettings >( associatedBody, associatedStation, correctionPowers );
}

inline std::shared_ptr< EstimatableParameterSettings > multiArcPolynomialClockCorrections( const std::string& associatedBody,
                                                                                           const std::string& associatedStation,
                                                                                           const std::vector< int > correctionPowers,
                                                                                           const std::vector< int > arcIndices )
{
...
inline std::shared_ptr< EstimatableParameterSettings > constantEmpiricalAccelerationMagnitudes( const std::string associatedBody,
                                                                                                const std::string centralBody )
{
    std::map< basic_astrodynamics::EmpiricalAccelerationComponents,
              std::vector< basic_astrodynamics::EmpiricalAccelerationFunctionalShapes > >
    ...
    return std::make_shared< EmpiricalAccelerationEstimatableParameterSettings >( associatedBody, centralBody, componentsToEstimate );
}

inline std::shared_ptr< EstimatableParameterSettings > empiricalAccelerationMagnitudesFull( const std::string associatedBody,
                                                                                            const std::string centralBody )
{
    std::map< basic_astrodynamics::EmpiricalAccelerationComponents,
              std::vector< basic_astrodynamics::EmpiricalAccelerationFunctionalShapes > >
    ...
    componentsToEstimate[ basic_astrodynamics::radial_empirical_acceleration_component ].push_back(
            basic_astrodynamics::constant_empirical );
    componentsToEstimate[ basic_astrodynamics::radial_empirical_acceleration_component ].push_back( basic_astrodynamics::sine_empirical );
    componentsToEstimate[ basic_astrodynamics::radial_empirical_acceleration_component ].push_back( basic_astrodynamics::cosine_empirical );
    componentsToEstimate[ basic_astrodynamics::along_track_empirical_acceleration_component ].push_back(
            basic_astrodynamics::constant_empirical );
    componentsToEstimate[ basic_astrodynamics::along_track_empirical_acceleration_component ].push_back(
...
inline std::shared_ptr< EstimatableParameterSettings > groundStationPosition( const std::string& body,
...
inline std::shared_ptr< EstimatableParameterSettings > referencePointPosition( const std::string& body,
                                                                               const std::string& groundStationName )
{
...
inline std::shared_ptr< EstimatableParameterSettings > directTidalDissipationLagTime( const std::string& body,
                                                                                      const std::vector< std::string >& deformingBodies )
{
    return std::make_shared< DirectTidalTimeLagEstimatableParameterSettings >( body, deformingBodies );
}
...
inline std::shared_ptr< EstimatableParameterSettings > inverseTidalQualityFactor( const std::string& body,
                                                                                  const std::vector< std::string >& deformingBodies )
{
    return std::make_shared< InverseTidalQualityFactorEstimatableParameterSettings >( body, deformingBodies );
}
...
inline std::shared_ptr< EstimatableParameterSettings > orderInvariantKLoveNumber( const std::string& associatedBody,
                                                                                  const int degree,
                                                                                  const std::string deformingBody,
                                                                                  const bool useComplexValue = 0 )
{
...
inline std::shared_ptr< EstimatableParameterSettings > orderVaryingKLoveNumber( const std::string& associatedBody,
                                                                                const int degree,
                                                                                const std::vector< int >& orders,
                                                                                const std::string deformingBody,
                                                                                const bool useComplexValue = 0 )
...
inline std::shared_ptr< EstimatableParameterSettings > coreFactor( const std::string& associatedBody )
{
    return std::make_shared< EstimatableParameterSettings >( associatedBody, core_factor );
}

inline std::shared_ptr< EstimatableParameterSettings > freeCoreNutationRate( const std::string& associatedBody )
{
    return std::make_shared< EstimatableParameterSettings >( associatedBody, free_core_nutation_rate );
}

inline std::shared_ptr< EstimatableParameterSettings > periodicSpinVariations( const std::string& associatedBody )
{
    return std::make_shared< EstimatableParameterSettings >( associatedBody, periodic_spin_variation );
}

inline std::shared_ptr< EstimatableParameterSettings > polarMotionAmplitudes( const std::string& associatedBody )
{
    return std::make_shared< EstimatableParameterSettings >( associatedBody, polar_motion_amplitude );
}

inline std::shared_ptr< EstimatableParameterSettings > quasiImpulsiveShots( const std::string& associatedBody )
{
    return std::make_shared< EstimatableParameterSettings >( associatedBody, desaturation_delta_v_values );
}
...
inline std::shared_ptr< EstimatableParameterSettings > radiationPressureTargetPerpendicularDirectionScaling( const std::string targetName,
                                                                                                             const std::string sourceName )
{
    return std::make_shared< EstimatableParameterSettings >(
            targetName, source_perpendicular_direction_radiation_pressure_scaling_factor, sourceName );
}

inline std::shared_ptr< EstimatableParameterSettings > diffuseReflectivity( const std::string bodyName, const std::string panel_group_id )
{
