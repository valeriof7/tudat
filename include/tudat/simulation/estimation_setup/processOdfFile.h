...
>>>>>>> tudat-team/develop
    ...
    std::pair< double, double > getTimeBounds( )
    {
        ...
        return std::make_pair(
                *std::min_element( obsTimesDouble.begin( ), obsTimesDouble.end( ) ),
...
>>>>>>> tudat-team/develop
    ...
    {
        return utilities::createMapFromVectors( this->processedObservationTimes_,
                                                receiverRampingFlags_ );
    }

    std::map< TimeType, double > getReferenceFrequencies( )
    {
        return utilities::createMapFromVectors( this->processedObservationTimes_,
                                                referenceFrequencies_ );
    }
...
class ProcessedOdfFileSequentialRangeData : public ProcessedOdfFileSingleLinkData< TimeType >
{
   public:
    ...
    {
        return utilities::createMapFromVectors( this->processedObservationTimes_,
                                                uplinkRangingCoderInPhaseTimeOffset_ );
    }
 ...
 */
bool compareRawOdfDataByStartDate(
        std::shared_ptr< input_output::OdfRawFileContents > rawOdfData1,
...
class ProcessedOdfFileContents
{
   public:
    ...
    std::vector< std::string > getGroundStationsNames( )
    {
        ...
        for( auto observableIt = processedDataBlocks_.begin( );
             observableIt != processedDataBlocks_.end( );
             ++observableIt )
        {
            for( auto linkEndIt = observableIt->second.begin( );
                 linkEndIt != observableIt->second.end( );
                 ++linkEndIt )
            {
                observation_models::LinkEnds linkEnd = linkEndIt->first;

                for( auto linkEndTypeIt = linkEnd.begin( ); linkEndTypeIt != linkEnd.end( );
                     ++linkEndTypeIt )
                {
                    // Check if linkEndId is a ground station
                    if( linkEndTypeIt->second.stationName_ != "" &&
                        linkEndTypeIt->second.bodyName_ != spacecraftName_ )
                    {
                        if( !std::count( groundStations.begin( ),
                                         groundStations.end( ),
                                         linkEndTypeIt->second.stationName_ ) )
                        {
    ...
    std::pair< double, double > getStartAndEndTime( )
    {
             ...
             ++observableIt )
        {
            for( auto linkEndIt = observableIt->second.begin( );
                 ...
                 ++linkEndIt )
            {
                std::shared_ptr< ProcessedOdfFileSingleLinkData< TimeType > >
                        processedSingleLinkData = linkEndIt->second;

                // Extract the start and end times
                std::vector< TimeType > timeVectorTimeType =
                        processedSingleLinkData->processedObservationTimes_;
                ...
                if( timeVector.front( ) < startTimeTdbSinceJ2000 ||
                    std::isnan( startTimeTdbSinceJ2000 ) )
                {
                    startTimeTdbSinceJ2000 = timeVector.front( );
                }

                if( timeVector.back( ) > endTimeTdbSinceJ2000 ||
                    std::isnan( endTimeTdbSinceJ2000 ) )
                {
    ...
    const std::map< std::string,
                    std::shared_ptr< ground_stations::PiecewiseLinearFrequencyInterpolator > > &
    getRampInterpolators( )
    {
    ...
    void defineSpacecraftAntennaId( const std::string &spacecraft, const std::string &antennaName )
    {
        ...
        for( auto observationIterator: processedDataBlocks_ )
        {
            for( auto linkEndIterator: observationIterator.second )
            {
                ...
                reprocessedDataBlocks[ observationIterator.first ][ newLinkEnds ] =
                                    ...
                                    const std::map< double, double > &timeIntervals )
    {
        ...
        std::vector< double > timeIntervalStarts =
                utilities::createVectorFromMapKeys( timeIntervals );
        std::shared_ptr< interpolators::LookUpScheme< double > > lookUpScheme_ =
                std::make_shared< interpolators::HuntingAlgorithmLookupScheme< double > >(
                        timeIntervalStarts );
        ...
        for( auto observationIterator: processedDataBlocks_ )
        {
            ...
            for( auto linkEndIterator: observationIterator.second )
            {
                ...
                std::shared_ptr< ProcessedOdfFileSingleLinkData< TimeType > > currentDataBlock =
                        linkEndIterator.second;
                ...
                if( timeBounds.first > timeIntervalStarts.at( 0 ) )
                {
                    int timeIntervalIndex =
                            lookUpScheme_->findNearestLowerNeighbour( timeBounds.first );
   ...
   private:
     ...
     */
    void sortAndValidateOdfDataVector(
            std::vector< std::shared_ptr< input_output::OdfRawFileContents > > &rawOdfDataVector )
    {
        ...
        for( unsigned int i = 0; i < rawOdfDataVector.size( ); ++i )
        {
            ...
            if( rawOdfDataVector.at( i )->spacecraftId_ != spacecraftId )
            {
                throw std::runtime_error(
                        ...
                        "spacecraft IDs"
                        "found (" +
                        std::to_string( spacecraftId ) + " and " +
                        std::to_string( rawOdfDataVector.at( i )->spacecraftId_ ) + ")." );
            }
        }

        std::stable_sort(
                rawOdfDataVector.begin( ), rawOdfDataVector.end( ), &compareRawOdfDataByStartDate );
    }
                             ...
                             observation_models::ObservableType currentObservableType )
    {
        ...
        if( requiresTransmittingStation( currentObservableType ) )
        {
            ...
            {
                if( std::count( ignoredGroundStations_.begin( ),
                                ignoredGroundStations_.end( ),
                                transmittingStation ) == 0 )
                {
                    ignoredGroundStations_.push_back( transmittingStation );
                    if( verbose_ )
                    {
                        std::cerr << "Warning: ground station " << transmittingStation
                                  << " not available in ramp tables,"
                ...
                unprocessedRampStartTimesPerStation_[ transmittingStation ].front( ) )
            {
                if( verbose_ )
                {
                    std::cerr << "Warning: observation of ODF type " << currentObservableId
                              << " not covered by ramp table of station " << transmittingStation
        ...
        if( requiresFirstReceivingStation( currentObservableType ) )
        {
            ...
            if( rampInterpolators_.count( receivingStation ) == 0 )
            {
                                ...
                                receivingStation ) == 0 )
                {
                    ...
                    {
                        std::cerr << "Warning: observation of ODF type " << currentObservableId
                                  << " not covered by ramp table of station " << receivingStation
                                  << ", ignoring it." << std::endl;
                    }
                }
                ignoredOdfRawDataBlocks_.push_back( rawDataBlock );
    ...
    void extractRawOdfOrbitData( std::shared_ptr< input_output::OdfRawFileContents > rawOdfData )
    {
        ...
        std::vector< std::shared_ptr< input_output::OdfDataBlock > > rawDataBlocks =
                rawOdfData->getDataBlocks( );
        ...
        for( unsigned int i = 0; i < rawDataBlocks.size( ); i++ )
        {
            ...
            int currentObservableId =
                    rawDataBlocks.at( i )->getObservableSpecificDataBlock( )->dataType_;
            ...
            {
                if( std::find( ignoredRawOdfObservableTypes_.begin( ),
                               ...
                               currentObservableId ) == ignoredRawOdfObservableTypes_.end( ) )
                {
                    ...
                    if( verbose_ )
                    {
                        std::cerr << "Warning: processing of ODF data type " << currentObservableId
                                  << " is not implemented, ignoring the corresponding data."
                                  << std::endl;
                    }
                }
            ...
            observation_models::LinkEnds linkEnds =
                    getLinkEndsFromOdfBlock( rawDataBlocks.at( i ), spacecraftName_ );
            ...
            if( isObservationValid( rawDataBlocks.at( i ), linkEnds, currentObservableType ) )
            {
                ...
                if( createNewObject )
                {
                    switch( currentObservableType )
                    {
                        ...
                        default: {
                            std::cerr << "Warning: Observable type "
                                      << observation_models::getObservableName(
                                                 currentObservableType )
    ...
    void addOdfRawDataBlockToProcessedData(
            ...
            const std::shared_ptr< ProcessedOdfFileSingleLinkData< TimeType > >
            ...
            const std::string rawDataFileName )
    {
        ...
        if( rawDataBlock->getCommonDataBlock( )->validity_ == 0 )
        {
            ...
            singleLinkProcessedData->downlinkBandIds_.push_back(
                    rawDataBlock->getCommonDataBlock( )->downlinkBandId_ );
            singleLinkProcessedData->uplinkBandIds_.push_back(
                    rawDataBlock->getCommonDataBlock( )->uplinkBandId_ );
            singleLinkProcessedData->referenceBandIds_.push_back(
                    rawDataBlock->getCommonDataBlock( )->referenceBandId_ );
            singleLinkProcessedData->unprocessedObservationTimes_.push_back(
                    rawDataBlock->getCommonDataBlock( )->getObservableTime( ) );
            ...
            if( singleLinkProcessedData->getObservableType( ) ==
                observation_models::dsn_n_way_averaged_doppler )
            {
                ...
                std::shared_ptr< ProcessedOdfFileDopplerData< TimeType > >
                        odfParsedDopplerDataBlock = std::dynamic_pointer_cast<
                                ProcessedOdfFileDopplerData< TimeType > >(
                                singleLinkProcessedData );

                singleLinkProcessedData->observableValues_.push_back(
                        ( Eigen::Matrix< double, 1, 1 >( )
                          << rawDataBlock->getCommonDataBlock( )->getObservableValue( ) )
                                .finished( ) );

                odfParsedDopplerDataBlock->countInterval_.push_back(
                        odfDopplerDataBlock->getCompressionTime( ) );
                odfParsedDopplerDataBlock->receiverChannels_.push_back(
                        odfDopplerDataBlock->getReceiverChannel( ) );
                odfParsedDopplerDataBlock->receiverRampingFlags_.push_back(
                        odfDopplerDataBlock->getReceiverExciterFlag( ) );
                odfParsedDopplerDataBlock->referenceFrequencies_.push_back(
                        odfDopplerDataBlock->getReferenceFrequency( ) );
                     ...
                     observation_models::n_way_range )
            {
                std::shared_ptr< input_output::OdfSequentialRangeDataBlock >
                        odfSequentialRangeDataBlock = std::dynamic_pointer_cast<
                                input_output::OdfSequentialRangeDataBlock >(
                                rawDataBlock->getObservableSpecificDataBlock( ) );
                std::shared_ptr< ProcessedOdfFileSequentialRangeData< TimeType > >
                        odfParsedSequentialRangeDataBlock = std::dynamic_pointer_cast<
                                ProcessedOdfFileSequentialRangeData< TimeType > >(
                                singleLinkProcessedData );
                ...
                odfParsedSequentialRangeDataBlock->composite2_.push_back(
                        odfSequentialRangeDataBlock->getCompositeTwo( ) );
            ...
            std::vector< std::shared_ptr< input_output::OdfRawFileContents > > rawOdfDataVector )
    {
        ...
        for( unsigned int i = 0; i < rawOdfDataVector.size( ); ++i )
        {
            ...
            for( auto it = rampBlocksPerStation.begin( ); it != rampBlocksPerStation.end( ); it++ )
            {
                ...
                std::vector< std::shared_ptr< input_output::OdfRampBlock > > rampBlocks =
                        it->second;

                for( unsigned int j = 0; j < it->second.size( ); j++ )
                {
                    // Check if zero time ramp
                    if( rampBlocks.at( j )->getRampStartTime( ) ==
                        rampBlocks.at( j )->getRampEndTime( ) )
                    {
                        continue;
                    }
                    ...
                    if( j == 0 && !unprocessedRampStartTimesPerStation_[ stationName ].empty( ) )
                    {
                        ...
                        unprocessedRampEndTimesPerStation_[ stationName ].push_back(
                                rampBlocks.at( j )->getRampStartTime( ) );
                    ...
                    unprocessedRampEndTimesPerStation_[ stationName ].push_back(
                            rampBlocks.at( j )->getRampEndTime( ) );
                    rampRatesPerStation[ stationName ].push_back(
                            rampBlocks.at( j )->getRampRate( ) );
                    startFrequenciesPerStation[ stationName ].push_back(
                            rampBlocks.at( j )->getRampStartFrequency( ) );
                }
            }
        }

        for( auto it = unprocessedRampStartTimesPerStation_.begin( );
             it != unprocessedRampStartTimesPerStation_.end( );
             ++it )
        {
            ...
            std::vector< Time > rampStartTimesPerStationUtc =
                    computeObservationTimesUtcFromJ2000< Time >(
                            unprocessedRampStartTimesPerStation_[ stationName ] );
    ...
    void updateProcessedObservationTimes( )
    {
        ...
        for( auto observableTypeIterator = processedDataBlocks_.begin( );
             observableTypeIterator != processedDataBlocks_.end( );
             ++observableTypeIterator )
        {
            for( auto linkEndsIterator = observableTypeIterator->second.begin( );
                 linkEndsIterator != observableTypeIterator->second.end( );
                 ++linkEndsIterator )
            {
                linkEndsIterator->second->processedObservationTimes_ =
                        computeObservationTimesTdbFromJ2000(
            ...
            const std::vector< InputTimeType > &observationTimesUtcFromEME1950 )
    {
        ...
        Time EME1950ToJ2000Offset =
                Time( basic_astrodynamics::convertCalendarDateToJulianDaysSinceEpoch< double >(
                              1950, 1, 1, 0, 0, 0, basic_astrodynamics::JULIAN_DAY_ON_J2000 ) *
                      physical_constants::JULIAN_DAY );

        for( unsigned int i = 0; i < observationTimesUtcFromEME1950.size( ); ++i )
        {
            observationTimesUtcFromJ2000.push_back(
                    static_cast< Time >( observationTimesUtcFromEME1950.at( i ) ) +
                    EME1950ToJ2000Offset );
        }
            ...
            const std::vector< TimeType > &observationTimesUtcFromEME1950 )
    {
        ...
        for( unsigned int i = 0; i < observationTimesUtcFromJ2000.size( ); ++i )
        {
            ...
            earthFixedPositions.push_back(
                    approximateEarthFixedGroundStationPositions_.at( groundStation ) );
        }

        std::vector< TimeType > observationTimesTdbFromJ2000 =
                utilities::staticCastVector< TimeType, Time >(
                        timeScaleConverter.getCurrentTimes< Time >( basic_astrodynamics::utc_scale,
...
class ProcessedOdfFileContentsPrivateFunctionTest
{
   public:
    ...
    {
        return processedOdfFileContents
                ->computeObservationTimesTdbFromJ2000( groundStation,
                                                       { observationTimeUtcFromEME1950 } )
                .front( );
    }
                ...
                simulation_setup::getApproximateDsnGroundStationPositions( ) )
{
    ...
    for( std::string odfFile: odfFileNames )
    {
        rawOdfDataVector.push_back(
                std::make_shared< input_output::OdfRawFileContents >( odfFile ) );
    }
...
inline std::shared_ptr< ProcessedOdfFileContents< TimeType > > processOdfData(
        ...
        const std::map< std::string, Eigen::Vector3d > &earthFixedGroundStationPositions =
                simulation_setup::getApproximateDsnGroundStationPositions( ) )
{
    return processOdfData< TimeType >( std::vector< std::string >{ odfFileName },
                                       spacecraftName,
                                       verbose,
                                       earthFixedGroundStationPositions );
}
        ...
        unsigned int dataIndex )
{
    ...
    observation_models::ObservableType currentObservableType =
            odfDataContents->getObservableType( );
    ...
    ancillarySettings.setAncilliaryDoubleVectorData(
            observation_models::frequency_bands,
    ...
    ancillarySettings.setAncilliaryDoubleData(
            ...
            convertFrequencyBandToDouble( getFrequencyBandForOdfId(
                    odfDataContents->referenceBandIds_.at( dataIndex ) ) ) );

    if( std::dynamic_pointer_cast< ProcessedOdfFileDopplerData< TimeType > >( odfDataContents ) !=
        nullptr )
    {
        ...
        ancillarySettings.setAncilliaryDoubleData(
                observation_models::doppler_integration_time,
                dopplerDataBlock->countInterval_.at( dataIndex ) );

        ancillarySettings.setAncilliaryDoubleData(
                observation_models::doppler_reference_frequency,
    ...
    else if( std::dynamic_pointer_cast< ProcessedOdfFileSequentialRangeData< TimeType > >(
                     odfDataContents ) != nullptr )
    {
        std::shared_ptr< ProcessedOdfFileSequentialRangeData< TimeType > >
                sequentialRangeDataBlock = std::dynamic_pointer_cast<
                        ProcessedOdfFileSequentialRangeData< TimeType > >( odfDataContents );

        ancillarySettings.setAncilliaryDoubleData(
                observation_models::sequential_range_reference_frequency,
        ...
        ancillarySettings.setAncilliaryDoubleVectorData(
                observation_models::link_ends_delays,
                std::vector< double >{
                        sequentialRangeDataBlock->transmitterUplinkDelay_.at( dataIndex ),
                        0.0,
    ...
    {
        throw std::runtime_error(
                "Error when casting ODF processed data: data type not identified." );
    }
...
void separateSingleLinkOdfData(
        ...
        std::vector< std::vector< Eigen::Matrix< ObservationScalarType, Eigen::Dynamic, 1 > > >
                &observables,
        std::vector< observation_models::ObservationAncilliarySimulationSettings >
                &ancillarySettings )
{
    // Initialize vectors
    ...
    for( unsigned int i = 0; i < odfSingleLinkData->unprocessedObservationTimes_.size( ); ++i )
    {
        ...
        {
            observationTimes.push_back( std::vector< TimeType >{
                    /*static_cast< TimeType >(*/ observationTimesTdb.at( i ) /*)*/ } );
...
createOdfObservedObservationCollection(
        std::shared_ptr< ProcessedOdfFileContents< TimeType > > processedOdfFileContents,
        std::vector< observation_models::ObservableType > observableTypesToProcess =
                std::vector< observation_models::ObservableType >( ),
        std::pair< TimeType, TimeType > startAndEndTimesToProcess =
                std::make_pair< TimeType, TimeType >( TUDAT_NAN, TUDAT_NAN ) )
{
    ...
    std::map< observation_models::ObservableType,
              std::map< observation_models::LinkEnds,
                        std::vector< std::shared_ptr<
                                observation_models::SingleObservationSet< ObservationScalarType,
                                                                          TimeType > > > > >
         ...
         ++observableTypeIterator )
    {
        ...
        if( std::count( observableTypesToProcess.begin( ),
                        observableTypesToProcess.end( ),
                        currentObservableType ) == 0 )
        {
            continue;
        }
             ...
             ++linkEndsIterator )
        {
            ...
            std::shared_ptr< ProcessedOdfFileSingleLinkData< TimeType > > currentOdfSingleLinkData =
                    linkEndsIterator->second;
            ...
            // Fill vectors
            separateSingleLinkOdfData( currentObservableType,
                                       currentOdfSingleLinkData,
                                       observationTimes,
                                       observables,
                                       ancillarySettings );
            ...
            else
            {
                for( unsigned int i = 0; i < observationTimes.size( ); ++i )
                {
                    ...
                    std::vector< Eigen::Matrix< ObservationScalarType, Eigen::Dynamic, 1 > >
                            singleTruncatedObservables;

                    for( unsigned int j = 0; j < observationTimes.at( i ).size( ); ++j )
                    {
                        if( ( ( !std::isnan( static_cast< double >(
                                        startAndEndTimesToProcess.first ) ) &&
                                observationTimes.at( i ).at( j ) >=
                                        startAndEndTimesToProcess.first ) ||
                              std::isnan( static_cast< double >(
                                      startAndEndTimesToProcess.first ) ) ) &&
                            ( ( !std::isnan( static_cast< double >(
                                        startAndEndTimesToProcess.second ) ) &&
                                observationTimes.at( i ).at( j ) <=
                                        startAndEndTimesToProcess.second ) ||
                              std::isnan( static_cast< double >(
                                      startAndEndTimesToProcess.second ) ) ) )
                        {
                            singleTruncatedObservationTimes.push_back(
                                    observationTimes.at( i ).at( j ) );
            ...
            for( unsigned int i = 0; i < observationTimes.size( ); ++i )
            {
                sortedObservationSets[ currentObservableType ][ currentLinkEnds ].push_back(
                        std::make_shared<
                                ...
                                nullptr,
                                std::make_shared< observation_models::
    ...
    return std::make_shared<
            observation_models::ObservationCollection< ObservationScalarType, TimeType > >(
            sortedObservationSets );
}
        ...
        const unsigned int compressionRatio )
{
    ...
    double currentCompressionTime =
            originalDopplerData->getAncilliarySettings( )->getAncilliaryDoubleData(
    ...
    std::vector< TimeType > originalObservationTimesTdb =
            originalDopplerData->getObservationTimesReference( );
    ...
    for( unsigned int i = 0; i < originalObservations.size( ); i += compressionRatio )
    {
        ...
        {
            Eigen::Matrix< ObservationScalarType, Eigen::Dynamic, 1 > newObservable =
                    originalObservations.at( i );
            ...
            {
                if( ( originalObservationTimesUtc.at( i + j ) -
                      originalObservationTimesUtc.at( i + j - 1 ) - currentCompressionTime ) <
    ...
    if( simulation_setup::getCombinedApproximateGroundStationPositions( ).count( stationName ) ==
        0 )
    {
        throw std::runtime_error(
    ...
    std::vector< TimeType > compressedObservationTimesTdb =
            timeScaleConverter.getCurrentTimes< TimeType >( basic_astrodynamics::utc_scale,
                                                            basic_astrodynamics::tdb_scale,
    ...
    std::shared_ptr< ObservationAncilliarySimulationSettings > ancilliarySimulationSettings =
            std::make_shared< ObservationAncilliarySimulationSettings >(
                    *( originalDopplerData->getAncilliarySettings( ) ) );
    double originalIntegrationTime =
            ancilliarySimulationSettings->getAncilliaryDoubleData( doppler_integration_time );
    ancilliarySimulationSettings->setAncilliaryDoubleData(
            doppler_integration_time,
...
template< typename ObservationScalarType = double, typename TimeType = double >
std::shared_ptr< observation_models::ObservationCollection< ObservationScalarType, TimeType > >
createCompressedDopplerCollection(
        const std::shared_ptr<
                observation_models::ObservationCollection< ObservationScalarType, TimeType > >
                originalDopplerData,
        const unsigned int compressionRatio,
        const unsigned int minNumberObservations = 10 )
{
    ...
    double compressionRatioFloat =
            mathematical_constants::getFloatingInteger< double >( compressionRatio );
    std::shared_ptr< observation_models::ObservationCollection< ObservationScalarType, TimeType > >
            compressedData = splitObservationSets(
                    originalDopplerData,
                    observationSetSplitter(
                            time_interval_splitter, compressionRatioFloat, minNumberObservations ),
    ...
    compressedData->removeSingleObservationSets(
            { { dsn_n_way_averaged_doppler, indicesSetsToRemove } } );
...
void changeObservableTypesOfObservationSimulationSettings(
        std::vector<
                std::shared_ptr< simulation_setup::ObservationSimulationSettings< TimeType > > >
                ...
                { dsn_one_way_averaged_doppler, one_way_differenced_range } } )
{
    for( unsigned int i = 0; i < observationSimulationSettings.size( ); ++i )
    {
        ObservableType currentObservableType =
                observationSimulationSettings.at( i )->getObservableType( );

        if( replacementObservableTypes.count( currentObservableType ) )
        {
            observationSimulationSettings.at( i )->setObservableType(
                    replacementObservableTypes.at( currentObservableType ) );
        }
    }
}
        ...
        std::shared_ptr< simulation_setup::Body > bodyWithGroundStations )
{
         ...
         it++ )
    {
        ...
        {
            throw std::runtime_error( "Error when setting frequencies for station " + it->first +
                                      ", station not found." );
        }
        bodyWithGroundStations->getGroundStation( it->first )
                ->setTransmittingFrequencyCalculator( it->second );
    }
}
...
void setOdfInformationInBodies(
        ...
        const std::function< double( FrequencyBands uplinkBand, FrequencyBands downlinkBand ) >
                getTurnaroundRatio = &getDsnDefaultTurnaroundRatios )
{
    // Set transmitting frequency objects in ground stations
    setTransmittingFrequenciesInGroundStations< TimeType >(
            processedOdfFileContents, bodies.getBody( bodyWithGroundStations ) );
