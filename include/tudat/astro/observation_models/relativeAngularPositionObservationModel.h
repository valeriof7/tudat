...
public:
    ...
    /*!
     *  This function compute ideal relative angular position observation at a given time, between two transmitters. The time argument can
     ...
     * times are returned by reference. \param time Time at which observation is to be simulated \param linkEndAssociatedWithTime Link end
     * at which given time is valid, i.e. link end for which associated time is kept constant (to input value) \param linkEndTimes List of
     * times at each link end during observation (returned by reference). \param linkEndStates List of states at each link end during
     * observation (returned by reference). \return Calculated relative angular position observable values.
     */
...

    {
        ...
        //                coordinate_conversions::convertCartesianToSpherical< ObservationScalarType >(
        //                        secondTransmitterState.segment( 0, 3 ) - receiverState.segment( 0, 3 ) ).template cast<
