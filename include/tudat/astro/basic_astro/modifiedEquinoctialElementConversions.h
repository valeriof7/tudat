...
Eigen::Matrix< ScalarType, 6, 1 > convertKeplerianToModifiedEquinoctialElements( const Eigen::Matrix< ScalarType, 6, 1 >& keplerianElements,
                                                                                 const bool flipSingularityToZeroInclination )
{
    ...
    else  // eccentricity is significantly away from singularity and can be computed with p=a(1-e).
    {
        modifiedEquinoctialState( semiLatusRectumIndex ) =
                keplerianElements( semiMajorAxisIndex ) * ( getFloatingInteger< ScalarType >( 1 ) - eccentricity * eccentricity );
    }
    ...
    modifiedEquinoctialState( trueLongitudeIndex ) =
            basic_mathematics::computeModulo( argumentOfPeriapsisAndAscendingNode + keplerianElements( trueAnomalyIndex ),
...
// Based on Hintz, 2008.
{
    ...
    bool flipSingularityToZeroInclination = mission_geometry::isOrbitRetrograde( keplerianElements );

    // Convert to modified equinoctial elements.
    return convertKeplerianToModifiedEquinoctialElements( keplerianElements, flipSingularityToZeroInclination );
}
        ...
        const bool flipSingularityToZeroInclination )
{
    ...
    convertedKeplerianElements( longitudeOfAscendingNodeIndex ) =
            basic_mathematics::computeModulo( longitudeOfAscendingNode, getFloatingInteger< ScalarType >( 2 ) * getPi< ScalarType >( ) );
    ...
    // Argument of periapsis can be found from composite argument.
    {
        ...
        convertedKeplerianElements( argumentOfPeriapsisIndex ) =
                basic_mathematics::computeModulo( argumentOfPeriapsisAndLongitude - retrogradeFactor * longitudeOfAscendingNode,
                                                  getFloatingInteger< ScalarType >( 2 ) * getPi< ScalarType >( ) );
    }

    // Compute true anomaly.
    convertedKeplerianElements( trueAnomalyIndex ) =
            basic_mathematics::computeModulo( modifiedEquinoctialElements( trueLongitudeIndex ) - argumentOfPeriapsisAndLongitude,
                                              getFloatingInteger< ScalarType >( 2 ) * getPi< ScalarType >( ) );
 ...
 */
template< typename ScalarType = double >
Eigen::Matrix< ScalarType, 6, 1 > convertCartesianToModifiedEquinoctialElements( const Eigen::Matrix< ScalarType, 6, 1 >& cartesianElements,
                                                                                 const ScalarType centralBodyGravitationalParameter )

{
    // Convert to keplerian elements.
    Eigen::Matrix< ScalarType, 6, 1 > keplerianElements =
            convertCartesianToKeplerianElements( cartesianElements, centralBodyGravitationalParameter );
...
{
    return convertCartesianToModifiedEquinoctialElements( cartesianElementsFunction( ), centralBodyGravitationalParameterFunction( ) );
}
                                                                                 ...
                                                                                 const bool flipSingularityToZeroInclination )
{
    ...
    return convertKeplerianToModifiedEquinoctialElements< ScalarType >(
            convertCartesianToKeplerianElements< ScalarType >( cartesianElements, centralBodyGravitationalParameter ),
...
// Using unnamed pdf and code archive Bart Rmgens.
{
    ...
    if( !flipSingularityToZeroInclination )
    {
        ...
        ScalarType wParameter = getFloatingInteger< ScalarType >( 1 ) + fElement * cosineTrueLongitude + gElement * sineTrueLongitude;
        ...
        convertedCartesianElements( xCartesianPositionIndex ) = radius / sSquaredParameter *
                ( cosineTrueLongitude + aSquaredParameter * cosineTrueLongitude +
                  getFloatingInteger< ScalarType >( 2 ) * hElement * kElement * sineTrueLongitude );
        convertedCartesianElements( yCartesianPositionIndex ) = radius / sSquaredParameter *
                ( sineTrueLongitude - aSquaredParameter * sineTrueLongitude +
                  getFloatingInteger< ScalarType >( 2 ) * hElement * kElement * cosineTrueLongitude );
    ...
    else  // use the indirect transformation via an intermediate Kepler state.
    {
        ...
        convertedCartesianElements = convertKeplerianToCartesianElements(
                convertModifiedEquinoctialToKeplerianElements( modifiedEquinoctialElements, flipSingularityToZeroInclination ),
        ...
        const bool flipSingularityToZeroInclination )
{
    if( flipSingularityToZeroInclination )
    {
        throw std::runtime_error(
                "Error in direct conversion from MEE to Cartesian elements, implementation with singularity at 0 degrees not yet "
                "implemented" );
    }
    using mathematical_constants::getFloatingInteger;
    ...
    ScalarType parameterW = getFloatingInteger< ScalarType >( 1 ) + parameterF * cosineTrueLongitude + parameterG * sineTrueLongitude;
    ...
    cartesianElements( 2 ) = getFloatingInteger< ScalarType >( 2 ) * ( parameterH * sineTrueLongitude - parameterK * cosineTrueLongitude );
    ...
    cartesianElements( 3 ) =
            -( sineTrueLongitude + parameterAlphaSquared * sineTrueLongitude -
               ...
               getFloatingInteger< ScalarType >( 2 ) * parameterF * parameterH * parameterK + parameterAlphaSquared * parameterG );
    cartesianElements( 4 ) =
            -( -cosineTrueLongitude + parameterAlphaSquared * cosineTrueLongitude +
               getFloatingInteger< ScalarType >( 2 ) * parameterH * parameterK * sineTrueLongitude - parameterF +
               getFloatingInteger< ScalarType >( 2 ) * parameterG * parameterH * parameterK + parameterAlphaSquared * parameterF );
    cartesianElements( 5 ) = getFloatingInteger< ScalarType >( 2 ) *
            ( parameterH * cosineTrueLongitude + parameterK * sineTrueLongitude + parameterF * parameterH + parameterG * parameterK );
