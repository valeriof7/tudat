...
Eigen::Vector6d convertStateFromTEMEtoJ2000( const double epochSinceJ2000, const Eigen::Vector6d stateTEME )
{  // First, rotate to the True Of Date (TOD) frame.
    ...
    // These angles (zeta, z, and theta) do not really have descriptive names. For a description of the precession geometry and these
    // angles, see pages 226-228 and figure 3-31 in Vallado (2013).
    double precessionAngleModToGcrfZeta;
    double precessionAngleModToGcrfZ;
    double precessionAngleModToGcrfTheta;
    sofa_interface::getPrecessionAngles(
            precessionAngleModToGcrfZeta, precessionAngleModToGcrfZ, precessionAngleModToGcrfTheta, epochSinceJ2000 );
...
Eigen::Vector6d convertStateFromTEMEtoEclipJ2000( const double epochSinceJ2000, const Eigen::Vector6d stateTEME )
{
    ...
    Eigen::Quaterniond itrsToEclipticQuaternion =
            spice_interface::computeRotationQuaternionBetweenFrames( "J2000", "ECLIPJ2000", epochSinceJ2000 );
...
Eigen::Vector6d convertStateFromJ2000ToTEME( const double epochSinceJ2000, const Eigen::Vector6d stateJ2000 )
{
    // First, rotate to the True Of Date (TOD) frame.
    double equationOfEquinoxes = sofa_interface::calculateEquationOfEquinoxes( epochSinceJ2000 );
...
Eigen::Vector6d convertStateFromEclipJ2000ToTEME( const double epochSinceJ2000, const Eigen::Vector6d stateEclipJ2000 )
{
    // First, rotate to the True Of Date (TOD) frame.
    double equationOfEquinoxes = sofa_interface::calculateEquationOfEquinoxes( epochSinceJ2000 );
    ...
    Eigen::Vector3d positionTEME =
            rotationMatrix1.transpose( ) * rotationMatrix2.transpose( ) * rotationMatrix3.transpose( ) * positionEclipJ2000;
    Eigen::Vector3d velocityTEME =
            rotationMatrix1.transpose( ) * rotationMatrix2.transpose( ) * rotationMatrix3.transpose( ) * velocityEclipJ2000;
    Eigen::Vector6d stateTEME;
    stateTEME.segment( 0, 3 ) = positionTEME;
    stateTEME.segment( 3, 3 ) = velocityTEME;
