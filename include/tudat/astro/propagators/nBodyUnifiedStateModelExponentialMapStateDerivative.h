 ...
 */
Eigen::Vector7d computeStateDerivativeForUnifiedStateModelExponentialMap( const Eigen::Vector7d& currentUnifiedStateModelElements,
...
/*!
 * Function to evaluate the equations of motion for the unifies state model with exponential map (USMEM), providing the
 * time-derivatives of USMEM elements from the accelerations expressed in an RSW frame (see Vallado, 2001). This function takes the
 * accelerations in the inertial frame, as well as the Cartesian inertial state, and converts the accelerations to the RSW frame. \param
 * currentUnifiedStateModelElements Current USMEM elements of the body for which the equations of motion are to be evaluated \param
 * currentCartesianState Current Cartesian state of the body for which the equations of motion are to be evaluated \param
 * accelerationsInInertialFrame Accelerations acting on body, expressed in inertial frame \param centralBodyGravitationalParameter
 * Gravitational parameter of sum of central body and body for which orbit is propagated. \return Time derivatives of USMEM elements.
 */
Eigen::Vector7d computeStateDerivativeForUnifiedStateModelExponentialMap( const Eigen::Vector7d& currentUnifiedStateModelElements,
...
public:
                                         ...
                                         Eigen::Block< Eigen::Matrix< StateScalarType, Eigen::Dynamic, Eigen::Dynamic > > stateDerivative )
    {
        ...
        for( unsigned int i = 0; i < this->bodiesToBeIntegratedNumerically_.size( ); i++ )
        {
            ...
            stateDerivative.block( i * 7, 0, 7, 1 ) =
                    computeStateDerivativeForUnifiedStateModelExponentialMap(
            ...
            const TimeType& time )
    {
        ...
        for( unsigned int i = 0; i < this->bodiesToBeIntegratedNumerically_.size( ); i++ )
        {
            currentState.segment( i * 7, 7 ) = orbital_element_conversions::convertCartesianToUnifiedStateModelExponentialMapElements(
