        ...
        const std::shared_ptr< SingleVariableLimitPropagationTerminationCondition > dependentVariableTerminationCondition )
{
    ...
    integrator->getStateDerivativeFunction( )( integrator->getCurrentIndependentVariable( ), integrator->getCurrentState( ) );
    TimeStepType dependentVariableError = static_cast< TimeStepType >( dependentVariableTerminationCondition->getStopConditionError( ) );
        ...
        const bool isOnlyTerminationCondition = true )
{
    ...
    if( increasingTime )
    {
        finalConditionRootFinder =
                root_finders::createRootFinder< TimeStepType >( dependentVariableTerminationCondition->getTerminationRootFinderSettings( ),
    ...
    try
    {
        finalTimeStep = finalConditionRootFinder->execute(
                std::make_shared< basic_mathematics::FunctionProxy< TimeStepType, TimeStepType > >( dependentVariableErrorFunction ),
                ( lastTime - secondToLastTime ) / 2.0 );
        ...
        StateType& endState )
{
    ...
    TimeStepType minimumTimeStep = static_cast< TimeStepType >( std::numeric_limits< double >::max( ) ),
                 maximumTimeStep = static_cast< TimeStepType >( 0.0 );
    ...
    for( unsigned int i = 0; i < terminationConditionList.size( ); i++ )
    {
        if( terminationConditionList.at( i )->getcheckTerminationToExactCondition( ) )
        {
            ...
            getFinalStateForExactTerminationCondition( integrator,
                                                       terminationConditionList.at( i ),
                                                       secondToLastTime,
                                                       lastTime,
                                                       secondToLastState,
                                                       lastState,
        ...
        const bool isOnlyTerminationCondition = true )
{
    ...
    switch( terminationCondition->getTerminationType( ) )
    {
        ...
        case dependent_variable_stopping_condition: {
            ...
            getFinalStateForExactDependentVariableTerminationCondition( integrator,
                                                                        dependentVariableTerminationCondition,
                                                                        secondToLastTime,
                                                                        lastTime,
                                                                        secondToLastState,
        ...
        case hybrid_stopping_condition: {
            ...
            useNewSolution = getFinalStateForExactHybridVariableTerminationCondition(
                    integrator, hyrbidTerminationCondition, secondToLastTime, lastTime, secondToLastState, lastState, endTime, endState );
            break;
        }
        default:
        ...
        const double currentCpuTime )
{
    ...
    TimeType endTime;
    StateType endState;
    if( getFinalStateForExactTerminationCondition( integrator,
                ...
                std::make_shared< SingleArcPropagatorProcessingSettings >( ) )
{
    ...
    std::shared_ptr< PropagationTerminationDetails > propagationTerminationReason =
            std::make_shared< PropagationTerminationDetails >( unknown_propagation_termination_reason );
    ...
    do
    {
        try
        {
            ...
            {
                // Print solutions
                if( processingSettings->getPrintSettings( )->printCurrentStep(
                            stepsSinceLastPrint, std::fabs( static_cast< double >( currentTime ) - timeOfLastPrint ) ) )
                {
                    stepsSinceLastPrint = 0;
                    timeOfLastPrint = currentTime;
                ...
                {
                    propagationTerminationReason = std::make_shared< PropagationTerminationDetails >( termination_condition_reached, 0 );
                    break;
                }
                ...
                if( processingSettings->saveCurrentStep( stepsSinceLastSave,
                                                         std::fabs( static_cast< double >( currentTime ) - timeOfLastSave ) ) )
                {
                    solutionHistory[ currentTime ] = newState;
                        ...
                        static_cast< double >( currentTime ), currentCPUTime, newState.template cast< double >( ) ) )
            {
                ...
                if( propagationTerminationCondition->getTerminationType( ) != hybrid_stopping_condition )
                {
                    propagationTerminationReason = std::make_shared< PropagationTerminationDetails >(
                            termination_condition_reached, propagationTerminationCondition->getcheckTerminationToExactCondition( ) );
                }
                else
                {
                    if( std::dynamic_pointer_cast< HybridPropagationTerminationCondition >( propagationTerminationCondition ) == nullptr )
                    {
                    ...
                    propagationTerminationReason = std::make_shared< PropagationTerminationDetailsFromHybridCondition >(
                            ...
                            std::dynamic_pointer_cast< HybridPropagationTerminationCondition >( propagationTerminationCondition ) );
                }
        ...
        catch( const std::exception& caughtException )
        {
            ...
            std::cerr << "Error, propagation terminated at t=" + std::to_string( static_cast< double >( currentTime ) ) +
                            ", returning propagation data up to current time."
                      << std::endl;
            breakPropagation = true;
            propagationTerminationReason = std::make_shared< PropagationTerminationDetails >( runtime_error_caught_in_propagation );
        }
    ...
    simulationResults->reset( solutionHistory,
                              dependentVariableHistory,
                              cumulativeComputationTimeHistory,
                              std::map< TimeType, unsigned int >( ),
                                 ...
                                 std::make_shared< SingleArcPropagatorProcessingSettings >( ) )
{
    ...
    std::shared_ptr<
            numerical_integrators::NumericalIntegrator< TimeType, StateType, StateType, typename scalar_type< TimeType >::value_type > >
            integrator = numerical_integrators::createIntegrator< TimeType, StateType, typename scalar_type< TimeType >::value_type >(
