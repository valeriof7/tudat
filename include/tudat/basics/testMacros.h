...
#define _TUDAT_CHECK_MATRIX_MESSAGE "Element [%d, %d] not within expected tolerance (delta %e): expected %e, was %e, tolerance %e %s."
#endif
...
#define TUDAT_CHECK_MATRIX_BASE( L, R )                                                                                                   \
    boost::test_tools::predicate_result equalRows = boost::test_tools::tt_detail::equal_impl( L.rows( ), R.rows( ) );                     \
    BOOST_CHECK_MESSAGE( equalRows, boost::str( boost::format( "Matrix number of rows not equal: %d != %d" ) % L.rows( ) % R.rows( ) ) ); \
    boost::test_tools::predicate_result equalCols = boost::test_tools::tt_detail::equal_impl( L.cols( ), R.cols( ) );                     \
    BOOST_CHECK_MESSAGE( equalCols,                                                                                                       \
                         boost::str( boost::format( "Matrix number of columns not equal: %d != %d" ) % L.cols( ) % R.cols( ) ) );         \
...
#define TUDAT_CHECK_MATRIX_CLOSE( L, R, T )                                                                                                \
    {                                                                                                                                      \
        ...
        BOOST_CHECK_MESSAGE(                                                                                                               \
                ::boost::test_tools::check_is_close( L.coeff( row, col ), R.coeff( row, col ), percent_tolerance( T ) ).p_predicate_value, \
...
#define TUDAT_CHECK_MATRIX_CLOSE_FRACTION( L, R, T )                                                                                       \
    {                                                                                                                                      \
        _TUDAT_USING_PERCENT_TOLERANCE                                                                                                     \
        TUDAT_CHECK_MATRIX_BASE( L, R )                                                                                                    \
        BOOST_CHECK_MESSAGE( ::boost::test_tools::check_is_close( L.coeff( row, col ), R.coeff( row, col ), percent_tolerance( T * 100 ) ) \
                                     .p_predicate_value,                                                                                   \
                             _TUDAT_CHECK_MESSAGE( L, R, T, "" ) );                                                                        \
    }
