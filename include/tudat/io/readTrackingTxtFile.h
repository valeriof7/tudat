 ...
 * A TrackingFileFieldConverter is an interface that can convert a raw string input field to its associated double value as expected for the
 * TrackingDataType Reading a trackingFile can be done by specifying a list of trackingFileFields, each corresponding to such a converter.
 */
...
public:
    ...
    virtual double toDouble( std::string& rawField ) const
    {
        ...
        {
            throw std::runtime_error( "The tracking file field cannot be converted correctly. Check your columnTypes. Raw field was \"" +
                                      rawField + "\".\n" );
        }
    }

    //! Getter for doubleDataType_
    const TrackingDataType& getTrackingDataType( )
    {
        return doubleDataType_;
    }
...
public:
    ...
    {
        std::map< std::string, double > monthsMap{ { "JAN", 1. }, { "FEB", 2. },  { "MAR", 3. },  { "APR", 4. },
...
static const std::map< std::string, std::shared_ptr< TrackingFileFieldConverter > > trackingFileFieldConverterMap = {
    ...
    { "time_tag_delay_microseconds", std::make_shared< TrackingFileFieldMultiplyingConverter >( TrackingDataType::time_tag_delay, 1.e-6 ) },
    ...
    { "reference_body_distance",
      std::make_shared< TrackingFileFieldMultiplyingConverter >( TrackingDataType::reference_body_distance, 1.0E3 ) },
    { "ramp_reference_time", std::make_shared< TrackingFileFieldUTCTimeConverter >( TrackingDataType::utc_ramp_referencee_j2000 ) },
    { "transmission_frequency_constant_term",
      std::make_shared< TrackingFileFieldConverter >( TrackingDataType::transmission_frequency_constant_term ) },
    { "transmission_frequency_linear_term",
      std::make_shared< TrackingFileFieldConverter >( TrackingDataType::transmission_frequency_linear_term ) },
    { "doppler_predicted_frequency_hz",
      std::make_shared< TrackingFileFieldConverter >( TrackingDataType::doppler_predicted_frequency_hz ) },
    { "doppler_troposphere_correction",
      std::make_shared< TrackingFileFieldConverter >( TrackingDataType::doppler_troposphere_correction ) },
...
public:
    ...
    TrackingTxtFileContents( const std::string fileName,
        ...
        fileName_( fileName ), columnFieldTypes_( columnTypes ), commentSymbol_( commentSymbol ), valueSeparators_( valueSeparators ),
        ignoreOmittedColumns_( ignoreOmittedColumns )
    {
        parseData( );
    }
...
public:
     ...
     */
    void addMetaData( TrackingDataType dataType, double value )
    {
        metaDataMapDouble_[ dataType ] = value;
    }

    void addMetaData( TrackingDataType dataType, const std::string& value )
    {
        metaDataMapStr_[ dataType ] = value;
    }

    //! Number of columns expected in the file
    size_t getNumColumns( ) const
    {
        return columnFieldTypes_.size( );
    }

    //! Number of rows read out in the raw data map
    size_t getNumRows( ) const
    {
        return rawDataMap_.at( columnFieldTypes_[ 0 ] ).size( );
    }

    //! Getter for the field types defined by the user
    const std::vector< std::string >& getRawColumnTypes( )
    {
        return columnFieldTypes_;
    }
    ...
    const auto& getRawDataMap( )
    {
        return rawDataMap_;
    }

    //! Getter for `doubleDataMap_`
    const auto& getDoubleDataMap( )
    {
        return doubleDataMap_;
    }
    ...
    const auto& getMetaDataDoubleMap( )
    {
        return metaDataMapDouble_;
    }

    //! Getter for `metaDataMapStr_`
    const auto& getMetaDataStrMap( )
    {
        return metaDataMapStr_;
    }
